...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="oslocurrency/oslo-env:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t oslocurrency/oslo-beta:master --cache-from oslocurrency/oslo-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push oslocurrency/oslo-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=oslocurrency/oslo-beta:master"
            docker pull oslocurrency/oslo-beta:master
        fi
