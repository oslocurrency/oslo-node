...
run_build() {
    ...
    mv -f ~/oslo-${VERSIONS}.tar.gz ~/rpmbuild/SOURCES/.
    scl enable llvm-toolset-7 devtoolset-7 'rpmbuild -ba oslocurrency.spec'
    scl enable llvm-toolset-7 devtoolset-7 'rpmbuild -ba oslocurrency-beta.spec'
}

run_update() {
    for file in ./oslocurrency*.in; do
