...
parser.add_argument("--rpc", help="node rpc host:port", default="http://[::1]:9096")
...
with open(args.output, 'wb') as of:
    ...
    for rep in reps:
        if rep["weight"] == 0:
            break
        acc_val = int(hexlify(b32decode(rep["account"].encode('utf-8').replace(b"oslo_", b"").translate(tbl) + b"====")), 16)
