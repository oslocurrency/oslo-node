...
#include <oslo/boost/beast/http.hpp>
...
#include <oslo/rpc/rpc_request_processor.hpp>
...
public:
	...
	{
		sock.async_connect (oslo::tcp_endpoint (boost::asio::ip::address_v6::loopback (), port), [this](boost::system::error_code const & ec) {
			if (!ec)
			{
...
std::shared_ptr<oslo::node> add_ipc_enabled_node (oslo::system & system, oslo::node_config & node_config)
{
	node_config.ipc_config.transport_tcp.enabled = true;
	node_config.ipc_config.transport_tcp.port = oslo::get_available_port ();
	return system.add_node (node_config);
}

std::shared_ptr<oslo::node> add_ipc_enabled_node (oslo::system & system)
{
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	return add_ipc_enabled_node (system, node_config);
}

void reset_confirmation_height (oslo::block_store & store, oslo::account const & account)
{
...
void check_block_response_count (oslo::system & system, oslo::rpc & rpc, boost::property_tree::ptree & request, uint64_t size_count)
{
...
public:
	...
	void reset ()
	{
		oslo::thread_role::set (oslo::thread_role::name::unknown);
	}
...
TEST (rpc, wrapped_task)
{
	...
	auto handler_l (std::make_shared<oslo::json_handler> (node, node_rpc_config, "", response_handler_l));
	auto task (handler_l->create_worker_task ([](std::shared_ptr<oslo::json_handler>) {
...
TEST (rpc, account_balance)
{
	...
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
	oslo::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	oslo::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", oslo::test_genesis_key.pub.to_account ());
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
...
TEST (rpc, account_create)
{
	...
	ASSERT_EQ (std::error_code (oslo::error_common::invalid_index).message (), response2.json.get<std::string> ("error"));
}
...
TEST (rpc, account_weight)
{
	...
	oslo::change_block block (latest, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (oslo::process_result::progress, node1.process (block).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
...
TEST (rpc, validate_account_invalid)
{
	...
	rpc.start ();
	std::string account;
	oslo::test_genesis_key.pub.encode_account (account);
	account[0] ^= 0x1;
	boost::property_tree::ptree request;
	request.put ("action", "validate_account_number");
...
TEST (rpc, send)
{
	...
	request.put ("source", oslo::test_genesis_key.pub.to_account ());
	request.put ("destination", oslo::test_genesis_key.pub.to_account ());
	request.put ("amount", "100");
	system.deadline_set (10s);
	boost::thread thread2 ([&system, node]() {
		while (node->balance (oslo::test_genesis_key.pub) == oslo::genesis_amount)
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	});
...
TEST (rpc, send_fail)
{
	...
	ASSERT_EQ (std::error_code (oslo::error_common::account_not_found_wallet).message (), response.json.get<std::string> ("error"));
	thread2.join ();
}
...
TEST (rpc, send_work)
{
	...
	ASSERT_EQ (std::error_code (oslo::error_common::invalid_work).message (), response.json.get<std::string> ("error"));
	request.erase ("work");
	request.put ("work", oslo::to_string_hex (*node->work_generate_blocking (node->latest (oslo::test_genesis_key.pub))));
	test_response response2 (request, rpc.config.port, system.io_ctx);
	system.deadline_set (10s);
	while (response2.status == 0)
	{
...
TEST (rpc, send_work_disabled)
{
	...
	request.put ("amount", "100");
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_common::disabled_work_generation).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, send_idempotent)
{
	...
	request.put ("action", "send");
	request.put ("source", oslo::test_genesis_key.pub.to_account ());
	request.put ("destination", oslo::account (0).to_account ());
	request.put ("amount", (oslo::genesis_amount - (oslo::genesis_amount / 4)).convert_to<std::string> ());
	...
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->ledger.block_exists (block));
	ASSERT_EQ (node->balance (oslo::test_genesis_key.pub), oslo::genesis_amount / 4);
	test_response response2 (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response2.status == 0)
	{
	...
	ASSERT_EQ (std::error_code (oslo::error_common::insufficient_balance).message (), response3.json.get<std::string> ("error"));
}
...
TEST (rpc, send_epoch_2)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv, false);

	auto target_difficulty = oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (oslo::epoch::epoch_2, true, false, false));
	...
	request.put ("action", "send");
	request.put ("source", oslo::test_genesis_key.pub.to_account ());
	request.put ("destination", oslo::keypair ().pub.to_account ());
	request.put ("amount", "1");

	// Test that the correct error is given if there is insufficient work
	auto insufficient = system.work_generate_limited (oslo::genesis_hash, min_difficulty, target_difficulty);
	request.put ("work", oslo::to_string_hex (insufficient));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (oslo::error_common::invalid_work);
...
TEST (rpc, wallet_representative)
{
	...
	ASSERT_EQ (account_text1, oslo::genesis_account.to_account ());
}
...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		if (!node->store.account_get (transaction, oslo::test_genesis_key.pub, info))
		{
...
TEST (rpc, wallet_create_seed)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::raw_key seed;
	oslo::random_pool::generate_block (seed.data.bytes.data (), seed.data.bytes.size ());
	auto prv = oslo::deterministic_key (seed, 0);
	auto pub (oslo::pub_key (prv));
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
...
TEST (rpc, wallet_export)
{
	...
	oslo::kdf kdf;
	oslo::wallet_store store (error, kdf, transaction, oslo::genesis_account, 1, "0", wallet_json);
	ASSERT_FALSE (error);
	ASSERT_TRUE (store.exists (transaction, oslo::test_genesis_key.pub));
}
...
TEST (rpc, account_move)
{
	...
	destination->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key;
	auto source_id = oslo::random_wallet_id ();
	...
	ASSERT_TRUE (destination->exists (oslo::test_genesis_key.pub));
	auto transaction (node->wallets.tx_begin_read ());
...
TEST (rpc, block)
{
	...
	request.put ("hash", node->latest (oslo::genesis_account).to_string ());
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
...
TEST (rpc, chain)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key;
	auto genesis (node->latest (oslo::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, 1));
	...
	std::vector<oslo::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
		blocks.push_back (oslo::block_hash (i->second.get<std::string> ("")));
	}
...
TEST (rpc, frontier)
{
	oslo::system system;
	auto node = add_ipc_enabled_node (system);
	std::unordered_map<oslo::account, oslo::block_hash> source;
	{
		...
		for (auto i (0); i < 1000; ++i)
		{
			...
			source[key.pub] = hash;
			node->store.confirmation_height_put (transaction, key.pub, { 0, oslo::block_hash (0) });
			node->store.account_put (transaction, key.pub, oslo::account_info (hash, 0, 0, 0, 0, 0, oslo::epoch::epoch_0));
		}
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::keypair key;
	...
	request.put ("account", oslo::account (0).to_account ());
	request.put ("count", std::to_string (std::numeric_limits<uint64_t>::max ()));
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	...
	std::unordered_map<oslo::account, oslo::block_hash> frontiers;
	...
	ASSERT_EQ (1, frontiers.erase (oslo::test_genesis_key.pub));
...
TEST (rpc, history)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto change (system.wallet (0)->change_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (*send, oslo::test_genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	oslo::genesis genesis;
	oslo::state_block usend (oslo::genesis_account, node0->latest (oslo::genesis_account), oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::genesis_account, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node0->work_generate_blocking (node0->latest (oslo::genesis_account)));
	oslo::state_block ureceive (oslo::genesis_account, usend.hash (), oslo::genesis_account, oslo::genesis_amount, usend.hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node0->work_generate_blocking (usend.hash ()));
	oslo::state_block uchange (oslo::genesis_account, ureceive.hash (), oslo::keypair ().pub, oslo::genesis_amount, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node0->work_generate_blocking (ureceive.hash ()));
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction, usend).code);
		ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction, ureceive).code);
		ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction, uchange).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node0, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<1> (history_l[0]));
	ASSERT_EQ (oslo::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	ASSERT_EQ ("send", std::get<0> (history_l[1]));
	ASSERT_EQ (usend.hash ().to_string (), std::get<3> (history_l[1]));
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<1> (history_l[1]));
	ASSERT_EQ (oslo::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
	ASSERT_EQ ("receive", std::get<0> (history_l[2]));
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<1> (history_l[2]));
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<1> (history_l[4]));
	ASSERT_EQ (oslo::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[4]));
...
TEST (rpc, account_history)
{
	...
	rpc.start ();
	{
		...
		request.put ("account", oslo::genesis_account.to_account ());
		request.put ("count", 100);
		test_response response (request, rpc.config.port, system.io_ctx);
		while (response.status == 0)
		{
	...
	auto send2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, account2, node0->config.receive_minimum.number ()));
	...
	// Test filter for receive state blocks
	{
		...
		boost::property_tree::ptree other_account;
		other_account.put ("", oslo::test_genesis_key.pub.to_account ());
		boost::property_tree::ptree filtered_accounts;
		filtered_accounts.push_back (std::make_pair ("", other_account));
		request.add_child ("account_filter", filtered_accounts);
		request.put ("count", 100);
...
TEST (rpc, history_count)
{
	...
	auto change (system.wallet (0)->change_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (*send, oslo::test_genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
...
TEST (rpc, process_block)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::keypair key;
	auto latest (node1.latest (oslo::test_genesis_key.pub));
	oslo::send_block send (latest, key.pub, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	request.put ("block", json);
	{
		...
		ASSERT_EQ (200, response.status);
		system.deadline_set (10s);
		while (node1.latest (oslo::test_genesis_key.pub) != send.hash ())
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	...
	request.put ("json_block", true);
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (oslo::error_blocks::invalid_block);
...
TEST (rpc, process_block_with_work_watcher)
{
	...
	oslo::keypair key;
	auto latest (node1.latest (oslo::test_genesis_key.pub));
	auto send (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, latest, oslo::test_genesis_key.pub, oslo::genesis_amount - 100, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest)));
	auto difficulty1 (send->difficulty ());
	auto multiplier1 = oslo::normalized_multiplier (oslo::difficulty::to_multiplier (difficulty1, node1.network_params.network.publish_thresholds.epoch_1), node1.network_params.network.publish_thresholds.epoch_1);
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	{
		oslo::rpc_config rpc_config (oslo::get_available_port (), false);
		rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
		oslo::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
		oslo::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
		rpc.start ();
		...
		request.put ("block", json);
		{
			...
			ASSERT_EQ (200, response.status);
			std::error_code ec (oslo::error_rpc::rpc_control_disabled);
			ASSERT_EQ (ec.message (), response.json.get<std::string> ("error"));
		}
...
TEST (rpc, process_republish)
{
	...
	oslo::keypair key;
	auto latest (node1.latest (oslo::test_genesis_key.pub));
	oslo::send_block send (latest, key.pub, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node3.work_generate_blocking (latest));
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node3, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	ASSERT_EQ (200, response.status);
	system.deadline_set (10s);
	while (node2.latest (oslo::test_genesis_key.pub) != send.hash ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, process_subtype_send)
{
	...
	oslo::keypair key;
	auto latest (node1.latest (oslo::test_genesis_key.pub));
	oslo::state_block send (oslo::genesis_account, latest, oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	ASSERT_EQ (200, response.status);
	std::error_code ec (oslo::error_rpc::invalid_subtype_balance);
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	...
	system.deadline_set (10s);
	while (system.nodes[1]->latest (oslo::test_genesis_key.pub) != send.hash ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, process_subtype_open)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.active.insert (std::make_shared<oslo::state_block> (send));
	oslo::state_block open (key.pub, 0, key.pub, oslo::Gxrb_ratio, send.hash (), key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
...
TEST (rpc, process_subtype_receive)
{
	...
	auto & node2 = *system.add_node ();
	auto latest (node1.latest (oslo::test_genesis_key.pub));
	oslo::state_block send (oslo::genesis_account, latest, oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (oslo::process_result::progress, node1.process (send).code);
	ASSERT_EQ (oslo::process_result::progress, node2.process (send).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.active.insert (std::make_shared<oslo::state_block> (send));
	oslo::state_block receive (oslo::test_genesis_key.pub, send.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, send.hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	ASSERT_EQ (200, response2.status);
	ec = oslo::error_rpc::invalid_subtype_previous;
	ASSERT_EQ (response2.json.get<std::string> ("error"), ec.message ());
	request.put ("subtype", "receive");
	test_response response3 (request, rpc.config.port, system.io_ctx);
	while (response3.status == 0)
	...
	system.deadline_set (10s);
	while (node2.latest (oslo::test_genesis_key.pub) != receive.hash ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, process_ledger_insufficient_work)
{
	...
	auto latest (node.latest (oslo::test_genesis_key.pub));
	auto min_difficulty = node.network_params.network.publish_thresholds.entry;
	auto max_difficulty = node.network_params.network.publish_thresholds.epoch_1;
	oslo::state_block send (oslo::genesis_account, latest, oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, system.work_generate_limited (latest, min_difficulty, max_difficulty));
	...
	ASSERT_EQ (200, response.status);
	std::error_code ec (oslo::error_process::insufficient_work);
	ASSERT_EQ (1, response.json.count ("error"));
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
}
...
TEST (rpc, process_difficulty_update_flood)
{
	...
	auto latest (node.latest (oslo::test_genesis_key.pub));
	oslo::state_block send (oslo::genesis_account, latest, oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node.work_generate_blocking (latest));
	...
	auto expected_multiplier = oslo::normalized_multiplier (oslo::difficulty::to_multiplier (send.difficulty (), oslo::work_threshold (send.work_version (), oslo::block_details (oslo::epoch::epoch_0, true, false, false))), node.network_params.network.publish_thresholds.epoch_1);

	{
		std::string json;
		send.serialize_json (json);
		request.put ("block", json);
		std::error_code ec (oslo::error_process::old);
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	...
	ASSERT_NO_ERROR (system.poll_until_true (5s, [&node, &node_passive, &send, expected_multiplier] {
		...
		oslo::lock_guard<std::mutex> guard_passive (node_passive.active.mutex);
...
TEST (rpc, payment_begin_end)
{
	...
	uint64_t work (0);
	while (oslo::work_difficulty (oslo::work_version::work_1, root1, work) >= oslo::work_threshold_base (oslo::work_version::work_1))
	{
		++work;
		ASSERT_LT (work, 50);
	}
	system.deadline_set (10s);
	while (oslo::work_difficulty (oslo::work_version::work_1, root1, work) < node1->default_difficulty (oslo::work_version::work_1))
	{
		auto ec = system.poll ();
...
TEST (rpc, payment_end_nonempty)
{
	...
	request1.put ("wallet", wallet_id.to_string ());
	request1.put ("account", oslo::test_genesis_key.pub.to_account ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, payment_zero_balance)
{
	...
	oslo::account account;
	ASSERT_FALSE (account.decode_account (account_text));
	ASSERT_NE (oslo::test_genesis_key.pub, account);
}
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", oslo::amount (oslo::Mxrb_ratio).to_string_dec ());
	...
	scoped_thread_name_io.reset ();
	system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Mxrb_ratio);
	...
	request1.put ("amount", oslo::amount (oslo::Mxrb_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response3.status == 0)
	{
...
TEST (rpc, peers)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::endpoint endpoint (boost::asio::ip::make_address_v6 ("fc00::1"), 4000);
...
TEST (rpc, pending)
{
	...
	oslo::keypair key1;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (5s);
	...
	request.put ("count", "100");
	{
		...
		oslo::block_hash hash (blocks_node.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		oslo::block_hash hash (blocks_node.begin ()->first);
		ASSERT_EQ (block1->hash (), hash);
	...
	request.put ("threshold", "100"); // Threshold test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<oslo::block_hash, oslo::uint128_union> blocks;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			oslo::block_hash hash;
			hash.decode_hex (i->first);
			oslo::uint128_union amount;
	...
	request.put ("min_version", "true");
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<oslo::block_hash, oslo::uint128_union> amounts;
		std::unordered_map<oslo::block_hash, oslo::account> sources;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			oslo::block_hash hash;
			hash.decode_hex (i->first);
		...
		ASSERT_EQ (sources[block1->hash ()], oslo::test_genesis_key.pub);
	}
...
TEST (rpc, pending_burn)
{
	...
	oslo::account burn (0);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, burn, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (5s);
	while (node->active.active (*block1))
	{
...
TEST (rpc, search_pending)
{
	...
	auto latest (node->latest (oslo::test_genesis_key.pub));
	oslo::send_block block (latest, oslo::test_genesis_key.pub, oslo::genesis_amount - node->config.receive_minimum.number (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node->work_generate_blocking (latest));
	{
		auto transaction (node->store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, block).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("Oslo %1%") % OSLO_VERSION_STRING), response1.json.get<std::string> ("node_vendor"));
	...
	auto genesis_open (node1->latest (oslo::test_genesis_key.pub));
...
TEST (rpc, work_generate)
{
	...
	auto verify_response = [node, &rpc, &system](auto & request, auto & hash) {
		...
		uint64_t work;
		ASSERT_FALSE (oslo::from_string_hex (work_text, work));
		auto result_difficulty (oslo::work_difficulty (oslo::work_version::work_1, hash, work));
		auto response_difficulty_text (response.json.get<std::string> ("difficulty"));
		uint64_t response_difficulty;
		ASSERT_FALSE (oslo::from_string_hex (response_difficulty_text, response_difficulty));
		ASSERT_EQ (result_difficulty, response_difficulty);
		auto multiplier = response.json.get<double> ("multiplier");
		ASSERT_NEAR (oslo::difficulty::to_multiplier (result_difficulty, node->default_difficulty (oslo::work_version::work_1)), multiplier, 1e-6);
	};
...
TEST (rpc, work_generate_difficulty)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", oslo::to_string_hex (difficulty));
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (10s);
		while (response.status == 0)
		{
		...
		ASSERT_NEAR (oslo::difficulty::to_multiplier (result_difficulty, node->default_difficulty (oslo::work_version::work_1)), multiplier, 1e-10);
	...
	{
		uint64_t difficulty (node->max_work_generate_difficulty (oslo::work_version::work_1) + 1);
		request.put ("difficulty", oslo::to_string_hex (difficulty));
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (oslo::error_rpc::difficulty_limit);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, work_generate_multiplier)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", oslo::to_string_hex (0xff00000000000000));
	...
	}
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (oslo::error_rpc::bad_multiplier_format);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
	{
		double max_multiplier (oslo::difficulty::to_multiplier (node->max_work_generate_difficulty (oslo::work_version::work_1), node->default_difficulty (oslo::work_version::work_1)));
...
TEST (rpc, work_generate_epoch_2)
{
	oslo::system system;
	auto node = add_ipc_enabled_node (system);
	auto epoch1 = system.upgrade_genesis_epoch (*node, oslo::epoch::epoch_1);
	...
	rpc.start ();
	boost::property_tree::ptree request;
	auto verify_response = [node, &rpc, &system](auto & request, oslo::block_hash const & hash, uint64_t & out_difficulty) {
		request.put ("hash", hash.to_string ());
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		uint64_t work{ 0 };
		ASSERT_FALSE (oslo::from_string_hex (work_text, work));
		out_difficulty = oslo::work_difficulty (oslo::work_version::work_1, hash, work);
	};
	request.put ("action", "work_generate");
...
TEST (rpc, work_generate_block_high)
{
	...
	rpc.start ();
	oslo::keypair key;
	oslo::state_block block (key.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, 123, key.prv, key.pub, *node->work_generate_blocking (key.pub));
	oslo::block_hash hash (block.root ());
	auto block_difficulty (oslo::work_difficulty (oslo::work_version::work_1, hash, block.block_work ()));
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("json_block", "true");
	...
	request.add_child ("block", json);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (1, response.json.count ("error"));
		ASSERT_EQ (std::error_code (oslo::error_rpc::block_work_enough).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_low)
{
	...
	rpc.start ();
	oslo::keypair key;
	oslo::state_block block (key.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, 123, key.prv, key.pub, 0);
	auto threshold (node->default_difficulty (block.work_version ()));
	block.block_work_set (system.work_generate_limited (block.root (), threshold, oslo::difficulty::from_multiplier (node->config.max_work_generate_multiplier / 10, threshold)));
	oslo::block_hash hash (block.root ());
	auto block_difficulty (block.difficulty ());
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("difficulty", oslo::to_string_hex (block_difficulty + 1));
...
TEST (rpc, work_generate_block_root_mismatch)
{
	...
	request.put ("block", json);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (1, response.json.count ("error"));
		ASSERT_EQ (std::error_code (oslo::error_rpc::block_root_mismatch).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	oslo::keypair key;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	...
	oslo::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	oslo::state_block block (key.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, send_block->hash (), key.prv, key.pub, 0);
	auto threshold (oslo::work_threshold (block.work_version (), oslo::block_details (oslo::epoch::epoch_2, false, true, false)));
...
TEST (rpc, work_cancel)
{
	...
	while (!done)
	{
		system.work.generate (oslo::work_version::work_1, hash1, node1.network_params.network.publish_thresholds.base, [&done](boost::optional<uint64_t> work_a) {
			done = !work_a;
		});
		test_response response1 (request1, rpc.config.port, system.io_ctx);
		std::error_code ec;
...
TEST (rpc, work_peer_bad)
{
	...
	node2.work_generate (oslo::work_version::work_1, hash1, node2.network_params.network.publish_thresholds.base, [&work](boost::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.is_initialized ());
		work = *work_a;
	});
	system.deadline_set (5s);
	while (oslo::work_difficulty (oslo::work_version::work_1, hash1, work) < oslo::work_threshold_base (oslo::work_version::work_1))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, work_peer_one)
{
	...
	oslo::keypair key1;
	std::atomic<uint64_t> work (0);
	node2.work_generate (oslo::work_version::work_1, key1.pub, node1.network_params.network.publish_thresholds.base, [&work](boost::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.is_initialized ());
		work = *work_a;
	});
	system.deadline_set (5s);
	while (oslo::work_difficulty (oslo::work_version::work_1, key1.pub, work) < oslo::work_threshold_base (oslo::work_version::work_1))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, work_peer_many)
{
	...
	oslo::system system2;
	oslo::system system3 (1);
	oslo::system system4 (1);
	...
	oslo::keypair key;
	oslo::rpc_config config2 (oslo::get_available_port (), true);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server2 (node2, node_rpc_config);
	oslo::ipc_rpc_processor ipc_rpc_processor2 (system2.io_ctx, config2);
	oslo::rpc rpc2 (system2.io_ctx, config2, ipc_rpc_processor2);
	rpc2.start ();
	oslo::rpc_config config3 (oslo::get_available_port (), true);
	oslo::ipc::ipc_server ipc_server3 (node3, node_rpc_config);
	oslo::ipc_rpc_processor ipc_rpc_processor3 (system3.io_ctx, config3);
	oslo::rpc rpc3 (system3.io_ctx, config3, ipc_rpc_processor3);
	rpc3.start ();
	oslo::rpc_config config4 (oslo::get_available_port (), true);
	oslo::ipc::ipc_server ipc_server4 (node4, node_rpc_config);
	oslo::ipc_rpc_processor ipc_rpc_processor4 (system4.io_ctx, config4);
	oslo::rpc rpc4 (system2.io_ctx, config4, ipc_rpc_processor4);
	rpc4.start ();
	...
	{
		oslo::keypair key1;
		node1.work_generate (oslo::work_version::work_1, key1.pub, node1.network_params.network.publish_thresholds.base, [& work = works[i]](boost::optional<uint64_t> work_a) {
			work = *work_a;
		});
		while (oslo::work_difficulty (oslo::work_version::work_1, key1.pub, works[i]) < oslo::work_threshold_base (oslo::work_version::work_1))
		{
			system1.poll ();
			system2.poll ();
			system3.poll ();
...
TEST (rpc, work_version_invalid)
{
	...
	request.put ("version", "work_invalid");
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (1, response.json.count ("error"));
		ASSERT_EQ (std::error_code (oslo::error_rpc::bad_work_version).message (), response.json.get<std::string> ("error"));
	}
...
TEST (rpc, available_supply)
{
	...
	scoped_thread_name_io.reset ();
	auto block2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, 0, 100)); // Sending to burning 0 account
	scoped_thread_name_io.renew ();
	test_response response3 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response3.status == 0)
...
TEST (rpc, mrai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (oslo::Mxrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, mrai_from_raw)
{
	...
	request1.put ("amount", oslo::Mxrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, krai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (oslo::kxrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, krai_from_raw)
{
	...
	request1.put ("amount", oslo::kxrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, oslo_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "oslo_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (oslo::xrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, oslo_from_raw)
{
	oslo::system system;
	auto & node1 = *add_ipc_enabled_node (system);
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "oslo_from_raw");
	request1.put ("amount", oslo::xrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, account_representative_set_epoch_2)
{
	...
	auto target_difficulty = oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (oslo::epoch::epoch_2, false, false, false));
	ASSERT_LT (node.network_params.network.publish_thresholds.entry, target_difficulty);
	auto min_difficulty = node.network_params.network.publish_thresholds.entry;
	...
	request.put ("action", "account_representative_set");
	request.put ("account", oslo::test_genesis_key.pub.to_account ());
	request.put ("representative", oslo::keypair ().pub.to_account ());
...
TEST (rpc, bootstrap)
{
	...
	oslo::system system1 (1);
	auto latest (system1.nodes[0]->latest (oslo::test_genesis_key.pub));
	oslo::send_block send (latest, oslo::genesis_account, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system1.nodes[0]->work_generate_blocking (latest));
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, system1.nodes[0]->ledger.process (transaction, send).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
	oslo::ipc_rpc_processor ipc_rpc_processor (system0.io_ctx, rpc_config);
	oslo::rpc rpc (system0.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
	...
	while (system0.nodes[0]->latest (oslo::genesis_account) != system1.nodes[0]->latest (oslo::genesis_account))
	{
...
TEST (rpc, representatives)
{
	...
	std::vector<oslo::account> representatives;
	...
	ASSERT_EQ (oslo::genesis_account, representatives[0]);
}
...
TEST (rpc, wallet_change_seed)
{
	...
	oslo::random_pool::generate_block (seed.data.bytes.data (), seed.data.bytes.size ());
	{
		...
		oslo::raw_key seed0;
		oslo::random_pool::generate_block (seed0.data.bytes.data (), seed0.data.bytes.size ());
...
TEST (rpc, wallet_frontiers)
{
	...
	ASSERT_EQ (200, response.status);
	auto & frontiers_node (response.json.get_child ("frontiers"));
	std::vector<oslo::account> frontiers;
	for (auto i (frontiers_node.begin ()), n (frontiers_node.end ()); i != n; ++i)
	{
		frontiers.push_back (oslo::account (i->second.get<std::string> ("")));
	}
	ASSERT_EQ (1, frontiers.size ());
	ASSERT_EQ (system0.nodes[0]->latest (oslo::genesis_account), frontiers[0]);
}
...
TEST (rpc, work_validate)
{
	...
	request.put ("action", "work_validate");
	request.put ("hash", hash.to_string ());
	request.put ("work", oslo::to_string_hex (work1));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		uint64_t difficulty;
		ASSERT_FALSE (oslo::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (difficulty, node1.default_difficulty (oslo::work_version::work_1));
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, oslo::difficulty::to_multiplier (difficulty, node1.default_difficulty (oslo::work_version::work_1)), 1e-6);
	}
	uint64_t work2 (0);
	request.put ("work", oslo::to_string_hex (work2));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		uint64_t difficulty;
		ASSERT_FALSE (oslo::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (node1.default_difficulty (oslo::work_version::work_1), difficulty);
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, oslo::difficulty::to_multiplier (difficulty, node1.default_difficulty (oslo::work_version::work_1)), 1e-6);
	}
	auto result_difficulty (oslo::work_difficulty (oslo::work_version::work_1, hash, work1));
	ASSERT_GE (result_difficulty, node1.default_difficulty (oslo::work_version::work_1));
	request.put ("work", oslo::to_string_hex (work1));
	request.put ("difficulty", oslo::to_string_hex (result_difficulty));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
	...
	request.put ("work", oslo::to_string_hex (work1));
	request.put ("difficulty", oslo::to_string_hex (difficulty4));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		ASSERT_EQ (result_difficulty >= node1.default_difficulty (oslo::work_version::work_1), response.json.get<bool> ("valid_all"));
	...
	request.put ("work", oslo::to_string_hex (work3));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
...
TEST (rpc, work_validate_epoch_2)
{
	...
	request.put ("work", oslo::to_string_hex (work));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		ASSERT_FALSE (oslo::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, oslo::difficulty::to_multiplier (difficulty, node->network_params.network.publish_thresholds.epoch_1), 1e-6);
	};
	...
	scoped_thread_name_io.renew ();
	{
		...
		ASSERT_FALSE (oslo::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, oslo::difficulty::to_multiplier (difficulty, node->default_difficulty (oslo::work_version::work_1)), 1e-6);
	};
}
...
TEST (rpc, republish)
{
	...
	oslo::send_block send (latest, key.pub, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	oslo::open_block open (send.hash (), key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	...
	ASSERT_EQ (200, response.status);
	system.deadline_set (10s);
	while (system.nodes[1]->balance (oslo::test_genesis_key.pub) == oslo::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto & blocks_node (response.json.get_child ("blocks"));
...
TEST (rpc, deterministic_key)
{
	...
	oslo::account account0 (system0.wallet (0)->deterministic_insert ());
	oslo::account account1 (system0.wallet (0)->deterministic_insert ());
	oslo::account account2 (system0.wallet (0)->deterministic_insert ());
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
...
TEST (rpc, accounts_balances)
{
	...
	entry.put ("", oslo::test_genesis_key.pub.to_account ());
	...
	for (auto & balances : response.json.get_child ("balances"))
	{
		...
		ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), account_text);
...
TEST (rpc, accounts_frontiers)
{
	...
	for (auto & frontiers : response.json.get_child ("frontiers"))
	{
		...
		ASSERT_EQ (node->latest (oslo::genesis_account), frontier_text);
	}
}
...
TEST (rpc, accounts_pending)
{
	...
	request.put ("count", "100");
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			...
			oslo::block_hash hash1 (blocks.second.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			std::string account_text (blocks.first);
			ASSERT_EQ (key1.pub.to_account (), account_text);
			oslo::block_hash hash1 (blocks.second.begin ()->first);
			ASSERT_EQ (block1->hash (), hash1);
...
TEST (rpc, blocks)
{
	...
	boost::property_tree::ptree entry;
	boost::property_tree::ptree peers_l;
	entry.put ("", node->latest (oslo::genesis_account).to_string ());
	peers_l.push_back (std::make_pair ("", entry));
	...
	for (auto & blocks : response.json.get_child ("blocks"))
	{
		...
		ASSERT_EQ (node->latest (oslo::genesis_account).to_string (), hash_text);
...
TEST (rpc, wallet_info)
{
	...
	oslo::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, 1));
	oslo::account account (system.wallet (0)->deterministic_insert ());
	{
		auto transaction (node->wallets.tx_begin_write ());
...
TEST (rpc, wallet_balances)
{
	...
	auto send (system0.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, 1));
	scoped_thread_name_io.renew ();
...
TEST (rpc, pending_exists)
{
	...
	oslo::keypair key1;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto hash0 (node->latest (oslo::genesis_account));
	auto block1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (5s);
	while (node->active.active (*block1))
...
TEST (rpc, wallet_pending)
{
	...
	auto block1 (system0.wallet (0)->send_action (oslo::test_genesis_key.pub, key1.pub, 100));
	auto iterations (0);
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	for (auto & pending : response.json.get_child ("blocks"))
	{
		std::string account_text (pending.first);
		ASSERT_EQ (key1.pub.to_account (), account_text);
		oslo::block_hash hash1 (pending.second.begin ()->second.get<std::string> (""));
		ASSERT_EQ (block1->hash (), hash1);
	}
	request.put ("threshold", "100"); // Threshold test
	test_response response0 (request, rpc.config.port, system0.io_ctx);
...
TEST (rpc, work_get)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	system.wallet (0)->work_cache_blocking (oslo::test_genesis_key.pub, node->latest (oslo::test_genesis_key.pub));
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	uint64_t work (1);
	auto transaction (node->wallets.tx_begin_read ());
	node->wallets.items.begin ()->second->store.work_get (transaction, oslo::genesis_account, work);
	ASSERT_EQ (oslo::to_string_hex (work), work_text);
}
...
TEST (rpc, work_set)
{
	...
	request.put ("wallet", node->wallets.items.begin ()->first.to_string ());
	request.put ("account", oslo::test_genesis_key.pub.to_account ());
	request.put ("work", oslo::to_string_hex (work0));
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
	...
	uint64_t work1 (1);
	auto transaction (node->wallets.tx_begin_read ());
	node->wallets.items.begin ()->second->store.work_get (transaction, oslo::genesis_account, work1);
...
TEST (rpc, wallet_republish)
{
	...
	oslo::genesis genesis;
	oslo::keypair key;
	while (key.pub < oslo::test_genesis_key.pub)
	{
		oslo::keypair key1;
		key.pub = key1.pub;
...
TEST (rpc, delegators)
{
	...
	oslo::send_block send (latest, key.pub, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	oslo::open_block open (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (oslo::process_result::progress, node1.process (open).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	...
	ASSERT_EQ ("100", delegators.get<std::string> (oslo::test_genesis_key.pub.to_account ()));
...
TEST (rpc, account_info)
{
	...
	request.put ("account", oslo::account ().to_account ());
	...
	// Test for a non existing account
	{
		...
		ASSERT_EQ (error.get (), std::error_code (oslo::error_common::account_not_found).message ());
	}
	...
	oslo::send_block send (latest, key.pub, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	auto time (oslo::seconds_since_epoch ());
	{
		auto transaction = node1.store.tx_begin_write ();
		node1.store.confirmation_height_put (transaction, oslo::test_genesis_key.pub, { 1, genesis.hash () });
	}
	scoped_thread_name_io.renew ();
	...
	request.put ("representative", "1");
	{
		...
		ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), representative2);
	}
}
...
TEST (rpc, json_block_input)
{
	...
	oslo::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	oslo::state_block send (oslo::genesis_account, node1.latest (oslo::test_genesis_key.pub), oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	oslo::state_block block (json_error, response.json.get_child ("block"));
	ASSERT_FALSE (json_error);

	ASSERT_FALSE (oslo::validate_message (key.pub, send.hash (), block.block_signature ()));
...
TEST (rpc, json_block_output)
{
	...
	bool json_error{ false };
	oslo::send_block send_from_json (json_error, response.json.get_child ("contents"));
	ASSERT_FALSE (json_error);
}
...
TEST (rpc, blocks_info)
{
	...
	auto check_blocks = [node](test_response & response) {
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			...
			ASSERT_EQ (oslo::genesis_amount.convert_to<std::string> (), amount_text);
			...
			ASSERT_EQ (oslo::genesis_amount.convert_to<std::string> (), balance_text);
	...
	std::string random_hash = oslo::block_hash ().to_string ();
	...
	request.add_child ("hashes", hashes);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_blocks::not_found).message (), response.json.get<std::string> ("error"));
	}
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, oslo::Gxrb_ratio));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (*send, key.pub, oslo::Gxrb_ratio));
	ASSERT_NE (nullptr, receive);
	auto change (system.wallet (0)->change_action (oslo::test_genesis_key.pub, key.pub));
	ASSERT_NE (nullptr, change);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
...
TEST (rpc, ledger)
{
	...
	auto & node1 (*system.nodes[0]);
	auto latest (node1.latest (oslo::test_genesis_key.pub));
	auto genesis_balance (oslo::genesis_amount);
	auto send_amount (genesis_balance - 100);
	genesis_balance -= send_amount;
	oslo::send_block send (latest, key.pub, genesis_balance, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	oslo::open_block open (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (oslo::process_result::progress, node1.process (open).code);
	auto time (oslo::seconds_since_epoch ());
	...
	request.put ("representative", true);
	{
		...
		for (auto & account : response.json.get_child ("accounts"))
		{
			...
			ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), representative.get ());
		}
	}
	// Test threshold
	request.put ("count", 2);
	...
	oslo::send_block send2 (send.hash (), key.pub, genesis_balance, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	scoped_thread_name_io.reset ();
	node1.process (send2);
	scoped_thread_name_io.renew ();
...
TEST (rpc, block_create)
{
	...
	oslo::send_block send (latest, key.pub, 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, send_work);
	auto open_work = *node1.work_generate_blocking (key.pub);
	oslo::open_block open (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, open_work);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node1, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	request.put ("work", oslo::to_string_hex (send_work));
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
	...
	ASSERT_EQ (oslo::to_string_hex (send.difficulty ()), send_difficulty);
	auto send_text (response.json.get<std::string> ("block"));
	boost::property_tree::ptree block_l;
	std::stringstream block_stream (send_text);
	boost::property_tree::read_json (block_stream, block_l);
	auto send_block (oslo::deserialize_block_json (block_l));
	...
	request1.put ("representative", oslo::test_genesis_key.pub.to_account ());
	request1.put ("source", send.hash ().to_string ());
	request1.put ("work", oslo::to_string_hex (open_work));
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
	...
	auto open_block (oslo::deserialize_block_json (block_l));
	...
	oslo::change_block change (open.hash (), key.pub, key.prv, key.pub, change_work);
	request1.put ("type", "change");
	request1.put ("work", oslo::to_string_hex (change_work));
	...
	auto change_block (oslo::deserialize_block_json (block_l));
	ASSERT_EQ (change.hash (), change_block->hash ());
	scoped_thread_name_io.reset ();
	ASSERT_EQ (oslo::process_result::progress, node1.process (change).code);
	oslo::send_block send2 (send.hash (), key.pub, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	...
	request2.put ("work", oslo::to_string_hex (*node1.work_generate_blocking (change.hash ())));
	...
	auto receive_block (oslo::deserialize_block_json (block_l));
...
TEST (rpc, block_create_state)
{
	...
	request.put ("representative", oslo::test_genesis_key.pub.to_account ());
	request.put ("balance", (oslo::genesis_amount - oslo::Gxrb_ratio).convert_to<std::string> ());
	request.put ("link", key.pub.to_account ());
	request.put ("work", oslo::to_string_hex (*node->work_generate_blocking (genesis.hash ())));
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	boost::property_tree::ptree block_l;
	boost::property_tree::read_json (block_stream, block_l);
	auto state_block (oslo::deserialize_block_json (block_l));
	ASSERT_NE (nullptr, state_block);
	ASSERT_EQ (oslo::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (oslo::process_result::progress, process_result.code);
}
...
TEST (rpc, block_create_state_open)
{
	...
	request.put ("representative", oslo::test_genesis_key.pub.to_account ());
	request.put ("balance", oslo::Gxrb_ratio.convert_to<std::string> ());
	request.put ("link", send_block->hash ().to_string ());
	request.put ("work", oslo::to_string_hex (*node->work_generate_blocking (key.pub)));
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	ASSERT_GT (difficulty, oslo::work_threshold (state_block->work_version (), oslo::block_details (oslo::epoch::epoch_0, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (oslo::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, oslo::epoch::epoch_0);
...
TEST (rpc, block_create_state_request_work)
{
	...
	for (auto previous : previous_test_input)
	{
		...
		boost::property_tree::read_json (block_stream, block_l);
		auto block (oslo::deserialize_block_json (block_l));
		ASSERT_NE (nullptr, block);
		ASSERT_GE (block->difficulty (), node->default_difficulty (oslo::work_version::work_1));
	}
}
...
TEST (rpc, block_create_open_epoch_v2)
{
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (state_block->difficulty ());
	ASSERT_GT (difficulty, oslo::work_threshold (state_block->work_version (), oslo::block_details (oslo::epoch::epoch_2, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (oslo::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, oslo::epoch::epoch_2);
	ASSERT_TRUE (state_block->sideband ().details.is_receive);
	ASSERT_FALSE (node->latest (key.pub).is_zero ());
}
...
TEST (rpc, block_create_receive_epoch_v2)
{
	...
	auto send_block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	ASSERT_NE (nullptr, send_block);
	oslo::state_block open (key.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, send_block->hash (), key.prv, key.pub, *node->work_generate_blocking (key.pub));
	ASSERT_EQ (oslo::process_result::progress, node->process (open).code);
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, oslo::epoch::epoch_2));
	auto send_block_2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	boost::property_tree::ptree request;
	request.put ("action", "block_create");
	request.put ("type", "state");
	request.put ("key", key.prv.data.to_string ());
	...
	request.put ("representative", oslo::test_genesis_key.pub.to_account ());
	request.put ("balance", (2 * oslo::Gxrb_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_send_epoch_v2)
{
	...
	request.put ("link", oslo::test_genesis_key.pub.to_string ());
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (state_block->difficulty ());
	ASSERT_GT (difficulty, oslo::work_threshold (state_block->work_version (), oslo::block_details (oslo::epoch::epoch_2, true, false, false)));
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (oslo::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, oslo::epoch::epoch_2);
...
TEST (rpc, wallet_create_fail)
{
	...
	ASSERT_EQ (std::error_code (oslo::error_common::wallet_lmdb_max_dbs).message (), response.json.get<std::string> ("error"));
}
...
TEST (rpc, wallet_add_watch)
{
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_TRUE (system.wallet (0)->exists (oslo::test_genesis_key.pub));

	// Make sure using special wallet key as pubkey fails
	oslo::public_key bad_key (1);
	...
	std::error_code ec (oslo::error_common::bad_public_key);
...
TEST (rpc, online_reps)
{
	...
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*system.nodes[1], node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), item->second.get<std::string> (""));
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), item2->first);
	auto weight2 (item2->second.get<std::string> ("weight"));
	ASSERT_EQ (node2->weight (oslo::test_genesis_key.pub).convert_to<std::string> (), weight2);
	//Test accounts filter
	scoped_thread_name_io.reset ();
	auto new_rep (system.wallet (1)->deterministic_insert ());
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, new_rep, node1->config.receive_minimum.number ()));
	scoped_thread_name_io.renew ();
	ASSERT_NE (nullptr, send);
	system.deadline_set (5s);
	...
	scoped_thread_name_io.reset ();
	auto change (system.wallet (0)->change_action (oslo::test_genesis_key.pub, new_rep));
	scoped_thread_name_io.renew ();
	ASSERT_NE (nullptr, change);
	system.deadline_set (5s);
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (oslo::test_genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		oslo::keypair key1;
		oslo::send_block send (previous_genesis_chain_hash, key1.pub, oslo::genesis_amount - oslo::Gxrb_ratio - 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
	...
	std::shared_ptr<oslo::block> frontier;
	{
		auto transaction = node->store.tx_begin_read ();
	...
	// Make the same request, it should now return an error
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (oslo::error_rpc::confirmation_height_not_processing);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, confirmation_history)
{
	...
	auto block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (10s);
	...
	oslo::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == oslo::genesis_amount || tally_num == (oslo::genesis_amount - oslo::Gxrb_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	ASSERT_TRUE (node->active.list_recently_cemented ().empty ());
	auto send1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	auto send2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	auto send3 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, oslo::Gxrb_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (10s);
	...
	oslo::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == oslo::genesis_amount || tally_num == (oslo::genesis_amount - oslo::Gxrb_ratio) || tally_num == (oslo::genesis_amount - 2 * oslo::Gxrb_ratio) || tally_num == (oslo::genesis_amount - 3 * oslo::Gxrb_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm_confirmed)
{
	...
	config.callback_port = oslo::get_available_port ();
	...
	ASSERT_EQ (0, node->stats.count (oslo::stat::type::error, oslo::stat::detail::http_callback, oslo::stat::dir::out));
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	ASSERT_EQ (oslo::genesis_hash, confirmed.begin ()->winner->hash ());
	// Check callback
	system.deadline_set (5s);
	while (node->stats.count (oslo::stat::type::error, oslo::stat::detail::http_callback, oslo::stat::dir::out) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	// Callback result is error because callback target port isn't listening
	ASSERT_EQ (1, node->stats.count (oslo::stat::type::error, oslo::stat::detail::http_callback, oslo::stat::dir::out));
	node->stop ();
}
...
TEST (rpc, stats_clear)
{
	...
	oslo::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	node->stats.inc (oslo::stat::type::ledger, oslo::stat::dir::in);
	ASSERT_EQ (1, node->stats.count (oslo::stat::type::ledger, oslo::stat::dir::in));
	boost::property_tree::ptree request;
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_EQ (0, node->stats.count (oslo::stat::type::ledger, oslo::stat::dir::in));
...
TEST (rpc, unchecked)
{
	...
	auto open2 (std::make_shared<oslo::state_block> (key.pub, 0, key.pub, 2, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
...
TEST (rpc, unchecked_get)
{
	...
	request.put ("hash", open->hash ().to_string ());
	{
		...
		ASSERT_LE (timestamp, oslo::seconds_since_epoch ());
	}
	request.put ("json_block", true);
	{
		test_response response (request, rpc.config.port, system.io_ctx);
...
TEST (rpc, unopened)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::account account1 (1), account2 (account1.number () + 1);
	auto genesis (node->latest (oslo::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, account1, 1));
	ASSERT_NE (nullptr, send);
	auto send2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, account2, 10));
	ASSERT_NE (nullptr, send2);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	...
	}
	{
		...
		boost::property_tree::ptree request;
		request.put ("action", "unopened");
		request.put ("account", oslo::account (account2.number () + 1).to_account ());
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		{
...
TEST (rpc, unopened_burn)
{
	...
	auto genesis (node->latest (oslo::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, oslo::burn_account, 1));
	ASSERT_NE (nullptr, send);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
...
TEST (rpc, wallet_history)
{
	...
	auto node = add_ipc_enabled_node (system, node_config);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto timestamp1 (oslo::seconds_since_epoch ());
	auto send (system.wallet (0)->send_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto timestamp2 (oslo::seconds_since_epoch ());
	auto receive (system.wallet (0)->receive_action (*send, oslo::test_genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	oslo::keypair key;
	auto timestamp3 (oslo::seconds_since_epoch ());
	auto send2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send2);
	system.deadline_set (10s);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<4> (history_l[0]));
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<4> (history_l[1]));
	...
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<4> (history_l[2]));
	ASSERT_EQ (std::to_string (timestamp1), std::get<5> (history_l[2]));
	// Genesis block
	ASSERT_EQ ("receive", std::get<0> (history_l[3]));
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	ASSERT_EQ (oslo::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[3]));
	ASSERT_EQ (oslo::genesis_hash.to_string (), std::get<3> (history_l[3]));
	ASSERT_EQ (oslo::test_genesis_key.pub.to_account (), std::get<4> (history_l[3]));
}
...
TEST (rpc, sign_hash)
{
	...
	ASSERT_EQ (200, response.status);
	std::error_code ec (oslo::error_rpc::sign_hash_disabled);
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	...
	ASSERT_FALSE (oslo::validate_message (key.pub, send.hash (), signature));
}
...
TEST (rpc, memory_stats)
{
	...
	auto vote (std::make_shared<oslo::vote> (key.pub, key.prv, 0, hashes));
...
TEST (rpc, block_confirmed)
{
	...
	ASSERT_EQ (200, response.status);
	ASSERT_EQ (std::error_code (oslo::error_blocks::invalid_block_hash).message (), response.json.get<std::string> ("error"));
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (std::error_code (oslo::error_blocks::not_found).message (), response1.json.get<std::string> ("error"));
	...
	// Open an account directly in the ledger
	{
		...
		oslo::send_block send1 (latest, key.pub, 300, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send1).code);

		oslo::open_block open1 (send1.hash (), oslo::genesis_account, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	auto send = std::make_shared<oslo::send_block> (latest, key.pub, 10, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest));
...
TEST (rpc, database_txn_tracker)
{
	...
	// First try when database tracking is disabled
	{
		...
		request.put ("action", "database_txn_tracker");
		{
			...
			ASSERT_EQ (200, response.status);
			std::error_code ec (oslo::error_common::tracking_not_enabled);
			ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
		}
	}
	...
	auto check_not_correct_amount = [&system, &request, &rpc_port = rpc.config.port]() {
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (oslo::error_common::invalid_amount);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	};
	...
	auto thread_name = oslo::thread_role::get_string (oslo::thread_role::name::rpc_process_container);
...
TEST (rpc, active_difficulty)
{
	...
	ASSERT_EQ (node->default_difficulty (oslo::work_version::work_1), node->network_params.network.publish_thresholds.epoch_2);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	auto expected_multiplier{ (1.5 + 4.2 + (trend_size - 2) * 1) / trend_size };
	{
		...
		ASSERT_FALSE (oslo::from_string_hex (network_minimum_text, network_minimum));
		ASSERT_EQ (node->default_difficulty (oslo::work_version::work_1), network_minimum);
		auto multiplier (response.json.get<double> ("multiplier"));
		...
		ASSERT_FALSE (oslo::from_string_hex (network_current_text, network_current));
		ASSERT_EQ (oslo::difficulty::from_multiplier (expected_multiplier, node->default_difficulty (oslo::work_version::work_1)), network_current);
...
TEST (rpc, simultaneous_calls)
{
	...
	auto node = add_ipc_enabled_node (system);
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::thread_runner runner (system.io_ctx, node->config.io_threads);
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
...
TEST (rpc, in_process)
{
	...
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	oslo::inprocess_rpc_handler inprocess_rpc_handler (*node, ipc_server, node_rpc_config);
	oslo::rpc rpc (system.io_ctx, rpc_config, inprocess_rpc_handler);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", oslo::test_genesis_key.pub.to_account ());
...
TEST (rpc_config, migrate)
{
	...
	bool updated = false;
	auto data_path = oslo::unique_path ();
	boost::filesystem::create_directory (data_path);
	oslo::node_rpc_config oslo_rpc_config;
	oslo_rpc_config.deserialize_json (updated, rpc, data_path);
	ASSERT_TRUE (updated);

	// Check that the rpc config file is created
	auto rpc_path = oslo::get_rpc_config_path (data_path);
	oslo::rpc_config rpc_config;
...
TEST (rpc, deprecated_account_format)
{
	...
	std::string account_text (oslo::test_genesis_key.pub.to_account ());
	account_text[4] = '-';
	...
	ASSERT_EQ (200, response2.status);
	std::string frontier (response.json.get<std::string> ("frontier"));
	ASSERT_EQ (oslo::genesis_hash.to_string (), frontier);
...
TEST (rpc, epoch_upgrade)
{
	...
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, oslo::genesis_hash, oslo::test_genesis_key.pub, oslo::genesis_amount - 1, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (oslo::genesis_hash))); // to opened account
	ASSERT_EQ (oslo::process_result::progress, node->process (*send1).code);
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send1->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2, key2.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send1->hash ()))); // to unopened account (pending)
	ASSERT_EQ (oslo::process_result::progress, node->process (*send2).code);
	auto send3 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send2->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 3, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send2->hash ()))); // to burn (0)
	ASSERT_EQ (oslo::process_result::progress, node->process (*send3).code);
	oslo::account max_account (std::numeric_limits<oslo::uint256_t>::max ());
	auto send4 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send3->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 4, max_account, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send3->hash ()))); // to max account
	ASSERT_EQ (oslo::process_result::progress, node->process (*send4).code);
	auto open (std::make_shared<oslo::state_block> (key1.pub, 0, key1.pub, 1, send1->hash (), key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (oslo::process_result::progress, node->process (*open).code);
	// Check accounts epochs
	{
		...
		{
			oslo::account_info info (i->second);
			ASSERT_EQ (info.epoch (), oslo::epoch::epoch_0);
		}
	}
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node, node_rpc_config);
	...
	// Check upgrade
	{
		...
		ASSERT_TRUE (node->store.account_exists (transaction, std::numeric_limits<oslo::uint256_t>::max ()));
		ASSERT_FALSE (node->store.account_exists (transaction, 0));
	}

	// Epoch 2 upgrade
	auto genesis_latest (node->latest (oslo::test_genesis_key.pub));
	auto send5 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis_latest, oslo::test_genesis_key.pub, oslo::genesis_amount - 5, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis_latest))); // to burn (0)
	ASSERT_EQ (oslo::process_result::progress, node->process (*send5).code);
	auto send6 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send5->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 6, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send5->hash ()))); // to key1 (again)
	ASSERT_EQ (oslo::process_result::progress, node->process (*send6).code);
	auto key1_latest (node->latest (key1.pub));
	auto send7 (std::make_shared<oslo::state_block> (key1.pub, key1_latest, key1.pub, 0, key3.pub, key1.prv, key1.pub, *system.work.generate (key1_latest))); // to key3
	ASSERT_EQ (oslo::process_result::progress, node->process (*send7).code);
	{
		// Check pending entry
		auto transaction (node->store.tx_begin_read ());
		oslo::pending_info info;
		ASSERT_FALSE (node->store.pending_get (transaction, oslo::pending_key (key3.pub, send7->hash ()), info));
		ASSERT_EQ (oslo::epoch::epoch_1, info.epoch);
	}
	...
	// Check upgrade
	{
		...
		{
			oslo::account_info info (i->second);
			ASSERT_EQ (info.epoch (), oslo::epoch::epoch_2);
		}
		ASSERT_TRUE (node->store.account_exists (transaction, key1.pub));
		ASSERT_TRUE (node->store.account_exists (transaction, key2.pub));
...
TEST (rpc, account_lazy_start)
{
	...
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, oslo::genesis_hash, oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (oslo::genesis_hash)));
	...
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (*node2, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
...
TEST (rpc, receive)
{
	...
	auto send1 (wallet->send_action (oslo::test_genesis_key.pub, key1.pub, node.config.receive_minimum.number (), *node.work_generate_blocking (oslo::genesis_hash)));
	system.deadline_set (5s);
	while (node.balance (oslo::test_genesis_key.pub) == oslo::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	...
	auto send2 (wallet->send_action (oslo::test_genesis_key.pub, key1.pub, node.config.receive_minimum.number () - 1, *node.work_generate_blocking (send1->hash ())));
	scoped_io_thread_name_change scoped_thread_name_io;
	oslo::node_rpc_config node_rpc_config;
	oslo::ipc::ipc_server ipc_server (node, node_rpc_config);
	oslo::rpc_config rpc_config (oslo::get_available_port (), true);
	...
	// Trying to receive the same block should fail with unreceivable
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_process::unreceivable).message (), response.json.get<std::string> ("error"));
	}
	// Trying to receive a non-existing block should fail
	request.put ("block", oslo::block_hash (send2->hash ().number () + 1).to_string ());
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
...
TEST (rpc, receive_unopened)
{
	...
	oslo::keypair key1;
	auto send1 (wallet->send_action (oslo::test_genesis_key.pub, key1.pub, node.config.receive_minimum.number () - 1, *node.work_generate_blocking (oslo::genesis_hash)));
	system.deadline_set (5s);
	while (node.balance (oslo::test_genesis_key.pub) == oslo::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	...
	request.put ("block", send1->hash ().to_string ());
	{
		...
		ASSERT_EQ (info.representative, oslo::test_genesis_key.pub);
	}
	scoped_thread_name_io.reset ();

	// Test receiving for an unopened with a different wallet representative
	oslo::keypair key2;
	auto prev_amount (node.balance (oslo::test_genesis_key.pub));
	auto send2 (wallet->send_action (oslo::test_genesis_key.pub, key2.pub, node.config.receive_minimum.number () - 1, *node.work_generate_blocking (send1->hash ())));
	system.deadline_set (5s);
	while (node.balance (oslo::test_genesis_key.pub) == prev_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (rpc, receive_work_disabled)
{
	...
	auto send1 (wallet->send_action (oslo::test_genesis_key.pub, key1.pub, node.config.receive_minimum.number () - 1, *worker_node.work_generate_blocking (genesis.hash ()), false));
...
TEST (rpc, telemetry_single)
{
...

	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_rpc::requires_port_and_address).message (), response.json.get<std::string> ("error"));
	}
...

	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_common::invalid_ip_address).message (), response.json.get<std::string> ("error"));
	}
	...
	request.put ("port", "invalid port");
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_common::invalid_port).message (), response.json.get<std::string> ("error"));
	}
	...
	request.put ("port", node->network.endpoint ().port ());
	{
		...
		oslo::jsonconfig config (response.json);
		...
		oslo::compare_default_telemetry_response_data (telemetry_data, node->network_params, node->config.bandwidth_limit, node->active.active_difficulty (), node->node_id);
	}
}
...
TEST (rpc, telemetry_all)
{
	...
	std::atomic<bool> done{ false };
	auto node = system.nodes.front ();
	node1.telemetry->get_metrics_single_peer_async (node1.network.find_channel (node->network.endpoint ()), [&done](oslo::telemetry_data_response const & telemetry_data_response_a) {
	...
	request.put ("action", "telemetry");
	{
		...
		ASSERT_FALSE (telemetry_data.deserialize_json (config, should_ignore_identification_metrics));
		oslo::compare_default_telemetry_response_data_excluding_signature (telemetry_data, node->network_params, node->config.bandwidth_limit, node->active.active_difficulty ());
	...
	oslo::jsonconfig config (metrics);
	oslo::telemetry_data data;
	auto const should_ignore_identification_metrics = false;
	ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
	oslo::compare_default_telemetry_response_data (data, node->network_params, node->config.bandwidth_limit, node->active.active_difficulty (), node->node_id);
...
TEST (rpc, telemetry_self)
{
	...
	node1.network.udp_channels.insert (oslo::endpoint (boost::asio::ip::make_address_v6 ("::1"), oslo::get_available_port ()), 0);
	...
	auto const should_ignore_identification_metrics = false;
	{
		...
		oslo::jsonconfig config (response.json);
		ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
		oslo::compare_default_telemetry_response_data (data, node1.network_params, node1.config.bandwidth_limit, node1.active.active_difficulty (), node1.node_id);
	}
	...
	request.put ("port", "0");
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (oslo::error_rpc::peer_not_found).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, confirmation_active)
{
	...
	oslo::genesis genesis;
	auto send1 (std::make_shared<oslo::send_block> (genesis.hash (), oslo::public_key (), oslo::genesis_amount - 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<oslo::send_block> (send1->hash (), oslo::public_key (), oslo::genesis_amount - 200, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send1->hash ())));
