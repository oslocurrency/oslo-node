...
TEST (toml, diff)
{
	oslo::tomlconfig defaults, other;
...
TEST (toml, daemon_config_update_array)
{
	...
	oslo::daemon_config c (data_path);
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	oslo::tomlconfig t;
	t.read (ss);
	oslo::rpc_config conf;
	oslo::rpc_config defaults;
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	oslo::tomlconfig t;
	t.read (ss);
	oslo::daemon_config conf;
	oslo::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == oslo::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == oslo::error_config::invalid_value);
}

TEST (toml, put)
{
	oslo::tomlconfig config;
	oslo::tomlconfig config_node;
	// Overwrite value and add to child node
	config_node.put ("port", "9094");
	config_node.put ("port", "9095");
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["oslo_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml, daemon_config_deserialize_errors)
{
	...
	oslo::tomlconfig toml2;
	toml2.read (ss_frontiers_confirmation);
	oslo::daemon_config conf2;
	...
	ASSERT_EQ (conf2.node.frontiers_confirmation, oslo::frontiers_confirmation_mode::invalid);
}
...
TEST (toml, daemon_read_config)
{
	...
	ASSERT_FALSE (boost::filesystem::exists (oslo::get_node_toml_config_path (path)));
	ASSERT_FALSE (oslo::read_node_config_toml (path, config));
	{
		auto error = oslo::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = oslo::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	oslo::tomlconfig toml;
	toml.write (oslo::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (oslo::get_node_toml_config_path (path)));
	ASSERT_FALSE (oslo::read_node_config_toml (path, config));
	{
		auto error = oslo::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
