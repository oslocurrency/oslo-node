...
TEST (logger, changing_time_interval)
{
	...
	oslo::logger_mt my_logger (logging.min_time_between_log_output);
...
TEST (logger, try_log)
{
	auto path1 (oslo::unique_path ());
	std::stringstream ss;
	oslo::boost_log_cerr_redirect redirect_cerr (ss.rdbuf ());
	oslo::logger_mt my_logger (100ms);
...
TEST (logger, always_log)
{
	...
	std::stringstream ss;
	oslo::boost_log_cerr_redirect redirect_cerr (ss.rdbuf ());
	oslo::logger_mt my_logger (20s); // Make time interval effectively unreachable
