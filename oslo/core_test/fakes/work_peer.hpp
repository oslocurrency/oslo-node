...
public:
	work_peer_connection (asio::io_context & ioc_a, work_peer_type const type_a, oslo::work_version const version_a, oslo::work_pool & pool_a, std::function<void(bool const)> on_generation_a, std::function<void()> on_cancel_a) :
	socket (ioc_a),
	type (type_a),
	version (version_a),
	work_pool (pool_a),
...
private:
	...
	void handle_generate (oslo::block_hash const & hash_a)
	{
		if (type == work_peer_type::good)
		{
			auto hash = hash_a;
			auto request_difficulty = oslo::work_threshold_base (version);
			auto this_l (shared_from_this ());
			work_pool.generate (version, hash, request_difficulty, [this_l, hash](boost::optional<uint64_t> work_a) {
				...
				auto result_difficulty (oslo::work_difficulty (this_l->version, hash, result));
				static oslo::network_params params;
				ptree::ptree message_l;
				message_l.put ("work", oslo::to_string_hex (result));
				message_l.put ("difficulty", oslo::to_string_hex (result_difficulty));
				message_l.put ("multiplier", oslo::to_string (oslo::difficulty::to_multiplier (result_difficulty, oslo::work_threshold_base (this_l->version))));
...
public:
	...
	fake_work_peer (oslo::work_pool & pool_a, asio::io_context & ioc_a, unsigned short port_a, work_peer_type const type_a, oslo::work_version const version_a = oslo::work_version::work_1) :
	pool (pool_a),
