...
namespace
{
void add_required_children_node_config_tree (oslo::jsonconfig & tree);
}
...
TEST (node, work_generate)
{
	...
	oslo::block_hash root{ 1 };
	oslo::work_version version{ oslo::work_version::work_1 };
	{
		auto difficulty = oslo::difficulty::from_multiplier (1.5, node.network_params.network.publish_thresholds.base);
		auto work = node.work_generate_blocking (version, root, difficulty);
		ASSERT_TRUE (work.is_initialized ());
		ASSERT_TRUE (oslo::work_difficulty (version, root, *work) >= difficulty);
	}
	{
		auto difficulty = oslo::difficulty::from_multiplier (0.5, node.network_params.network.publish_thresholds.base);
		boost::optional<uint64_t> work;
		do
		{
			work = node.work_generate_blocking (version, root, difficulty);
		} while (oslo::work_difficulty (version, root, *work) >= node.network_params.network.publish_thresholds.base);
		ASSERT_TRUE (work.is_initialized ());
		ASSERT_TRUE (oslo::work_difficulty (version, root, *work) >= difficulty);
		ASSERT_FALSE (oslo::work_difficulty (version, root, *work) >= node.network_params.network.publish_thresholds.base);
	}
}
...
TEST (node, block_store_path_failure)
{
	...
	oslo::alarm alarm (*service);
	auto path (oslo::unique_path ());
	oslo::logging logging;
	logging.init (path);
	oslo::work_pool work (std::numeric_limits<unsigned>::max ());
	auto node (std::make_shared<oslo::node> (*service, oslo::get_available_port (), path, alarm, logging, work));
...
TEST (node, password_fanout)
{
	...
	auto node (std::make_shared<oslo::node> (*service, path, alarm, config, work));
...
TEST (node, balance)
{
	...
	ASSERT_EQ (std::numeric_limits<oslo::uint128_t>::max (), system.nodes[0]->ledger.account_balance (transaction, oslo::test_genesis_key.pub));
}

TEST (node, representative)
{
	oslo::system system (1);
	auto block1 (system.nodes[0]->rep_block (oslo::test_genesis_key.pub));
	{
...
TEST (node, send_unkeyed)
{
	...
	oslo::keypair key2;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	system.wallet (0)->store.password.value_set (oslo::keypair ().prv);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
}
...
TEST (node, send_self)
{
	...
	ASSERT_EQ (std::numeric_limits<oslo::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (oslo::test_genesis_key.pub));
}
...
TEST (node, send_single_observing_peer)
{
	...
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<oslo::node> const & node_a) { return node_a->balance (key2.pub).is_zero (); }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, send_out_of_order)
{
	...
	oslo::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ()));
	oslo::send_block send2 (send1.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number () * 2, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send1.hash ()));
	oslo::send_block send3 (send2.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number () * 3, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send2.hash ()));
	node1.process_active (std::make_shared<oslo::send_block> (send3));
	node1.process_active (std::make_shared<oslo::send_block> (send2));
	node1.process_active (std::make_shared<oslo::send_block> (send1));
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<oslo::node> const & node_a) { return node_a->balance (oslo::test_genesis_key.pub) != oslo::genesis_amount - node1.config.receive_minimum.number () * 3; }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, quick_confirm)
{
	...
	auto genesis_start_balance (node1.balance (oslo::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto send (std::make_shared<oslo::send_block> (previous, key.pub, node1.config.online_weight_minimum.number () + 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (previous)));
	...
	ASSERT_EQ (node1.balance (oslo::test_genesis_key.pub), node1.config.online_weight_minimum.number () + 1);
...
TEST (node, node_receive_quorum)
{
	...
	oslo::block_hash previous (node1.latest (oslo::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (std::make_shared<oslo::send_block> (previous, key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (previous)));
	node1.process_active (send);
	system.deadline_set (10s);
	...
	}
	{
		...
		auto info (node1.active.roots.find (oslo::qualified_root (previous, previous)));
	...
	system2.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	ASSERT_TRUE (node1.balance (key.pub).is_zero ());
	auto channel (std::make_shared<oslo::transport::channel_udp> (node1.network.udp_channels, system2.nodes[0]->network.endpoint (), node1.network_params.protocol.protocol_version));
...
TEST (node, auto_bootstrap)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key2.prv);
	auto send1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, node0->config.receive_minimum.number ()));
	...
	auto channel (std::make_shared<oslo::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.protocol.protocol_version));
	node1->network.send_keepalive (channel);
	node1->start ();
...
TEST (node, auto_bootstrap_reverse)
{
	...
	auto node1 (std::make_shared<oslo::node> (system.io_ctx, oslo::get_available_port (), oslo::unique_path (), system.alarm, system.logging, system.work, node_flags));
	ASSERT_FALSE (node1->init_error ());
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, node0->config.receive_minimum.number ()));
	auto channel (std::make_shared<oslo::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node0->network_params.protocol.protocol_version));
...
TEST (node, receive_gap)
{
	...
	auto block (std::make_shared<oslo::send_block> (5, 1, 2, oslo::keypair ().prv, 4, 0));
...
{
	oslo::system system (1);
	std::array<oslo::endpoint, 8> endpoints;
	endpoints.fill (oslo::endpoint (boost::asio::ip::address_v6::loopback (), oslo::get_available_port ()));
	endpoints[0] = oslo::endpoint (boost::asio::ip::address_v6::loopback (), oslo::get_available_port ());
...
TEST (node, search_pending)
{
	...
	oslo::keypair key2;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	system.wallet (0)->insert_adhoc (key2.prv);
...
TEST (node, search_pending_multiple)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key3.pub, node->config.receive_minimum.number ()));
	system.deadline_set (10s);
...
TEST (node, search_pending_confirmed)
{
	...
	oslo::keypair key2;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto send1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	...
	}
	{
		...
		system.wallet (0)->store.erase (transaction, oslo::test_genesis_key.pub);
	}
	system.wallet (0)->insert_adhoc (key2.prv);
	ASSERT_FALSE (system.wallet (0)->search_pending ());
	{
...
TEST (node, unlock_search)
{
	...
	auto node (system.nodes[0]);
	oslo::keypair key2;
	oslo::uint128_t balance (node->balance (oslo::test_genesis_key.pub));
	{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	system.deadline_set (10s);
	while (node->balance (oslo::test_genesis_key.pub) == balance)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	while (!node->active.empty ())
	...
	system.wallet (0)->insert_adhoc (key2.prv);
	{
		oslo::lock_guard<std::recursive_mutex> lock (system.wallet (0)->store.mutex);
		system.wallet (0)->store.password.value_set (oslo::keypair ().prv);
	}
	{
		auto transaction (system.wallet (0)->wallets.tx_begin_write ());
...
TEST (node, connect_after_junk)
{
	...
	channel1->send_buffer (oslo::shared_const_buffer (std::move (junk_buffer)), oslo::stat::detail::bulk_pull, [](boost::system::error_code const &, size_t) {});
	system.deadline_set (10s);
	while (node0->stats.count (oslo::stat::type::error) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	node1->start ();
	system.nodes.push_back (node1);
	auto channel2 (std::make_shared<oslo::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.protocol.protocol_version));
...
{
	oslo::system system (1);
	auto price1 (system.nodes[0]->price (oslo::Gxrb_ratio, 1));
	ASSERT_EQ (oslo::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (oslo::Gxrb_ratio * int(oslo::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (oslo::Gxrb_ratio * int(oslo::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (oslo::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (oslo::Gxrb_ratio * int(oslo::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node, confirm_locked)
{
	...
	auto block (std::make_shared<oslo::send_block> (0, 0, 0, oslo::keypair ().prv, 0, 0));
...
{
	auto path (oslo::unique_path ());
	oslo::logging logging1;
	logging1.init (path);
	oslo::node_config config1 (100, logging1);
...
TEST (node_config, v2_v3_upgrade)
{
	...
	config1.deserialize_json (upgraded, tree);
	//ASSERT_EQ (oslo::uint128_union (0).to_string_dec (), tree.get<std::string> ("inactive_supply"));
...
TEST (node_config, v15_v16_upgrade)
{
	auto test_upgrade = [](auto old_preconfigured_peers_url, auto new_preconfigured_peers_url) {
		...
		oslo::jsonconfig preconfigured_peers_json;
	...
	test_upgrade ("rai.raiblocks.net", "peering.oslo.org");
	test_upgrade ("rai-beta.raiblocks.net", "peering-beta.oslo.org");
}
...
TEST (node_config, v16_values)
{
	...
	tree.put ("vote_minimum", oslo::Gxrb_ratio.convert_to<std::string> ());
	config.deserialize_json (upgraded, tree);
	ASSERT_FALSE (upgraded);
	ASSERT_FALSE (config.allow_local_peers);
	ASSERT_EQ (config.signature_checker_threads, 1);
	ASSERT_EQ (config.vote_minimum.number (), oslo::Gxrb_ratio);

	// Check config is correct with other values
	tree.put ("allow_local_peers", true);
	tree.put ("signature_checker_threads", 4);
	tree.put ("vote_minimum", (std::numeric_limits<oslo::uint128_t>::max () - 100).convert_to<std::string> ());
	upgraded = false;
	config.deserialize_json (upgraded, tree);
	ASSERT_FALSE (upgraded);
	ASSERT_TRUE (config.allow_local_peers);
	ASSERT_EQ (config.signature_checker_threads, 4);
	ASSERT_EQ (config.vote_minimum.number (), std::numeric_limits<oslo::uint128_t>::max () - 100);
}
...
public:
	...
	oslo::error serialize_json (oslo::jsonconfig & json)
	{
		json.put ("thing", text);
...
{
public:
	oslo::error deserialize_json (bool & upgraded, oslo::jsonconfig & json)
	{
		if (!json.empty ())
		{
			...
			if (text_l == "error")
			{
				json.get_error () = oslo::error_common::generic;
			}
			text = text_l;
		}
		else
...
TEST (node_flags, disable_tcp_realtime)
{
	...
	ASSERT_EQ (oslo::transport::transport_type::udp, list1[0]->get_type ());
	ASSERT_EQ (1, node2->network.size ());
	auto list2 (node2->network.list (2));
	ASSERT_EQ (node1->network.endpoint (), list2[0]->get_endpoint ());
	ASSERT_EQ (oslo::transport::transport_type::udp, list2[0]->get_type ());
}
...
TEST (node_flags, disable_tcp_realtime_and_bootstrap_listener)
{
	...
	auto node2 = system.add_node (node_flags);
	ASSERT_EQ (oslo::tcp_endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->bootstrap.endpoint ());
	ASSERT_NE (oslo::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.endpoint ());
	ASSERT_EQ (1, node1->network.size ());
	auto list1 (node1->network.list (2));
	ASSERT_EQ (node2->network.endpoint (), list1[0]->get_endpoint ());
	ASSERT_EQ (oslo::transport::transport_type::udp, list1[0]->get_type ());
...
TEST (node_flags, disable_udp)
{
	...
	node_flags.disable_udp = false;
	auto node1 = system.add_node (node_flags);
	auto node2 (std::make_shared<oslo::node> (system.io_ctx, oslo::unique_path (), system.alarm, oslo::node_config (oslo::get_available_port (), system.logging), system.work));
	system.nodes.push_back (node2);
	node2->start ();
	ASSERT_EQ (oslo::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.udp_channels.get_local_endpoint ());
	ASSERT_NE (oslo::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.endpoint ());
	// Send UDP message
	auto channel (std::make_shared<oslo::transport::channel_udp> (node1->network.udp_channels, node2->network.endpoint (), node2->network_params.protocol.protocol_version));
	node1->network.send_keepalive (channel);
...
TEST (node, fork_publish)
{
	std::weak_ptr<oslo::node> node0;
	{
		...
		oslo::keypair key2;
		auto send2 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, oslo::genesis_amount - 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
		...
		node1.block_processor.flush ();
		lock.lock ();
		auto existing1 (election->last_votes.find (oslo::test_genesis_key.pub));
...
TEST (node, fork_publish_inactive)
{
	...
	auto send2 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, oslo::genesis_amount - 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, send1->block_work ()));
	auto & node (*system.nodes[0]);
	ASSERT_EQ (oslo::process_result::progress, node.process (*send1).code);
	ASSERT_EQ (oslo::process_result::fork, node.process_local (send2).code);
...
TEST (node, fork_multi_flip)
{
	...
	for (auto & type : types)
	{
		...
		auto send2 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, oslo::genesis_amount - 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
		oslo::publish publish2 (send2);
		auto send3 (std::make_shared<oslo::send_block> (publish2.block->hash (), key2.pub, oslo::genesis_amount - 100, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (publish2.block->hash ())));
		oslo::publish publish3 (send3);
...
TEST (node, fork_bootstrap_flip)
{
	oslo::system system0;
	oslo::system system1;
	oslo::node_config config0 (oslo::get_available_port (), system0.logging);
	config0.frontiers_confirmation = oslo::frontiers_confirmation_mode::disabled;
	oslo::node_flags node_flags;
	node_flags.disable_bootstrap_bulk_push_client = true;
	node_flags.disable_lazy_bootstrap = true;
	node_flags.disable_udp = false;
	auto & node1 (*system0.add_node (config0, node_flags));
	oslo::node_config config1 (oslo::get_available_port (), system1.logging);
	config1.frontiers_confirmation = oslo::frontiers_confirmation_mode::disabled;
	auto & node2 (*system1.add_node (config1, node_flags));
	system0.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::block_hash latest (node1.latest (oslo::test_genesis_key.pub));
	oslo::keypair key1;
	auto send1 (std::make_shared<oslo::send_block> (latest, key1.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system0.work.generate (latest)));
	oslo::keypair key2;
	auto send2 (std::make_shared<oslo::send_block> (latest, key2.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system0.work.generate (latest)));
	// Insert but don't rebroadcast, simulating settled blocks
	node1.block_processor.add (send1, oslo::seconds_since_epoch ());
	node1.block_processor.flush ();
	node2.block_processor.add (send2, oslo::seconds_since_epoch ());
	node2.block_processor.flush ();
	{
		auto transaction (node2.store.tx_begin_read ());
...
TEST (node, fork_open)
{
	...
	auto open1 (std::make_shared<oslo::open_block> (publish1.block->hash (), 1, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	oslo::publish publish2 (open1);
	node1.network.process_message (publish2, channel1);
	node1.block_processor.flush ();
	ASSERT_EQ (1, node1.active.size ());
	auto open2 (std::make_shared<oslo::open_block> (publish1.block->hash (), 2, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	oslo::publish publish3 (open2);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
...
TEST (node, fork_open_flip)
{
	...
	oslo::keypair rep1;
	oslo::keypair rep2;
	auto send1 (std::make_shared<oslo::send_block> (genesis.hash (), key1.pub, oslo::genesis_amount - 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	// A copy is necessary to avoid data races during ledger processing, which sets the sideband
	auto send1_copy (std::make_shared<oslo::send_block> (*send1));
	node1.process_active (send1);
	node2.process_active (send1_copy);
	// We should be keeping this block
	auto open1 (std::make_shared<oslo::open_block> (send1->hash (), rep1.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	// This block should be evicted
	auto open2 (std::make_shared<oslo::open_block> (send1->hash (), rep2.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	...
	ASSERT_EQ (oslo::genesis_amount - 1, winner.first);
...
TEST (node, coherent_observer)
{
	oslo::system system (1);
	auto & node1 (*system.nodes[0]);
	node1.observers.blocks.add ([&node1](oslo::election_status const & status_a, oslo::account const &, oslo::uint128_t const &, bool) {
		auto transaction (node1.store.tx_begin_read ());
		ASSERT_TRUE (node1.store.block_exists (transaction, status_a.winner->hash ()));
	});
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key;
	system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, 1);
}
...
TEST (node, fork_no_vote_quorum)
{
	...
	system.wallet (0)->send_action (oslo::test_genesis_key.pub, key4, oslo::genesis_amount / 4);
	...
	auto block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key1, node1.config.receive_minimum.number ()));
	...
	oslo::state_block send1 (oslo::test_genesis_key.pub, block->hash (), oslo::test_genesis_key.pub, (oslo::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), key1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (block->hash ()));
	ASSERT_EQ (oslo::process_result::progress, node1.process (send1).code);
	ASSERT_EQ (oslo::process_result::progress, node2.process (send1).code);
	ASSERT_EQ (oslo::process_result::progress, node3.process (send1).code);
	auto key2 (system.wallet (2)->deterministic_insert ());
	auto send2 (std::make_shared<oslo::send_block> (block->hash (), key2, (oslo::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (block->hash ())));
	oslo::raw_key key3;
	auto transaction (system.wallet (1)->wallets.tx_begin_read ());
	ASSERT_FALSE (system.wallet (1)->store.fetch (transaction, key1, key3));
	auto vote (std::make_shared<oslo::vote> (key1, key3, 0, send2));
	oslo::confirm_ack confirm (vote);
	std::vector<uint8_t> buffer;
	{
		oslo::vectorstream stream (buffer);
		confirm.serialize (stream, false);
	}
	oslo::transport::channel_udp channel (node2.network.udp_channels, node3.network.endpoint (), node1.network_params.protocol.protocol_version);
	channel.send_buffer (oslo::shared_const_buffer (std::move (buffer)), oslo::stat::detail::confirm_ack);
	while (node3.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::in) < 3)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_TRUE (node1.latest (oslo::test_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node2.latest (oslo::test_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node3.latest (oslo::test_genesis_key.pub) == send1.hash ());
}
...
TEST (node, DISABLED_fork_pre_confirm)
{
	...
	system.deadline_set (30s);
	auto block0 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key1.pub, oslo::genesis_amount / 3));
	ASSERT_NE (nullptr, block0);
	while (node0.balance (key1.pub) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto block1 (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, oslo::genesis_amount / 3));
	...
	oslo::keypair key3;
	oslo::keypair key4;
	auto block2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, node0.latest (oslo::test_genesis_key.pub), key3.pub, node0.balance (oslo::test_genesis_key.pub), 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	auto block3 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, node0.latest (oslo::test_genesis_key.pub), key4.pub, node0.balance (oslo::test_genesis_key.pub), 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	...
	while (!done)
	{
		done |= node0.latest (oslo::test_genesis_key.pub) == block2->hash () && node1.latest (oslo::test_genesis_key.pub) == block2->hash () && node2.latest (oslo::test_genesis_key.pub) == block2->hash ();
		done |= node0.latest (oslo::test_genesis_key.pub) == block3->hash () && node1.latest (oslo::test_genesis_key.pub) == block3->hash () && node2.latest (oslo::test_genesis_key.pub) == block3->hash ();
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
{
	oslo::system system1 (1);
	system1.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::system system2 (1);
	auto & node1 (*system1.nodes[0]);
	auto & node2 (*system2.nodes[0]);
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	std::shared_ptr<oslo::transport::channel> channel (std::make_shared<oslo::transport::channel_udp> (node2.network.udp_channels, node1.network.endpoint (), node2.network_params.protocol.protocol_version));
	auto vote = std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, std::vector<oslo::block_hash> ());
	node2.rep_crawler.response (channel, vote);
	oslo::genesis genesis;
	oslo::keypair key1;
	oslo::keypair key2;
	auto send3 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Mxrb_ratio, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	...
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send3->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Mxrb_ratio, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send3->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Mxrb_ratio, key2.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	{
		auto transaction1 (node1.store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node1.ledger.process (transaction1, *send1).code);
		auto transaction2 (node2.store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node2.ledger.process (transaction2, *send2).code);
	}
	node1.process_active (send1);
	node1.process_active (send2);
	node2.process_active (send1);
...
TEST (node, broadcast_elected)
{
	...
	for (auto & type : types)
	{
		...
		node_config.peering_port = oslo::get_available_port ();
		auto node2 = system.add_node (node_config, node_flags, type);
		oslo::keypair rep_big;
		oslo::keypair rep_small;
		oslo::keypair rep_other;
		{
			auto transaction0 (node0->store.tx_begin_write ());
			auto transaction1 (node1->store.tx_begin_write ());
			auto transaction2 (node2->store.tx_begin_write ());
			oslo::send_block fund_big (node0->ledger.latest (transaction0, oslo::test_genesis_key.pub), rep_big.pub, oslo::Gxrb_ratio * 5, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
			oslo::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
			oslo::send_block fund_small (fund_big.hash (), rep_small.pub, oslo::Gxrb_ratio * 2, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
			oslo::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
			oslo::send_block fund_other (fund_small.hash (), rep_other.pub, oslo::Gxrb_ratio * 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
			oslo::open_block open_other (fund_other.hash (), rep_other.pub, rep_other.pub, rep_other.prv, rep_other.pub, 0);
			...
			ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
			ASSERT_EQ (oslo::process_result::progress, node1->ledger.process (transaction1, fund_big).code);
			ASSERT_EQ (oslo::process_result::progress, node2->ledger.process (transaction2, fund_big).code);
			ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction0, open_big).code);
			ASSERT_EQ (oslo::process_result::progress, node1->ledger.process (transaction1, open_big).code);
			ASSERT_EQ (oslo::process_result::progress, node2->ledger.process (transaction2, open_big).code);
			ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction0, fund_small).code);
			ASSERT_EQ (oslo::process_result::progress, node1->ledger.process (transaction1, fund_small).code);
			ASSERT_EQ (oslo::process_result::progress, node2->ledger.process (transaction2, fund_small).code);
			ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction0, open_small).code);
			ASSERT_EQ (oslo::process_result::progress, node1->ledger.process (transaction1, open_small).code);
			ASSERT_EQ (oslo::process_result::progress, node2->ledger.process (transaction2, open_small).code);
			ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction0, fund_other).code);
			ASSERT_EQ (oslo::process_result::progress, node1->ledger.process (transaction1, fund_other).code);
			ASSERT_EQ (oslo::process_result::progress, node2->ledger.process (transaction2, fund_other).code);
			ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction0, open_other).code);
			ASSERT_EQ (oslo::process_result::progress, node1->ledger.process (transaction1, open_other).code);
			ASSERT_EQ (oslo::process_result::progress, node2->ledger.process (transaction2, open_other).code);
		}
		// Confirm blocks to allow voting
		for (auto & node : system.nodes)
		{
			auto block (node->block (node->latest (oslo::test_genesis_key.pub)));
			ASSERT_NE (nullptr, block);
		...
		auto fork0 (std::make_shared<oslo::send_block> (node2->latest (oslo::test_genesis_key.pub), rep_small.pub, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
		node0->work_generate_blocking (*fork0);
		// A copy is necessary to avoid data races during ledger processing, which sets the sideband
		auto fork0_copy (std::make_shared<oslo::send_block> (*fork0));
		node0->process_active (fork0);
		node1->process_active (fork0_copy);
		auto fork1 (std::make_shared<oslo::send_block> (node2->latest (oslo::test_genesis_key.pub), rep_big.pub, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
		...
		system.deadline_set (5s);
		while (node1->stats.count (oslo::stat::type::confirmation_observer, oslo::stat::detail::inactive_conf_height, oslo::stat::dir::out) == 0)
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	}
...
TEST (node, rep_self_vote)
{
	...
	oslo::keypair rep_big;
	oslo::send_block fund_big (node0->ledger.latest (node0->store.tx_begin_read (), oslo::test_genesis_key.pub), rep_big.pub, oslo::uint128_t ("0xb0000000000000000000000000000000"), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
	oslo::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
	node0->work_generate_blocking (fund_big);
	node0->work_generate_blocking (open_big);
	ASSERT_EQ (oslo::process_result::progress, node0->process (fund_big).code);
	ASSERT_EQ (oslo::process_result::progress, node0->process (open_big).code);
	...
	node0->block_confirm (node0->block (open_big.hash ()));
	{
		...
		ASSERT_NE (nullptr, election);
		oslo::lock_guard<std::mutex> guard (node0->active.mutex);
		election->confirm_once ();
	}
	...
	auto block0 (std::make_shared<oslo::send_block> (node0->latest (oslo::test_genesis_key.pub), rep_big.pub, oslo::uint128_t ("0x60000000000000000000000000000000"), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node0->work_generate_blocking (*block0);
	ASSERT_EQ (oslo::process_result::progress, node0->process (*block0).code);
	...
	oslo::unique_lock<std::mutex> lock (active.mutex);
	auto & rep_votes (election1.election->last_votes);
	ASSERT_NE (rep_votes.end (), rep_votes.find (oslo::test_genesis_key.pub));
...
{
	oslo::system system0 (1);
	oslo::system system1 (1);
	auto node0 (system0.nodes[0]);
	auto node1 (system1.nodes[0]);
	oslo::keypair key0;
	// node0 knows about send0 but node1 doesn't.
	oslo::send_block send0 (node0->latest (oslo::test_genesis_key.pub), key0.pub, 500, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node0->ledger.process (transaction, send0).code);
	}
...
TEST (node, bootstrap_fork_open)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key0;
	oslo::send_block send0 (node0->latest (oslo::test_genesis_key.pub), key0.pub, oslo::genesis_amount - 500, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
	oslo::open_block open0 (send0.hash (), 1, key0.pub, key0.prv, key0.pub, 0);
	oslo::open_block open1 (send0.hash (), 2, key0.pub, key0.prv, key0.pub, 0);
	node0->work_generate_blocking (send0);
	node0->work_generate_blocking (open0);
	node0->work_generate_blocking (open1);
	// Both know about send0
	ASSERT_EQ (oslo::process_result::progress, node0->process (send0).code);
	ASSERT_EQ (oslo::process_result::progress, node1->process (send0).code);
	// Confirm send0 to allow starting and voting on the following blocks
	for (auto node : system.nodes)
	{
		node->block_confirm (node->block (node->latest (oslo::test_genesis_key.pub)));
		{
	...
	ASSERT_EQ (oslo::process_result::progress, node0->process (open0).code);
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	wallet0->insert_adhoc (oslo::test_genesis_key.prv);
	auto send1 (wallet0->send_action (oslo::genesis_account, key0.pub, 2 * oslo::Mxrb_ratio));
	system.deadline_set (10s);
	while (node1.balance (key0.pub) != 2 * oslo::Mxrb_ratio || node1.bootstrap_initiator.in_progress ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto latest (node1.latest (key0.pub));
	oslo::state_block send2 (key0.pub, latest, oslo::genesis_account, oslo::Mxrb_ratio, oslo::genesis_account, key0.prv, key0.pub, *node0.work_generate_blocking (latest));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node1.ledger.process (transaction, send2).code);
	}
	auto send3 (wallet1->send_action (key0.pub, oslo::genesis_account, oslo::Mxrb_ratio));
	system.deadline_set (10s);
	while (node0.balance (oslo::genesis_account) != oslo::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (oslo::test_genesis_key.pub, key1.pub, oslo::Mxrb_ratio);
...
TEST (node, rep_weight)
{
	...
	auto & node (*system.nodes[0]);
	oslo::genesis genesis;
	oslo::keypair keypair1;
	oslo::keypair keypair2;
	...
	std::shared_ptr<oslo::block> block2 = builder
	                                      ...
	                                      .build ();
	std::shared_ptr<oslo::block> block3 = builder
	                                      .state ()
	                                      .account (oslo::test_genesis_key.pub)
	                                      .previous (block1->hash ())
	                                      .representative (oslo::test_genesis_key.pub)
	                                      .balance (oslo::genesis_amount - amount_not_pr - amount_pr)
	                                      .link (keypair2.pub)
	                                      .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                                      .work (*system.work.generate (block1->hash ()))
	                                      .build ();
	std::shared_ptr<oslo::block> block4 = builder
	                                      .state ()
	                                      ...
	                                      .build ();
	{
		...
		ASSERT_EQ (oslo::process_result::progress, node.ledger.process (transaction, *block1).code);
		ASSERT_EQ (oslo::process_result::progress, node.ledger.process (transaction, *block2).code);
		ASSERT_EQ (oslo::process_result::progress, node.ledger.process (transaction, *block3).code);
		ASSERT_EQ (oslo::process_result::progress, node.ledger.process (transaction, *block4).code);
	}
	node.network.udp_channels.insert (oslo::endpoint (boost::asio::ip::address_v6::loopback (), oslo::get_available_port ()), 0);
	ASSERT_TRUE (node.rep_crawler.representatives (1).empty ());
	oslo::endpoint endpoint0 (boost::asio::ip::address_v6::loopback (), oslo::get_available_port ());
	oslo::endpoint endpoint1 (boost::asio::ip::address_v6::loopback (), oslo::get_available_port ());
	oslo::endpoint endpoint2 (boost::asio::ip::address_v6::loopback (), oslo::get_available_port ());
	std::shared_ptr<oslo::transport::channel> channel0 (std::make_shared<oslo::transport::channel_udp> (node.network.udp_channels, endpoint0, node.network_params.protocol.protocol_version));
	std::shared_ptr<oslo::transport::channel> channel1 (std::make_shared<oslo::transport::channel_udp> (node.network.udp_channels, endpoint1, node.network_params.protocol.protocol_version));
	std::shared_ptr<oslo::transport::channel> channel2 (std::make_shared<oslo::transport::channel_udp> (node.network.udp_channels, endpoint2, node.network_params.protocol.protocol_version));
	...
	auto vote0 = std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, genesis.open);
	auto vote1 = std::make_shared<oslo::vote> (keypair1.pub, keypair1.prv, 0, genesis.open);
	auto vote2 = std::make_shared<oslo::vote> (keypair2.pub, keypair2.prv, 0, genesis.open);
	...
	ASSERT_EQ (node.balance (oslo::test_genesis_key.pub), reps[0].weight.number ());
	ASSERT_EQ (oslo::test_genesis_key.pub, reps[0].account);
...
TEST (node, rep_remove)
{
	...
	std::shared_ptr<oslo::block> block1 = builder
	                                      ...
	                                      .previous (genesis.hash ())
	                                      .representative (oslo::test_genesis_key.pub)
	                                      .balance (oslo::genesis_amount - node.minimum_principal_weight () * 2)
	                                      .link (keypair1.pub)
	                                      .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                                      .work (*system.work.generate (genesis.hash ()))
	                                      .build ();
	...
	std::shared_ptr<oslo::block> block3 = builder
	                                      ...
	                                      .previous (block1->hash ())
	                                      .representative (oslo::test_genesis_key.pub)
	                                      .balance (oslo::genesis_amount - node.minimum_principal_weight () * 4)
	                                      .link (keypair2.pub)
	                                      .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                                      .work (*system.work.generate (block1->hash ()))
	                                      .build ();
	...
	auto node1 = system.add_node (oslo::node_config (oslo::get_available_port (), system.logging));
	...
	auto vote2 = std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, genesis.open);
	...
	auto vote3 = std::make_shared<oslo::vote> (keypair2.pub, keypair2.prv, 0, genesis.open);
	node.network.tcp_channels.start_tcp (node2->network.endpoint (), [node_w, &done, &vote3, &system](std::shared_ptr<oslo::transport::channel> channel2) {
...
TEST (node, no_voting)
{
	...
	auto wallet1 (system.wallet (1));
	// Node1 has a rep
	wallet1->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key1;
	wallet1->insert_adhoc (key1.prv);
	// Broadcast a confirm so others should know this is a rep node
	wallet1->send_action (oslo::test_genesis_key.pub, key1.pub, oslo::Mxrb_ratio);
	system.deadline_set (10s);
	while (!node0.active.empty ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (0, node0.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::in));
}
...
TEST (node, send_callback)
{
	...
	ASSERT_EQ (std::numeric_limits<oslo::uint128_t>::max () - node0.config.receive_minimum.number (), node0.balance (oslo::test_genesis_key.pub));
}
...
TEST (node, vote_replay)
{
	...
	{
		auto transaction (node1.store.tx_begin_read ());
		auto vote (node1.store.vote_generate (transaction, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, genesis.open));
	}
	auto node2 = system.add_node ();
	{
		auto transaction (node2->store.tx_begin_read ());
		oslo::lock_guard<std::mutex> lock (node2->store.get_cache_mutex ());
		auto vote (node2->store.vote_current (transaction, oslo::test_genesis_key.pub));
...
TEST (node, balance_observer)
{
	...
	oslo::keypair key;
	node1.observers.account_balance.add ([&key, &balances](oslo::account const & account_a, bool is_pending) {
		if (key.pub == account_a && is_pending)
		{
			balances++;
		}
		else if (oslo::test_genesis_key.pub == account_a && !is_pending)
		{
			balances++;
		}
	});
...
TEST (node, stat_counting)
{
	oslo::system system (1);
	auto & node1 (*system.nodes[0]);
	node1.stats.add (oslo::stat::type::ledger, oslo::stat::dir::in, 1);
	node1.stats.add (oslo::stat::type::ledger, oslo::stat::dir::in, 5);
	node1.stats.inc (oslo::stat::type::ledger, oslo::stat::dir::in);
	node1.stats.inc (oslo::stat::type::ledger, oslo::stat::detail::send, oslo::stat::dir::in);
	node1.stats.inc (oslo::stat::type::ledger, oslo::stat::detail::send, oslo::stat::dir::in);
	node1.stats.inc (oslo::stat::type::ledger, oslo::stat::detail::receive, oslo::stat::dir::in);
	ASSERT_EQ (10, node1.stats.count (oslo::stat::type::ledger, oslo::stat::dir::in));
	ASSERT_EQ (2, node1.stats.count (oslo::stat::type::ledger, oslo::stat::detail::send, oslo::stat::dir::in));
	ASSERT_EQ (1, node1.stats.count (oslo::stat::type::ledger, oslo::stat::detail::receive, oslo::stat::dir::in));
}
...
TEST (node, online_reps)
{
	...
	auto vote (std::make_shared<oslo::vote> ());
	node1.online_reps.observe (oslo::test_genesis_key.pub);
	// 1 minimum, 1 maximum
	node1.online_reps.sample ();
	ASSERT_EQ (oslo::genesis_amount, node1.online_reps.online_stake ());
	// 2 minimum, 1 maximum
	node1.online_reps.sample ();
	ASSERT_EQ (node1.config.online_weight_minimum, node1.online_reps.online_stake ());
}
...
TEST (node, block_confirm)
{
	...
	for (auto & type : types)
	{
		...
		oslo::keypair key;
		system.wallet (1)->insert_adhoc (oslo::test_genesis_key.prv);
		auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (genesis.hash ())));
		// A copy is necessary to avoid data races during ledger processing, which sets the sideband
		auto send1_copy (std::make_shared<oslo::state_block> (*send1));
		node1.block_processor.add (send1, oslo::seconds_since_epoch ());
		node2.block_processor.add (send1_copy, oslo::seconds_since_epoch ());
		...
		auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send1->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio * 2, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (send1->hash ())));
		{
			auto transaction (node1.store.tx_begin_write ());
			ASSERT_EQ (oslo::process_result::progress, node1.ledger.process (transaction, *send2).code);
		}
		{
			...
			ASSERT_EQ (oslo::process_result::progress, node2.ledger.process (transaction, *send2).code);
		}
...
TEST (node, block_arrival_size)
{
	oslo::system system (1);
	auto & node (*system.nodes[0]);
	auto time (std::chrono::steady_clock::now () - oslo::block_arrival::arrival_time_min - std::chrono::seconds (5));
	oslo::block_hash hash (0);
	for (auto i (0); i < oslo::block_arrival::arrival_size_min * 2; ++i)
	{
		node.block_arrival.arrival.push_back (oslo::block_arrival_info{ time, hash });
		++hash.qwords[0];
	}
	ASSERT_EQ (oslo::block_arrival::arrival_size_min * 2, node.block_arrival.arrival.size ());
	node.block_arrival.recent (0);
	ASSERT_EQ (oslo::block_arrival::arrival_size_min, node.block_arrival.arrival.size ());
}
...
TEST (node, confirm_quorum)
{
	...
	oslo::amount new_balance (node1.config.online_weight_minimum.number () - oslo::Gxrb_ratio);
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, new_balance, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1.work_generate_blocking (genesis.hash ())));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node1.ledger.process (transaction, *send1).code);
	}
	system.wallet (0)->send_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, new_balance.number ());
	system.deadline_set (10s);
	while (node1.active.empty ())
	{
		ASSERT_NO_ERROR (system.poll ());
	...
	oslo::lock_guard<std::mutex> guard (node1.active.mutex);
	auto info (node1.active.roots.find (oslo::qualified_root (send1->hash (), send1->hash ())));
	ASSERT_NE (node1.active.roots.end (), info);
	ASSERT_FALSE (info->election->confirmed ());
	ASSERT_EQ (1, info->election->last_votes.size ());
	ASSERT_EQ (0, node1.balance (oslo::test_genesis_key.pub));
}
...
TEST (node, local_votes_cache)
{
	...
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.frontiers_confirmation = oslo::frontiers_confirmation_mode::disabled;
	node_config.receive_minimum = oslo::genesis_amount;
	auto & node (*system.add_node (node_config));
	oslo::genesis genesis;
	...
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send1->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node.work_generate_blocking (send1->hash ())));
	auto send3 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send2->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 3 * oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node.work_generate_blocking (send2->hash ())));
	{
		auto transaction (node.store.tx_begin_write ());
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::confirm_req message1 (send1);
	oslo::confirm_req message2 (send2);
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	node.network.process_message (message1, channel);
	auto wait_vote_sequence = [&node, &system](unsigned sequence) {
		std::shared_ptr<oslo::vote> current_vote;
		system.deadline_set (5s);
		while (current_vote == nullptr || current_vote->sequence < sequence)
		{
			{
				oslo::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
				auto transaction (node.store.tx_begin_read ());
				current_vote = node.store.vote_current (transaction, oslo::test_genesis_key.pub);
			}
			ASSERT_NO_ERROR (system.poll ());
		}
	...
	{
		oslo::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
		ASSERT_EQ (2, node.store.vote_current (node.store.tx_begin_read (), oslo::test_genesis_key.pub)->sequence);
	}
	// Max cache
	{
		auto transaction (node.store.tx_begin_write ());
	...
	oslo::confirm_req message3 (send3);
	for (auto i (0); i < 100; ++i)
	{
...
TEST (node, local_votes_cache_batch)
{
	...
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node.work_generate_blocking (genesis.hash ())));
	std::vector<std::shared_ptr<oslo::block>> blocks{ genesis.open, send1 };
	std::vector<std::pair<oslo::block_hash, oslo::root>> batch{ { genesis.open->hash (), genesis.open->root () }, { send1->hash (), send1->root () } };
	{
		auto transaction (node.store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node.ledger.process (transaction, *send1).code);
	}
	oslo::confirm_req message (batch);
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	// Generates and sends one vote for both hashes which is then cached
	node.network.process_message (message, channel);
	system.deadline_set (3s);
	while (node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out) < 1)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (1, node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out));
	...
	node.network.process_message (message, channel);
	system.deadline_set (3s);
	while (node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out) < 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (2, node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out));
	// Test when votes are different
	node.votes_cache.remove (genesis.open->hash ());
	node.votes_cache.remove (send1->hash ());
	node.network.process_message (oslo::confirm_req (genesis.open->hash (), genesis.open->root ()), channel);
	system.deadline_set (3s);
	while (node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out) < 3)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (3, node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out));
	node.network.process_message (oslo::confirm_req (send1->hash (), send1->root ()), channel);
	system.deadline_set (3s);
	while (node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out) < 4)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (4, node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out));
	// There are two different votes, so both should be sent in response
	node.network.process_message (message, channel);
	system.deadline_set (3s);
	while (node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out) < 6)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (6, node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out));
}
...
TEST (node, local_votes_cache_generate_new_vote)
{
	...
	oslo::confirm_req message1 (genesis.open);
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	node.network.process_message (message1, channel);
	system.deadline_set (3s);
	...
	ASSERT_EQ (genesis.open->hash (), boost::get<oslo::block_hash> (votes1[0]->blocks[0]));
	{
		oslo::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
		auto transaction (node.store.tx_begin_read ());
		auto current_vote (node.store.vote_current (transaction, oslo::test_genesis_key.pub));
	...
	std::vector<std::pair<oslo::block_hash, oslo::root>> roots_hashes{ std::make_pair (genesis.open->hash (), genesis.open->root ()), std::make_pair (send1->hash (), send1->root ()) };
	oslo::confirm_req message2 (roots_hashes);
	node.network.process_message (message2, channel);
	system.deadline_set (3s);
...
TEST (node, local_votes_cache_size)
{
	...
	wallet.insert_adhoc (oslo::keypair ().prv);
	ASSERT_EQ (2, node.wallets.reps ().voting);
	auto transaction (node.store.tx_begin_read ());
	auto vote1 (node.store.vote_generate (transaction, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, { oslo::genesis_hash }));
	oslo::block_hash hash (1);
	auto vote2 (node.store.vote_generate (transaction, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, { hash }));
	...
	ASSERT_EQ (0, node.votes_cache.find (oslo::genesis_hash).size ());
}
...
TEST (node, vote_republish)
{
	...
	system.wallet (1)->insert_adhoc (key2.prv);
	oslo::genesis genesis;
	auto send1 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number () * 2, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	system.deadline_set (5s);
	...
	auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, send2));
	ASSERT_TRUE (node1.active.active (*send1));
	ASSERT_TRUE (node2.active.active (*send1));
	node1.vote_processor.vote (vote, std::make_shared<oslo::transport::channel_udp> (node1.network.udp_channels, node1.network.endpoint (), node1.network_params.protocol.protocol_version));
...
TEST (node, vote_by_hash_bundle)
{
	...
	system.nodes[0]->observers.vote.add ([&max_hashes](std::shared_ptr<oslo::vote> vote_a, std::shared_ptr<oslo::transport::channel>, oslo::vote_code) {
	...
	for (int i = 1; i <= 200; i++)
	{
		system.nodes[0]->active.generator.add (oslo::genesis_hash);
	}
...
TEST (node, vote_by_hash_republish)
{
	...
	for (auto & type : types)
	{
		...
		auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, vote_blocks));
		ASSERT_TRUE (node1.active.active (*send1));
		ASSERT_TRUE (node2.active.active (*send1));
		node1.vote_processor.vote (vote, std::make_shared<oslo::transport::channel_udp> (node1.network.udp_channels, node1.network.endpoint (), node1.network_params.protocol.protocol_version));
		while (!node1.block (send2->hash ()))
...
TEST (node, vote_by_hash_epoch_block_republish)
{
	...
	oslo::genesis genesis;
	auto send1 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto epoch1 (std::make_shared<oslo::state_block> (oslo::genesis_account, genesis.hash (), oslo::genesis_account, oslo::genesis_amount, node1.ledger.epoch_link (oslo::epoch::epoch_1), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	system.deadline_set (5s);
	while (!node2.block (send1->hash ()))
	{
...
TEST (node, epoch_conflict_confirm)
{
	...
	oslo::keypair key;
	oslo::genesis genesis;
	oslo::keypair epoch_signer (oslo::test_genesis_key);
	auto send (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 1, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto open (std::make_shared<oslo::state_block> (key.pub, 0, key.pub, 1, send->hash (), key.prv, key.pub, *system.work.generate (key.pub)));
	auto change (std::make_shared<oslo::state_block> (key.pub, open->hash (), key.pub, 1, 0, key.prv, key.pub, *system.work.generate (open->hash ())));
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2, open->hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send->hash ())));
	auto epoch_open (std::make_shared<oslo::state_block> (change->root (), 0, 0, 0, node0->ledger.epoch_link (oslo::epoch::epoch_1), epoch_signer.prv, epoch_signer.pub, *system.work.generate (open->hash ())));
	...
	ASSERT_EQ (oslo::process_result::progress, node1->process (*open).code);
	...
	{
		oslo::block_post_events events;
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node0->block_processor.process_one (transaction, events, send).code);
		ASSERT_EQ (oslo::process_result::progress, node0->block_processor.process_one (transaction, events, send2).code);
		ASSERT_EQ (oslo::process_result::progress, node0->block_processor.process_one (transaction, events, open).code);
	}
	...
	oslo::blocks_confirm (*node0, { change, epoch_open });
	ASSERT_EQ (2, node0->active.size ());
	{
		oslo::lock_guard<std::mutex> lock (node0->active.mutex);
...
TEST (node, fork_invalid_block_signature)
{
	...
	auto send1 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<oslo::send_block> (genesis.hash (), key2.pub, std::numeric_limits<oslo::uint128_t>::max () - node1.config.receive_minimum.number () * 2, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto send2_corrupt (std::make_shared<oslo::send_block> (*send2));
	send2_corrupt->signature = oslo::signature (123);
	auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, send2));
	auto vote_corrupt (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, send2_corrupt));
	...
	ASSERT_TIMELY (5s, node1.stats.count (oslo::stat::type::rollback, oslo::stat::detail::all));
...
TEST (node, fork_election_invalid_block_signature)
{
	...
	oslo::genesis genesis;
	oslo::block_builder builder;
	std::shared_ptr<oslo::block> send1 = builder.state ()
	                                     .account (oslo::test_genesis_key.pub)
	                                     .previous (genesis.hash ())
	                                     .representative (oslo::test_genesis_key.pub)
	                                     ...
	                                     .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                                     .build ();
	std::shared_ptr<oslo::block> send2 = builder.state ()
	                                     .account (oslo::test_genesis_key.pub)
	                                     .previous (genesis.hash ())
	                                     .representative (oslo::test_genesis_key.pub)
	                                     ...
	                                     .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                                     .build ();
	std::shared_ptr<oslo::block> send3 = builder.state ()
	                                     .account (oslo::test_genesis_key.pub)
	                                     .previous (genesis.hash ())
	                                     .representative (oslo::test_genesis_key.pub)
	                                     .balance (oslo::genesis_amount - 2 * oslo::Gxrb_ratio)
	                                     .link (oslo::test_genesis_key.pub)
	                                     .work (*system.work.generate (genesis.hash ()))
	                                     .sign (oslo::test_genesis_key.prv, 0) // Invalid signature
	                                     .build ();
	auto channel1 (node1.network.udp_channels.create (node1.network.endpoint ()));
	node1.network.process_message (oslo::publish (send1), channel1);
	system.deadline_set (5s);
	...
	lock.unlock ();
	node1.network.process_message (oslo::publish (send3), channel1);
	node1.network.process_message (oslo::publish (send2), channel1);
	lock.lock ();
...
TEST (node, block_processor_signatures)
{
	...
	auto & node1 (*system0.nodes[0]);
	system0.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::block_hash latest (system0.nodes[0]->latest (oslo::test_genesis_key.pub));
	oslo::keypair key1;
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, latest, oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	oslo::keypair key2;
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send1->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Gxrb_ratio, key2.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	oslo::keypair key3;
	auto send3 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send2->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 3 * oslo::Gxrb_ratio, key3.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send3);
	// Invalid signature bit
	auto send4 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send3->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 4 * oslo::Gxrb_ratio, key3.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send4);
	send4->signature.bytes[32] ^= 0x1;
	// Invalid signature bit (force)
	auto send5 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send3->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 5 * oslo::Gxrb_ratio, key3.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	...
	auto receive1 (std::make_shared<oslo::state_block> (key1.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, send1->hash (), key1.prv, key1.pub, 0));
	node1.work_generate_blocking (*receive1);
	auto receive2 (std::make_shared<oslo::state_block> (key2.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, send2->hash (), key2.prv, key2.pub, 0));
	node1.work_generate_blocking (*receive2);
	// Invalid private key
	auto receive3 (std::make_shared<oslo::state_block> (key3.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, send3->hash (), key2.prv, key3.pub, 0));
...
TEST (node, block_processor_reject_state)
{
	...
	auto & node (*system.nodes[0]);
	oslo::genesis genesis;
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	...
	ASSERT_FALSE (node.ledger.block_exists (send1->hash ()));
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
...
TEST (node, block_processor_full)
{
	...
	auto & node = *system.add_node (oslo::node_config (oslo::get_available_port (), system.logging), node_flags);
	oslo::genesis genesis;
	auto send1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node.work_generate_blocking (*send1);
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send1->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	node.work_generate_blocking (*send2);
	auto send3 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send2->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 3 * oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
...
TEST (node, confirm_back)
{
	...
	auto & node (*system.nodes[0]);
	oslo::genesis genesis;
	auto genesis_start_balance (node.balance (oslo::test_genesis_key.pub));
	auto send1 (std::make_shared<oslo::send_block> (genesis.hash (), key.pub, genesis_start_balance - 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto open (std::make_shared<oslo::state_block> (key.pub, 0, key.pub, 1, send1->hash (), key.prv, key.pub, *system.work.generate (key.pub)));
	auto send2 (std::make_shared<oslo::state_block> (key.pub, open->hash (), key.pub, 0, oslo::test_genesis_key.pub, key.prv, key.pub, *system.work.generate (open->hash ())));
	node.process_active (send1);
	node.process_active (open);
	node.process_active (send2);
	oslo::blocks_confirm (node, { send1, open, send2 });
	ASSERT_EQ (3, node.active.size ());
	std::vector<oslo::block_hash> vote_blocks;
	vote_blocks.push_back (send2->hash ());
	auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, vote_blocks));
	node.vote_processor.vote_blocking (vote, std::make_shared<oslo::transport::channel_udp> (node.network.udp_channels, node.network.endpoint (), node.network_params.protocol.protocol_version));
	system.deadline_set (10s);
	while (!node.active.empty ())
	{
		ASSERT_NO_ERROR (system.poll ());
...
TEST (node, peers)
{
	...
	auto node2 (std::make_shared<oslo::node> (system.io_ctx, oslo::get_available_port (), oslo::unique_path (), system.alarm, system.logging, system.work));
	system.nodes.push_back (node2);

	auto endpoint = node1->network.endpoint ();
	oslo::endpoint_key endpoint_key{ endpoint.address ().to_v6 ().to_bytes (), endpoint.port () };
	auto & store = node2->store;
	{
		...
		store.peer_put (transaction, oslo::endpoint_key{ boost::asio::ip::address_v6::any ().to_bytes (), 55555 });
	}
...
TEST (node, peer_cache_restart)
{
	...
	auto path (oslo::unique_path ());
	{
		auto node2 (std::make_shared<oslo::node> (system.io_ctx, oslo::get_available_port (), path, system.alarm, system.logging, system.work));
		system.nodes.push_back (node2);
		auto & store = node2->store;
		{
			// Add a peer to the database
	...
	// Restart node
	{
		...
		auto node3 (std::make_shared<oslo::node> (system.io_ctx, oslo::get_available_port (), path, system.alarm, system.logging, system.work, node_flags));
...
TEST (node, unchecked_cleanup)
{
	...
	oslo::keypair key;
	auto & node (*system.nodes[0]);
	auto open (std::make_shared<oslo::state_block> (key.pub, 0, key.pub, 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
...
TEST (node, dont_write_lock_node)
{
	...
	std::thread ([&path, &write_lock_held_promise, &finished_promise]() {
		...
		auto store = oslo::make_store (logger, path, false, true);
		{
			oslo::genesis genesis;
			oslo::ledger_cache ledger_cache;
			auto transaction (store->tx_begin_write ());
...
TEST (node, bidirectional_tcp)
{
	...
	auto send2 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, send1->hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - 2 * oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *node1->work_generate_blocking (send1->hash ())));
	node2->process_active (send2);
	node2->block_processor.flush ();
	system.deadline_set (5s);
...
TEST (node, aggressive_flooding)
{
	...
	wallet1.insert_adhoc (oslo::test_genesis_key.prv);
	std::vector<std::pair<std::shared_ptr<oslo::node>, std::shared_ptr<oslo::wallet>>> nodes_wallets;
	bool const sanitizer_or_valgrind (is_sanitizer_build || oslo::running_within_valgrind ());
	...
	auto large_amount = (oslo::genesis_amount / 2) / nodes_wallets.size ();
	std::vector<std::shared_ptr<oslo::block>> genesis_blocks;
	for (auto & node_wallet : nodes_wallets)
	{
		...
		auto block (wallet1.send_action (oslo::test_genesis_key.pub, keypair.pub, large_amount));
	...
	for (auto & node_wallet : nodes_wallets)
	{
		...
		ASSERT_EQ (node1.latest (oslo::test_genesis_key.pub), node_wallet.first->latest (oslo::test_genesis_key.pub));
	}
	...
	oslo::block_builder builder;
	std::shared_ptr<oslo::state_block> block{};
	{
		auto transaction (node1.store.tx_begin_read ());
		block = builder.state ()
		        .account (oslo::test_genesis_key.pub)
		        .representative (oslo::test_genesis_key.pub)
		        .previous (node1.ledger.latest (transaction, oslo::test_genesis_key.pub))
		        .balance (node1.ledger.account_balance (transaction, oslo::test_genesis_key.pub) - 1)
		        .link (oslo::test_genesis_key.pub)
		        .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
		        .work (*node1.work_generate_blocking (node1.ledger.latest (transaction, oslo::test_genesis_key.pub)))
		        .build ();
	}
	// Processing locally goes through the aggressive block flooding path
	node1.process_local (block, false);

	auto all_have_block = [&nodes_wallets](oslo::block_hash const & hash_a) {
	...
	auto wallet_block = wallet1.send_sync (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, 10);
...
TEST (node, max_work_generate_difficulty)
{
	...
	auto & node = *system.add_node (node_config);
	auto initial_difficulty = node.default_difficulty (oslo::work_version::work_1);
	ASSERT_EQ (node.max_work_generate_difficulty (oslo::work_version::work_1), oslo::difficulty::from_multiplier (node.config.max_work_generate_multiplier, initial_difficulty));
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (node, oslo::epoch::epoch_1));
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (node, oslo::epoch::epoch_2));
	auto final_difficulty = node.default_difficulty (oslo::work_version::work_1);
	ASSERT_NE (final_difficulty, initial_difficulty);
	ASSERT_EQ (node.max_work_generate_difficulty (oslo::work_version::work_1), oslo::difficulty::from_multiplier (node.config.max_work_generate_multiplier, final_difficulty));
}
...
TEST (active_difficulty, recalculate_work)
{
	...
	node1.work_generate_blocking (*send1);
	auto multiplier1 = oslo::difficulty::to_multiplier (send1->difficulty (), node1.network_params.network.publish_thresholds.epoch_1);
	...
	ASSERT_EQ (node1.active.active_difficulty (), oslo::difficulty::from_multiplier (sum / node1.active.multipliers_cb.size (), node1.network_params.network.publish_thresholds.epoch_1));
	oslo::unique_lock<std::mutex> lock (node1.active.mutex);
...
TEST (node, rollback_vote_self)
{
	...
	std::shared_ptr<oslo::state_block> send1 = builder.make_block ()
	                                           ...
	                                           .representative (oslo::test_genesis_key.pub)
	                                           .link (key.pub)
	                                           .balance (oslo::genesis_amount - weight)
	                                           .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                                           .work (*system.work.generate (oslo::genesis_hash))
	                                           .build ();
	std::shared_ptr<oslo::state_block> open = builder.make_block ()
	                                          .account (key.pub)
	                                          .previous (0)
	...
	std::shared_ptr<oslo::state_block> send2 = builder.make_block ()
	                                           ...
	                                           .build ();
	std::shared_ptr<oslo::state_block> fork = builder.make_block ()
	                                          .from (*send2)
	...
	auto vote (election->last_votes.find (oslo::test_genesis_key.pub));
...
TEST (node, dependency_graph)
{
	...
	// Send to key1
	std::shared_ptr<oslo::state_block> gen_send1 = builder.make_block ()
	                                               .account (oslo::test_genesis_key.pub)
	                                               .previous (oslo::genesis_hash)
	                                               .representative (oslo::test_genesis_key.pub)
	                                               .link (key1.pub)
	...
	auto key3_epoch = builder.make_block ()
	                  ...
	                  .link (node.ledger.epoch_link (oslo::epoch::epoch_1))
	                  .balance (key3_receive->balance ())
	                  .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
	                  .work (*system.work.generate (key3_receive->hash ()))
	                  .build ();

	ASSERT_EQ (oslo::process_result::progress, node.process (*gen_send1).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key1_open).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key1_send1).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*gen_receive).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*gen_send2).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key2_open).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key2_send1).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key3_open).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key2_send2).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key1_receive).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key1_send2).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key3_receive).code);
	ASSERT_EQ (oslo::process_result::progress, node.process (*key3_epoch).code);
	ASSERT_TRUE (node.active.empty ());

	// Hash -> Ancestors
	std::unordered_map<oslo::block_hash, std::vector<oslo::block_hash>> dependency_graph{
...
TEST (node, dependency_graph_frontier)
{
	...
	{
		auto transaction (node->store.tx_begin_write ());
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *gen_send1).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key1_open).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key1_send1).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *gen_receive).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *gen_send2).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key2_open).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key2_send1).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key3_open).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key2_send2).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key1_receive).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key1_send2).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key3_receive).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *key3_epoch).code);
	}
...
TEST (node, deferred_dependent_elections)
{
	...
	std::shared_ptr<oslo::state_block> send2 = builder.make_block ()
	                                           ...
	                                           .work (*system.work.generate (send1->hash ()))
	                                           .build ();
	std::shared_ptr<oslo::state_block> receive = builder.make_block ()
	                                             .from (*open)
	                                             .previous (open->hash ())
	                                             .link (send2->hash ())
	                                             .balance (2)
	...
	ASSERT_EQ (oslo::process_result::fork, node.process (*fork).code);
	node.process_active (fork);
	node.block_processor.flush ();
	ASSERT_FALSE (node.active.active (receive->qualified_root ()));
...
void add_required_children_node_config_tree (oslo::jsonconfig & tree)
{
	...
	oslo::jsonconfig preconfigured_representatives_l;
	tree.put_child ("preconfigured_representatives", preconfigured_representatives_l);
	oslo::jsonconfig work_peers_l;
	tree.put_child ("work_peers", work_peers_l);
	tree.put ("version", std::to_string (oslo::node_config::json_version ()));
}
}
