...
#if OSLO_ROCKSDB
#include <oslo/node/rocksdb/rocksdb.hpp>
#endif
...
namespace
{
void modify_account_info_to_v13 (oslo::mdb_store & store, oslo::transaction const & transaction_a, oslo::account const & account_a, oslo::block_hash const & rep_block);
void modify_account_info_to_v14 (oslo::mdb_store & store, oslo::transaction const & transaction_a, oslo::account const & account_a, uint64_t confirmation_height, oslo::block_hash const & rep_block);
void modify_genesis_account_info_to_v5 (oslo::mdb_store & store, oslo::transaction const & transaction_a);
void modify_confirmation_height_to_v15 (oslo::mdb_store & store, oslo::transaction const & transaction, oslo::account const & account, uint64_t confirmation_height);
void write_sideband_v12 (oslo::mdb_store & store_a, oslo::transaction & transaction_a, oslo::block & block_a, oslo::block_hash const & successor_a, MDB_dbi db_a);
void write_sideband_v14 (oslo::mdb_store & store_a, oslo::transaction & transaction_a, oslo::block const & block_a, MDB_dbi db_a);
void write_sideband_v15 (oslo::mdb_store & store_a, oslo::transaction & transaction_a, oslo::block const & block_a);
}
...
{
	oslo::block_details details_send (oslo::epoch::epoch_0, true, false, false);
	ASSERT_TRUE (details_send.is_send);
	ASSERT_FALSE (details_send.is_receive);
	ASSERT_FALSE (details_send.is_epoch);
	ASSERT_EQ (oslo::epoch::epoch_0, details_send.epoch);

	oslo::block_details details_receive (oslo::epoch::epoch_1, false, true, false);
	ASSERT_FALSE (details_receive.is_send);
	ASSERT_TRUE (details_receive.is_receive);
	ASSERT_FALSE (details_receive.is_epoch);
	ASSERT_EQ (oslo::epoch::epoch_1, details_receive.epoch);

	oslo::block_details details_epoch (oslo::epoch::epoch_2, false, false, true);
	ASSERT_FALSE (details_epoch.is_send);
	ASSERT_FALSE (details_epoch.is_receive);
	ASSERT_TRUE (details_epoch.is_epoch);
	ASSERT_EQ (oslo::epoch::epoch_2, details_epoch.epoch);

	oslo::block_details details_none (oslo::epoch::unspecified, false, false, false);
	ASSERT_FALSE (details_none.is_send);
	ASSERT_FALSE (details_none.is_receive);
	ASSERT_FALSE (details_none.is_epoch);
	ASSERT_EQ (oslo::epoch::unspecified, details_none.epoch);
}

TEST (block_store, block_details_serialization)
{
	oslo::block_details details1;
	details1.epoch = oslo::epoch::epoch_2;
	details1.is_epoch = false;
	details1.is_receive = true;
	details1.is_send = false;
	std::vector<uint8_t> vector;
	{
		oslo::vectorstream stream1 (vector);
		details1.serialize (stream1);
	}
	oslo::bufferstream stream2 (vector.data (), vector.size ());
	oslo::block_details details2;
...
TEST (block_store, sideband_serialization)
{
	...
	{
		oslo::vectorstream stream1 (vector);
		sideband1.serialize (stream1, oslo::block_type::receive);
	}
	oslo::bufferstream stream2 (vector.data (), vector.size ());
	oslo::block_sideband sideband2;
	ASSERT_FALSE (sideband2.deserialize (stream2, oslo::block_type::receive));
...
TEST (block_store, add_item)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	oslo::open_block block (0, 1, 0, oslo::keypair ().prv, 0, 0);
	block.sideband_set ({});
...
TEST (block_store, clear_successor)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	oslo::open_block block1 (0, 1, 0, oslo::keypair ().prv, 0, 0);
	block1.sideband_set ({});
	auto transaction (store->tx_begin_write ());
	store->block_put (transaction, block1.hash (), block1);
	oslo::open_block block2 (0, 2, 0, oslo::keypair ().prv, 0, 0);
	block2.sideband_set ({});
...
TEST (block_store, add_nonempty_block)
{
	...
	block.sideband_set ({});
	auto hash1 (block.hash ());
	block.signature = oslo::sign_message (key1.prv, key1.pub, hash1);
	auto transaction (store->tx_begin_write ());
	auto latest1 (store->block_get (transaction, hash1));
	ASSERT_EQ (nullptr, latest1);
	store->block_put (transaction, hash1, block);
...
TEST (block_store, add_two_items)
{
	...
	ASSERT_TRUE (!store->init_error ());
	oslo::keypair key1;
	oslo::open_block block (0, 1, 1, oslo::keypair ().prv, 0, 0);
	block.sideband_set ({});
	auto hash1 (block.hash ());
	block.signature = oslo::sign_message (key1.prv, key1.pub, hash1);
	auto transaction (store->tx_begin_write ());
	auto latest1 (store->block_get (transaction, hash1));
	ASSERT_EQ (nullptr, latest1);
	oslo::open_block block2 (0, 1, 3, oslo::keypair ().prv, 0, 0);
	block2.sideband_set ({});
	block2.hashables.account = 3;
	auto hash2 (block2.hash ());
	block2.signature = oslo::sign_message (key1.prv, key1.pub, hash2);
...
TEST (block_store, add_receive)
{
	...
	auto transaction (store->tx_begin_write ());
	store->block_put (transaction, block1.hash (), block1);
	oslo::receive_block block (block1.hash (), 1, oslo::keypair ().prv, 2, 3);
	block.sideband_set ({});
	oslo::block_hash hash1 (block.hash ());
	auto latest1 (store->block_get (transaction, hash1));
	ASSERT_EQ (nullptr, latest1);
	store->block_put (transaction, hash1, block);
	auto latest2 (store->block_get (transaction, hash1));
...
TEST (block_store, pending_iterator)
{
	...
	store->pending_put (transaction, oslo::pending_key (1, 2), { 2, 3, oslo::epoch::epoch_1 });
	auto current (store->pending_begin (transaction));
	ASSERT_NE (store->pending_end (), current);
	oslo::pending_key key1 (current->first);
	ASSERT_EQ (oslo::account (1), key1.account);
	ASSERT_EQ (oslo::block_hash (2), key1.hash);
	oslo::pending_info pending (current->second);
	ASSERT_EQ (oslo::account (2), pending.source);
	ASSERT_EQ (oslo::amount (3), pending.amount);
	ASSERT_EQ (oslo::epoch::epoch_1, pending.epoch);
}
...
TEST (block_store, pending_iterator_comparison)
{
	...
	auto transaction (store->tx_begin_write ());
	// Populate pending
	store->pending_put (transaction, oslo::pending_key (oslo::account (3), oslo::block_hash (1)), oslo::pending_info (oslo::account (10), oslo::amount (1), oslo::epoch::epoch_0));
	store->pending_put (transaction, oslo::pending_key (oslo::account (3), oslo::block_hash (4)), oslo::pending_info (oslo::account (10), oslo::amount (0), oslo::epoch::epoch_0));
	// Populate pending_v1
	store->pending_put (transaction, oslo::pending_key (oslo::account (2), oslo::block_hash (2)), oslo::pending_info (oslo::account (10), oslo::amount (2), oslo::epoch::epoch_1));
	store->pending_put (transaction, oslo::pending_key (oslo::account (2), oslo::block_hash (3)), oslo::pending_info (oslo::account (10), oslo::amount (3), oslo::epoch::epoch_1));

	// Iterate account 3 (pending)
	{
		size_t count = 0;
		oslo::account begin (3);
		oslo::account end (begin.number () + 1);
		for (auto i (store->pending_begin (transaction, oslo::pending_key (begin, 0))), n (store->pending_begin (transaction, oslo::pending_key (end, 0))); i != n; ++i, ++count)
		{
			oslo::pending_key key (i->first);
...
TEST (block_store, genesis)
{
	...
	auto receive1 (dynamic_cast<oslo::open_block *> (block1.get ()));
	ASSERT_NE (nullptr, receive1);
	ASSERT_LE (info.modified, oslo::seconds_since_epoch ());
	...
	auto test_pub_text (oslo::test_genesis_key.pub.to_string ());
	auto test_pub_account (oslo::test_genesis_key.pub.to_account ());
	auto test_prv_text (oslo::test_genesis_key.prv.data.to_string ());
	ASSERT_EQ (oslo::genesis_account, oslo::test_genesis_key.pub);
}
...
TEST (bootstrap, simple)
{
	...
	store->unchecked_del (transaction, oslo::unchecked_key (block1->previous (), block1->hash ()));
...
{
	oslo::logger_mt logger;
	oslo::mdb_store store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store.init_error ());
	auto block1 (std::make_shared<oslo::send_block> (4, 1, 2, oslo::keypair ().prv, 4, 5));
	auto transaction (store.tx_begin_write ());
	auto block2 (store.unchecked_get (transaction, block1->previous ()));
	ASSERT_TRUE (block2.empty ());
	store.unchecked_put (transaction, block1->previous (), block1);
...
TEST (unchecked, multiple_get)
{
	...
	ASSERT_TRUE (!store->init_error ());
	auto block1 (std::make_shared<oslo::send_block> (4, 1, 2, oslo::keypair ().prv, 4, 5));
	auto block2 (std::make_shared<oslo::send_block> (3, 1, 2, oslo::keypair ().prv, 4, 5));
	auto block3 (std::make_shared<oslo::send_block> (5, 1, 2, oslo::keypair ().prv, 4, 5));
	{
		auto transaction (store->tx_begin_write ());
	...
	std::vector<oslo::block_hash> unchecked1;
	...
	std::vector<oslo::block_hash> unchecked2;
...
TEST (block_store, unchecked_begin_search)
{
	...
	ASSERT_TRUE (!store->init_error ());
	oslo::keypair key0;
	oslo::send_block block1 (0, 1, 2, key0.prv, key0.pub, 3);
	oslo::send_block block2 (5, 6, 7, key0.prv, key0.pub, 8);
}
...
TEST (block_store, frontier_retrieval)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	oslo::account account1 (0);
	oslo::account_info info1 (0, 0, 0, 0, 0, 0, oslo::epoch::epoch_0);
	auto transaction (store->tx_begin_write ());
	store->confirmation_height_put (transaction, account1, { 0, oslo::block_hash (0) });
...
TEST (block_store, one_account)
{
	...
	auto transaction (store->tx_begin_write ());
	store->confirmation_height_put (transaction, account, { 20, oslo::block_hash (15) });
	store->account_put (transaction, account, { hash, account, hash, 42, 100, 200, oslo::epoch::epoch_0 });
	auto begin (store->latest_begin (transaction));
	auto end (store->latest_end ());
	ASSERT_NE (end, begin);
	ASSERT_EQ (account, oslo::account (begin->first));
	oslo::account_info info (begin->second);
	ASSERT_EQ (hash, info.head);
	...
	ASSERT_EQ (oslo::block_hash (15), confirmation_height_info.frontier);
	++begin;
	ASSERT_EQ (end, begin);
}
...
TEST (block_store, two_account)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	oslo::account account1 (1);
	oslo::block_hash hash1 (2);
	oslo::account account2 (3);
	oslo::block_hash hash2 (4);
	auto transaction (store->tx_begin_write ());
	store->confirmation_height_put (transaction, account1, { 20, oslo::block_hash (10) });
	store->account_put (transaction, account1, { hash1, account1, hash1, 42, 100, 300, oslo::epoch::epoch_0 });
	store->confirmation_height_put (transaction, account2, { 30, oslo::block_hash (20) });
	store->account_put (transaction, account2, { hash2, account2, hash2, 84, 200, 400, oslo::epoch::epoch_0 });
	auto begin (store->latest_begin (transaction));
	auto end (store->latest_end ());
	ASSERT_NE (end, begin);
	ASSERT_EQ (account1, oslo::account (begin->first));
	oslo::account_info info1 (begin->second);
	...
	ASSERT_EQ (20, confirmation_height_info.height);
	ASSERT_EQ (oslo::block_hash (10), confirmation_height_info.frontier);
	++begin;
	ASSERT_NE (end, begin);
	ASSERT_EQ (account2, oslo::account (begin->first));
	oslo::account_info info2 (begin->second);
	...
	ASSERT_EQ (oslo::block_hash (20), confirmation_height_info.frontier);
	++begin;
	ASSERT_EQ (end, begin);
}
...
TEST (block_store, latest_find)
{
	...
	auto transaction (store->tx_begin_write ());
	store->confirmation_height_put (transaction, account1, { 0, oslo::block_hash (0) });
	store->account_put (transaction, account1, { hash1, account1, hash1, 100, 0, 300, oslo::epoch::epoch_0 });
	store->confirmation_height_put (transaction, account2, { 0, oslo::block_hash (0) });
	store->account_put (transaction, account2, { hash2, account2, hash2, 200, 0, 400, oslo::epoch::epoch_0 });
...
TEST (block_store, DISABLED_already_open) // File can be shared
{
	...
	oslo::set_secure_perm_directory (path.parent_path ());
	std::ofstream file;
	file.open (path.string ().c_str ());
	ASSERT_TRUE (file.is_open ());
	oslo::logger_mt logger;
	auto store = oslo::make_store (logger, path);
	ASSERT_TRUE (store->init_error ());
}
...
TEST (block_store, roots)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	oslo::send_block send_block (0, 1, 2, oslo::keypair ().prv, 4, 5);
	ASSERT_EQ (send_block.hashables.previous, send_block.root ());
	oslo::change_block change_block (0, 1, oslo::keypair ().prv, 3, 4);
	ASSERT_EQ (change_block.hashables.previous, change_block.root ());
	oslo::receive_block receive_block (0, 1, oslo::keypair ().prv, 3, 4);
	ASSERT_EQ (receive_block.hashables.previous, receive_block.root ());
	oslo::open_block open_block (0, 1, 2, oslo::keypair ().prv, 4, 5);
...
TEST (block_store, latest_exists)
{
	...
	oslo::account_info info;
	auto transaction (store->tx_begin_write ());
	store->confirmation_height_put (transaction, two, { 0, oslo::block_hash (0) });
...
TEST (block_store, large_iteration)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	std::unordered_set<oslo::account> accounts1;
	for (auto i (0); i < 1000; ++i)
	{
		auto transaction (store->tx_begin_write ());
		oslo::account account;
		oslo::random_pool::generate_block (account.bytes.data (), account.bytes.size ());
		accounts1.insert (account);
		store->confirmation_height_put (transaction, account, { 0, oslo::block_hash (0) });
		store->account_put (transaction, account, oslo::account_info ());
	}
	std::unordered_set<oslo::account> accounts2;
	oslo::account previous (0);
	auto transaction (store->tx_begin_read ());
	for (auto i (store->latest_begin (transaction, 0)), n (store->latest_end ()); i != n; ++i)
	{
		oslo::account current (i->first);
...
TEST (block_store, frontier)
{
	...
	ASSERT_TRUE (!store->init_error ());
	auto transaction (store->tx_begin_write ());
	oslo::block_hash hash (100);
	oslo::account account (200);
...
TEST (block_store, block_replace)
{
	...
	auto store = oslo::make_store (logger, oslo::unique_path ());
	ASSERT_TRUE (!store->init_error ());
	oslo::send_block send1 (0, 0, 0, oslo::keypair ().prv, 0, 1);
	send1.sideband_set ({});
	oslo::send_block send2 (0, 0, 0, oslo::keypair ().prv, 0, 2);
	send2.sideband_set ({});
	auto transaction (store->tx_begin_write ());
...
TEST (block_store, sequence_increment)
{
	...
	oslo::keypair key1;
	oslo::keypair key2;
	auto block1 (std::make_shared<oslo::open_block> (0, 1, 0, oslo::keypair ().prv, 0, 0));
	auto transaction (store->tx_begin_write ());
...
TEST (mdb_block_store, upgrade_v2_v3)
{
	...
	auto path (oslo::unique_path ());
	{
		...
		oslo::change_block change (hash, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (hash));
		...
		ASSERT_EQ (oslo::genesis_amount, ledger.weight (key1.pub));
		...
		oslo::account_info info;
		ASSERT_FALSE (store.account_get (transaction, oslo::test_genesis_key.pub, info));
		auto rep_block = ledger.representative (transaction, ledger.latest (transaction, oslo::test_genesis_key.pub));
		oslo::account_info_v5 info_old (info.head, rep_block, info.open_block, info.balance, info.modified);
		auto status (mdb_put (store.env.tx (transaction), store.accounts_v0, oslo::mdb_val (oslo::test_genesis_key.pub), oslo::mdb_val (sizeof (info_old), &info_old), 0));
		ASSERT_EQ (status, 0);
		store.confirmation_height_del (transaction, oslo::genesis_account);
	}
	oslo::logger_mt logger;
	oslo::mdb_store store (logger, path);
	oslo::stat stats;
	...
	oslo::account_info info;
	ASSERT_FALSE (store.account_get (transaction, oslo::test_genesis_key.pub, info));
	ASSERT_EQ (change_hash, ledger.representative (transaction, ledger.latest (transaction, oslo::test_genesis_key.pub)));
}
...
TEST (mdb_block_store, upgrade_v3_v4)
{
	...
	auto path (oslo::unique_path ());
	{
		...
		oslo::pending_info_v3 info (key1.pub, 100, key2.pub);
		auto status (mdb_put (store.env.tx (transaction), store.pending_v0, oslo::mdb_val (key3.pub), oslo::mdb_val (sizeof (info), &info), 0));
		ASSERT_EQ (0, status);
	}
	oslo::logger_mt logger;
	oslo::mdb_store store (logger, path);
	...
	oslo::pending_key key (key2.pub, reinterpret_cast<oslo::block_hash const &> (key3.pub));
	oslo::pending_info info;
	auto error (store.pending_get (transaction, key, info));
	ASSERT_FALSE (error);
	ASSERT_EQ (key1.pub, info.source);
	ASSERT_EQ (oslo::amount (100), info.amount);
	ASSERT_EQ (oslo::epoch::epoch_0, info.epoch);
}

TEST (mdb_block_store, upgrade_v4_v5)
{
	oslo::block_hash genesis_hash (0);
	oslo::block_hash hash (0);
	auto path (oslo::unique_path ());
	{
		oslo::logger_mt logger;
		...
		oslo::keypair key0;
		oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
		oslo::send_block block0 (info.head, key0.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (info.head));
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, block0).code);
		hash = block0.hash ();
		...
		auto status (mdb_put (store.env.tx (transaction), store.pending_v0, oslo::mdb_val (oslo::pending_key (key0.pub, block0.hash ())), oslo::mdb_val (oslo::pending_info_v14 (oslo::genesis_account, oslo::Gxrb_ratio, oslo::epoch::epoch_0)), 0));
...
TEST (mdb_block_store, upgrade_v5_v6)
{
	...
	auto transaction (store.tx_begin_read ());
	oslo::account_info info;
	store.account_get (transaction, oslo::test_genesis_key.pub, info);
...
TEST (mdb_block_store, upgrade_v6_v7)
{
	auto path (oslo::unique_path ());
	{
		...
		modify_account_info_to_v13 (store, transaction, oslo::genesis_account, oslo::genesis_hash);
		auto send1 (std::make_shared<oslo::send_block> (0, 0, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
...
TEST (block_store, DISABLED_change_dupsort) // Unchecked is no longer dupsort table
{
	...
	auto send1 (std::make_shared<oslo::send_block> (0, 0, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
	auto send2 (std::make_shared<oslo::send_block> (1, 0, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
...
TEST (block_store, sequence_flush_by_hash)
{
	...
	auto transaction (store->tx_begin_write ());
	oslo::keypair key1;
	std::vector<oslo::block_hash> blocks1;
	blocks1.push_back (oslo::genesis_hash);
...
TEST (mdb_block_store, upgrade_v8_v9)
{
	...
	oslo::keypair key;
	{
		...
		ASSERT_EQ (0, mdb_put (store.env.tx (transaction), store.vote, oslo::mdb_val (key.pub), oslo::mdb_val (sizeof (sequence), &sequence), 0));
...
TEST (block_store, state_block)
{
	...
	oslo::genesis genesis;
	oslo::keypair key1;
	oslo::state_block block1 (1, genesis.hash (), 3, 4, 6, key1.prv, key1.pub, 7);
	block1.sideband_set ({});
	{
		oslo::ledger_cache ledger_cache;
		auto transaction (store->tx_begin_write ());
		store->initialize (transaction, genesis, ledger_cache);
		ASSERT_EQ (oslo::block_type::state, block1.type ());
		store->block_put (transaction, block1.hash (), block1);
		ASSERT_TRUE (store->block_exists (transaction, block1.hash ()));
...
TEST (mdb_block_store, upgrade_sideband_genesis)
{
	...
	auto path (oslo::unique_path ());
	{
		...
		oslo::block_sideband_v14 sideband1;
...
TEST (mdb_block_store, upgrade_sideband_two_blocks)
{
	...
	auto path (oslo::unique_path ());
	{
		...
		ASSERT_FALSE (store.init_error ());
		oslo::stat stat;
		oslo::ledger ledger (store, stat);
		auto transaction (store.tx_begin_write ());
		store.version_put (transaction, 11);
		store.initialize (transaction, genesis, ledger.cache);
		oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
		oslo::state_block block (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
		hash2 = block.hash ();
		...
		modify_account_info_to_v13 (store, transaction, oslo::genesis_account, hash2);
		auto status (mdb_put (store.env.tx (transaction), store.pending_v0, oslo::mdb_val (oslo::pending_key (oslo::test_genesis_key.pub, block.hash ())), oslo::mdb_val (oslo::pending_info_v14 (oslo::genesis_account, oslo::Gxrb_ratio, oslo::epoch::epoch_0)), 0));
		ASSERT_EQ (status, MDB_SUCCESS);
		store.confirmation_height_del (transaction, oslo::genesis_account);
	}
	oslo::logger_mt logger;
...
TEST (mdb_block_store, upgrade_sideband_two_accounts)
{
	...
	auto path (oslo::unique_path ());
	{
		...
		store.initialize (transaction, genesis, ledger.cache);
		oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
		oslo::state_block block1 (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
		hash2 = block1.hash ();
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, block1).code);
		oslo::state_block block2 (key.pub, 0, oslo::test_genesis_key.pub, oslo::Gxrb_ratio, hash2, key.prv, key.pub, *pool.generate (key.pub));
		hash3 = block2.hash ();
...
TEST (mdb_block_store, legacy_account_computed)
{
	...
	mdb_dbi_open (store.env.tx (transaction), "state_v1", MDB_CREATE, &store.state_blocks_v1);
	write_sideband_v12 (store, transaction, *genesis.open, 0, store.open_blocks);
	ASSERT_EQ (oslo::genesis_account, ledger.account (transaction, genesis.hash ()));
}
...
TEST (mdb_block_store, upgrade_sideband_epoch)
{
	...
	oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
	{
		...
		store.version_put (transaction, 11);
		store.initialize (transaction, genesis, ledger.cache);
		oslo::state_block block1 (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, ledger.epoch_link (oslo::epoch::epoch_1), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
		hash2 = block1.hash ();
		ASSERT_FALSE (mdb_dbi_open (store.env.tx (transaction), "state_v1", MDB_CREATE, &store.state_blocks_v1));
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, block1).code);
		ASSERT_EQ (oslo::epoch::epoch_1, store.block_version (transaction, hash2));
		store.block_del (transaction, hash2, block1.type ());
		...
		oslo::mdb_val value;
		ASSERT_FALSE (mdb_get (store.env.tx (transaction), store.state_blocks_v1, oslo::mdb_val (hash2), value));
		ASSERT_FALSE (mdb_get (store.env.tx (transaction), store.open_blocks, oslo::mdb_val (oslo::genesis_hash), value));

		ASSERT_FALSE (mdb_dbi_open (store.env.tx (transaction), "accounts_v1", MDB_CREATE, &store.accounts_v1));
		modify_account_info_to_v13 (store, transaction, oslo::genesis_account, hash2);
		store.account_del (transaction, oslo::genesis_account);
		store.confirmation_height_del (transaction, oslo::genesis_account);
	}
	oslo::logger_mt logger;
	oslo::mdb_store store (logger, path);
	...
	oslo::state_block block2 (oslo::test_genesis_key.pub, hash2, oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (hash2));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, block2).code);
	ASSERT_EQ (oslo::epoch::epoch_1, store.block_version (transaction, block2.hash ()));
}
...
TEST (mdb_block_store, sideband_height)
{
	...
	oslo::change_block change (receive.hash (), 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (receive.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, change).code);
	oslo::state_block state_send1 (oslo::test_genesis_key.pub, change.hash (), 0, oslo::genesis_amount - oslo::Gxrb_ratio, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (change.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send1).code);
	oslo::state_block state_send2 (oslo::test_genesis_key.pub, state_send1.hash (), 0, oslo::genesis_amount - 2 * oslo::Gxrb_ratio, key2.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_send1.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send2).code);
	oslo::state_block state_send3 (oslo::test_genesis_key.pub, state_send2.hash (), 0, oslo::genesis_amount - 3 * oslo::Gxrb_ratio, key3.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_send2.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send3).code);
	oslo::state_block state_open (key1.pub, 0, 0, oslo::Gxrb_ratio, state_send1.hash (), key1.prv, key1.pub, *pool.generate (key1.pub));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_open).code);
	oslo::state_block epoch (key1.pub, state_open.hash (), 0, oslo::Gxrb_ratio, ledger.epoch_link (oslo::epoch::epoch_1), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_open.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, epoch).code);
	ASSERT_EQ (oslo::epoch::epoch_1, store.block_version (transaction, epoch.hash ()));
	oslo::state_block epoch_open (key2.pub, 0, 0, 0, ledger.epoch_link (oslo::epoch::epoch_1), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (key2.pub));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, epoch_open).code);
	ASSERT_EQ (oslo::epoch::epoch_1, store.block_version (transaction, epoch_open.hash ()));
	oslo::state_block state_receive (key2.pub, epoch_open.hash (), 0, oslo::Gxrb_ratio, state_send2.hash (), key2.prv, key2.pub, *pool.generate (epoch_open.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_receive).code);
	oslo::open_block open (state_send3.hash (), oslo::test_genesis_key.pub, key3.pub, key3.prv, key3.pub, *pool.generate (key3.pub));
...
TEST (block_store, peers)
{
	...
	oslo::endpoint_key endpoint (boost::asio::ip::address_v6::any ().to_bytes (), 100);
	{
		auto transaction (store->tx_begin_write ());
	...
	oslo::endpoint_key endpoint1 (boost::asio::ip::address_v6::any ().to_bytes (), 101);
	{
		auto transaction (store->tx_begin_write ());
...
TEST (block_store, endpoint_key_byte_order)
{
	...
	uint16_t port = 100;
	oslo::endpoint_key endpoint_key (address.to_bytes (), port);

	std::vector<uint8_t> bytes;
	{
		oslo::vectorstream stream (bytes);
		oslo::write (stream, endpoint_key);
	}
	...
	oslo::endpoint_key endpoint_key1;
	oslo::read (stream1, endpoint_key1);
...
TEST (mdb_block_store, upgrade_v13_v14)
{
	...
	oslo::genesis genesis;
	{
		...
		ASSERT_FALSE (store.account_get (transaction, oslo::genesis_account, account_info));
		store.version_put (transaction, 13);
		modify_account_info_to_v13 (store, transaction, oslo::genesis_account, oslo::genesis_hash);

		// This should fail as sizes are no longer correct for account_info_v14
		oslo::mdb_val value;
		ASSERT_FALSE (mdb_get (store.env.tx (transaction), store.accounts_v0, oslo::mdb_val (oslo::genesis_account), value));
		oslo::account_info_v14 info;
	...
	oslo::uint256_union node_id_mdb_key (3);
	auto error_node_id (mdb_get (store.env.tx (transaction), store.meta, oslo::mdb_val (node_id_mdb_key), value));
...
TEST (mdb_block_store, upgrade_v14_v15)
{
	...
	oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
	oslo::send_block send (genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
	oslo::state_block epoch (oslo::test_genesis_key.pub, send.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, network_params.ledger.epochs.link (oslo::epoch::epoch_1), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (send.hash ()));
	oslo::state_block state_send (oslo::test_genesis_key.pub, epoch.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio * 2, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (epoch.hash ()));
	{
		oslo::logger_mt logger;
		oslo::mdb_store store (logger, path);
		oslo::stat stats;
		...
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, send).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, epoch).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send).code);
		// Lower the database to the previous version
		store.version_put (transaction, 14);
		store.confirmation_height_del (transaction, oslo::genesis_account);
		modify_account_info_to_v14 (store, transaction, oslo::genesis_account, confirmation_height_info.height, state_send.hash ());

		store.pending_del (transaction, oslo::pending_key (oslo::genesis_account, state_send.hash ()));
		...
		ASSERT_FALSE (mdb_put (store.env.tx (transaction), store.pending_v0, oslo::mdb_val (oslo::pending_key (oslo::test_genesis_key.pub, send.hash ())), oslo::mdb_val (oslo::pending_info_v14 (oslo::genesis_account, oslo::Gxrb_ratio, oslo::epoch::epoch_0)), 0));
		ASSERT_FALSE (mdb_put (store.env.tx (transaction), store.pending_v1, oslo::mdb_val (oslo::pending_key (oslo::test_genesis_key.pub, state_send.hash ())), oslo::mdb_val (oslo::pending_info_v14 (oslo::genesis_account, oslo::Gxrb_ratio, oslo::epoch::epoch_1)), 0));

		// This should fail as sizes are no longer correct for account_info
		oslo::mdb_val value;
		ASSERT_FALSE (mdb_get (store.env.tx (transaction), store.accounts_v1, oslo::mdb_val (oslo::genesis_account), value));
		oslo::account_info info;
		ASSERT_NE (value.size (), info.db_size ());
		store.account_del (transaction, oslo::genesis_account);

		// Confirmation height for the account should be deleted
		ASSERT_TRUE (mdb_get (store.env.tx (transaction), store.confirmation_height, oslo::mdb_val (oslo::genesis_account), value));
	}
	...
	// Size of account_info should now equal that set in db
	oslo::mdb_val value;
	ASSERT_FALSE (mdb_get (store.env.tx (transaction), store.accounts, oslo::mdb_val (oslo::genesis_account), value));
	oslo::account_info info (value);
	ASSERT_EQ (value.size (), info.db_size ());
	...
	auto error_get_accounts_v1 (mdb_get (store.env.tx (transaction), store.accounts_v1, oslo::mdb_val (oslo::genesis_account), value));
	ASSERT_NE (error_get_accounts_v1, MDB_SUCCESS);
	auto error_get_pending_v1 (mdb_get (store.env.tx (transaction), store.pending_v1, oslo::mdb_val (oslo::pending_key (oslo::test_genesis_key.pub, state_send.hash ())), value));
	ASSERT_NE (error_get_pending_v1, MDB_SUCCESS);
	auto error_get_state_v1 (mdb_get (store.env.tx (transaction), store.state_blocks_v1, oslo::mdb_val (state_send.hash ()), value));
	...
	ASSERT_EQ (block->sideband ().details.epoch, oslo::epoch::epoch_1);
	block = store.block_get (transaction, send.hash ());
	ASSERT_NE (block, nullptr);
	ASSERT_EQ (block->sideband ().details.epoch, oslo::epoch::epoch_0);
	ASSERT_EQ (info.epoch (), oslo::epoch::epoch_1);
	oslo::pending_info pending_info;
	store.pending_get (transaction, oslo::pending_key (oslo::test_genesis_key.pub, send.hash ()), pending_info);
	ASSERT_EQ (pending_info.epoch, oslo::epoch::epoch_0);
	store.pending_get (transaction, oslo::pending_key (oslo::test_genesis_key.pub, state_send.hash ()), pending_info);
	ASSERT_EQ (pending_info.epoch, oslo::epoch::epoch_1);
...
TEST (mdb_block_store, upgrade_v15_v16)
{
	...
	oslo::mdb_val value;
	{
		...
		auto weight = ledger.cache.rep_weights.representation_get (oslo::genesis_account);
		ASSERT_EQ (MDB_SUCCESS, mdb_put (txn, store.representation, oslo::mdb_val (oslo::genesis_account), oslo::mdb_val (oslo::uint128_union (weight)), 0));
		// Lower the database to the previous version
		store.version_put (transaction, 15);
		// Confirm the rep weight exists in the database
		ASSERT_EQ (MDB_SUCCESS, mdb_get (store.env.tx (transaction), store.representation, oslo::mdb_val (oslo::genesis_account), value));
		store.confirmation_height_del (transaction, oslo::genesis_account);
	}
	...
	auto error_get_representation (mdb_get (store.env.tx (transaction), store.representation, oslo::mdb_val (oslo::genesis_account), value));
...
TEST (mdb_block_store, upgrade_v16_v17)
{
	oslo::genesis genesis;
	oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
	oslo::state_block block1 (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
	oslo::state_block block2 (oslo::test_genesis_key.pub, block1.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio - 1, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (block1.hash ()));
	oslo::state_block block3 (oslo::test_genesis_key.pub, block2.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio - 2, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (block2.hash ()));

	auto code = [&block1, &block2, &block3](auto confirmation_height, oslo::block_hash const & expected_cemented_frontier) {
		auto path (oslo::unique_path ());
		oslo::mdb_val value;
		{
			oslo::genesis genesis;
			...
			modify_confirmation_height_to_v15 (store, transaction, oslo::genesis_account, confirmation_height);
...
TEST (mdb_block_store, upgrade_v17_v18)
{
	...
	oslo::network_params network_params;
	oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
	oslo::send_block send_zero (genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
	oslo::state_block state_receive_zero (oslo::test_genesis_key.pub, send_zero.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, send_zero.hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (send_zero.hash ()));
	oslo::state_block epoch (oslo::test_genesis_key.pub, state_receive_zero.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, network_params.ledger.epochs.link (oslo::epoch::epoch_1), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_receive_zero.hash ()));
	oslo::state_block state_send (oslo::test_genesis_key.pub, epoch.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Gxrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (epoch.hash ()));
	oslo::state_block state_receive (oslo::test_genesis_key.pub, state_send.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, state_send.hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_send.hash ()));
	oslo::state_block state_change (oslo::test_genesis_key.pub, state_receive.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_receive.hash ()));
	oslo::state_block state_send_change (oslo::test_genesis_key.pub, state_change.hash (), key1.pub, oslo::genesis_amount - oslo::Gxrb_ratio, key1.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_change.hash ()));
	oslo::state_block epoch_first (key1.pub, 0, 0, 0, network_params.ledger.epochs.link (oslo::epoch::epoch_2), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (key1.pub));
	oslo::state_block state_receive2 (key1.pub, epoch_first.hash (), key1.pub, oslo::Gxrb_ratio, state_send_change.hash (), key1.prv, key1.pub, *pool.generate (epoch_first.hash ()));
	oslo::state_block state_send2 (oslo::test_genesis_key.pub, state_send_change.hash (), key1.pub, oslo::genesis_amount - oslo::Gxrb_ratio * 2, key2.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (state_send_change.hash ()));
	oslo::state_block state_open (key2.pub, 0, key2.pub, oslo::Gxrb_ratio, state_send2.hash (), key2.prv, key2.pub, *pool.generate (key2.pub));
	oslo::state_block state_send_epoch_link (key2.pub, state_open.hash (), key2.pub, 0, network_params.ledger.epochs.link (oslo::epoch::epoch_2), key2.prv, key2.pub, *pool.generate (state_open.hash ()));
	{
		oslo::logger_mt logger;
		oslo::mdb_store store (logger, path);
		auto transaction (store.tx_begin_write ());
		...
		oslo::ledger ledger (store, stats);
		store.initialize (transaction, genesis, ledger.cache);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, send_zero).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_receive_zero).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, epoch).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_receive).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_change).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send_change).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, epoch_first).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_receive2).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send2).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_open).code);
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, state_send_epoch_link).code);
	...
	oslo::mdb_val value;
	ASSERT_FALSE (mdb_get (store.env.tx (transaction), store.state_blocks, oslo::mdb_val (state_send.hash ()), value));
	ASSERT_EQ (value.size (), oslo::state_block::size + oslo::block_sideband::size (oslo::block_type::state));
	...
	}
	{
		...
		ASSERT_NE (block, nullptr);
		ASSERT_EQ (block->sideband ().details.epoch, oslo::epoch::epoch_2);
		ASSERT_TRUE (block->sideband ().details.is_epoch);
		ASSERT_FALSE (block->sideband ().details.is_send);
		ASSERT_FALSE (block->sideband ().details.is_receive);
	}
...
TEST (mdb_block_store, upgrade_backup)
{
	...
	oslo::logger_mt logger;
	oslo::mdb_store store (logger, path, oslo::txn_tracking_config{}, std::chrono::seconds (5), oslo::lmdb_config{}, 512, true);
	ASSERT_FALSE (store.init_error ());
	auto transaction (store.tx_begin_read ());
	ASSERT_LT (14, store.version_get (transaction));
...
TEST (block_store, confirmation_height)
{
	...
	oslo::account account1 (0);
	oslo::account account2 (1);
	oslo::account account3 (2);
	oslo::block_hash cemented_frontier1 (3);
	oslo::block_hash cemented_frontier2 (4);
	oslo::block_hash cemented_frontier3 (5);
	{
		auto transaction (store.tx_begin_write ());
	...
	ASSERT_EQ (confirmation_height_info.frontier, oslo::block_hash (0));
	ASSERT_FALSE (store.confirmation_height_get (transaction, account2, confirmation_height_info));
	ASSERT_EQ (confirmation_height_info.height, 0);
	ASSERT_EQ (confirmation_height_info.frontier, oslo::block_hash (0));
	ASSERT_FALSE (store.confirmation_height_get (transaction, account3, confirmation_height_info));
...
TEST (mdb_block_store, upgrade_confirmation_height_many)
{
	...
	auto path (oslo::unique_path ());
	{
		...
		{
			oslo::account account (i);
			oslo::open_block open (1, oslo::genesis_account, 3, nullptr);
			open.sideband_set ({});
			store.block_put (transaction, open.hash (), open);
			oslo::account_info_v13 account_info_v13 (open.hash (), open.hash (), open.hash (), 3, 4, 1, oslo::epoch::epoch_0);
			auto status (mdb_put (store.env.tx (transaction), store.accounts_v0, oslo::mdb_val (account), oslo::mdb_val (account_info_v13), 0));
			ASSERT_EQ (status, 0);
		}
		store.confirmation_height_del (transaction, oslo::genesis_account);
	...
	{
		ASSERT_EQ (i->second.height, (i->first == oslo::genesis_account) ? 1 : 0);
		ASSERT_EQ (i->second.frontier, (i->first == oslo::genesis_account) ? genesis.hash () : oslo::block_hash (0));
	}
}
...
TEST (block_store, incompatible_version)
{
	...
	{
		auto store = oslo::make_store (logger, path, true);
		ASSERT_TRUE (store->init_error ());
...
TEST (block_store, rocksdb_force_test_env_variable)
{
	...
	auto mdb_cast = dynamic_cast<oslo::mdb_store *> (store.get ());

#if OSLO_ROCKSDB
	if (value && boost::lexical_cast<int> (value) == 1)
	{
		ASSERT_NE (boost::polymorphic_downcast<oslo::rocksdb_store *> (store.get ()), nullptr);
	}
	else
	{
...
void write_sideband_v12 (oslo::mdb_store & store_a, oslo::transaction & transaction_a, oslo::block & block_a, oslo::block_hash const & successor_a, MDB_dbi db_a)
{
	std::vector<uint8_t> vector;
	{
		...
		oslo::write (stream, successor_a);
	}
	MDB_val val{ vector.size (), vector.data () };
	auto hash (block_a.hash ());
	auto status (mdb_put (store_a.env.tx (transaction_a), db_a, oslo::mdb_val (hash), &val, 0));
	ASSERT_EQ (0, status);
	oslo::block_sideband_v14 sideband_v14;
...
void write_sideband_v14 (oslo::mdb_store & store_a, oslo::transaction & transaction_a, oslo::block const & block_a, MDB_dbi db_a)
{
	...
	oslo::block_sideband_v14 sideband_v14 (block->type (), block->sideband ().account, block->sideband ().successor, block->sideband ().balance, block->sideband ().timestamp, block->sideband ().height);
	std::vector<uint8_t> data;
	{
		oslo::vectorstream stream (data);
		block_a.serialize (stream);
		sideband_v14.serialize (stream);
	}

	MDB_val val{ data.size (), data.data () };
	ASSERT_FALSE (mdb_put (store_a.env.tx (transaction_a), block->sideband ().details.epoch == oslo::epoch::epoch_0 ? store_a.state_blocks_v0 : store_a.state_blocks_v1, oslo::mdb_val (block_a.hash ()), &val, 0));
}
...
void write_sideband_v15 (oslo::mdb_store & store_a, oslo::transaction & transaction_a, oslo::block const & block_a)
{
	...
	ASSERT_LE (block->sideband ().details.epoch, oslo::epoch::max);
	// Simulated by writing 0 on every of the most significant bits, leaving out epoch only, as if pre-upgrade
	oslo::block_sideband sideband_v15 (block->sideband ().account, block->sideband ().successor, block->sideband ().balance, block->sideband ().timestamp, block->sideband ().height, block->sideband ().details.epoch, false, false, false);
	std::vector<uint8_t> data;
	{
		oslo::vectorstream stream (data);
		block_a.serialize (stream);
	...
	MDB_val val{ data.size (), data.data () };
	ASSERT_FALSE (mdb_put (store_a.env.tx (transaction_a), store_a.state_blocks, oslo::mdb_val (block_a.hash ()), &val, 0));
}

// These functions take the latest account_info and create a legacy one so that upgrade tests can be emulated more easily.
void modify_account_info_to_v13 (oslo::mdb_store & store, oslo::transaction const & transaction, oslo::account const & account, oslo::block_hash const & rep_block)
{
	oslo::account_info info;
	...
	auto status (mdb_put (store.env.tx (transaction), (info.epoch () == oslo::epoch::epoch_0) ? store.accounts_v0 : store.accounts_v1, oslo::mdb_val (account), oslo::mdb_val (account_info_v13), 0));
	ASSERT_EQ (status, 0);
}

void modify_account_info_to_v14 (oslo::mdb_store & store, oslo::transaction const & transaction, oslo::account const & account, uint64_t confirmation_height, oslo::block_hash const & rep_block)
{
	oslo::account_info info;
	ASSERT_FALSE (store.account_get (transaction, account, info));
	oslo::account_info_v14 account_info_v14 (info.head, rep_block, info.open_block, info.balance, info.modified, info.block_count, confirmation_height, info.epoch ());
	auto status (mdb_put (store.env.tx (transaction), info.epoch () == oslo::epoch::epoch_0 ? store.accounts_v0 : store.accounts_v1, oslo::mdb_val (account), oslo::mdb_val (account_info_v14), 0));
	ASSERT_EQ (status, 0);
}

void modify_confirmation_height_to_v15 (oslo::mdb_store & store, oslo::transaction const & transaction, oslo::account const & account, uint64_t confirmation_height)
{
	auto status (mdb_put (store.env.tx (transaction), store.confirmation_height, oslo::mdb_val (account), oslo::mdb_val (confirmation_height), 0));
	ASSERT_EQ (status, 0);
}

void modify_genesis_account_info_to_v5 (oslo::mdb_store & store, oslo::transaction const & transaction)
{
	oslo::account_info info;
	store.account_get (transaction, oslo::test_genesis_key.pub, info);
	oslo::representative_visitor visitor (transaction, store);
	visitor.compute (info.head);
	oslo::account_info_v5 info_old (info.head, visitor.result, info.open_block, info.balance, info.modified);
	auto status (mdb_put (store.env.tx (transaction), store.accounts_v0, oslo::mdb_val (oslo::test_genesis_key.pub), oslo::mdb_val (sizeof (info_old), &info_old), 0));
	ASSERT_EQ (status, 0);
}
}
