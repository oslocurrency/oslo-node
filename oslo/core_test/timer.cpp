...
TEST (timer, states)
{
	oslo::timer<std::chrono::milliseconds> t1;
	ASSERT_EQ (t1.current_state (), oslo::timer_state::stopped);
	t1.start ();
	ASSERT_EQ (t1.current_state (), oslo::timer_state::started);
	t1.restart ();
	ASSERT_EQ (t1.current_state (), oslo::timer_state::started);
	t1.pause ();
	ASSERT_EQ (t1.current_state (), oslo::timer_state::stopped);
	...
	oslo::timer<std::chrono::milliseconds> t2 (oslo::timer_state::started);
	ASSERT_EQ (t2.current_state (), oslo::timer_state::started);
	t2.stop ();
	ASSERT_EQ (t2.current_state (), oslo::timer_state::stopped);
}

TEST (timer, measure_and_compare)
{
	using namespace std::chrono_literals;
	oslo::timer<std::chrono::milliseconds> t1 (oslo::timer_state::started);
	ASSERT_EQ (t1.current_state (), oslo::timer_state::started);
