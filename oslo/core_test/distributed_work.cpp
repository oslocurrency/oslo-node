#include <oslo/core_test/fakes/work_peer.hpp>
...
TEST (distributed_work, stopped)
{
	...
	ASSERT_TRUE (system.nodes[0]->distributed_work.make (oslo::work_version::work_1, oslo::block_hash (), {}, oslo::network_constants ().publish_thresholds.base, {}));
}
...
TEST (distributed_work, no_peers)
{
	...
	ASSERT_FALSE (node->distributed_work.make (oslo::work_version::work_1, hash, node->config.work_peers, node->network_params.network.publish_thresholds.base, callback, oslo::account ()));
	system.deadline_set (5s);
	while (!done)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_GE (oslo::work_difficulty (oslo::work_version::work_1, hash, *work), node->network_params.network.publish_thresholds.base);
...
TEST (distributed_work, no_peers_disabled)
{
	...
	ASSERT_TRUE (node.distributed_work.make (oslo::work_version::work_1, oslo::block_hash (), node.config.work_peers, oslo::network_constants ().publish_thresholds.base, {}));
}
...
TEST (distributed_work, no_peers_cancel)
{
	...
	ASSERT_FALSE (node.distributed_work.make (oslo::work_version::work_1, hash, node.config.work_peers, oslo::difficulty::from_multiplier (1e6, node.network_params.network.publish_thresholds.base), callback_to_cancel));
	ASSERT_EQ (1, node.distributed_work.items.size ());
...
TEST (distributed_work, no_peers_multi)
{
	...
	{
		ASSERT_FALSE (node->distributed_work.make (oslo::work_version::work_1, hash, node->config.work_peers, oslo::difficulty::from_multiplier (10, node->network_params.network.publish_thresholds.base), callback));
	}
	...
	for (unsigned i{ 0 }; i < total; ++i)
	{
		oslo::block_hash hash_i (i + 1);
		ASSERT_FALSE (node->distributed_work.make (oslo::work_version::work_1, hash_i, node->config.work_peers, node->network_params.network.publish_thresholds.base, callback));
	}
...
TEST (distributed_work, peer)
{
	...
	auto work_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, oslo::get_available_port (), work_peer_type::good));
	work_peer->start ();
	decltype (node->config.work_peers) peers;
	peers.emplace_back ("::ffff:127.0.0.1", work_peer->port ());
	ASSERT_FALSE (node->distributed_work.make (oslo::work_version::work_1, hash, peers, node->network_params.network.publish_thresholds.base, callback, oslo::account ()));
	system.deadline_set (5s);
	while (!done)
	{
		ASSERT_NO_ERROR (system.poll ());
...
TEST (distributed_work, peer_malicious)
{
	...
	auto malicious_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, oslo::get_available_port (), work_peer_type::malicious));
	...
	ASSERT_FALSE (node->local_work_generation_enabled ());
	auto malicious_peer2 (std::make_shared<fake_work_peer> (node->work, node->io_ctx, oslo::get_available_port (), work_peer_type::malicious));
	malicious_peer2->start ();
	peers[0].second = malicious_peer2->port ();
	ASSERT_FALSE (node->distributed_work.make (oslo::work_version::work_1, hash, peers, node->network_params.network.publish_thresholds.base, {}, oslo::account ()));
	system.deadline_set (5s);
...
TEST (distributed_work, peer_multi)
{
	...
	auto good_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, oslo::get_available_port (), work_peer_type::good));
	auto malicious_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, oslo::get_available_port (), work_peer_type::malicious));
	auto slow_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, oslo::get_available_port (), work_peer_type::slow));
	good_peer->start ();
	malicious_peer->start ();
	slow_peer->start ();
	decltype (node->config.work_peers) peers;
