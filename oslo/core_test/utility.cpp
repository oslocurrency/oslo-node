...
TEST (rate, basic)
{
	oslo::rate::token_bucket bucket (10, 10);
...
TEST (rate, network)
{
	...
	oslo::rate::token_bucket bucket (10, 5);
...
TEST (rate, unlimited)
{
	oslo::rate::token_bucket bucket (0, 0);
	ASSERT_TRUE (bucket.try_consume (5));
	ASSERT_EQ (bucket.largest_burst (), 5);
...
TEST (filesystem, remove_all_files)
{
	...
	oslo::remove_all_files_in_dir (path);
...
TEST (filesystem, move_all_files)
{
	...
	oslo::move_all_files_to_dir (dummy_directory, path);
...
{
	oslo::relaxed_atomic_integral<uint32_t> atomic{ 0 };
	ASSERT_EQ (0, atomic++);
	ASSERT_EQ (1, atomic);
	ASSERT_EQ (2, ++atomic);
	ASSERT_EQ (2, atomic);
