...
class public_key;
class block_hash;
extern oslo::keypair const & zero_key;
extern oslo::keypair const & test_genesis_key;
extern std::string const & oslo_test_genesis;
extern std::string const & genesis_block;
extern oslo::block_hash const & genesis_hash;
extern oslo::public_key const & oslo_test_account;
extern oslo::public_key const & genesis_account;
extern oslo::public_key const & burn_account;
extern oslo::uint128_t const & genesis_amount;
...
namespace util
{
	...
	public:
		...
		{
			oslo::timer<UNIT> timer (oslo::timer_state::started);
			bool error = true;
...
#ifndef IGNORE_GTEST_INCL
inline void compare_default_telemetry_response_data_excluding_signature (oslo::telemetry_data const & telemetry_data_a, oslo::network_params const & network_params_a, uint64_t bandwidth_limit_a, uint64_t active_difficulty_a)
{
	...
	ASSERT_EQ (telemetry_data_a.major_version, oslo::get_major_node_version ());
	ASSERT_EQ (telemetry_data_a.minor_version, oslo::get_minor_node_version ());
	ASSERT_EQ (telemetry_data_a.patch_version, oslo::get_patch_node_version ());
	ASSERT_EQ (telemetry_data_a.pre_release_version, oslo::get_pre_release_node_version ());
...
inline void compare_default_telemetry_response_data (oslo::telemetry_data const & telemetry_data_a, oslo::network_params const & network_params_a, uint64_t bandwidth_limit_a, uint64_t active_difficulty_a, oslo::keypair const & node_id_a)
{
	ASSERT_FALSE (telemetry_data_a.validate_signature (oslo::telemetry_data::size));
	oslo::telemetry_data telemetry_data_l = telemetry_data_a;
