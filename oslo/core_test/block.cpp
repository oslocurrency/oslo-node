...
{
	oslo::private_key prv (0);
	auto pub (oslo::pub_key (prv));
	oslo::uint256_union message (0);
...
{
	oslo::keypair key1;
	oslo::send_block block (0, 1, 13, key1.prv, key1.pub, 2);
	auto hash (block.hash ());
	ASSERT_FALSE (oslo::validate_message (key1.pub, hash, block.signature));
	block.signature.bytes[32] ^= 0x1;
	ASSERT_TRUE (oslo::validate_message (key1.pub, hash, block.signature));
}

TEST (block, send_serialize)
{
	oslo::send_block block1 (0, 1, 2, oslo::keypair ().prv, 4, 5);
	...
	ASSERT_NE (0, size);
	oslo::bufferstream stream2 (data, size);
	bool error (false);
	oslo::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (block, send_serialize_json)
{
	...
	bool error (false);
	oslo::send_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	oslo::receive_block block1 (0, 1, oslo::keypair ().prv, 3, 4);
	oslo::keypair key1;
	std::vector<uint8_t> bytes;
	{
		oslo::vectorstream stream1 (bytes);
	...
	bool error (false);
	oslo::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, receive_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	oslo::receive_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, open_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	oslo::open_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	oslo::change_block block1 (0, 1, oslo::keypair ().prv, 3, 4);
	std::string string1;
	block1.serialize_json (string1);
	ASSERT_NE (0, string1.size ());
	boost::property_tree::ptree tree1;
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	oslo::change_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	oslo::uint512_union input (oslo::uint512_t (0));
	std::string text;
	input.encode_hex (text);
	oslo::uint512_union output;
...
{
	oslo::uint512_union input (oslo::uint512_t (1));
	std::string text;
	input.encode_hex (text);
	oslo::uint512_union output;
	auto error (output.decode_hex (text));
...
{
	oslo::uint512_union input (oslo::uint512_t (1000));
	std::string text;
	input.encode_hex (text);
	text[5] = '!';
	oslo::uint512_union output;
...
{
	oslo::uint512_union input (std::numeric_limits<oslo::uint512_t>::max ());
	std::string text;
	input.encode_hex (text);
	oslo::uint512_union output;
	auto error (output.decode_hex (text));
	ASSERT_FALSE (error);
	ASSERT_EQ (input, output);
	ASSERT_EQ (oslo::uint512_t ("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"), output.number ());
}
...
TEST (send_block, deserialize)
{
	...
	ASSERT_EQ (oslo::send_block::size, bytes.size ());
	oslo::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	oslo::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (receive_block, deserialize)
{
	...
	ASSERT_EQ (oslo::receive_block::size, bytes.size ());
	oslo::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	oslo::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (open_block, deserialize)
{
	...
	ASSERT_EQ (oslo::open_block::size, bytes.size ());
	oslo::bufferstream stream (bytes.data (), bytes.size ());
	bool error (false);
	oslo::open_block block2 (error, stream);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	oslo::change_block block1 (1, 2, oslo::keypair ().prv, 4, 5);
	ASSERT_EQ (block1.hash (), block1.hash ());
	std::vector<uint8_t> bytes;
	{
		oslo::vectorstream stream1 (bytes);
	...
	ASSERT_EQ (oslo::change_block::size, bytes.size ());
	auto data (bytes.data ());
	auto size (bytes.size ());
	ASSERT_NE (nullptr, data);
	ASSERT_NE (0, size);
	oslo::bufferstream stream2 (data, size);
	bool error (false);
	oslo::change_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (frontier_req, serialization)
{
	...
	ASSERT_FALSE (error);
	oslo::frontier_req request2 (error, stream, header);
	ASSERT_FALSE (error);
...
TEST (block, publish_req_serialization)
{
	...
	ASSERT_FALSE (error);
	oslo::publish req2 (error, stream2, header);
	ASSERT_FALSE (error);
	ASSERT_EQ (req, req2);
...
{
	oslo::send_block block (0, 1, 2, oslo::keypair ().prv, 4, 5);
	ASSERT_EQ (block.difficulty (), oslo::work_difficulty (block.work_version (), block.root (), block.block_work ()));
}
...
TEST (state_block, serialization)
{
	oslo::keypair key1;
	oslo::keypair key2;
	oslo::state_block block1 (key1.pub, 1, key2.pub, 2, 4, key1.prv, key1.pub, 5);
	ASSERT_EQ (key1.pub, block1.hashables.account);
	ASSERT_EQ (oslo::block_hash (1), block1.previous ());
	ASSERT_EQ (key2.pub, block1.hashables.representative);
	ASSERT_EQ (oslo::amount (2), block1.hashables.balance);
	ASSERT_EQ (oslo::uint256_union (4), block1.hashables.link);
	std::vector<uint8_t> bytes;
	{
		oslo::vectorstream stream (bytes);
		block1.serialize (stream);
	...
	ASSERT_EQ (oslo::state_block::size, bytes.size ());
	bool error1 (false);
	oslo::bufferstream stream (bytes.data (), bytes.size ());
	oslo::state_block block2 (error1, stream);
	ASSERT_FALSE (error1);
	ASSERT_EQ (block1, block2);
	...
	bool error2 (false);
	oslo::state_block block3 (error2, tree);
	ASSERT_FALSE (error2);
...
{
	ASSERT_EQ (oslo::work_version::work_1, oslo::send_block ().work_version ());
	ASSERT_EQ (oslo::work_version::work_1, oslo::receive_block ().work_version ());
	ASSERT_EQ (oslo::work_version::work_1, oslo::change_block ().work_version ());
	ASSERT_EQ (oslo::work_version::work_1, oslo::open_block ().work_version ());
	ASSERT_EQ (oslo::work_version::work_1, oslo::state_block ().work_version ());
}
...
TEST (block_uniquer, single)
{
	...
	ASSERT_EQ (*block1, *block2);
	std::weak_ptr<oslo::state_block> block3 (block2);
...
TEST (block_uniquer, cleanup)
{
	oslo::keypair key;
	auto block1 (std::make_shared<oslo::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0));
	auto block2 (std::make_shared<oslo::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 1));
	oslo::block_uniquer uniquer;
...
TEST (block_builder, zeroed_state_block)
{
	...
	auto zero_block_manual (std::make_shared<oslo::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0));
	...
	ASSERT_FALSE (oslo::validate_message (key.pub, zero_block_build->hash (), zero_block_build->signature));
}
...
TEST (block_builder, state_missing_rep)
{
	...
	auto block = builder
	             ...
	             .work (0)
	             .build (ec);
	ASSERT_EQ (ec, oslo::error_common::missing_representative);
}
...
TEST (block_builder, state_errors)
{
	...
	ASSERT_EQ (ec, oslo::error_common::bad_account_number);
...
TEST (block_builder, open_equality)
{
	...
	// With constructor
	oslo::keypair key1, key2;
	oslo::open_block block1 (1, key1.pub, key2.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, change_equality)
{
	...
	// With constructor
	oslo::keypair key1, key2;
	oslo::change_block block1 (1, key1.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, send_equality)
{
	...
	// With constructor
	oslo::keypair key1, key2;
	oslo::send_block block1 (1, key1.pub, 2, key1.prv, key1.pub, 5);
...
TEST (block_builder, receive_equality)
{
	...
	// With constructor
	oslo::keypair key1;
	oslo::receive_block block1 (1, 2, key1.prv, key1.pub, 5);
