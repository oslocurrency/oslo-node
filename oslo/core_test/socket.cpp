...
TEST (socket, drop_policy)
{
	...
	oslo::inactive_node inactivenode (oslo::unique_path (), node_flags);
	auto node = inactivenode.node;

	oslo::thread_runner runner (node->io_ctx, 1);

	std::vector<std::shared_ptr<oslo::socket>> connections;
	...
	{
		auto client_dummy (std::make_shared<oslo::socket> (node, boost::none, oslo::socket::concurrency::multi_writer));
		max_write_queue_size = client_dummy->get_max_write_queue_size ();
	}

	auto func = [&](size_t total_message_count, oslo::buffer_drop_policy drop_policy) {
		auto server_port (oslo::get_available_port ());
		boost::asio::ip::tcp::endpoint endpoint (boost::asio::ip::address_v4::any (), server_port);

		auto server_socket (std::make_shared<oslo::server_socket> (node, endpoint, 1, oslo::socket::concurrency::multi_writer));
		boost::system::error_code ec;
		server_socket->start (ec);
		ASSERT_FALSE (ec);

		// Accept connection, but don't read so the writer will drop.
		server_socket->on_connection ([&connections](std::shared_ptr<oslo::socket> new_connection, boost::system::error_code const & ec_a) {
			connections.push_back (new_connection);
			return true;
		});

		auto client (std::make_shared<oslo::socket> (node, boost::none, oslo::socket::concurrency::multi_writer));
		oslo::util::counted_completion write_completion (total_message_count);
		...
		[client, total_message_count, node, &write_completion, &drop_policy](boost::system::error_code const & ec_a) {
			for (int i = 0; i < total_message_count; i++)
			{
				...
				client->async_write (
				oslo::shared_const_buffer (std::move (buff)), [&write_completion](boost::system::error_code const & ec, size_t size_a) {
	...
	func (max_write_queue_size * 2 + 1, oslo::buffer_drop_policy::no_socket_drop);
	ASSERT_EQ (1, node->stats.count (oslo::stat::type::tcp, oslo::stat::detail::tcp_write_no_socket_drop, oslo::stat::dir::out));
	ASSERT_EQ (0, node->stats.count (oslo::stat::type::tcp, oslo::stat::detail::tcp_write_drop, oslo::stat::dir::out));

	func (max_write_queue_size + 1, oslo::buffer_drop_policy::limiter);
	// The stats are accumulated from before
	ASSERT_EQ (1, node->stats.count (oslo::stat::type::tcp, oslo::stat::detail::tcp_write_no_socket_drop, oslo::stat::dir::out));
	ASSERT_EQ (1, node->stats.count (oslo::stat::type::tcp, oslo::stat::detail::tcp_write_drop, oslo::stat::dir::out));
...
TEST (socket, concurrent_writes)
{
	...
	oslo::util::counted_completion read_count_completion (total_message_count);
	std::function<void(std::shared_ptr<oslo::socket>)> reader = [&read_count_completion, &total_message_count, &reader](std::shared_ptr<oslo::socket> socket_a) {
...
#pragma GCC diagnostic pop
	...
	auto server_socket (std::make_shared<oslo::server_socket> (node, endpoint, max_connections, oslo::socket::concurrency::multi_writer));
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	std::vector<std::shared_ptr<oslo::socket>> connections;
	...
	server_socket->on_connection ([&connections, &reader](std::shared_ptr<oslo::socket> new_connection, boost::system::error_code const & ec_a) {
		if (ec_a)
		{
	...
	oslo::util::counted_completion connection_count_completion (client_count);
	std::vector<std::shared_ptr<oslo::socket>> clients;
...
#pragma GCC diagnostic ignored "-Wunused-lambda-capture"
		...
		client_threads.emplace_back ([&client, &message_count]() {
			for (int i = 0; i < message_count; i++)
			{
				...
				buff.push_back ('A' + i);
				client->async_write (oslo::shared_const_buffer (std::move (buff)));
			}
		});
#ifndef _WIN32
#pragma GCC diagnostic pop
	...
	ASSERT_EQ (node->stats.count (oslo::stat::type::tcp, oslo::stat::detail::tcp_accept_success, oslo::stat::dir::in), client_count);
	// We may exhaust max connections and have some tcp accept failures, but no more than the client count
	ASSERT_LT (node->stats.count (oslo::stat::type::tcp, oslo::stat::detail::tcp_accept_failure, oslo::stat::dir::in), client_count);
