...
TEST (vote_processor, codes)
{
	...
	oslo::keypair key;
	auto vote (std::make_shared<oslo::vote> (key.pub, key.prv, 1, std::vector<oslo::block_hash>{ genesis.open->hash () }));
	auto vote_invalid = std::make_shared<oslo::vote> (*vote);
	...
	// Invalid signature
	ASSERT_EQ (oslo::vote_code::invalid, node.vote_processor.vote_blocking (vote_invalid, channel, false));

	// Hint of pre-validation
	ASSERT_NE (oslo::vote_code::invalid, node.vote_processor.vote_blocking (vote_invalid, channel, true));

	// No ongoing election
	ASSERT_EQ (oslo::vote_code::indeterminate, node.vote_processor.vote_blocking (vote, channel));
	...
	ASSERT_EQ (oslo::vote_code::vote, node.vote_processor.vote_blocking (vote, channel));

	// Processing the same vote is a replay
	ASSERT_EQ (oslo::vote_code::replay, node.vote_processor.vote_blocking (vote, channel));

	// Invalid takes precedence
	ASSERT_EQ (oslo::vote_code::invalid, node.vote_processor.vote_blocking (vote_invalid, channel));

	// A higher sequence is not a replay
	++vote->sequence;
	ASSERT_EQ (oslo::vote_code::invalid, node.vote_processor.vote_blocking (vote, channel));
	vote->signature = oslo::sign_message (key.prv, key.pub, vote->hash ());
	ASSERT_EQ (oslo::vote_code::vote, node.vote_processor.vote_blocking (vote, channel));
...
TEST (vote_processor, flush)
{
	...
	auto & node (*system.nodes[0]);
	oslo::genesis genesis;
	auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 1, std::vector<oslo::block_hash>{ genesis.open->hash () }));
	auto channel (std::make_shared<oslo::transport::channel_udp> (node.network.udp_channels, node.network.endpoint (), node.network_params.protocol.protocol_version));
	for (unsigned i = 0; i < 2000; ++i)
	{
		auto new_vote (std::make_shared<oslo::vote> (*vote));
...
TEST (vote_processor, overflow)
{
	...
	ASSERT_EQ (not_processed, node.stats.count (oslo::stat::type::vote, oslo::stat::detail::vote_overflow));
}
...
TEST (vote_processor, weights)
{
	...
	system.wallet (0)->send_sync (oslo::test_genesis_key.pub, key0.pub, level0);
	system.wallet (0)->send_sync (oslo::test_genesis_key.pub, key1.pub, level1);
	system.wallet (0)->send_sync (oslo::test_genesis_key.pub, key2.pub, level2);
	...
	ASSERT_NE (node.vote_processor.representatives_1.end (), node.vote_processor.representatives_1.find (oslo::test_genesis_key.pub));
	ASSERT_NE (node.vote_processor.representatives_2.end (), node.vote_processor.representatives_2.find (oslo::test_genesis_key.pub));
	ASSERT_NE (node.vote_processor.representatives_3.end (), node.vote_processor.representatives_3.find (oslo::test_genesis_key.pub));
}
}
...
TEST (vote_processor, no_broadcast_local)
{
	...
	oslo::keypair key;
	std::shared_ptr<oslo::block> send = builder.state ()
	...
	ASSERT_EQ (2 * node.config.vote_minimum.number (), node.weight (oslo::test_genesis_key.pub));
	// Insert account in wallet
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	node.wallets.compute_reps ();
	ASSERT_TRUE (node.wallets.reps ().exists (oslo::test_genesis_key.pub));
	ASSERT_FALSE (node.wallets.reps ().have_half_rep ());
	// Process a vote
	auto vote (node.store.vote_generate (node.store.tx_begin_read (), oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, { send->hash () }));
	ASSERT_EQ (oslo::vote_code::vote, node.active.vote (vote));
	// Make sure the vote was processed
	auto election (node.active.election (send->qualified_root ()));
	ASSERT_NE (nullptr, election);
	auto existing (election->last_votes.find (oslo::test_genesis_key.pub));
	...
	ASSERT_EQ (1, node.stats.count (oslo::stat::type::message, oslo::stat::detail::publish, oslo::stat::dir::out));

	// Repeat test with no representative
	// Erase account from the wallet
	system.wallet (0)->store.erase (node.wallets.tx_begin_write (), oslo::test_genesis_key.pub);
	node.wallets.compute_reps ();
	ASSERT_FALSE (node.wallets.reps ().exists (oslo::test_genesis_key.pub));
	...
	std::shared_ptr<oslo::block> send2 = builder.state ()
	                                     ...
	                                     .build (ec);
	ASSERT_FALSE (ec);
	ASSERT_EQ (oslo::process_result::progress, node.process_local (send2).code);
	ASSERT_EQ (node.config.vote_minimum, node.weight (oslo::test_genesis_key.pub));
	node.block_confirm (send2);
	// Process a vote
	auto vote2 (node.store.vote_generate (node.store.tx_begin_read (), oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, { send2->hash () }));
	ASSERT_EQ (oslo::vote_code::vote, node.active.vote (vote2));
	// Make sure the vote was processed
	auto election2 (node.active.election (send2->qualified_root ()));
	ASSERT_NE (nullptr, election2);
	auto existing2 (election2->last_votes.find (oslo::test_genesis_key.pub));
	...
	ASSERT_EQ (2, node.stats.count (oslo::stat::type::message, oslo::stat::detail::publish, oslo::stat::dir::out));

	// Repeat test with a PR in the wallet
	// Increase the genesis weight again
	std::shared_ptr<oslo::block> open = builder.state ()
	                                    .account (key.pub)
	                                    .representative (oslo::test_genesis_key.pub)
	                                    .previous (0)
	                                    .balance (oslo::genesis_amount - 2 * node.config.vote_minimum.number ())
	                                    .link (send->hash ())
	                                    .sign (key.prv, key.pub)
	                                    .work (*system.work.generate (key.pub))
	                                    .build (ec);
	ASSERT_FALSE (ec);
	ASSERT_EQ (oslo::process_result::progress, node.process_local (open).code);
	ASSERT_EQ (oslo::genesis_amount - node.config.vote_minimum.number (), node.weight (oslo::test_genesis_key.pub));
	...
	// Process a vote
	auto vote3 (node.store.vote_generate (node.store.tx_begin_read (), oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, { open->hash () }));
	ASSERT_EQ (oslo::vote_code::vote, node.active.vote (vote3));
	// Make sure the vote was processed
	auto election3 (node.active.election (open->qualified_root ()));
	ASSERT_NE (nullptr, election3);
	auto existing3 (election3->last_votes.find (oslo::test_genesis_key.pub));
	...
	ASSERT_EQ (1, node.stats.count (oslo::stat::type::message, oslo::stat::detail::confirm_ack, oslo::stat::dir::out));
	ASSERT_EQ (3, node.stats.count (oslo::stat::type::message, oslo::stat::detail::publish, oslo::stat::dir::out));
}
