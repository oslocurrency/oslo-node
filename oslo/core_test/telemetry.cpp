...
TEST (telemetry, consolidate_data)
{
	...
	data.uptime = 6;
	data.genesis_block = oslo::block_hash (3);
	...
	data1.uptime = 10;
	data1.genesis_block = oslo::block_hash (4);
	...
	data2.uptime = 3;
	data2.genesis_block = oslo::block_hash (4);
	...
	std::vector<oslo::telemetry_data> all_data{ data, data1, data2 };

	auto consolidated_telemetry_data = oslo::consolidate_telemetry_data (all_data);
	...
	ASSERT_EQ (consolidated_telemetry_data.genesis_block, oslo::block_hash (4));
	...
	all_data[2].genesis_block = oslo::block_hash (3);
	...
	auto consolidated_telemetry_data1 = oslo::consolidate_telemetry_data (all_data);
	...
	ASSERT_EQ (consolidated_telemetry_data1.genesis_block, oslo::block_hash (3));
...
TEST (telemetry, consolidate_data_remove_outliers)
{
	...
	std::vector<oslo::telemetry_data> all_data (20, data);

	// Insert some outliers
	oslo::telemetry_data lower_bound_outlier_data;
	...
	lower_bound_outlier_data.genesis_block = oslo::block_hash (2);
	...
	oslo::telemetry_data upper_bound_outlier_data;
	...
	upper_bound_outlier_data.genesis_block = oslo::block_hash (99);
...
TEST (telemetry, signatures)
{
	...
	data.timestamp = std::chrono::system_clock::time_point (100ms);
	data.sign (node_id);
	ASSERT_FALSE (data.validate_signature (oslo::telemetry_data::size));
...
TEST (telemetry, basic)
{
	...
	auto channel = node_client->network.find_channel (node_server->network.endpoint ());
	{
		...
		node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &server_endpoint, &telemetry_data](oslo::telemetry_data_response const & response_a) {
	...
	oslo::compare_default_telemetry_response_data (telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->active.active_difficulty (), node_server->node_id);
	...
	std::this_thread::sleep_for (oslo::telemetry_cache_cutoffs::test);
...
TEST (telemetry, receive_from_non_listening_channel)
{
	...
	oslo::telemetry_ack message (oslo::telemetry_data{});
...
TEST (telemetry, over_udp)
{
	...
	std::atomic<bool> done{ false };
	auto channel = node_client->network.find_channel (node_server->network.endpoint ());
	node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &node_server](oslo::telemetry_data_response const & response_a) {
		ASSERT_FALSE (response_a.error);
		oslo::compare_default_telemetry_response_data (response_a.telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->active.active_difficulty (), node_server->node_id);
		done = true;
	});
...
TEST (telemetry, invalid_channel)
{
	...
	std::atomic<bool> done{ false };
	node_client->telemetry->get_metrics_single_peer_async (nullptr, [&done](oslo::telemetry_data_response const & response_a) {
...
TEST (telemetry, blocking_request)
{
	...
	oslo::compare_default_telemetry_response_data (telemetry_data_response.telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->active.active_difficulty (), node_server->node_id);
...
TEST (telemetry, disconnects)
{
	...
	std::atomic<bool> done{ false };
	node_client->telemetry->get_metrics_single_peer_async (channel, [&done](oslo::telemetry_data_response const & response_a) {
		ASSERT_TRUE (response_a.error);
		done = true;
	});
...
TEST (telemetry, dos_tcp)
{
	...
	oslo::telemetry_req message;
	auto channel = node_client->network.tcp_channels.find_channel (oslo::transport::map_endpoint_to_tcp (node_server->network.endpoint ()));
	channel->send (message, [](boost::system::error_code const & ec, size_t size_a) {
		ASSERT_FALSE (ec);
	});

	system.deadline_set (10s);
	while (1 != node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::in))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	system.deadline_set (10s);
	while ((oslo::telemetry_cache_cutoffs::test + orig) > std::chrono::steady_clock::now ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	while (node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::in) == 1)
	{
...
TEST (telemetry, disable_metrics)
{
	...
	node_server->telemetry->get_metrics_single_peer_async (channel1, [&done, node_client](oslo::telemetry_data_response const & response_a) {
		ASSERT_FALSE (response_a.error);
		oslo::compare_default_telemetry_response_data (response_a.telemetry_data, node_client->network_params, node_client->config.bandwidth_limit, node_client->active.active_difficulty (), node_client->node_id);
		done = true;
	});
...
TEST (telemetry, remove_peer_different_genesis)
{
	...
	node1->network_params.ledger.genesis_hash = oslo::block_hash ("0");
	node1->start ();
	system.nodes.push_back (node1);
	node0->network.merge_peer (node1->network.endpoint ());
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (10s, node0->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::different_genesis_hash) != 0 && node1->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::different_genesis_hash) != 0);

	ASSERT_TIMELY (1s, 0 == node0->network.size ());
	ASSERT_TIMELY (1s, 0 == node1->network.size ());
	ASSERT_EQ (node0->stats.count (oslo::stat::type::message, oslo::stat::detail::node_id_handshake, oslo::stat::dir::out), 1);
	ASSERT_EQ (node1->stats.count (oslo::stat::type::message, oslo::stat::detail::node_id_handshake, oslo::stat::dir::out), 1);

	oslo::lock_guard<std::mutex> guard (node0->network.excluded_peers.mutex);
	ASSERT_EQ (1, node0->network.excluded_peers.peers.get<oslo::peer_exclusion::tag_endpoint> ().count (node1->network.endpoint ().address ()));
	ASSERT_EQ (1, node1->network.excluded_peers.peers.get<oslo::peer_exclusion::tag_endpoint> ().count (node0->network.endpoint ().address ()));
}
...
TEST (telemetry, remove_peer_different_genesis_udp)
{
	...
	node_flags.disable_tcp_realtime = true;
	node_flags.disable_ongoing_telemetry_requests = true;
	oslo::system system (1, oslo::transport::transport_type::udp, node_flags);
	auto node0 (system.nodes[0]);
	ASSERT_EQ (0, node0->network.size ());
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto channel0 (std::make_shared<oslo::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node0->network_params.protocol.protocol_version));
	auto channel1 (std::make_shared<oslo::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.protocol.protocol_version));
	...
	node0->telemetry->get_metrics_single_peer_async (channel1, [&done0](oslo::telemetry_data_response const & response_a) {
		done0 = true;
	});

	node1->telemetry->get_metrics_single_peer_async (channel0, [&done1](oslo::telemetry_data_response const & response_a) {
		done1 = true;
	});
...
TEST (telemetry, remove_peer_invalid_signature)
{
	...
	node->telemetry->recent_or_initial_request_telemetry_data.emplace (channel->get_endpoint (), oslo::telemetry_data (), std::chrono::steady_clock::now (), true);
	...
	auto telemetry_ack = oslo::telemetry_ack (telemetry_data);
	node->network.process_message (telemetry_ack, channel);

	ASSERT_TIMELY (10s, node->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::invalid_signature) > 0);
	ASSERT_NO_ERROR (system.poll_until_true (3s, [&node, address = channel->get_endpoint ().address ()]() -> bool {
		oslo::lock_guard<std::mutex> guard (node->network.excluded_peers.mutex);
		return node->network.excluded_peers.peers.get<oslo::peer_exclusion::tag_endpoint> ().count (address);
	}));
}
}
