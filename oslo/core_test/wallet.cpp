#include <oslo/core_test/testutil.hpp>
#include <oslo/crypto_lib/random_pool.hpp>
#include <oslo/lib/threading.hpp>
#include <oslo/node/lmdb/wallet_value.hpp>
#include <oslo/node/testing.hpp>

#include <gtest/gtest.h>

#include <boost/filesystem.hpp>

using namespace std::chrono_literals;
unsigned constexpr oslo::wallet_store::version_current;

TEST (wallet, no_special_keys_accounts)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::keypair key1;
	ASSERT_FALSE (wallet.exists (transaction, key1.pub));
	wallet.insert_adhoc (transaction, key1.prv);
	ASSERT_TRUE (wallet.exists (transaction, key1.pub));

	for (uint64_t account = 0; account < oslo::wallet_store::special_count; account++)
	{
		oslo::account account_l (account);
		ASSERT_FALSE (wallet.exists (transaction, account_l));
	}
}

TEST (wallet, no_key)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::keypair key1;
	oslo::raw_key prv1;
	ASSERT_TRUE (wallet.fetch (transaction, key1.pub, prv1));
	ASSERT_TRUE (wallet.valid_password (transaction));
}

TEST (wallet, fetch_locked)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_TRUE (wallet.valid_password (transaction));
	oslo::keypair key1;
	ASSERT_EQ (key1.pub, wallet.insert_adhoc (transaction, key1.prv));
	auto key2 (wallet.deterministic_insert (transaction));
	ASSERT_FALSE (key2.is_zero ());
	oslo::raw_key key3;
	key3.data = 1;
	wallet.password.value_set (key3);
	ASSERT_FALSE (wallet.valid_password (transaction));
	oslo::raw_key key4;
	ASSERT_TRUE (wallet.fetch (transaction, key1.pub, key4));
	ASSERT_TRUE (wallet.fetch (transaction, key2, key4));
}

TEST (wallet, retrieval)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::keypair key1;
	ASSERT_TRUE (wallet.valid_password (transaction));
	wallet.insert_adhoc (transaction, key1.prv);
	oslo::raw_key prv1;
	ASSERT_FALSE (wallet.fetch (transaction, key1.pub, prv1));
	ASSERT_TRUE (wallet.valid_password (transaction));
	ASSERT_EQ (key1.prv, prv1);
	wallet.password.values[0]->bytes[16] ^= 1;
	oslo::raw_key prv2;
	ASSERT_TRUE (wallet.fetch (transaction, key1.pub, prv2));
	ASSERT_FALSE (wallet.valid_password (transaction));
}

TEST (wallet, empty_iteration)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	auto i (wallet.begin (transaction));
	auto j (wallet.end ());
	ASSERT_EQ (i, j);
}

TEST (wallet, one_item_iteration)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::keypair key1;
	wallet.insert_adhoc (transaction, key1.prv);
	for (auto i (wallet.begin (transaction)), j (wallet.end ()); i != j; ++i)
	{
		ASSERT_EQ (key1.pub, oslo::uint256_union (i->first));
		oslo::raw_key password;
		wallet.wallet_key (password, transaction);
		oslo::raw_key key;
		key.decrypt (oslo::wallet_value (i->second).key, password, (oslo::uint256_union (i->first)).owords[0].number ());
		ASSERT_EQ (key1.prv, key);
	}
}

TEST (wallet, two_item_iteration)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	oslo::keypair key1;
	oslo::keypair key2;
	ASSERT_NE (key1.pub, key2.pub);
	std::unordered_set<oslo::public_key> pubs;
	std::unordered_set<oslo::private_key> prvs;
	oslo::kdf kdf;
	{
		auto transaction (env.tx_begin_write ());
		oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
		ASSERT_FALSE (init);
		wallet.insert_adhoc (transaction, key1.prv);
		wallet.insert_adhoc (transaction, key2.prv);
		for (auto i (wallet.begin (transaction)), j (wallet.end ()); i != j; ++i)
		{
			pubs.insert (i->first);
			oslo::raw_key password;
			wallet.wallet_key (password, transaction);
			oslo::raw_key key;
			key.decrypt (oslo::wallet_value (i->second).key, password, (i->first).owords[0].number ());
			prvs.insert (key.as_private_key ());
		}
	}
	ASSERT_EQ (2, pubs.size ());
	ASSERT_EQ (2, prvs.size ());
	ASSERT_NE (pubs.end (), pubs.find (key1.pub));
	ASSERT_NE (prvs.end (), prvs.find (key1.prv.as_private_key ()));
	ASSERT_NE (pubs.end (), pubs.find (key2.pub));
	ASSERT_NE (prvs.end (), prvs.find (key2.prv.as_private_key ()));
}

TEST (wallet, insufficient_spend_one)
{
	oslo::system system (1);
	oslo::keypair key1;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key1.pub, 500));
	ASSERT_NE (nullptr, block);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key1.pub, oslo::genesis_amount));
}

TEST (wallet, spend_all_one)
{
	oslo::system system (1);
	auto & node1 (*system.nodes[0]);
	oslo::block_hash latest1 (node1.latest (oslo::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, std::numeric_limits<oslo::uint128_t>::max ()));
	oslo::account_info info2;
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.store.account_get (transaction, oslo::test_genesis_key.pub, info2);
		ASSERT_NE (latest1, info2.head);
		auto block (node1.store.block_get (transaction, info2.head));
		ASSERT_NE (nullptr, block);
		ASSERT_EQ (latest1, block->previous ());
	}
	ASSERT_TRUE (info2.balance.is_zero ());
	ASSERT_EQ (0, node1.balance (oslo::test_genesis_key.pub));
}

TEST (wallet, send_async)
{
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key2;
	boost::thread thread ([&system]() {
		system.deadline_set (10s);
		while (!system.nodes[0]->balance (oslo::test_genesis_key.pub).is_zero ())
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	});
	std::atomic<bool> success (false);
	system.wallet (0)->send_async (oslo::test_genesis_key.pub, key2.pub, std::numeric_limits<oslo::uint128_t>::max (), [&success](std::shared_ptr<oslo::block> block_a) { ASSERT_NE (nullptr, block_a); success = true; });
	thread.join ();
	ASSERT_TIMELY (2s, success);
}

TEST (wallet, spend)
{
	oslo::system system (1);
	auto & node1 (*system.nodes[0]);
	oslo::block_hash latest1 (node1.latest (oslo::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key2;
	// Sending from empty accounts should always be an error.  Accounts need to be opened with an open block, not a send block.
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (0, key2.pub, 0));
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, std::numeric_limits<oslo::uint128_t>::max ()));
	oslo::account_info info2;
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.store.account_get (transaction, oslo::test_genesis_key.pub, info2);
		ASSERT_NE (latest1, info2.head);
		auto block (node1.store.block_get (transaction, info2.head));
		ASSERT_NE (nullptr, block);
		ASSERT_EQ (latest1, block->previous ());
	}
	ASSERT_TRUE (info2.balance.is_zero ());
	ASSERT_EQ (0, node1.balance (oslo::test_genesis_key.pub));
}

TEST (wallet, change)
{
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key2;
	auto block1 (system.nodes[0]->rep_block (oslo::test_genesis_key.pub));
	ASSERT_FALSE (block1.is_zero ());
	ASSERT_NE (nullptr, system.wallet (0)->change_action (oslo::test_genesis_key.pub, key2.pub));
	auto block2 (system.nodes[0]->rep_block (oslo::test_genesis_key.pub));
	ASSERT_FALSE (block2.is_zero ());
	ASSERT_NE (block1, block2);
}

TEST (wallet, partial_spend)
{
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, 500));
	ASSERT_EQ (std::numeric_limits<oslo::uint128_t>::max () - 500, system.nodes[0]->balance (oslo::test_genesis_key.pub));
}

TEST (wallet, spend_no_previous)
{
	oslo::system system (1);
	{
		system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		oslo::account_info info1;
		ASSERT_FALSE (system.nodes[0]->store.account_get (transaction, oslo::test_genesis_key.pub, info1));
		for (auto i (0); i < 50; ++i)
		{
			oslo::keypair key;
			system.wallet (0)->insert_adhoc (key.prv);
		}
	}
	oslo::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, 500));
	ASSERT_EQ (std::numeric_limits<oslo::uint128_t>::max () - 500, system.nodes[0]->balance (oslo::test_genesis_key.pub));
}

TEST (wallet, find_none)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::account account (1000);
	ASSERT_EQ (wallet.end (), wallet.find (transaction, account));
}

TEST (wallet, find_existing)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::keypair key1;
	ASSERT_FALSE (wallet.exists (transaction, key1.pub));
	wallet.insert_adhoc (transaction, key1.prv);
	ASSERT_TRUE (wallet.exists (transaction, key1.pub));
	auto existing (wallet.find (transaction, key1.pub));
	ASSERT_NE (wallet.end (), existing);
	++existing;
	ASSERT_EQ (wallet.end (), existing);
}

TEST (wallet, rekey)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::raw_key password;
	wallet.password.value (password);
	ASSERT_TRUE (password.data.is_zero ());
	ASSERT_FALSE (init);
	oslo::keypair key1;
	wallet.insert_adhoc (transaction, key1.prv);
	oslo::raw_key prv1;
	wallet.fetch (transaction, key1.pub, prv1);
	ASSERT_EQ (key1.prv, prv1);
	ASSERT_FALSE (wallet.rekey (transaction, "1"));
	wallet.password.value (password);
	oslo::raw_key password1;
	wallet.derive_key (password1, transaction, "1");
	ASSERT_EQ (password1, password);
	oslo::raw_key prv2;
	wallet.fetch (transaction, key1.pub, prv2);
	ASSERT_EQ (key1.prv, prv2);
	*wallet.password.values[0] = 2;
	ASSERT_TRUE (wallet.rekey (transaction, "2"));
}

TEST (account, encode_zero)
{
	oslo::account number0 (0);
	std::string str0;
	number0.encode_account (str0);

	/*
	 * Handle different lengths for "xrb_" prefixed and "oslo_" prefixed accounts
	 */
	ASSERT_EQ ((str0.front () == 'x') ? 64 : 65, str0.size ());
	ASSERT_EQ (65, str0.size ());
	oslo::account number1;
	ASSERT_FALSE (number1.decode_account (str0));
	ASSERT_EQ (number0, number1);
}

TEST (account, encode_all)
{
	oslo::account number0;
	number0.decode_hex ("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
	std::string str0;
	number0.encode_account (str0);

	/*
	 * Handle different lengths for "xrb_" prefixed and "oslo_" prefixed accounts
	 */
	ASSERT_EQ ((str0.front () == 'x') ? 64 : 65, str0.size ());
	oslo::account number1;
	ASSERT_FALSE (number1.decode_account (str0));
	ASSERT_EQ (number0, number1);
}

TEST (account, encode_fail)
{
	oslo::account number0 (0);
	std::string str0;
	number0.encode_account (str0);
	str0[16] ^= 1;
	oslo::account number1;
	ASSERT_TRUE (number1.decode_account (str0));
}

TEST (wallet, hash_password)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	oslo::raw_key hash1;
	wallet.derive_key (hash1, transaction, "");
	oslo::raw_key hash2;
	wallet.derive_key (hash2, transaction, "");
	ASSERT_EQ (hash1, hash2);
	oslo::raw_key hash3;
	wallet.derive_key (hash3, transaction, "a");
	ASSERT_NE (hash1, hash3);
}

TEST (fan, reconstitute)
{
	oslo::uint256_union value0 (0);
	oslo::fan fan (value0, 1024);
	for (auto & i : fan.values)
	{
		ASSERT_NE (value0, *i);
	}
	oslo::raw_key value1;
	fan.value (value1);
	ASSERT_EQ (value0, value1.data);
}

TEST (fan, change)
{
	oslo::raw_key value0;
	value0.data = 0;
	oslo::raw_key value1;
	value1.data = 1;
	ASSERT_NE (value0, value1);
	oslo::fan fan (value0.data, 1024);
	ASSERT_EQ (1024, fan.values.size ());
	oslo::raw_key value2;
	fan.value (value2);
	ASSERT_EQ (value0, value2);
	fan.value_set (value1);
	fan.value (value2);
	ASSERT_EQ (value1, value2);
}

TEST (wallet, reopen_default_password)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	auto transaction (env.tx_begin_write ());
	ASSERT_FALSE (init);
	oslo::kdf kdf;
	{
		oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
		ASSERT_FALSE (init);
		ASSERT_TRUE (wallet.valid_password (transaction));
	}
	{
		bool init;
		oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
		ASSERT_FALSE (init);
		ASSERT_TRUE (wallet.valid_password (transaction));
	}
	{
		oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
		ASSERT_FALSE (init);
		wallet.rekey (transaction, "");
		ASSERT_TRUE (wallet.valid_password (transaction));
	}
	{
		bool init;
		oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
		ASSERT_FALSE (init);
		ASSERT_FALSE (wallet.valid_password (transaction));
		wallet.attempt_password (transaction, " ");
		ASSERT_FALSE (wallet.valid_password (transaction));
		wallet.attempt_password (transaction, "");
		ASSERT_TRUE (wallet.valid_password (transaction));
	}
}

TEST (wallet, representative)
{
	auto error (false);
	oslo::mdb_env env (error, oslo::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (error, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	ASSERT_FALSE (wallet.is_representative (transaction));
	ASSERT_EQ (oslo::genesis_account, wallet.representative (transaction));
	ASSERT_FALSE (wallet.is_representative (transaction));
	oslo::keypair key;
	wallet.representative_set (transaction, key.pub);
	ASSERT_FALSE (wallet.is_representative (transaction));
	ASSERT_EQ (key.pub, wallet.representative (transaction));
	ASSERT_FALSE (wallet.is_representative (transaction));
	wallet.insert_adhoc (transaction, key.prv);
	ASSERT_TRUE (wallet.is_representative (transaction));
}

TEST (wallet, serialize_json_empty)
{
	auto error (false);
	oslo::mdb_env env (error, oslo::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet1 (error, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	oslo::wallet_store wallet2 (error, kdf, transaction, oslo::genesis_account, 1, "1", serialized);
	ASSERT_FALSE (error);
	oslo::raw_key password1;
	oslo::raw_key password2;
	wallet1.wallet_key (password1, transaction);
	wallet2.wallet_key (password2, transaction);
	ASSERT_EQ (password1, password2);
	ASSERT_EQ (wallet1.salt (transaction), wallet2.salt (transaction));
	ASSERT_EQ (wallet1.check (transaction), wallet2.check (transaction));
	ASSERT_EQ (wallet1.representative (transaction), wallet2.representative (transaction));
	ASSERT_EQ (wallet1.end (), wallet1.begin (transaction));
	ASSERT_EQ (wallet2.end (), wallet2.begin (transaction));
}

TEST (wallet, serialize_json_one)
{
	auto error (false);
	oslo::mdb_env env (error, oslo::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet1 (error, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	oslo::keypair key;
	wallet1.insert_adhoc (transaction, key.prv);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	oslo::wallet_store wallet2 (error, kdf, transaction, oslo::genesis_account, 1, "1", serialized);
	ASSERT_FALSE (error);
	oslo::raw_key password1;
	oslo::raw_key password2;
	wallet1.wallet_key (password1, transaction);
	wallet2.wallet_key (password2, transaction);
	ASSERT_EQ (password1, password2);
	ASSERT_EQ (wallet1.salt (transaction), wallet2.salt (transaction));
	ASSERT_EQ (wallet1.check (transaction), wallet2.check (transaction));
	ASSERT_EQ (wallet1.representative (transaction), wallet2.representative (transaction));
	ASSERT_TRUE (wallet2.exists (transaction, key.pub));
	oslo::raw_key prv;
	wallet2.fetch (transaction, key.pub, prv);
	ASSERT_EQ (key.prv, prv);
}

TEST (wallet, serialize_json_password)
{
	auto error (false);
	oslo::mdb_env env (error, oslo::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet1 (error, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	oslo::keypair key;
	wallet1.rekey (transaction, "password");
	wallet1.insert_adhoc (transaction, key.prv);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	oslo::wallet_store wallet2 (error, kdf, transaction, oslo::genesis_account, 1, "1", serialized);
	ASSERT_FALSE (error);
	ASSERT_FALSE (wallet2.valid_password (transaction));
	ASSERT_FALSE (wallet2.attempt_password (transaction, "password"));
	ASSERT_TRUE (wallet2.valid_password (transaction));
	oslo::raw_key password1;
	oslo::raw_key password2;
	wallet1.wallet_key (password1, transaction);
	wallet2.wallet_key (password2, transaction);
	ASSERT_EQ (password1, password2);
	ASSERT_EQ (wallet1.salt (transaction), wallet2.salt (transaction));
	ASSERT_EQ (wallet1.check (transaction), wallet2.check (transaction));
	ASSERT_EQ (wallet1.representative (transaction), wallet2.representative (transaction));
	ASSERT_TRUE (wallet2.exists (transaction, key.pub));
	oslo::raw_key prv;
	wallet2.fetch (transaction, key.pub, prv);
	ASSERT_EQ (key.prv, prv);
}

TEST (wallet_store, move)
{
	auto error (false);
	oslo::mdb_env env (error, oslo::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet1 (error, kdf, transaction, oslo::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	oslo::keypair key1;
	wallet1.insert_adhoc (transaction, key1.prv);
	oslo::wallet_store wallet2 (error, kdf, transaction, oslo::genesis_account, 1, "1");
	ASSERT_FALSE (error);
	oslo::keypair key2;
	wallet2.insert_adhoc (transaction, key2.prv);
	ASSERT_FALSE (wallet1.exists (transaction, key2.pub));
	ASSERT_TRUE (wallet2.exists (transaction, key2.pub));
	std::vector<oslo::public_key> keys;
	keys.push_back (key2.pub);
	ASSERT_FALSE (wallet1.move (transaction, wallet2, keys));
	ASSERT_TRUE (wallet1.exists (transaction, key2.pub));
	ASSERT_FALSE (wallet2.exists (transaction, key2.pub));
}

TEST (wallet_store, import)
{
	oslo::system system (2);
	auto wallet1 (system.wallet (0));
	auto wallet2 (system.wallet (1));
	oslo::keypair key1;
	wallet1->insert_adhoc (key1.prv);
	std::string json;
	wallet1->serialize (json);
	ASSERT_FALSE (wallet2->exists (key1.pub));
	auto error (wallet2->import (json, ""));
	ASSERT_FALSE (error);
	ASSERT_TRUE (wallet2->exists (key1.pub));
}

TEST (wallet_store, fail_import_bad_password)
{
	oslo::system system (2);
	auto wallet1 (system.wallet (0));
	auto wallet2 (system.wallet (1));
	oslo::keypair key1;
	wallet1->insert_adhoc (key1.prv);
	std::string json;
	wallet1->serialize (json);
	ASSERT_FALSE (wallet2->exists (key1.pub));
	auto error (wallet2->import (json, "1"));
	ASSERT_TRUE (error);
}

TEST (wallet_store, fail_import_corrupt)
{
	oslo::system system (2);
	auto wallet1 (system.wallet (1));
	std::string json;
	auto error (wallet1->import (json, "1"));
	ASSERT_TRUE (error);
}

// Test work is precached when a key is inserted
TEST (wallet, work)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	wallet->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::genesis genesis;
	auto done (false);
	system.deadline_set (20s);
	while (!done)
	{
		auto transaction (system.wallet (0)->wallets.tx_begin_read ());
		uint64_t work (0);
		if (!wallet->store.work_get (transaction, oslo::test_genesis_key.pub, work))
		{
			done = oslo::work_difficulty (genesis.open->work_version (), genesis.hash (), work) >= system.nodes[0]->default_difficulty (genesis.open->work_version ());
		}
		ASSERT_NO_ERROR (system.poll ());
	}
}

TEST (wallet, work_generate)
{
	oslo::system system (1);
	auto & node1 (*system.nodes[0]);
	auto wallet (system.wallet (0));
	oslo::uint128_t amount1 (node1.balance (oslo::test_genesis_key.pub));
	uint64_t work1;
	wallet->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::account account1;
	{
		auto transaction (node1.wallets.tx_begin_read ());
		account1 = system.account (transaction, 0);
	}
	oslo::keypair key;
	auto block (wallet->send_action (oslo::test_genesis_key.pub, key.pub, 100));
	system.deadline_set (10s);
	auto transaction (node1.store.tx_begin_read ());
	while (node1.ledger.account_balance (transaction, oslo::test_genesis_key.pub) == amount1)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	system.deadline_set (10s);
	auto again (true);
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto block_transaction (node1.store.tx_begin_read ());
		auto transaction (system.wallet (0)->wallets.tx_begin_read ());
		again = wallet->store.work_get (transaction, account1, work1) || oslo::work_difficulty (block->work_version (), node1.ledger.latest_root (block_transaction, account1), work1) < node1.default_difficulty (block->work_version ());
	}
}

TEST (wallet, work_cache_delayed)
{
	oslo::system system (1);
	auto & node1 (*system.nodes[0]);
	auto wallet (system.wallet (0));
	uint64_t work1;
	wallet->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::account account1;
	{
		auto transaction (node1.wallets.tx_begin_read ());
		account1 = system.account (transaction, 0);
	}
	oslo::keypair key;
	auto block1 (wallet->send_action (oslo::test_genesis_key.pub, key.pub, 100));
	ASSERT_EQ (block1->hash (), node1.latest (oslo::test_genesis_key.pub));
	auto block2 (wallet->send_action (oslo::test_genesis_key.pub, key.pub, 100));
	ASSERT_EQ (block2->hash (), node1.latest (oslo::test_genesis_key.pub));
	ASSERT_EQ (block2->hash (), node1.wallets.delayed_work->operator[] (oslo::test_genesis_key.pub));
	auto threshold (node1.default_difficulty (oslo::work_version::work_1));
	auto again (true);
	system.deadline_set (10s);
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		if (!wallet->store.work_get (node1.wallets.tx_begin_read (), account1, work1))
		{
			again = oslo::work_difficulty (oslo::work_version::work_1, block2->hash (), work1) < threshold;
		}
	}
	ASSERT_GE (oslo::work_difficulty (oslo::work_version::work_1, block2->hash (), work1), threshold);
}

TEST (wallet, insert_locked)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	{
		auto transaction (wallet->wallets.tx_begin_write ());
		wallet->store.rekey (transaction, "1");
		ASSERT_TRUE (wallet->store.valid_password (transaction));
		wallet->enter_password (transaction, "");
	}
	auto transaction (wallet->wallets.tx_begin_read ());
	ASSERT_FALSE (wallet->store.valid_password (transaction));
	ASSERT_TRUE (wallet->insert_adhoc (oslo::keypair ().prv).is_zero ());
}

TEST (wallet, version_1_upgrade)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	wallet->enter_initial_password ();
	oslo::keypair key;
	auto transaction (wallet->wallets.tx_begin_write ());
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	wallet->store.rekey (transaction, "1");
	wallet->enter_password (transaction, "");
	ASSERT_FALSE (wallet->store.valid_password (transaction));
	oslo::raw_key password_l;
	oslo::wallet_value value (wallet->store.entry_get_raw (transaction, oslo::wallet_store::wallet_key_special));
	oslo::raw_key kdf;
	kdf.data.clear ();
	password_l.decrypt (value.key, kdf, wallet->store.salt (transaction).owords[0]);
	oslo::uint256_union ciphertext;
	ciphertext.encrypt (key.prv, password_l, wallet->store.salt (transaction).owords[0]);
	wallet->store.entry_put_raw (transaction, key.pub, oslo::wallet_value (ciphertext, 0));
	wallet->store.version_put (transaction, 1);
	wallet->enter_password (transaction, "1");
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	ASSERT_EQ (oslo::wallet_store::version_current, wallet->store.version (transaction));
	oslo::raw_key prv;
	ASSERT_FALSE (wallet->store.fetch (transaction, key.pub, prv));
	ASSERT_EQ (key.prv, prv);
	value = wallet->store.entry_get_raw (transaction, oslo::wallet_store::wallet_key_special);
	wallet->store.derive_key (kdf, transaction, "");
	password_l.decrypt (value.key, kdf, wallet->store.salt (transaction).owords[0]);
	ciphertext.encrypt (key.prv, password_l, wallet->store.salt (transaction).owords[0]);
	wallet->store.entry_put_raw (transaction, key.pub, oslo::wallet_value (ciphertext, 0));
	wallet->store.version_put (transaction, 1);
	wallet->enter_password (transaction, "1");
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	ASSERT_EQ (oslo::wallet_store::version_current, wallet->store.version (transaction));
	oslo::raw_key prv2;
	ASSERT_FALSE (wallet->store.fetch (transaction, key.pub, prv2));
	ASSERT_EQ (key.prv, prv2);
}

TEST (wallet, deterministic_keys)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	auto key1 = wallet.deterministic_key (transaction, 0);
	auto key2 = wallet.deterministic_key (transaction, 0);
	ASSERT_EQ (key1, key2);
	auto key3 = wallet.deterministic_key (transaction, 1);
	ASSERT_NE (key1, key3);
	ASSERT_EQ (0, wallet.deterministic_index_get (transaction));
	wallet.deterministic_index_set (transaction, 1);
	ASSERT_EQ (1, wallet.deterministic_index_get (transaction));
	auto key4 (wallet.deterministic_insert (transaction));
	oslo::raw_key key5;
	ASSERT_FALSE (wallet.fetch (transaction, key4, key5));
	ASSERT_EQ (key3, key5.as_private_key ());
	ASSERT_EQ (2, wallet.deterministic_index_get (transaction));
	wallet.deterministic_index_set (transaction, 1);
	ASSERT_EQ (1, wallet.deterministic_index_get (transaction));
	wallet.erase (transaction, key4);
	ASSERT_FALSE (wallet.exists (transaction, key4));
	auto key8 (wallet.deterministic_insert (transaction));
	ASSERT_EQ (key4, key8);
	auto key6 (wallet.deterministic_insert (transaction));
	oslo::raw_key key7;
	ASSERT_FALSE (wallet.fetch (transaction, key6, key7));
	ASSERT_NE (key5, key7);
	ASSERT_EQ (3, wallet.deterministic_index_get (transaction));
	oslo::keypair key9;
	ASSERT_EQ (key9.pub, wallet.insert_adhoc (transaction, key9.prv));
	ASSERT_TRUE (wallet.exists (transaction, key9.pub));
	wallet.deterministic_clear (transaction);
	ASSERT_EQ (0, wallet.deterministic_index_get (transaction));
	ASSERT_FALSE (wallet.exists (transaction, key4));
	ASSERT_FALSE (wallet.exists (transaction, key6));
	ASSERT_FALSE (wallet.exists (transaction, key8));
	ASSERT_TRUE (wallet.exists (transaction, key9.pub));
}

TEST (wallet, reseed)
{
	bool init;
	oslo::mdb_env env (init, oslo::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	oslo::kdf kdf;
	oslo::wallet_store wallet (init, kdf, transaction, oslo::genesis_account, 1, "0");
	oslo::raw_key seed1;
	seed1.data = 1;
	oslo::raw_key seed2;
	seed2.data = 2;
	wallet.seed_set (transaction, seed1);
	oslo::raw_key seed3;
	wallet.seed (seed3, transaction);
	ASSERT_EQ (seed1, seed3);
	auto key1 (wallet.deterministic_insert (transaction));
	ASSERT_EQ (1, wallet.deterministic_index_get (transaction));
	wallet.seed_set (transaction, seed2);
	ASSERT_EQ (0, wallet.deterministic_index_get (transaction));
	oslo::raw_key seed4;
	wallet.seed (seed4, transaction);
	ASSERT_EQ (seed2, seed4);
	auto key2 (wallet.deterministic_insert (transaction));
	ASSERT_NE (key1, key2);
	wallet.seed_set (transaction, seed1);
	oslo::raw_key seed5;
	wallet.seed (seed5, transaction);
	ASSERT_EQ (seed1, seed5);
	auto key3 (wallet.deterministic_insert (transaction));
	ASSERT_EQ (key1, key3);
}

TEST (wallet, insert_deterministic_locked)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	auto transaction (wallet->wallets.tx_begin_write ());
	wallet->store.rekey (transaction, "1");
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	wallet->enter_password (transaction, "");
	ASSERT_FALSE (wallet->store.valid_password (transaction));
	ASSERT_TRUE (wallet->deterministic_insert (transaction).is_zero ());
}

TEST (wallet, version_2_upgrade)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	auto transaction (wallet->wallets.tx_begin_write ());
	wallet->store.rekey (transaction, "1");
	ASSERT_TRUE (wallet->store.attempt_password (transaction, ""));
	wallet->store.erase (transaction, oslo::wallet_store::deterministic_index_special);
	wallet->store.erase (transaction, oslo::wallet_store::seed_special);
	wallet->store.version_put (transaction, 2);
	ASSERT_EQ (2, wallet->store.version (transaction));
	ASSERT_EQ (wallet->store.find (transaction, oslo::wallet_store::deterministic_index_special), wallet->store.end ());
	ASSERT_EQ (wallet->store.find (transaction, oslo::wallet_store::seed_special), wallet->store.end ());
	wallet->store.attempt_password (transaction, "1");
	ASSERT_EQ (oslo::wallet_store::version_current, wallet->store.version (transaction));
	ASSERT_NE (wallet->store.find (transaction, oslo::wallet_store::deterministic_index_special), wallet->store.end ());
	ASSERT_NE (wallet->store.find (transaction, oslo::wallet_store::seed_special), wallet->store.end ());
	ASSERT_FALSE (wallet->deterministic_insert (transaction).is_zero ());
}

TEST (wallet, version_3_upgrade)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	auto transaction (wallet->wallets.tx_begin_write ());
	wallet->store.rekey (transaction, "1");
	wallet->enter_password (transaction, "1");
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	ASSERT_EQ (oslo::wallet_store::version_current, wallet->store.version (transaction));
	oslo::keypair key;
	oslo::raw_key seed;
	oslo::uint256_union seed_ciphertext;
	oslo::random_pool::generate_block (seed.data.bytes.data (), seed.data.bytes.size ());
	oslo::raw_key password_l;
	oslo::wallet_value value (wallet->store.entry_get_raw (transaction, oslo::wallet_store::wallet_key_special));
	oslo::raw_key kdf;
	wallet->store.derive_key (kdf, transaction, "1");
	password_l.decrypt (value.key, kdf, wallet->store.salt (transaction).owords[0]);
	oslo::uint256_union ciphertext;
	ciphertext.encrypt (key.prv, password_l, wallet->store.salt (transaction).owords[0]);
	wallet->store.entry_put_raw (transaction, key.pub, oslo::wallet_value (ciphertext, 0));
	seed_ciphertext.encrypt (seed, password_l, wallet->store.salt (transaction).owords[0]);
	wallet->store.entry_put_raw (transaction, oslo::wallet_store::seed_special, oslo::wallet_value (seed_ciphertext, 0));
	wallet->store.version_put (transaction, 3);
	wallet->enter_password (transaction, "1");
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	ASSERT_EQ (oslo::wallet_store::version_current, wallet->store.version (transaction));
	oslo::raw_key prv;
	ASSERT_FALSE (wallet->store.fetch (transaction, key.pub, prv));
	ASSERT_EQ (key.prv, prv);
	oslo::raw_key seed_compare;
	wallet->store.seed (seed_compare, transaction);
	ASSERT_EQ (seed, seed_compare);
	ASSERT_NE (seed_ciphertext, wallet->store.entry_get_raw (transaction, oslo::wallet_store::seed_special).key);
}

TEST (wallet, upgrade_backup)
{
	oslo::system system (1);
	auto dir (oslo::unique_path ());
	namespace fs = boost::filesystem;
	fs::create_directory (dir);
	/** Returns 'dir' if backup file cannot be found */
	auto get_backup_path = [&dir]() {
		for (fs::directory_iterator itr (dir); itr != fs::directory_iterator (); ++itr)
		{
			if (itr->path ().filename ().string ().find ("wallets_backup_") != std::string::npos)
			{
				return itr->path ();
			}
		}
		return dir;
	};

	auto wallet_id = oslo::random_wallet_id ();
	{
		auto node1 (std::make_shared<oslo::node> (system.io_ctx, oslo::get_available_port (), dir, system.alarm, system.logging, system.work));
		ASSERT_FALSE (node1->init_error ());
		auto wallet (node1->wallets.create (wallet_id));
		ASSERT_NE (nullptr, wallet);
		auto transaction (node1->wallets.tx_begin_write ());
		wallet->store.version_put (transaction, 3);
	}
	ASSERT_EQ (get_backup_path ().string (), dir.string ());

	// Check with config backup_before_upgrade = false
	{
		auto node1 (std::make_shared<oslo::node> (system.io_ctx, oslo::get_available_port (), dir, system.alarm, system.logging, system.work));
		ASSERT_FALSE (node1->init_error ());
		auto wallet (node1->wallets.open (wallet_id));
		ASSERT_NE (nullptr, wallet);
		auto transaction (node1->wallets.tx_begin_write ());
		ASSERT_LT (3u, wallet->store.version (transaction));
		wallet->store.version_put (transaction, 3);
	}
	ASSERT_EQ (get_backup_path ().string (), dir.string ());

	// Now do the upgrade and confirm that backup is saved
	{
		oslo::node_config node_config (oslo::get_available_port (), system.logging);
		node_config.backup_before_upgrade = true;
		auto node1 (std::make_shared<oslo::node> (system.io_ctx, dir, system.alarm, node_config, system.work));
		ASSERT_FALSE (node1->init_error ());
		auto wallet (node1->wallets.open (wallet_id));
		ASSERT_NE (nullptr, wallet);
		auto transaction (node1->wallets.tx_begin_read ());
		ASSERT_LT (3u, wallet->store.version (transaction));
	}
	ASSERT_NE (get_backup_path ().string (), dir.string ());
}

TEST (wallet, no_work)
{
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv, false);
	oslo::keypair key2;
	auto block (system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, std::numeric_limits<oslo::uint128_t>::max (), false));
	ASSERT_NE (nullptr, block);
	ASSERT_NE (0, block->block_work ());
	ASSERT_GE (block->difficulty (), oslo::work_threshold (block->work_version (), block->sideband ().details));
	auto transaction (system.wallet (0)->wallets.tx_begin_read ());
	uint64_t cached_work (0);
	system.wallet (0)->store.work_get (transaction, oslo::test_genesis_key.pub, cached_work);
	ASSERT_EQ (0, cached_work);
}

TEST (wallet, send_race)
{
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key2;
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (oslo::test_genesis_key.pub, key2.pub, oslo::Gxrb_ratio));
		ASSERT_EQ (oslo::genesis_amount - oslo::Gxrb_ratio * i, system.nodes[0]->balance (oslo::test_genesis_key.pub));
	}
}

TEST (wallet, password_race)
{
	oslo::system system (1);
	oslo::thread_runner runner (system.io_ctx, system.nodes[0]->config.io_threads);
	auto wallet = system.wallet (0);
	std::thread thread ([&wallet]() {
		for (int i = 0; i < 100; i++)
		{
			auto transaction (wallet->wallets.tx_begin_write ());
			wallet->store.rekey (transaction, std::to_string (i));
		}
	});
	for (int i = 0; i < 100; i++)
	{
		auto transaction (wallet->wallets.tx_begin_read ());
		// Password should always be valid, the rekey operation should be atomic.
		bool ok = wallet->store.valid_password (transaction);
		EXPECT_TRUE (ok);
		if (!ok)
		{
			break;
		}
	}
	thread.join ();
	system.stop ();
	runner.join ();
}

TEST (wallet, password_race_corrupt_seed)
{
	oslo::system system (1);
	oslo::thread_runner runner (system.io_ctx, system.nodes[0]->config.io_threads);
	auto wallet = system.wallet (0);
	oslo::raw_key seed;
	{
		auto transaction (wallet->wallets.tx_begin_write ());
		ASSERT_FALSE (wallet->store.rekey (transaction, "4567"));
		wallet->store.seed (seed, transaction);
		ASSERT_FALSE (wallet->store.attempt_password (transaction, "4567"));
	}
	std::vector<std::thread> threads;
	for (int i = 0; i < 100; i++)
	{
		threads.emplace_back ([&wallet]() {
			for (int i = 0; i < 10; i++)
			{
				auto transaction (wallet->wallets.tx_begin_write ());
				wallet->store.rekey (transaction, "0000");
			}
		});
		threads.emplace_back ([&wallet]() {
			for (int i = 0; i < 10; i++)
			{
				auto transaction (wallet->wallets.tx_begin_write ());
				wallet->store.rekey (transaction, "1234");
			}
		});
		threads.emplace_back ([&wallet]() {
			for (int i = 0; i < 10; i++)
			{
				auto transaction (wallet->wallets.tx_begin_read ());
				wallet->store.attempt_password (transaction, "1234");
			}
		});
	}
	for (auto & thread : threads)
	{
		thread.join ();
	}
	system.stop ();
	runner.join ();
	{
		auto transaction (wallet->wallets.tx_begin_write ());
		if (!wallet->store.attempt_password (transaction, "1234"))
		{
			oslo::raw_key seed_now;
			wallet->store.seed (seed_now, transaction);
			ASSERT_TRUE (seed_now == seed);
		}
		else if (!wallet->store.attempt_password (transaction, "0000"))
		{
			oslo::raw_key seed_now;
			wallet->store.seed (seed_now, transaction);
			ASSERT_TRUE (seed_now == seed);
		}
		else if (!wallet->store.attempt_password (transaction, "4567"))
		{
			oslo::raw_key seed_now;
			wallet->store.seed (seed_now, transaction);
			ASSERT_TRUE (seed_now == seed);
		}
		else
		{
			ASSERT_FALSE (true);
		}
	}
}

TEST (wallet, change_seed)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	wallet->enter_initial_password ();
	oslo::raw_key seed1;
	seed1.data = 1;
	oslo::public_key pub;
	uint32_t index (4);
	auto prv = oslo::deterministic_key (seed1, index);
	pub = oslo::pub_key (prv);
	wallet->insert_adhoc (oslo::test_genesis_key.prv, false);
	auto block (wallet->send_action (oslo::test_genesis_key.pub, pub, 100));
	ASSERT_NE (nullptr, block);
	system.nodes[0]->block_processor.flush ();
	{
		auto transaction (wallet->wallets.tx_begin_write ());
		wallet->change_seed (transaction, seed1);
		oslo::raw_key seed2;
		wallet->store.seed (seed2, transaction);
		ASSERT_EQ (seed1, seed2);
		ASSERT_EQ (index + 1, wallet->store.deterministic_index_get (transaction));
	}
	ASSERT_TRUE (wallet->exists (pub));
}

TEST (wallet, deterministic_restore)
{
	oslo::system system (1);
	auto wallet (system.wallet (0));
	wallet->enter_initial_password ();
	oslo::raw_key seed1;
	seed1.data = 1;
	oslo::public_key pub;
	uint32_t index (4);
	{
		auto transaction (wallet->wallets.tx_begin_write ());
		wallet->change_seed (transaction, seed1);
		oslo::raw_key seed2;
		wallet->store.seed (seed2, transaction);
		ASSERT_EQ (seed1, seed2);
		ASSERT_EQ (1, wallet->store.deterministic_index_get (transaction));
		auto prv = oslo::deterministic_key (seed1, index);
		pub = oslo::pub_key (prv);
	}
	wallet->insert_adhoc (oslo::test_genesis_key.prv, false);
	auto block (wallet->send_action (oslo::test_genesis_key.pub, pub, 100));
	ASSERT_NE (nullptr, block);
	system.nodes[0]->block_processor.flush ();
	{
		auto transaction (wallet->wallets.tx_begin_write ());
		wallet->deterministic_restore (transaction);
		ASSERT_EQ (index + 1, wallet->store.deterministic_index_get (transaction));
	}
	ASSERT_TRUE (wallet->exists (pub));
}

TEST (work_watcher, update)
{
	oslo::system system;
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.enable_voting = false;
	node_config.work_watcher_period = 1s;
	node_config.max_work_generate_multiplier = 1e6;
	oslo::node_flags node_flags;
	node_flags.disable_request_loop = true;
	auto & node = *system.add_node (node_config, node_flags);
	auto & wallet (*system.wallet (0));
	wallet.insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key;
	auto const block1 (wallet.send_action (oslo::test_genesis_key.pub, key.pub, 100));
	auto difficulty1 (block1->difficulty ());
	auto multiplier1 (oslo::normalized_multiplier (oslo::difficulty::to_multiplier (difficulty1, oslo::work_threshold (block1->work_version (), oslo::block_details (oslo::epoch::epoch_0, true, false, false))), node.network_params.network.publish_thresholds.epoch_1));
	auto const block2 (wallet.send_action (oslo::test_genesis_key.pub, key.pub, 200));
	auto difficulty2 (block2->difficulty ());
	auto multiplier2 (oslo::normalized_multiplier (oslo::difficulty::to_multiplier (difficulty2, oslo::work_threshold (block2->work_version (), oslo::block_details (oslo::epoch::epoch_0, true, false, false))), node.network_params.network.publish_thresholds.epoch_1));
	double updated_multiplier1{ multiplier1 }, updated_multiplier2{ multiplier2 }, target_multiplier{ std::max (multiplier1, multiplier2) + 1e-6 };
	{
		oslo::lock_guard<std::mutex> guard (node.active.mutex);
		node.active.trended_active_multiplier = target_multiplier;
	}
	system.deadline_set (20s);
	while (updated_multiplier1 == multiplier1 || updated_multiplier2 == multiplier2)
	{
		{
			oslo::lock_guard<std::mutex> guard (node.active.mutex);
			{
				auto const existing (node.active.roots.find (block1->qualified_root ()));
				//if existing is junk the block has been confirmed already
				ASSERT_NE (existing, node.active.roots.end ());
				updated_multiplier1 = existing->multiplier;
			}
			{
				auto const existing (node.active.roots.find (block2->qualified_root ()));
				//if existing is junk the block has been confirmed already
				ASSERT_NE (existing, node.active.roots.end ());
				updated_multiplier2 = existing->multiplier;
			}
		}
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_GT (updated_multiplier1, multiplier1);
	ASSERT_GT (updated_multiplier2, multiplier2);
}

TEST (work_watcher, propagate)
{
	oslo::system system;
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.enable_voting = false;
	node_config.work_watcher_period = 1s;
	node_config.max_work_generate_multiplier = 1e6;
	oslo::node_flags node_flags;
	node_flags.disable_request_loop = true;
	auto & node = *system.add_node (node_config, node_flags);
	auto & wallet (*system.wallet (0));
	wallet.insert_adhoc (oslo::test_genesis_key.prv);
	node_config.peering_port = oslo::get_available_port ();
	auto & node_passive = *system.add_node (node_config);
	oslo::keypair key;
	auto const block (wallet.send_action (oslo::test_genesis_key.pub, key.pub, 100));
	system.deadline_set (5s);
	while (!node_passive.ledger.block_exists (block->hash ()))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto const multiplier (oslo::normalized_multiplier (oslo::difficulty::to_multiplier (block->difficulty (), oslo::work_threshold (block->work_version (), oslo::block_details (oslo::epoch::epoch_0, false, false, false))), node.network_params.network.publish_thresholds.epoch_1));
	auto updated_multiplier{ multiplier };
	auto propagated_multiplier{ multiplier };
	{
		oslo::lock_guard<std::mutex> guard (node.active.mutex);
		node.active.trended_active_multiplier = multiplier * 1.001;
	}
	bool updated{ false };
	bool propagated{ false };
	system.deadline_set (10s);
	while (!(updated && propagated))
	{
		{
			oslo::lock_guard<std::mutex> guard (node.active.mutex);
			{
				auto const existing (node.active.roots.find (block->qualified_root ()));
				ASSERT_NE (existing, node.active.roots.end ());
				updated_multiplier = existing->multiplier;
			}
		}
		{
			oslo::lock_guard<std::mutex> guard (node_passive.active.mutex);
			{
				auto const existing (node_passive.active.roots.find (block->qualified_root ()));
				ASSERT_NE (existing, node_passive.active.roots.end ());
				propagated_multiplier = existing->multiplier;
			}
		}
		updated = updated_multiplier != multiplier;
		propagated = propagated_multiplier != multiplier;
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_GT (updated_multiplier, multiplier);
	ASSERT_EQ (propagated_multiplier, updated_multiplier);
}

TEST (work_watcher, removed_after_win)
{
	oslo::system system (1);
	auto & node (*system.nodes[0]);
	auto & wallet (*system.wallet (0));
	wallet.insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key;
	ASSERT_EQ (0, wallet.wallets.watcher->size ());
	auto const block1 (wallet.send_action (oslo::test_genesis_key.pub, key.pub, 100));
	ASSERT_EQ (1, wallet.wallets.watcher->size ());
	system.deadline_set (5s);
	while (node.wallets.watcher->is_watched (block1->qualified_root ()))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (0, node.wallets.watcher->size ());
}

TEST (work_watcher, removed_after_lose)
{
	oslo::system system;
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.enable_voting = false;
	node_config.work_watcher_period = 1s;
	auto & node = *system.add_node (node_config);
	auto & wallet (*system.wallet (0));
	wallet.insert_adhoc (oslo::test_genesis_key.prv);
	oslo::keypair key;
	auto const block1 (wallet.send_action (oslo::test_genesis_key.pub, key.pub, 100));
	ASSERT_TRUE (node.wallets.watcher->is_watched (block1->qualified_root ()));
	oslo::genesis genesis;
	auto fork1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::xrb_ratio, oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	node.process_active (fork1);
	node.block_processor.flush ();
	auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, 0, fork1));
	oslo::confirm_ack message (vote);
	node.network.process_message (message, nullptr);
	system.deadline_set (5s);
	while (node.wallets.watcher->is_watched (block1->qualified_root ()))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (0, node.wallets.watcher->size ());
}

TEST (work_watcher, generation_disabled)
{
	oslo::system system;
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.enable_voting = false;
	node_config.work_watcher_period = 1s;
	node_config.work_threads = 0;
	oslo::node_flags node_flags;
	node_flags.disable_request_loop = true;
	auto & node = *system.add_node (node_config);
	ASSERT_FALSE (node.work_generation_enabled ());
	oslo::work_pool pool (std::numeric_limits<unsigned>::max ());
	oslo::genesis genesis;
	oslo::keypair key;
	auto block (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Mxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ())));
	auto difficulty (block->difficulty ());
	node.wallets.watcher->add (block);
	ASSERT_FALSE (node.process_local (block).code != oslo::process_result::progress);
	ASSERT_TRUE (node.wallets.watcher->is_watched (block->qualified_root ()));
	auto multiplier = oslo::normalized_multiplier (oslo::difficulty::to_multiplier (difficulty, oslo::work_threshold (block->work_version (), oslo::block_details (oslo::epoch::epoch_0, true, false, false))), node.network_params.network.publish_thresholds.epoch_1);
	double updated_multiplier{ multiplier };
	{
		oslo::lock_guard<std::mutex> guard (node.active.mutex);
		node.active.trended_active_multiplier = multiplier * 10;
	}
	std::this_thread::sleep_for (2s);
	ASSERT_TRUE (node.wallets.watcher->is_watched (block->qualified_root ()));
	{
		oslo::lock_guard<std::mutex> guard (node.active.mutex);
		auto const existing (node.active.roots.find (block->qualified_root ()));
		ASSERT_NE (existing, node.active.roots.end ());
		updated_multiplier = existing->multiplier;
	}
	ASSERT_EQ (updated_multiplier, multiplier);
	ASSERT_TRUE (node.distributed_work.items.empty ());
}

TEST (work_watcher, cancel)
{
	oslo::system system;
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.work_watcher_period = 1s;
	node_config.max_work_generate_multiplier = 1e6;
	node_config.enable_voting = false;
	auto & node = *system.add_node (node_config);
	auto & wallet (*system.wallet (0));
	wallet.insert_adhoc (oslo::test_genesis_key.prv, false);
	oslo::keypair key;
	auto work1 (node.work_generate_blocking (oslo::test_genesis_key.pub));
	auto const block1 (wallet.send_action (oslo::test_genesis_key.pub, key.pub, 100, *work1, false));
	{
		oslo::unique_lock<std::mutex> lock (node.active.mutex);
		// Prevent active difficulty repopulating multipliers
		node.network_params.network.request_interval_ms = 10000;
		// Fill multipliers_cb and update active difficulty;
		for (auto i (0); i < node.active.multipliers_cb.size (); i++)
		{
			node.active.multipliers_cb.push_back (node.config.max_work_generate_multiplier);
		}
		node.active.update_active_multiplier (lock);
	}
	// Wait for work generation to start
	system.deadline_set (5s);
	while (0 == node.work.size ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	// Cancel the ongoing work
	ASSERT_EQ (1, node.work.size ());
	node.work.cancel (block1->root ());
	ASSERT_EQ (0, node.work.size ());
	{
		oslo::unique_lock<std::mutex> lock (wallet.wallets.watcher->mutex);
		auto existing (wallet.wallets.watcher->watched.find (block1->qualified_root ()));
		ASSERT_NE (wallet.wallets.watcher->watched.end (), existing);
		auto block2 (existing->second);
		// Block must be the same
		ASSERT_NE (nullptr, block1);
		ASSERT_NE (nullptr, block2);
		ASSERT_EQ (*block1, *block2);
		// but should still be under watch
		lock.unlock ();
		ASSERT_TRUE (wallet.wallets.watcher->is_watched (block1->qualified_root ()));
	}
}

// Ensure the minimum limited difficulty is enough for the highest threshold
TEST (wallet, limited_difficulty)
{
	oslo::system system;
	oslo::genesis genesis;
	oslo::node_config node_config (oslo::get_available_port (), system.logging);
	node_config.max_work_generate_multiplier = 1;
	oslo::node_flags node_flags;
	node_flags.disable_request_loop = true;
	auto & node = *system.add_node (node_config, node_flags);
	auto & wallet (*system.wallet (0));
	// Upgrade the genesis account to epoch 2
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (node, oslo::epoch::epoch_1));
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (node, oslo::epoch::epoch_2));
	ASSERT_EQ (oslo::epoch::epoch_2, node.store.block_version (node.store.tx_begin_read (), node.latest (oslo::test_genesis_key.pub)));
	wallet.insert_adhoc (oslo::test_genesis_key.prv, false);
	{
		// Force active difficulty to an impossibly high value
		oslo::lock_guard<std::mutex> guard (node.active.mutex);
		node.active.trended_active_multiplier = 1024 * 1024 * 1024;
	}
	ASSERT_EQ (node.max_work_generate_difficulty (oslo::work_version::work_1), node.active.limited_active_difficulty (*genesis.open));
	auto send = wallet.send_action (oslo::test_genesis_key.pub, oslo::keypair ().pub, 1, 1);
	ASSERT_NE (nullptr, send);
}

TEST (wallet, epoch_2_validation)
{
	oslo::system system (1);
	auto & node (*system.nodes[0]);
	auto & wallet (*system.wallet (0));

	// Upgrade the genesis account to epoch 2
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (node, oslo::epoch::epoch_1));
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (node, oslo::epoch::epoch_2));

	wallet.insert_adhoc (oslo::test_genesis_key.prv, false);

	// Test send and receive blocks
	// An epoch 2 receive block should be generated with lower difficulty with high probability
	auto tries = 0;
	auto max_tries = 20;
	auto amount = node.config.receive_minimum.number ();
	while (++tries < max_tries)
	{
		auto send = wallet.send_action (oslo::test_genesis_key.pub, oslo::test_genesis_key.pub, amount, 1);
		ASSERT_NE (nullptr, send);

		auto receive = wallet.receive_action (*send, oslo::test_genesis_key.pub, amount, 1);
		ASSERT_NE (nullptr, receive);
		if (receive->difficulty () < node.network_params.network.publish_thresholds.base)
		{
			ASSERT_GE (receive->difficulty (), node.network_params.network.publish_thresholds.epoch_2_receive);
			break;
		}
	}
	ASSERT_LT (tries, max_tries);

	// Test a change block
	ASSERT_NE (nullptr, wallet.change_action (oslo::test_genesis_key.pub, oslo::keypair ().pub, 1));
}

// Receiving from an upgraded account uses the lower threshold and upgrades the receiving account
TEST (wallet, epoch_2_receive_propagation)
{
	auto tries = 0;
	auto const max_tries = 20;
	while (++tries < max_tries)
	{
		oslo::system system;
		oslo::node_flags node_flags;
		node_flags.disable_request_loop = true;
		auto & node (*system.add_node (node_flags));
		auto & wallet (*system.wallet (0));

		// Upgrade the genesis account to epoch 1
		auto epoch1 = system.upgrade_genesis_epoch (node, oslo::epoch::epoch_1);
		ASSERT_NE (nullptr, epoch1);

		oslo::keypair key;
		oslo::state_block_builder builder;

		// Send and open the account
		wallet.insert_adhoc (oslo::test_genesis_key.prv, false);
		wallet.insert_adhoc (key.prv, false);
		auto amount = node.config.receive_minimum.number ();
		auto send1 = wallet.send_action (oslo::test_genesis_key.pub, key.pub, amount, 1);
		ASSERT_NE (nullptr, send1);
		ASSERT_NE (nullptr, wallet.receive_action (*send1, oslo::test_genesis_key.pub, amount, 1));

		// Upgrade the genesis account to epoch 2
		auto epoch2 = system.upgrade_genesis_epoch (node, oslo::epoch::epoch_2);
		ASSERT_NE (nullptr, epoch2);

		// Send a block
		auto send2 = wallet.send_action (oslo::test_genesis_key.pub, key.pub, amount, 1);
		ASSERT_NE (nullptr, send2);

		// Receiving should use the lower difficulty
		{
			oslo::lock_guard<std::mutex> guard (node.active.mutex);
			node.active.trended_active_multiplier = 1.0;
		}
		auto receive2 = wallet.receive_action (*send2, key.pub, amount, 1);
		ASSERT_NE (nullptr, receive2);
		if (receive2->difficulty () < node.network_params.network.publish_thresholds.base)
		{
			ASSERT_GE (receive2->difficulty (), node.network_params.network.publish_thresholds.epoch_2_receive);
			ASSERT_EQ (oslo::epoch::epoch_2, node.store.block_version (node.store.tx_begin_read (), receive2->hash ()));
			break;
		}
	}
	ASSERT_LT (tries, max_tries);
}

// Opening an upgraded account uses the lower threshold
TEST (wallet, epoch_2_receive_unopened)
{
	// Ensure the lower receive work is used when receiving
	auto tries = 0;
	auto const max_tries = 20;
	while (++tries < max_tries)
	{
		oslo::system system;
		oslo::node_flags node_flags;
		node_flags.disable_request_loop = true;
		auto & node (*system.add_node (node_flags));
		auto & wallet (*system.wallet (0));

		// Upgrade the genesis account to epoch 1
		auto epoch1 = system.upgrade_genesis_epoch (node, oslo::epoch::epoch_1);
		ASSERT_NE (nullptr, epoch1);

		oslo::keypair key;
		oslo::state_block_builder builder;

		// Send
		wallet.insert_adhoc (oslo::test_genesis_key.prv, false);
		auto amount = node.config.receive_minimum.number ();
		auto send1 = wallet.send_action (oslo::test_genesis_key.pub, key.pub, amount, 1);

		// Upgrade unopened account to epoch_2
		auto epoch2_unopened = oslo::state_block (key.pub, 0, 0, 0, node.network_params.ledger.epochs.link (oslo::epoch::epoch_2), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (key.pub, node.network_params.network.publish_thresholds.epoch_2));
		ASSERT_EQ (oslo::process_result::progress, node.process (epoch2_unopened).code);

		wallet.insert_adhoc (key.prv, false);

		// Receiving should use the lower difficulty
		{
			oslo::lock_guard<std::mutex> guard (node.active.mutex);
			node.active.trended_active_multiplier = 1.0;
		}
		auto receive1 = wallet.receive_action (*send1, key.pub, amount, 1);
		ASSERT_NE (nullptr, receive1);
		if (receive1->difficulty () < node.network_params.network.publish_thresholds.base)
		{
			ASSERT_GE (receive1->difficulty (), node.network_params.network.publish_thresholds.epoch_2_receive);
			ASSERT_EQ (oslo::epoch::epoch_2, node.store.block_version (node.store.tx_begin_read (), receive1->hash ()));
			break;
		}
	}
	ASSERT_LT (tries, max_tries);
}

TEST (wallet, foreach_representative_deadlock)
{
	oslo::system system (1);
	auto & node (*system.nodes[0]);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	node.wallets.compute_reps ();
	ASSERT_EQ (1, node.wallets.reps ().voting);
	node.wallets.foreach_representative ([&node](oslo::public_key const & pub, oslo::raw_key const & prv) {
		if (node.wallets.mutex.try_lock ())
		{
			node.wallets.mutex.unlock ();
		}
		else
		{
			ASSERT_FALSE (true);
		}
	});
}
