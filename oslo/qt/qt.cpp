...
bool oslo_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<oslo_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<oslo_qt::eventloop_event *> (event_a)->action ();
	return true;
}

oslo_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

oslo_qt::self_pane::self_pane (oslo_qt::wallet & wallet_a, oslo::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your Oslo account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % OSLO_VERSION_STRING % network).c_str ());
...
void oslo_qt::self_pane::set_balance_text (std::pair<oslo::uint128_t, oslo::uint128_t> balance_a)
{
...
oslo_qt::accounts::accounts (oslo_qt::wallet & wallet_a) :
...
void oslo_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
...
oslo_qt::import::import (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
oslo_qt::history::history (oslo::ledger & ledger_a, oslo::account const & account_a, oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public oslo::block_visitor
{
public:
	short_text_visitor (oslo::transaction const & transaction_a, oslo::ledger & ledger_a) :
...
oslo_qt::block_viewer::block_viewer (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void oslo_qt::block_viewer::rebroadcast_action (oslo::block_hash const & hash_a)
{
	auto done (true);
...
oslo_qt::account_viewer::account_viewer (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (OSLO): ") + wallet.format_balance (balance.first));
...
oslo_qt::stats_viewer::stats_viewer (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
refresh (new QPushButton ("Refresh")),
...
void oslo_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
oslo_qt::status::status (oslo_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (oslo_qt::status_types::nominal);
	set_text ();
}

void oslo_qt::status::erase (oslo_qt::status_types status_a)
{
	debug_assert (status_a != oslo_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void oslo_qt::status::insert (oslo_qt::status_types status_a)
{
	debug_assert (status_a != oslo_qt::status_types::nominal);
...
std::string oslo_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case oslo_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case oslo_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case oslo_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case oslo_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case oslo_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case oslo_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case oslo_qt::status_types::nominal:
...
std::string oslo_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
oslo_qt::wallet::wallet (QApplication & application_a, oslo_qt::eventloop_processor & processor_a, oslo::node & node_a, std::shared_ptr<oslo::wallet> wallet_a, oslo::account & account_a) :
rendering_ratio (oslo::Mxrb_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void oslo_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<oslo_qt::wallet> wallet_w (shared_from_this ());
...
void oslo_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<oslo_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual]() {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<oslo::block> block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w](oslo::election_status const & status_a, oslo::account const & account_a, oslo::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](oslo::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (oslo_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (oslo_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](std::shared_ptr<oslo::transport::channel>) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (oslo_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (oslo_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (oslo_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (oslo_qt::status_types::working);
					}
				}
			}));
		}
...
void oslo_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (oslo_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (oslo_qt::status_types::disconnected);
	}
}

void oslo_qt::wallet::empty_password ()
{
...
void oslo_qt::wallet::change_rendering_ratio (oslo::uint128_t const & rendering_ratio_a)
{
...
std::string oslo_qt::wallet::format_balance (oslo::uint128_t const & balance) const
{
...
void oslo_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void oslo_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

oslo_qt::settings::settings (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
void oslo_qt::settings::refresh_representative ()
{
...
void oslo_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void oslo_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (oslo_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (oslo_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (oslo_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (oslo_qt::status_types::vulnerable);
	}
}

oslo_qt::advanced_actions::advanced_actions (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:9095\"")),
...
wallet (wallet_a)
{
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (oslo::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void oslo_qt::advanced_actions::refresh_peers ()
{
...
void oslo_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.latest_begin (transaction)), j (wallet.node.ledger.store.latest_end ()); i != j; ++i)
	{
		...
		std::string balance;
		oslo::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void oslo_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

oslo_qt::block_entry::block_entry (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (oslo::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
				show_label_ok (*status);
				this->status->setText ("");
				if (!oslo::work_validate_entry (*block_l))
				{
...
oslo_qt::block_creation::block_creation (oslo_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void oslo_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void oslo_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void oslo_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void oslo_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void oslo_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						oslo::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void oslo_qt::block_creation::create_receive ()
{
	oslo::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				oslo::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							oslo::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void oslo_qt::block_creation::create_change ()
{
	oslo::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					oslo::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void oslo_qt::block_creation::create_open ()
{
	oslo::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								oslo::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
