...
#include <oslo/boost/asio/connect.hpp>
...
{
	oslo::daemon_config daemon_config (data_path);
	...
	toml.write (oslo::get_node_toml_config_path (data_path));
	...
	// Write rpc config
	oslo::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (oslo::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the oslo_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the oslo_rpc to test");
	// clang-format on
	...
	{
		auto node_filepath = running_executable_filepath.parent_path () / "oslo_node";
	...
	{
		std::cerr << "oslo_node executable could not be found in " << node_path << std::endl;
		return 1;
	}
	...
	{
		std::cerr << "oslo_rpc executable could not be found in " << rpc_path << std::endl;
		return 1;
	}
	...
	std::thread t ([send_count, &ioc, &primary_node_results, &resolver, &node_count, &destination_count]() {
		...
		wallet_add_rpc (ioc, primary_node_results, wallet, oslo::test_genesis_key.prv.data.to_string ());
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			std::make_shared<send_session> (ioc, send_calls_remaining, wallet, oslo::genesis_account.to_account (), destination_account->as_string, primary_node_results)->run ();
		}
	...
	oslo::thread_runner runner (ioc, simultaneous_process_calls);
	t.join ();
	runner.join ();
