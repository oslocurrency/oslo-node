...
bool oslo::rpc_secure::on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx)
{
...
void oslo::rpc_secure::load_certs (boost::asio::ssl::context & context_a)
{
	try
	{
...
oslo::rpc_secure::rpc_secure (boost::asio::io_context & context_a, oslo::rpc_config const & config_a, oslo::rpc_handler_interface & rpc_handler_interface_a) :
...
void oslo::rpc_secure::accept ()
{
	auto connection (std::make_shared<oslo::rpc_connection_secure> (config, io_ctx, logger, rpc_handler_interface, this->ssl_context));
