...
oslo::rpc_handler::rpc_handler (oslo::rpc_config const & rpc_config, std::string const & body_a, std::string const & request_id_a, std::function<void(std::string const &)> const & response_a, oslo::rpc_handler_interface & rpc_handler_interface_a, oslo::logger_mt & logger) :
body (body_a),
...
void oslo::rpc_handler::process_request (oslo::rpc_handler_request_params const & request_params)
{
	try
	{
		...
		else
		{
			if (request_params.rpc_version == 1)
			{
				...
				std::error_code rpc_control_disabled_ec = oslo::error_rpc::rpc_control_disabled;
