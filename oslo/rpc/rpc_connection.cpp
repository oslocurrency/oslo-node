...
#include <oslo/rpc/rpc_handler.hpp>
...
oslo::rpc_connection::rpc_connection (oslo::rpc_config const & rpc_config, boost::asio::io_context & io_ctx, oslo::logger_mt & logger, oslo::rpc_handler_interface & rpc_handler_interface) :
socket (io_ctx),
...
void oslo::rpc_connection::parse_connection ()
{
	read (socket);
}

void oslo::rpc_connection::prepare_head (unsigned version, boost::beast::http::status status)
{
	res.version (version);
	res.result (status);
...
void oslo::rpc_connection::write_result (std::string body, unsigned version, boost::beast::http::status status)
{
...
void oslo::rpc_connection::write_completion_handler (std::shared_ptr<oslo::rpc_connection> rpc_connection)
{
	// Intentional no-op
}

template <typename STREAM_TYPE>
void oslo::rpc_connection::read (STREAM_TYPE & stream)
{
	...
	boost::beast::http::async_read_header (stream, buffer, *header_parser, boost::asio::bind_executor (strand, [this_l, &stream, header_parser](boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			if (boost::iequals (header_parser->get ()[boost::beast::http::field::expect], "100-continue"))
			{
				...
				continue_response->set (boost::beast::http::field::server, "oslo");
		...
		else
		{
			...
			oslo::json_error_response (response_handler, std::string ("Invalid header: ") + ec.message ());
		}
	}));
}

template <typename STREAM_TYPE>
void oslo::rpc_connection::parse_request (STREAM_TYPE & stream, std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>> header_parser)
{
	auto this_l (shared_from_this ());
	auto header_field_credentials_l (header_parser->get ()["oslo-api-key"]);
	auto header_corr_id_l (header_parser->get ()["oslo-correlation-id"]);
		...
		if (!ec)
		{
			this_l->io_ctx.post ([this_l, body_parser, header_field_credentials_l, header_corr_id_l, path_l, &stream]() {
				...
				switch (method)
				{
					...
					{
						auto handler (std::make_shared<oslo::rpc_handler> (this_l->rpc_config, req.body (), request_id, response_handler, this_l->rpc_handler_interface, this_l->logger));
						oslo::rpc_handler_request_params request_params;
					...
					default:
					{
						oslo::json_error_response (response_handler, "Can only POST requests");
						break;
					}
				}
			});
...
template void oslo::rpc_connection::read (socket_type &);
template void oslo::rpc_connection::parse_request (socket_type &, std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>>);
#ifdef OSLO_SECURE_RPC
template void oslo::rpc_connection::read (boost::asio::ssl::stream<socket_type &> &);
template void oslo::rpc_connection::parse_request (boost::asio::ssl::stream<socket_type &> &, std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>>);
#endif
