...
{
	QMessageBox message (QMessageBox::Critical, "Error starting Oslo", message_a.c_str ());
...
{
	QMessageBox message (QMessageBox::NoIcon, "Help", "see <a href=\"https://docs.oslo.org/commands/command-line-interface/#launch-options\">launch options</a> ");
...
oslo::error write_wallet_config (oslo::wallet_config & config_a, boost::filesystem::path const & data_path_a)
{
	oslo::tomlconfig wallet_config_toml;
	auto wallet_path (oslo::get_qtwallet_toml_config_path (data_path_a));
...
oslo::error read_wallet_config (oslo::wallet_config & config_a, boost::filesystem::path const & data_path_a)
{
	oslo::tomlconfig wallet_config_toml;
	auto wallet_path (oslo::get_qtwallet_toml_config_path (data_path_a));
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path, oslo::node_flags const & flags)
{
	int result (0);
	...
	{
		error = oslo::error_config::rocksdb_enabled_but_not_supported;
	}
#endif
	...
	if (!error)
	{
		...
		oslo::thread_runner runner (io_ctx, config.node.io_threads);

		std::shared_ptr<oslo::node> node;
		std::shared_ptr<oslo_qt::wallet> gui;
		oslo::set_application_icon (application);
		auto opencl (oslo::opencl_work::create (config.opencl_enable, config.opencl, logger));
		oslo::work_pool work (config.node.work_threads, config.node.pow_sleep_interval, opencl ? [&opencl](oslo::work_version const version_a, oslo::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
		...
		node = std::make_shared<oslo::node> (io_ctx, data_path, alarm, config.node, work, flags);
		if (!node->init_error ())
		{
			...
			node->start ();
			oslo::ipc::ipc_server ipc (*node, config.rpc);
			...
			if (config.pow_server.enable)
			{
				...
				{
					splash->hide ();
					show_error (std::string ("oslo_pow_server is configured to start as a child process, however the file cannot be found at: ") + config.pow_server.pow_server_path);
					std::exit (1);
				}
...
#else
				splash->hide ();
				show_error ("oslo_pow_server is configured to start as a child process, but this is not supported on this system. Disable startup and start the server manually.");
				std::exit (1);
#endif
			}
			...
			if (config.rpc_enable)
			{
				if (!config.rpc.child_process.enable)
				{
					...
					rpc_handler = std::make_unique<oslo::inprocess_rpc_handler> (*node, ipc, config.rpc);
			...
			QObject::connect (&application, &QApplication::aboutToQuit, [&]() {
				...
				if (oslo_pow_server_process)
				{
					oslo_pow_server_process->terminate ();
				}
#endif
				runner.stop_event_processing ();
			});
			application.postEvent (&processor, new oslo_qt::eventloop_event ([&]() {
				gui = std::make_shared<oslo_qt::wallet> (application, processor, *node, wallet, wallet_config.account);
				splash->close ();
				gui->start ();
...
int main (int argc, char * const * argv)
{
	...
	try
	{
		...
		if (network != vm.end ())
		{
			...
			if (err)
			{
				show_error (oslo::network_constants::active_network_err_msg);
				std::exit (1);
			}
		}
