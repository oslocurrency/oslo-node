...
{
	size_t operator() (::oslo::uint256_union const & value_a) const
	{
		return std::hash<::oslo::uint256_union> () (value_a);
	}
};
...
{
	size_t operator() (::oslo::block_hash const & value_a) const
	{
		return std::hash<::oslo::block_hash> () (value_a);
	}
};

template <>
struct hash<::oslo::public_key>
{
	size_t operator() (::oslo::public_key const & value_a) const
	{
		return std::hash<::oslo::public_key> () (value_a);
	}
};
template <>
struct hash<::oslo::uint512_union>
{
	size_t operator() (::oslo::uint512_union const & value_a) const
	{
		return std::hash<::oslo::uint512_union> () (value_a);
	}
};
template <>
struct hash<::oslo::qualified_root>
{
	size_t operator() (::oslo::qualified_root const & value_a) const
	{
		return std::hash<::oslo::qualified_root> () (value_a);
	}
};
}
namespace oslo
...
public:
	...
	account_info (oslo::block_hash const &, oslo::account const &, oslo::block_hash const &, oslo::amount const &, uint64_t, uint64_t, epoch);
	bool deserialize (oslo::stream &);
	bool operator== (oslo::account_info const &) const;
	bool operator!= (oslo::account_info const &) const;
	size_t db_size () const;
	oslo::epoch epoch () const;
	...
	oslo::epoch epoch_m{ oslo::epoch::epoch_0 };
};
...
public:
	...
	pending_info (oslo::account const &, oslo::amount const &, oslo::epoch);
	size_t db_size () const;
	bool deserialize (oslo::stream &);
	bool operator== (oslo::pending_info const &) const;
...
public:
	pending_key () = default;
	pending_key (oslo::account const &, oslo::block_hash const &);
	bool deserialize (oslo::stream &);
	bool operator== (oslo::pending_key const &) const;
	oslo::account const & key () const;
...
public:
	...
	unchecked_key (oslo::block_hash const &, oslo::block_hash const &);
	bool deserialize (oslo::stream &);
	bool operator== (oslo::unchecked_key const &) const;
	oslo::block_hash const & key () const;
...
public:
	...
	unchecked_info (std::shared_ptr<oslo::block>, oslo::account const &, uint64_t, oslo::signature_verification = oslo::signature_verification::unknown, bool = false);
	...
	/** Seconds since posix epoch */
	uint64_t modified{ 0 };
	oslo::signature_verification verified{ oslo::signature_verification::unknown };
...
public:
	block_info () = default;
	block_info (oslo::account const &, oslo::amount const &);
	oslo::account account{ 0 };
	oslo::amount balance{ 0 };
};
...
public:
	...
	confirmation_height_info (uint64_t, oslo::block_hash const &);
	void serialize (oslo::stream &) const;
	bool deserialize (oslo::stream &);
	uint64_t height;
...
using vote_blocks_vec_iter = std::vector<boost::variant<std::shared_ptr<oslo::block>, oslo::block_hash>>::const_iterator;
class iterate_vote_blocks_as_hash final
{
public:
	iterate_vote_blocks_as_hash () = default;
	oslo::block_hash operator() (boost::variant<std::shared_ptr<oslo::block>, oslo::block_hash> const & item) const;
};
class vote final
{
public:
	vote () = default;
	vote (oslo::vote const &);
	vote (bool &, oslo::stream &, oslo::block_uniquer * = nullptr);
	vote (bool &, oslo::stream &, oslo::block_type, oslo::block_uniquer * = nullptr);
	vote (oslo::account const &, oslo::raw_key const &, uint64_t, std::shared_ptr<oslo::block>);
	vote (oslo::account const &, oslo::raw_key const &, uint64_t, std::vector<oslo::block_hash> const &);
	std::string hashes_string () const;
	oslo::block_hash hash () const;
	oslo::block_hash full_hash () const;
	bool operator== (oslo::vote const &) const;
	bool operator!= (oslo::vote const &) const;
	...
	bool validate () const;
	boost::transform_iterator<oslo::iterate_vote_blocks_as_hash, oslo::vote_blocks_vec_iter> begin () const;
	boost::transform_iterator<oslo::iterate_vote_blocks_as_hash, oslo::vote_blocks_vec_iter> end () const;
	std::string to_json () const;
	// Vote round sequence number
	uint64_t sequence;
	// The blocks, or block hashes, that this vote is for
	std::vector<boost::variant<std::shared_ptr<oslo::block>, oslo::block_hash>> blocks;
...
{
public:
	using value_type = std::pair<const oslo::block_hash, std::weak_ptr<oslo::vote>>;

	vote_uniquer (oslo::block_uniquer &);
	std::shared_ptr<oslo::vote> unique (std::shared_ptr<oslo::vote>);
	size_t size ();

private:
	oslo::block_uniquer & uniquer;
	std::mutex mutex;
...
{
public:
	oslo::process_result code;
	oslo::account account;
	oslo::amount amount;
	oslo::account pending_account;
	boost::optional<bool> state_is_send;
	oslo::signature_verification verified;
	oslo::amount previous_balance;
};
...
public:
	genesis ();
	oslo::block_hash hash () const;
	std::shared_ptr<oslo::block> open;
};
...
{
public:
	ledger_constants (oslo::network_constants & network_constants);
	ledger_constants (oslo::oslo_networks network_a);
	oslo::keypair zero_key;
	oslo::keypair test_genesis_key;
	oslo::account oslo_test_account;
	oslo::account oslo_beta_account;
	oslo::account oslo_live_account;
	std::string oslo_test_genesis;
	std::string oslo_beta_genesis;
	std::string oslo_live_genesis;
	oslo::account genesis_account;
	std::string genesis_block;
	oslo::block_hash genesis_hash;
	oslo::uint128_t genesis_amount;
	oslo::account burn_account;
	oslo::epochs epochs;
};
...
public:
	random_constants ();
	oslo::account not_an_account;
	oslo::uint128_union random_128;
};
...
{
public:
	node_constants (oslo::network_constants & network_constants);
...
{
public:
	voting_constants (oslo::network_constants & network_constants);
	size_t max_cache;
};
...
{
public:
	portmapping_constants (oslo::network_constants & network_constants);
...
{
public:
	bootstrap_constants (oslo::network_constants & network_constants);
...
public:
	...
	network_params (oslo::oslo_networks network_a);
...
{
public:
	std::shared_ptr<oslo::block> winner;
	oslo::amount tally;
...
oslo::wallet_id random_wallet_id ();
}
