...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a oslo_node crash")
		("debug_generate_crash_report", "Consolidates the oslo_node_backtrace.dump file. Requires addr2line installed on Linux")
		...
		("debug_profile_process", "Profile active blocks processing (only for oslo_test_network)")
		("debug_profile_votes", "Profile votes processing (only for oslo_test_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for oslo_test_network)")
	...
	oslo::add_node_flag_options (description);
	...
	if (network != vm.end ())
	{
		auto err (oslo::network_constants::set_active_network (network->second.as<std::string> ()));
		if (err)
		{
			std::cerr << oslo::network_constants::active_network_err_msg << std::endl;
			std::exit (1);
		}
	}
	...
	boost::filesystem::path data_path ((data_path_it != vm.end ()) ? data_path_it->second.as<std::string> () : oslo::working_path ());
	auto ec = oslo::handle_node_options (vm);
	if (ec == oslo::error_cli::unknown_command)
	{
		if (vm.count ("daemon") > 0)
		{
			oslo_daemon::daemon daemon;
			oslo::node_flags flags;
			auto flags_ec = oslo::update_flags (flags, vm);
			if (flags_ec)
			{
		...
		{
			if (!oslo::network_constants ().is_test_network ())
			{
				auto node_flags = oslo::inactive_node_flag_defaults ();
				oslo::update_flags (node_flags, vm);
				node_flags.generate_cache.reps = true;
				oslo::inactive_node inactive_node (data_path, node_flags);
				...
				auto get_total = [](decltype (hardcoded) const & reps) -> oslo::uint128_union {
					return std::accumulate (reps.begin (), reps.end (), oslo::uint128_t{ 0 }, [](auto sum, auto const & rep) { return sum + rep.second; });
				};
				...
				{
					std::vector<std::pair<oslo::account, oslo::uint128_t>> sorted;
				...
				{
					oslo::account rep;
					oslo::uint128_union hardcoded;
					oslo::uint128_union ledger;
					oslo::uint128_union diff;
					std::string get_entry () const
					{
						return boost::str (boost::format ("representative %1% hardcoded %2% ledger %3% mismatch %4%")
						% rep.to_account () % hardcoded.format_balance (oslo::Mxrb_ratio, 0, true) % ledger.format_balance (oslo::Mxrb_ratio, 0, true) % diff.format_balance (oslo::Mxrb_ratio, 0, true));
					}
				};
				...
				std::transform (hardcoded.begin (), hardcoded.end (), std::back_inserter (mismatched), [&ledger](auto const & rep) {
					...
					oslo::uint128_t ledger_weight = (ledger_rep == ledger.end () ? 0 : ledger_rep->second);
				...
				oslo::uint128_union const mismatch_total = std::accumulate (mismatched.begin (), mismatched.end (), oslo::uint128_t{ 0 }, [](auto sum, mismatched_t const & sample) { return sum + sample.diff.number (); });
				oslo::uint128_union const mismatch_mean = mismatch_total.number () / mismatched.size ();

				oslo::uint512_union mismatch_variance = std::accumulate (mismatched.begin (), mismatched.end (), oslo::uint512_t (0), [M = mismatch_mean.number (), N = mismatched.size ()](oslo::uint512_t sum, mismatched_t const & sample) {
					auto x = sample.diff.number ();
					oslo::uint512_t const mean_diff = x > M ? x - M : M - x;
					oslo::uint512_t const sqr = mean_diff * mean_diff;
					return sum + sqr;
				})
				/ mismatched.size ();

				oslo::uint128_union const mismatch_stddev = oslo::narrow_cast<oslo::uint128_t> (boost::multiprecision::sqrt (mismatch_variance.number ()));

				auto const outlier_threshold = std::max (oslo::Gxrb_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (oslo::Gxrb_ratio, mismatch_mean.number ());
				std::vector<std::pair<oslo::account, oslo::uint128_t>> newcomers;
				...
				auto newcomer_entry = [](auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % oslo::uint128_union (rep.second).format_balance (oslo::Mxrb_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Moslo\nledger weight %2% Moslo\nmismatched\n\tsamples %3%\n\ttotal %4% Moslo\n\tmean %5% Moslo\n\tsigma %6% Moslo\n")
				% total_hardcoded.format_balance (oslo::Mxrb_ratio, 0, true)
				% total_ledger.format_balance (oslo::Mxrb_ratio, 0, true)
				% mismatched.size ()
				% mismatch_total.format_balance (oslo::Mxrb_ratio, 0, true)
				% mismatch_mean.format_balance (oslo::Mxrb_ratio, 0, true)
				% mismatch_stddev.format_balance (oslo::Mxrb_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = oslo::Gxrb_ratio;
		...
		{
			auto inactive_node = oslo::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				...
				{
					oslo::keypair genesis (key.to_string ());
					...
					oslo::uint128_t balance (std::numeric_limits<oslo::uint128_t>::max ());
					oslo::open_block genesis_block (reinterpret_cast<const oslo::block_hash &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (oslo::work_version::work_1, genesis.pub, network_constants.publish_thresholds.epoch_1));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					oslo::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						oslo::uint128_t yearly_distribution (oslo::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							oslo::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (oslo::work_version::work_1, previous, network_constants.publish_thresholds.epoch_1));
							previous = send.hash ();
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			auto node = inactive_node.node;
			auto transaction (node->store.tx_begin_read ());
			oslo::uint128_t total;
			auto rep_amounts = node->ledger.cache.rep_weights.get_rep_amounts ();
			std::map<oslo::account, oslo::uint128_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::unordered_set<oslo::block_hash> frontier_hashes;
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			oslo::uint256_union result;
			oslo::uint256_union salt (0);
			std::string password ("");
			while (true)
			{
		...
		else if (vm.count ("debug_profile_generate"))
		{
			...
			if (multiplier_it != vm.end ())
			{
				try
				{
					...
					difficulty = oslo::difficulty::from_multiplier (multiplier, difficulty);
				}
			...
			else
			{
				...
				{
					if (oslo::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
			...
			oslo::work_pool work (std::numeric_limits<unsigned>::max (), pow_rate_limiter);
			oslo::change_block block (0, 0, oslo::keypair ().prv, 0, 0);
			if (!result)
			{
				std::cerr << boost::str (boost::format ("Starting generation profiling. Difficulty: %1$#x (%2%x from base difficulty %3$#x)\n") % difficulty % oslo::to_string (oslo::difficulty::to_multiplier (difficulty, network_constants.publish_full.base), 4) % network_constants.publish_full.base);
				while (!result)
				{
					block.hashables.previous.qwords[0] += 1;
					auto begin1 (std::chrono::high_resolution_clock::now ());
					block.block_work_set (*work.generate (oslo::work_version::work_1, block.root (), difficulty));
					auto end1 (std::chrono::high_resolution_clock::now ());
		...
		{
			uint64_t difficulty{ oslo::network_constants ().publish_full.base };
			...
			{
				valid = oslo::work_v1::value (hash, i) > difficulty;
			}
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							oslo::opencl_config config (platform, device, threads);
							auto opencl (oslo::opencl_work::create (true, config, logger));
							oslo::work_pool work_pool (std::numeric_limits<unsigned>::max (), std::chrono::nanoseconds (0), opencl ? [&opencl](oslo::work_version const version_a, oslo::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
								return opencl->generate_work (version_a, root_a, difficulty_a);
							}
							                                                                                                       : std::function<boost::optional<uint64_t> (oslo::work_version const, oslo::root const &, uint64_t, std::atomic<int> &)> (nullptr));
							oslo::change_block block (0, 0, oslo::keypair ().prv, 0, 0);
							std::cerr << boost::str (boost::format ("Starting OpenCL generation profiling. Platform: %1%. Device: %2%. Threads: %3%. Difficulty: %4$#x (%5%x from base difficulty %6$#x)\n") % platform % device % threads % difficulty % oslo::to_string (oslo::difficulty::to_multiplier (difficulty, network_constants.publish_full.base), 4) % network_constants.publish_full.base);
							for (uint64_t i (0); true; ++i)
							{
								block.hashables.previous.qwords[0] += 1;
								auto begin1 (std::chrono::high_resolution_clock::now ());
								block.block_work_set (*work_pool.generate (oslo::work_version::work_1, block.root (), difficulty));
								auto end1 (std::chrono::high_resolution_clock::now ());
								std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
							}
						}
		...
		{
			if (boost::filesystem::exists ("oslo_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("oslo_node_backtrace.dump");
		...
		else if (vm.count ("debug_generate_crash_report"))
		{
			if (boost::filesystem::exists ("oslo_node_backtrace.dump"))
			{
				...
				std::string crash_report_filename = "oslo_node_crash_report.txt";
				...
				ofs << st;
#else
				// Read all the oslo node files
				boost::system::error_code err;
				auto running_executable_filepath = boost::dll::program_location (err);
				if (!err)
				{
					auto num = 0;
					auto format = boost::format ("oslo_node_crash_load_address_dump_%1%.txt");
					...
					{
						std::ofstream ofs (crash_report_filename);
						oslo::set_secure_perm_file (crash_report_filename);
					}
					...
					// Now run using relative addresses. This will give actual results for other dlls, the results from the oslo_node executable.
					run_addr2line (true);

					if (std::find (system_codes.begin (), system_codes.end (), 0) == system_codes.end ())
					{
						std::cerr << "Error: Check that addr2line is installed and that oslo_node_crash_load_address_dump_*.txt files exist." << std::endl;
						result = -1;
					}
					else
					{
						...
						boost::filesystem::remove ("oslo_node_backtrace.dump");
					}
				}
				else
				{
			...
			{
				std::cerr << "Error: oslo_node_backtrace.dump could not be found";
				result = -1;
			}
		}
		...
		{
			oslo::keypair key;
			oslo::uint256_union message;
			auto signature = oslo::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				oslo::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			oslo::uint256_union message;
			oslo::uint512_union signature (oslo::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			oslo::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				oslo::keypair key;
				oslo::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					oslo::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			oslo::network_params test_params;
			...
			oslo::update_flags (node_flags, vm);
			auto node (std::make_shared<oslo::node> (system.io_ctx, 24001, path, system.alarm, logging, work, node_flags));
			oslo::block_hash genesis_latest (node->latest (test_params.ledger.test_genesis_key.pub));
			oslo::uint128_t genesis_balance (std::numeric_limits<oslo::uint128_t>::max ());
			// Generating keys
			std::vector<oslo::keypair> keys (num_accounts);
			std::vector<oslo::root> frontiers (num_accounts);
			std::vector<oslo::uint128_t> balances (num_accounts, 1000000000);
			// Generating blocks
			std::deque<std::shared_ptr<oslo::block>> blocks;
			for (auto i (0); i != num_accounts; ++i)
			{
				...
				auto send = builder.state ()
				            ...
				            .work (*work.generate (oslo::work_version::work_1, genesis_latest, node->network_params.network.publish_thresholds.epoch_1))
				            .build ();
				...
				auto open = builder.state ()
				            ...
				            .work (*work.generate (oslo::work_version::work_1, keys[i].pub, node->network_params.network.publish_thresholds.epoch_1))
				            .build ();
			...
			for (auto i (0); i != num_iterations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto send = builder.state ()
					            ...
					            .work (*work.generate (oslo::work_version::work_1, frontiers[j], node->network_params.network.publish_thresholds.epoch_1))
					            .build ();
					...
					auto receive = builder.state ()
					               ...
					               .link (static_cast<oslo::block_hash const &> (frontiers[j]))
					               .sign (keys[other].prv, keys[other].pub)
					               .work (*work.generate (oslo::work_version::work_1, frontiers[other], node->network_params.network.publish_thresholds.epoch_1))
					               .build ();
			...
			oslo::timer<std::chrono::seconds> timer_l (oslo::timer_state::started);
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			auto path (oslo::unique_path ());
			logging.init (path);
			auto node (std::make_shared<oslo::node> (system.io_ctx, 24001, path, system.alarm, logging, work));
			oslo::block_hash genesis_latest (node->latest (test_params.ledger.test_genesis_key.pub));
			oslo::uint128_t genesis_balance (std::numeric_limits<oslo::uint128_t>::max ());
			// Generating keys
			std::vector<oslo::keypair> keys (num_representatives);
			oslo::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<oslo::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<oslo::vote> (keys[j].pub, keys[j].prv, sequence, std::vector<oslo::block_hash> (1, i->hash ())));
		...
		else if (vm.count ("debug_profile_frontiers_confirmation"))
		{
			...
			oslo::alarm alarm1 (io_ctx1);
			oslo::alarm alarm2 (io_ctx2);
			oslo::work_pool work (std::numeric_limits<unsigned>::max ());
			oslo::logging logging;
			auto path1 (oslo::unique_path ());
			auto path2 (oslo::unique_path ());
			logging.init (path1);
			oslo::node_config config1 (24000, logging);
			oslo::node_flags flags;
			flags.disable_lazy_bootstrap = true;
			flags.disable_legacy_bootstrap = true;
			flags.disable_wallet_bootstrap = true;
			flags.disable_bootstrap_listener = true;
			auto node1 (std::make_shared<oslo::node> (io_ctx1, path1, alarm1, config1, work, flags, 0));
			oslo::block_hash genesis_latest (node1->latest (test_params.ledger.test_genesis_key.pub));
			oslo::uint128_t genesis_balance (std::numeric_limits<oslo::uint128_t>::max ());
			// Generating blocks
			std::deque<std::shared_ptr<oslo::block>> blocks;
			for (auto i (0); i != count; ++i)
			{
				...
				auto send = builder.state ()
				            ...
				            .link (key.pub)
				            .sign (test_params.ledger.test_genesis_key.prv, test_params.ledger.test_genesis_key.pub)
				            .work (*work.generate (oslo::work_version::work_1, genesis_latest, test_params.network.publish_thresholds.epoch_1))
				            .build ();
				...
				auto open = builder.state ()
				            ...
				            .work (*work.generate (oslo::work_version::work_1, key.pub, test_params.network.publish_thresholds.epoch_1))
				            .build ();
			...
			node1->start ();
			oslo::thread_runner runner1 (io_ctx1, node1->config.io_threads);
			...
			// Start new node
			oslo::node_config config2 (24001, logging);
			// Config override
			...
			{
				auto path (oslo::unique_path ());
				oslo::daemon_config daemon_config (path);
				auto error = oslo::read_node_config_toml (path, daemon_config, config_overrides);
				if (error)
				{
					std::cerr << "\n"
			...
			auto node2 (std::make_shared<oslo::node> (io_ctx2, path2, alarm2, config2, work, flags, 1));
			node2->start ();
			oslo::thread_runner runner2 (io_ctx2, node2->config.io_threads);
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./oslo_node --debug_random_feed | dieharder -a -g 200
			 */
			oslo::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			oslo::inactive_node inactive_node_l (data_path, node_flags);

			oslo::node_rpc_config config;
			oslo::ipc::ipc_server server (*inactive_node_l.node, config);
			auto handler_l (std::make_shared<oslo::json_handler> (*inactive_node_l.node, config, command_l.str (), response_handler_l));
			handler_l->process_request ();
		}
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			oslo::timer<std::chrono::seconds> timer;
			timer.start ();
			auto inactive_node = oslo::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
			...
			std::deque<std::pair<oslo::account, oslo::account_info>> accounts;
			...
			auto print_error_message = [&silent, &errors](std::string const & error_message_a) {
				if (!silent)
				{
					...
					oslo::lock_guard<std::mutex> lock (cerr_mutex);
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count](std::shared_ptr<oslo::node> const & node, oslo::read_transaction const & transaction, oslo::account const & account, oslo::account_info const & info) {
				++count;
				...
				oslo::block_hash calculated_hash (0);
				auto block (node->store.block_get (transaction, hash)); // Block data
				uint64_t height (0);
				uint64_t previous_timestamp (0);
				oslo::account calculated_representative (0);
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (block->type () == oslo::block_type::open || block->type () == oslo::block_type::state)
					{
					...
					if (height == 0 && block->type () != oslo::block_type::open && block->type () != oslo::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<oslo::state_block &> (*block.get ()));
					...
					if (block->difficulty () < oslo::work_threshold (block->work_version (), block->sideband ().details))
					{
						print_error_message (boost::str (boost::format ("Invalid work for block %1% value: %2%\n") % hash.to_string () % oslo::to_string_hex (block->block_work ())));
					}
					...
					if (block->type () == oslo::block_type::open || block->type () == oslo::block_type::change || block->type () == oslo::block_type::state)
					{
			...
			count = 0;
			finished = false;
			std::deque<std::pair<oslo::pending_key, oslo::pending_info>> pending;

			auto check_pending = [&print_error_message, &silent, &count](std::shared_ptr<oslo::node> const & node, oslo::read_transaction const & transaction, oslo::pending_key const & key, oslo::pending_info const & info) {
				++count;
				...
				else
				{
					...
					if (auto state = dynamic_cast<oslo::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<oslo::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			oslo::update_flags (node_flags, vm);
			oslo::inactive_node node2 (oslo::unique_path (), node_flags);
				...
				for (auto i (node->store.latest_begin (transaction)), n (node->store.latest_end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							oslo::unchecked_info unchecked_info (block, account, 0, oslo::signature_verification::unknown);
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % oslo::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		else if (vm.count ("debug_cemented_block_count"))
		{
			...
			oslo::update_flags (node_flags, vm);
			oslo::inactive_node node (data_path, node_flags);
		...
		else if (vm.count ("debug_sys_logging"))
		{
			...
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Oslo\\Oslo registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = oslo::default_inactive_node (data_path, vm);
			inactive_node->node->logger.always_log (oslo::severity_level::error, "Testing system logger");
		}
		else if (vm.count ("debug_account_versions"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::vector<std::unordered_set<oslo::account>> opened_account_versions (oslo::normalized_epoch (oslo::epoch::max));
			...
			for (auto i (node->store.latest_begin (transaction)), n (node->store.latest_end ()); i != n; ++i)
			{
				...
				auto epoch_idx = oslo::normalized_epoch (account_info.epoch ());
			...
			std::unordered_map<oslo::account, std::underlying_type_t<oslo::epoch>> unopened_highest_pending;
			for (auto i (node->store.pending_begin (transaction)), n (node->store.pending_end ()); i != n; ++i)
			{
				...
				if (!exists)
				{
					...
					auto epoch = oslo::normalized_epoch (info.epoch);
			...
			std::vector<size_t> unopened_account_version_totals (oslo::normalized_epoch (oslo::epoch::max));
