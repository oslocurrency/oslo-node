...
inline boost::log::formatting_ostream & operator<< (boost::log::formatting_ostream & strm, boost::log::to_log_manip<oslo::severity_level, severity_tag> const & manip)
{
	...
	oslo::severity_level level = manip.get ();
...
private:
	...
	void output (oslo::severity_level severity_level, LogItems &&... log_items)
	{
...
public:
	 ...
	 */
	template <typename... LogItems>
	void always_log (oslo::severity_level severity_level, LogItems &&... log_items)
	{
	...
	{
		always_log (oslo::severity_level::normal, std::forward<LogItems> (log_items)...);
	}
	 ...
	 */
	template <typename... LogItems>
	bool try_log (oslo::severity_level severity_level, LogItems &&... log_items)
	{
		auto error (true);
		auto time_now = std::chrono::steady_clock::now ();
		oslo::unique_lock<std::mutex> lk (last_log_time_mutex);
	...
	{
		return try_log (oslo::severity_level::normal, std::forward<LogItems> (log_items)...);
	}
