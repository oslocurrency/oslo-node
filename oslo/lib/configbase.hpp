...
class configbase : public oslo::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<oslo::error> const & error_a) :
	error (error_a)
	{
	}

	/** Returns the current error */
	oslo::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a oslo::error_aware type. Child nodes share the error state. */
	std::shared_ptr<oslo::error> error;
