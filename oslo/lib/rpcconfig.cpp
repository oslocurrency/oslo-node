...
oslo::error oslo::rpc_secure_config::serialize_json (oslo::jsonconfig & json) const
{
...
oslo::error oslo::rpc_secure_config::deserialize_json (oslo::jsonconfig & json)
{
...
oslo::error oslo::rpc_secure_config::serialize_toml (oslo::tomlconfig & toml) const
{
...
oslo::error oslo::rpc_secure_config::deserialize_toml (oslo::tomlconfig & toml)
{
...
oslo::rpc_config::rpc_config (uint16_t port_a, bool enable_control_a) :
address (boost::asio::ip::address_v6::loopback ().to_string ()),
port (port_a),
...
oslo::error oslo::rpc_config::serialize_json (oslo::jsonconfig & json) const
{
...
oslo::error oslo::rpc_config::deserialize_json (bool & upgraded_a, oslo::jsonconfig & json)
{
	if (!json.empty ())
	{
...
oslo::error oslo::rpc_config::serialize_toml (oslo::tomlconfig & toml) const
{
...
oslo::rpc_process_config::rpc_process_config () :
...
namespace oslo
{
oslo::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, oslo::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	oslo::error error;
	auto json_config_path = oslo::get_rpc_config_path (data_path_a);
	auto toml_config_path = oslo::get_rpc_toml_config_path (data_path_a);
	if (boost::filesystem::exists (json_config_path))
	{
		...
		else
		{
			...
			if (!error)
			{
				...
				oslo::rpc_config config_defaults_l;
...
oslo::error read_and_update_rpc_config (boost::filesystem::path const & data_path, oslo::rpc_config & config_a)
{
	...
	auto config_path = oslo::get_rpc_config_path (data_path);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the oslo_rpc executable file path based on where the currently running executable is found.
	auto rpc_filepath = running_executable_filepath.parent_path () / "oslo_rpc";
