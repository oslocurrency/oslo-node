...
struct work_thresholds
{
	...
	work_thresholds operator= (oslo::work_thresholds const & other_a)
	{
		return other_a;
	}
};
...
public:
	...
	network_constants (oslo_networks network_a) :
	...
	publish_thresholds (is_live_network () ? publish_full : is_beta_network () ? publish_beta : publish_test)
	{
		...
		default_node_port = is_live_network () ? 9095 : is_beta_network () ? 54000 : 44000;
		default_rpc_port = is_live_network () ? 9096 : is_beta_network () ? 55000 : 45000;
		default_ipc_port = is_live_network () ? 9097 : is_beta_network () ? 56000 : 46000;
		default_websocket_port = is_live_network () ? 9098 : is_beta_network () ? 57000 : 47000;
		request_interval_ms = is_test_network () ? 20 : 500;
	}

	/** Network work thresholds. Define these inline as constexpr when moving to cpp17. */
	static const oslo::work_thresholds publish_full;
	static const oslo::work_thresholds publish_beta;
	static const oslo::work_thresholds publish_test;
	...
	oslo_networks current_network{ oslo::network_constants::active_network };
	oslo::work_thresholds publish_thresholds;
	 ...
	 */
	static void set_active_network (oslo_networks network_a)
	{
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = oslo::oslo_networks::oslo_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = oslo::oslo_networks::oslo_beta_network;
		}
		else if (network_a == "test")
		{
			active_network = oslo::oslo_networks::oslo_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == oslo_networks::oslo_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == oslo_networks::oslo_beta_network;
	}
	bool is_test_network () const
	{
		return current_network == oslo_networks::oslo_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static oslo::oslo_networks active_network;
};
