...
void output (const char * str, std::chrono::milliseconds time, Mutex & mutex)
{
	...
	auto stacktrace = oslo::generate_stacktrace ();
...
template <typename Mutex>
void output_if_held_long_enough (oslo::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
...
template <typename Mutex>
void output_if_blocked_long_enough (oslo::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
