...
public:
	...
	oslo::error deserialize_json (oslo::jsonconfig & json);
...
public:
	...
	oslo::observer_set<boost::circular_buffer<stat_datapoint> &> sample_observers;

	/** Observers for count. Called on each update. */
	oslo::observer_set<uint64_t, uint64_t> count_observers;
};
...
private:
	...
	std::shared_ptr<oslo::stat_entry> get_entry (uint32_t key);

	/** Get entry for key, creating a new entry if necessary */
	std::shared_ptr<oslo::stat_entry> get_entry (uint32_t key, size_t sample_interval, size_t max_samples);

	/** Unlocked implementation of get_entry() */
	std::shared_ptr<oslo::stat_entry> get_entry_impl (uint32_t key, size_t sample_interval, size_t max_samples);
	...
	oslo::stat_config config;

	/** Stat entries are sorted by key to simplify processing of log output */
	std::map<uint32_t, std::shared_ptr<oslo::stat_entry>> entries;
