...
oslo::tomlconfig::tomlconfig () :
tree (cpptoml::make_table ())
{
	error = std::make_shared<oslo::error> ();
}

oslo::tomlconfig::tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<oslo::error> const & error_a) :
oslo::configbase (error_a), tree (tree_a)
{
	if (!error)
	{
		error = std::make_shared<oslo::error> ();
...
void oslo::tomlconfig::doc (std::string const & key, std::string const & doc)
{
	tree->document (key, doc);
}

oslo::error & oslo::tomlconfig::read (boost::filesystem::path const & path_a)
{
...
oslo::error & oslo::tomlconfig::read (std::istream & stream_overrides, boost::filesystem::path const & path_a)
{
	std::fstream stream;
...
oslo::error & oslo::tomlconfig::read (std::istream & stream_a)
{
	std::stringstream stream_override_empty;
	stream_override_empty << std::endl;
...
oslo::error & oslo::tomlconfig::read (std::istream & stream_first_a, std::istream & stream_second_a)
{
	try
	{
...
void oslo::tomlconfig::write (boost::filesystem::path const & path_a)
{
	std::fstream stream;
	open_or_create (stream, path_a.string ());
	write (stream);
...
void oslo::tomlconfig::write (std::ostream & stream_a) const
{
...
void oslo::tomlconfig::open_or_create (std::fstream & stream_a, std::string const & path_a)
{
	if (!boost::filesystem::exists (path_a))
	{
		...
		oslo::set_secure_perm_file (path_a);
	}
...
std::shared_ptr<cpptoml::table> oslo::tomlconfig::get_tree ()
{
	return tree;
}

/** Returns true if the toml table is empty */
bool oslo::tomlconfig::empty () const
{
	return tree->empty ();
}

boost::optional<oslo::tomlconfig> oslo::tomlconfig::get_optional_child (std::string const & key_a)
{
...
oslo::tomlconfig oslo::tomlconfig::get_required_child (std::string const & key_a)
{
	if (!tree->contains (key_a))
	{
		*error = oslo::error_config::missing_value;
...
oslo::tomlconfig & oslo::tomlconfig::put_child (std::string const & key_a, oslo::tomlconfig & conf_a)
{
	tree->insert (key_a, conf_a.get_tree ());
	return *this;
}

oslo::tomlconfig & oslo::tomlconfig::replace_child (std::string const & key_a, oslo::tomlconfig & conf_a)
{
	tree->erase (key_a);
...
bool oslo::tomlconfig::has_key (std::string const & key_a)
{
	return tree->contains (key_a);
}

/** Erase the property of given key */
oslo::tomlconfig & oslo::tomlconfig::erase (std::string const & key_a)
{
	tree->erase (key_a);
	return *this;
}

std::shared_ptr<cpptoml::array> oslo::tomlconfig::create_array (std::string const & key, boost::optional<const char *> documentation_a)
{
	if (!has_key (key))
	{
...
/**
	 ...
	 */
void oslo::tomlconfig::erase_default_values (tomlconfig & defaults_a)
{
...
std::string oslo::tomlconfig::to_string ()
{
	std::stringstream ss;
...
std::string oslo::tomlconfig::to_string_commented_entries ()
{
...
oslo::tomlconfig & oslo::tomlconfig::get_config (bool optional, std::string const & key, uint8_t & target, uint8_t default_value)
{
	try
	{
		if (tree->contains_qualified (key))
		{
			...
			{
				conditionally_set_error<uint8_t> (oslo::error_config::invalid_value, optional, key);
			}
			else
			{
		...
		{
			conditionally_set_error<uint8_t> (oslo::error_config::missing_value, optional, key);
		}
		else
		{
...
oslo::tomlconfig & oslo::tomlconfig::get_config (bool optional, std::string const & key, bool & target, bool default_value)
{
	auto bool_conv = [this, &target, &key, optional](std::string val) {
		...
		{
			conditionally_set_error<bool> (oslo::error_config::invalid_value, optional, key);
		}
	};
	try
	{
		...
		{
			conditionally_set_error<bool> (oslo::error_config::missing_value, optional, key);
		}
		else
		{
			target = default_value;
...
void oslo::tomlconfig::erase_defaults (std::shared_ptr<cpptoml::table> base, std::shared_ptr<cpptoml::table> other, std::shared_ptr<cpptoml::table> update_target)
{
...
oslo::tomlconfig & oslo::tomlconfig::get_config (bool optional, std::string key, boost::asio::ip::address_v6 & target, boost::asio::ip::address_v6 const & default_value)
{
	try
	{
		if (tree->contains_qualified (key))
		{
			...
			if (bec)
			{
				conditionally_set_error<boost::asio::ip::address_v6> (oslo::error_config::invalid_value, optional, key);
			}
		}
		else if (!optional)
		{
			conditionally_set_error<boost::asio::ip::address_v6> (oslo::error_config::missing_value, optional, key);
		}
		else
		{
			target = default_value;
