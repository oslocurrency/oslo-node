...
REGISTER_ERROR_CODES (oslo, error_common);
REGISTER_ERROR_CODES (oslo, error_blocks);
REGISTER_ERROR_CODES (oslo, error_rpc);
REGISTER_ERROR_CODES (oslo, error_process);
REGISTER_ERROR_CODES (oslo, error_config);
...
public:
	error () = default;
	error (oslo::error const & error_a) = default;
	error (oslo::error && error_a) = default;
	...
	error & operator= (oslo::error const & err_a);
	error & operator= (oslo::error && err_a);
	...
	error & then (std::function<oslo::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = oslo::error_common::generic);
...
/**
 * A type that manages a oslo::error.
 * The default return type is oslo::error&, though shared_ptr<oslo::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = oslo::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, oslo::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<oslo::error>>::value, "Must be oslo::error& or shared_ptr<oslo::error>");
