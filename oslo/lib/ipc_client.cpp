...
public:
	...
	virtual void async_write (oslo::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a) = 0;
...
class socket_client : public oslo::ipc::socket_base, public channel, public std::enable_shared_from_this<socket_client<SOCKET_TYPE, ENDPOINT_TYPE>>
{
public:
	...
	void async_write (oslo::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a) override
	{
		auto this_l (this->shared_from_this ());
	...
	void write_queued_messages ()
	{
		...
		this_l->timer_start (io_timeout);
		oslo::async_write (socket, msg.buffer,
...
private:
	...
	{
	public:
		queue_item (oslo::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a) :
		buffer (buffer_a), callback (callback_a)
		{
		}
		oslo::shared_const_buffer buffer;
 ...
 */
class client_impl : public oslo::ipc::ipc_client_impl
{
public:
	...
	void connect (std::string const & host_a, uint16_t port_a, std::function<void(oslo::error)> callback_a)
	{
		...
		tcp_client->async_resolve (host_a, port_a, [this, callback_a](boost::system::error_code const & ec_resolve_a, boost::asio::ip::tcp::endpoint endpoint_a) {
			if (!ec_resolve_a)
			{
				this->tcp_client->async_connect ([callback_a](const boost::system::error_code & ec_connect_a) {
					callback_a (oslo::error (ec_connect_a));
				});
			}
			else
			{
				callback_a (oslo::error (ec_resolve_a));
			}
		});
	}

	oslo::error connect (std::string const & path_a)
	{
		oslo::error err;
...
#else
		err = oslo::error ("Domain sockets are not supported by this platform");
#endif
		return err;
	}
...
oslo::ipc::ipc_client::ipc_client (boost::asio::io_context & io_ctx_a) :
io_ctx (io_ctx_a)
{
}

oslo::error oslo::ipc::ipc_client::connect (std::string const & path_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
	return boost::polymorphic_downcast<client_impl *> (impl.get ())->connect (path_a);
}

void oslo::ipc::ipc_client::async_connect (std::string const & host_a, uint16_t port_a, std::function<void(oslo::error)> callback_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
...
oslo::error oslo::ipc::ipc_client::connect (std::string const & host, uint16_t port)
{
	std::promise<oslo::error> result_l;
	async_connect (host, port, [&result_l](oslo::error err_a) {
		result_l.set_value (err_a);
	});
	return result_l.get_future ().get ();
}

void oslo::ipc::ipc_client::async_write (oslo::shared_const_buffer const & buffer_a, std::function<void(oslo::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_write (buffer_a, [callback_a](const boost::system::error_code & ec_a, size_t bytes_written_a) {
		callback_a (oslo::error (ec_a), bytes_written_a);
	});
}

void oslo::ipc::ipc_client::async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, size_t size_a, std::function<void(oslo::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_read (buffer_a, size_a, [callback_a, buffer_a](const boost::system::error_code & ec_a, size_t bytes_read_a) {
		callback_a (oslo::error (ec_a), bytes_read_a);
	});
}

/** Read a length-prefixed message asynchronously. Received length must be a big endian 32-bit unsigned integer. */
void oslo::ipc::ipc_client::async_read_message (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, std::chrono::seconds timeout_a, std::function<void(oslo::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
...
std::vector<uint8_t> oslo::ipc::get_preamble (oslo::ipc::payload_encoding encoding_a)
{
...
oslo::shared_const_buffer oslo::ipc::prepare_flatbuffers_request (std::shared_ptr<flatbuffers::FlatBufferBuilder> const & flatbuffer_a)
{
	auto buffer_l (get_preamble (oslo::ipc::payload_encoding::flatbuffers));
	...
	return oslo::shared_const_buffer{ std::move (buffer_l) };
}

oslo::shared_const_buffer oslo::ipc::prepare_request (oslo::ipc::payload_encoding encoding_a, std::string const & payload_a)
{
	std::vector<uint8_t> buffer_l;
	if (encoding_a == oslo::ipc::payload_encoding::json_v1 || encoding_a == oslo::ipc::payload_encoding::flatbuffers_json)
	{
...
std::string oslo::ipc::request (oslo::ipc::payload_encoding encoding_a, oslo::ipc::ipc_client & ipc_client, std::string const & rpc_action_a)
{
	auto req (prepare_request (encoding_a, rpc_action_a));
	auto res (std::make_shared<std::vector<uint8_t>> ());

	std::promise<std::string> result_l;
	ipc_client.async_write (req, [&ipc_client, &res, &result_l](oslo::error err_a, size_t size_a) {
		// Read length
		ipc_client.async_read (res, sizeof (uint32_t), [&ipc_client, &res, &result_l](oslo::error err_read_a, size_t size_read_a) {
			uint32_t payload_size_l = boost::endian::big_to_native (*reinterpret_cast<uint32_t *> (res->data ()));
			// Read json payload
			ipc_client.async_read (res, payload_size_l, [&res, &result_l](oslo::error err_read_a, size_t size_read_a) {
