...
#include <oslo/lib/configbase.hpp>
...
class tomlconfig : public oslo::configbase
{
public:
	tomlconfig ();
	tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<oslo::error> const & error_a = nullptr);
	void doc (std::string const & key, std::string const & doc);
	oslo::error & read (boost::filesystem::path const & path_a);
	oslo::error & read (std::istream & stream_overrides, boost::filesystem::path const & path_a);
	oslo::error & read (std::istream & stream_a);
	oslo::error & read (std::istream & stream_first_a, std::istream & stream_second_a);
	...
	tomlconfig & put_child (std::string const & key_a, oslo::tomlconfig & conf_a);
	tomlconfig & replace_child (std::string const & key_a, oslo::tomlconfig & conf_a);
	...
	tomlconfig & array_entries_required (std::string const & key, std::function<void(T)> callback)
	{
		...
		{
			conditionally_set_error<T> (oslo::error_config::missing_value, false, key);
		}
		return *this;
	}
	 ...
	 * @return May return oslo::error_config::invalid_value
	 */
	template <typename T>
	...
	/**
	 * Get required value.
	 * @note May set oslo::error_config::missing_value if \p key is missing, oslo::error_config::invalid_value if value is invalid.
	 */
	template <typename T>
...
protected:
	template <typename T, typename = std::enable_if_t<oslo::is_lexical_castable<T>::value>>
	tomlconfig & get_config (bool optional, std::string const & key, T & target, T default_value = T ())
	{
		try
		{
			if (tree->contains_qualified (key))
			{
				...
				{
					conditionally_set_error<T> (oslo::error_config::invalid_value, optional, key);
				}
			}
			else if (!optional)
			{
				conditionally_set_error<T> (oslo::error_config::missing_value, optional, key);
			}
			else
			{
