...
class jsonconfig : public oslo::configbase
{
public:
	jsonconfig ();
	jsonconfig (boost::property_tree::ptree & tree_a, std::shared_ptr<oslo::error> error_a = nullptr);
	 ...
	 */
	template <typename T>
	oslo::error & read_and_update (T & object, boost::filesystem::path const & path_a)
	{
	...
	jsonconfig & put_child (std::string const & key_a, oslo::jsonconfig & conf_a);
	jsonconfig & replace_child (std::string const & key_a, oslo::jsonconfig & conf_a);
