...
inline oslo::build_flags operator| (oslo::build_flags a, oslo::build_flags b)
{
	return static_cast<oslo::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, oslo::build_flags b)
{
...
public:
	...
	abstract_builder & sign (oslo::raw_key const & private_key, oslo::public_key const & public_key);
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (oslo::build_flags::work_present | oslo::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<oslo::state_block, state_block_builder>
{
public:
	/** Creates a state block builder by calling make_block() */
	state_block_builder ();
	/** Initialize from an existing block */
	state_block_builder & from (oslo::state_block const & block);
	...
	/** Set link */
	state_block_builder & link (oslo::link const & link);
	/** Set link from hex representation */
	state_block_builder & link_hex (std::string const & link_hex);
	/** Set link from an xrb_ or oslo_ address */
	state_block_builder & link_address (std::string const & link_address);
	/** Provides validation for build() */
	void validate ();

private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (oslo::build_flags::account_present | oslo::build_flags::balance_present | oslo::build_flags::link_present | oslo::build_flags::previous_present | oslo::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<oslo::open_block, open_block_builder>
{
public:
	...
	open_block_builder & source (oslo::block_hash source);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (oslo::build_flags::account_present | oslo::build_flags::representative_present | oslo::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<oslo::change_block, change_block_builder>
{
public:
	...
	/** Set representative */
	change_block_builder & representative (oslo::account account);
	/** Set representative from hex representation of public key */
	...
	/** Set previous block hash */
	change_block_builder & previous (oslo::block_hash previous);
	/** Set previous block hash from hex representation */
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (oslo::build_flags::previous_present | oslo::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<oslo::send_block, send_block_builder>
{
public:
...
class receive_block_builder : public abstract_builder<oslo::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (oslo::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (oslo::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	oslo::state_block_builder & state ()
	{
	...
	oslo::change_block_builder & change ()
	{
	...
	oslo::receive_block_builder & receive ()
	{
...
private:
	oslo::state_block_builder state_builder;
	oslo::open_block_builder open_builder;
	oslo::change_block_builder change_builder;
	oslo::send_block_builder send_builder;
	oslo::receive_block_builder receive_builder;
};
}
