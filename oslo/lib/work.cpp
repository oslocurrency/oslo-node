...
std::string oslo::to_string (oslo::work_version const version_a)
{
	...
	switch (version_a)
	{
		case oslo::work_version::work_1:
			result = "work_1";
			break;
		case oslo::work_version::unspecified:
			result = "unspecified";
			break;
	}
	return result;
...
bool oslo::work_validate_entry (oslo::block const & block_a)
{
	return block_a.difficulty () < oslo::work_threshold_entry (block_a.work_version ());
}

bool oslo::work_validate_entry (oslo::work_version const version_a, oslo::root const & root_a, uint64_t const work_a)
{
	return oslo::work_difficulty (version_a, root_a, work_a) < oslo::work_threshold_entry (version_a);
}

uint64_t oslo::work_difficulty (oslo::work_version const version_a, oslo::root const & root_a, uint64_t const work_a)
{
	uint64_t result{ 0 };
	switch (version_a)
	{
		case oslo::work_version::work_1:
			result = oslo::work_v1::value (root_a, work_a);
			break;
		default:
...
uint64_t oslo::work_threshold_base (oslo::work_version const version_a)
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case oslo::work_version::work_1:
			result = oslo::work_v1::threshold_base ();
			break;
		default:
...
uint64_t oslo::work_threshold_entry (oslo::work_version const version_a)
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case oslo::work_version::work_1:
			result = oslo::work_v1::threshold_entry ();
			break;
		default:
...
uint64_t oslo::work_threshold (oslo::work_version const version_a, oslo::block_details const details_a)
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case oslo::work_version::work_1:
			result = oslo::work_v1::threshold (details_a);
			break;
		default:
...
uint64_t oslo::work_v1::threshold_base ()
{
	static oslo::network_constants network_constants;
	return network_constants.publish_thresholds.base;
}

uint64_t oslo::work_v1::threshold_entry ()
{
	static oslo::network_constants network_constants;
	return network_constants.publish_thresholds.entry;
}

uint64_t oslo::work_v1::threshold (oslo::block_details const details_a)
{
	static_assert (oslo::epoch::max == oslo::epoch::epoch_2, "work_v1::threshold is ill-defined");
	static oslo::network_constants network_constants;
...
uint64_t oslo::work_v1::value (oslo::root const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
double oslo::normalized_multiplier (double const multiplier_a, uint64_t const threshold_a)
{
	static oslo::network_constants network_constants;
	...
	*/
	...
	{
		auto ratio (oslo::difficulty::to_multiplier (network_constants.publish_thresholds.epoch_2, threshold_a));
...
double oslo::denormalized_multiplier (double const multiplier_a, uint64_t const threshold_a)
{
	static oslo::network_constants network_constants;
	debug_assert (multiplier_a >= 1);
	auto multiplier (multiplier_a);
...
oslo::work_pool::work_pool (unsigned max_threads_a, std::chrono::nanoseconds pow_rate_limiter_a, std::function<boost::optional<uint64_t> (oslo::work_version const, oslo::root const &, uint64_t, std::atomic<int> &)> opencl_a) :
ticket (0),
done (false),
...
opencl (opencl_a)
{
	...
	for (auto i (0u); i < count; ++i)
	{
		threads.emplace_back (attrs, [this, i]() {
			oslo::thread_role::set (oslo::thread_role::name::work);
			oslo::work_thread_reprioritize ();
			loop (i);
		});
	}
}

oslo::work_pool::~work_pool ()
{
	stop ();
	for (auto & i : threads)
	{
...
void oslo::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	oslo::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
	...
	while (!done)
	{
		...
		if (!empty)
		{
			...
			{
				work = *opt_work;
				output = oslo::work_v1::value (current_l.item, work);
			}
			else
			{
			...
			if (ticket == ticket_l)
			{
				...
				debug_assert (current_l.difficulty == 0 || oslo::work_v1::value (current_l.item, work) == output);
...
void oslo::work_pool::cancel (oslo::root const & root_a)
{
...
void oslo::work_pool::generate (oslo::work_version const version_a, oslo::root const & root_a, uint64_t difficulty_a, std::function<void(boost::optional<uint64_t> const &)> callback_a)
{
...
boost::optional<uint64_t> oslo::work_pool::generate (oslo::root const & root_a)
{
	static oslo::network_constants network_constants;
	debug_assert (network_constants.is_test_network ());
	return generate (oslo::work_version::work_1, root_a, network_constants.publish_thresholds.base);
}

boost::optional<uint64_t> oslo::work_pool::generate (oslo::root const & root_a, uint64_t difficulty_a)
{
	static oslo::network_constants network_constants;
	debug_assert (network_constants.is_test_network ());
	return generate (oslo::work_version::work_1, root_a, difficulty_a);
}

boost::optional<uint64_t> oslo::work_pool::generate (oslo::work_version const version_a, oslo::root const & root_a, uint64_t difficulty_a)
{
...
size_t oslo::work_pool::size ()
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return pending.size ();
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (work_pool & work_pool, const std::string & name)
{
	size_t count;
	{
		oslo::lock_guard<std::mutex> guard (work_pool.mutex);
