...
oslo::rate::token_bucket::token_bucket (size_t max_token_count_a, size_t refill_rate_a)
{
...
bool oslo::rate::token_bucket::try_consume (unsigned tokens_required_a)
{
	debug_assert (tokens_required_a <= 1e9);
	oslo::lock_guard<std::mutex> lk (bucket_mutex);
	refill ();
...
void oslo::rate::token_bucket::refill ()
{
...
size_t oslo::rate::token_bucket::largest_burst () const
{
	oslo::lock_guard<std::mutex> lk (bucket_mutex);
