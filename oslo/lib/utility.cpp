...
oslo::container_info_composite::container_info_composite (const std::string & name) :
name (name)
{
}

bool oslo::container_info_composite::is_composite () const
{
	return true;
}

void oslo::container_info_composite::add_component (std::unique_ptr<container_info_component> child)
{
	children.push_back (std::move (child));
}

const std::vector<std::unique_ptr<oslo::container_info_component>> & oslo::container_info_composite::get_children () const
{
	return children;
}

const std::string & oslo::container_info_composite::get_name () const
{
	return name;
}

oslo::container_info_leaf::container_info_leaf (const container_info & info) :
info (info)
{
}

bool oslo::container_info_leaf::is_composite () const
{
	return false;
}

const oslo::container_info & oslo::container_info_leaf::get_info () const
{
	return info;
}

void oslo::dump_crash_stacktrace ()
{
	boost::stacktrace::safe_dump_to ("oslo_node_backtrace.dump");
}

std::string oslo::generate_stacktrace ()
{
...
void oslo::remove_all_files_in_dir (boost::filesystem::path const & dir)
{
...
void oslo::move_all_files_to_dir (boost::filesystem::path const & from, boost::filesystem::path const & to)
{
...
void assert_internal (const char * check_expr, const char * func, const char * file, unsigned int line, bool is_release_assert)
{
	...
	auto backtrace_str = oslo::generate_stacktrace ();
...
#ifdef _WIN32
	{
		...
		std::string filename = is_release_assert ? "oslo_node_backtrace_release_assert.txt" : "oslo_node_backtrace_assert.txt";
		...
		oslo::set_secure_perm_file (filepath);
		file << backtrace_str;
	}
#endif
