...
namespace ipc
{
	...
	public:
		...
		oslo::error connect (std::string const & path);

		/** Connect to a tcp socket synchronously */
		oslo::error connect (std::string const & host, uint16_t port);

		/** Connect to a tcp socket asynchronously */
		void async_connect (std::string const & host, uint16_t port, std::function<void(oslo::error)> callback);

		/** Write buffer asynchronously */
		void async_write (oslo::shared_const_buffer const & buffer_a, std::function<void(oslo::error, size_t)> callback_a);

		/** Read \p size_a bytes asynchronously */
		void async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, size_t size_a, std::function<void(oslo::error, size_t)> callback_a);
		 ...
		 */
		void async_read_message (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, std::chrono::seconds timeout_a, std::function<void(oslo::error, size_t)> callback_a);
	...
	std::string request (oslo::ipc::payload_encoding encoding_a, oslo::ipc::ipc_client & ipc_client, std::string const & rpc_action_a);

	/**
	 * Returns a buffer with an IPC preamble for the given \p encoding_a
	 */
	std::vector<uint8_t> get_preamble (oslo::ipc::payload_encoding encoding_a);

	/**
	 * Returns a buffer with an IPC preamble, followed by 32-bit BE lenght, followed by payload
	 */
	oslo::shared_const_buffer prepare_flatbuffers_request (std::shared_ptr<flatbuffers::FlatBufferBuilder> const & flatbuffer_a);

	template <typename T>
	oslo::shared_const_buffer shared_buffer_from (T & object_a, std::string const & correlation_id_a = {}, std::string const & credentials_a = {})
	{
		auto buffer_l (oslo::ipc::flatbuffer_producer::make_buffer (object_a, correlation_id_a, credentials_a));
		return oslo::ipc::prepare_flatbuffers_request (buffer_l);
	}
	 ...
	 */
	oslo::shared_const_buffer prepare_request (oslo::ipc::payload_encoding encoding_a, std::string const & payload_a);
}
}
