...
oslo::error oslo::lmdb_config::serialize_toml (oslo::tomlconfig & toml) const
{
	std::string sync_string;
	switch (sync)
	{
		case oslo::lmdb_config::sync_strategy::always:
			sync_string = "always";
			break;
		case oslo::lmdb_config::sync_strategy::nosync_safe:
			sync_string = "nosync_safe";
			break;
		case oslo::lmdb_config::sync_strategy::nosync_unsafe:
			sync_string = "nosync_unsafe";
			break;
		case oslo::lmdb_config::sync_strategy::nosync_unsafe_large_memory:
...
oslo::error oslo::lmdb_config::deserialize_toml (oslo::tomlconfig & toml, bool is_deprecated_lmdb_dbs_used)
{
	...
	if (!toml.get_error ())
	{
		...
		{
			sync = oslo::lmdb_config::sync_strategy::always;
		}
		else if (sync_string == "nosync_safe")
		{
			sync = oslo::lmdb_config::sync_strategy::nosync_safe;
		}
		else if (sync_string == "nosync_unsafe")
		{
			sync = oslo::lmdb_config::sync_strategy::nosync_unsafe;
		}
		else if (sync_string == "nosync_unsafe_large_memory")
		{
			sync = oslo::lmdb_config::sync_strategy::nosync_unsafe_large_memory;
		}
		else
		{
