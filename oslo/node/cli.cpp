...
namespace
{
void reset_confirmation_heights (oslo::block_store & store);
bool is_using_rocksdb (boost::filesystem::path const & data_path, std::error_code & ec);
}

std::string oslo::error_cli_messages::message (int ev) const
{
	switch (static_cast<oslo::error_cli> (ev))
	{
		case oslo::error_cli::generic:
			return "Unknown error";
		case oslo::error_cli::parse_error:
			return "Coud not parse command line";
		case oslo::error_cli::invalid_arguments:
			return "Invalid arguments";
		case oslo::error_cli::unknown_command:
			return "Unknown command";
		case oslo::error_cli::database_write_error:
			return "Database write error";
		case oslo::error_cli::reading_config:
			return "Config file read error";
		case oslo::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case oslo::error_cli::ambiguous_udp_options:
...
void oslo::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void oslo::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code oslo::update_flags (oslo::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = oslo::error_cli::database_write_error;
}
...
std::error_code oslo::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : oslo::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = oslo::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						oslo::remove_all_files_in_dir (backup_path);
						oslo::move_all_files_to_dir (source_path, backup_path);
						oslo::move_all_files_to_dir (vacuum_path, source_path);
	...
	else if (vm.count ("generate_config"))
	{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Oslo. Visit https://docs.oslo.org/running-a-node/configuration/ for more information.\n#\n"
			          ...
			          << "# Additional information for notable configuration options is available in https://docs.oslo.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		oslo::raw_key key;
		key.data.clear ();
		oslo::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		oslo::raw_key junk1;
		junk1.data.clear ();
		oslo::uint256_union junk2 (0);
		oslo::kdf kdf;
		kdf.phs (junk1, "", junk2);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = oslo::default_inactive_node (data_path, vm);
			auto wallet_key = oslo::random_wallet_id ();
				...
				if (existing != inactive_node->node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (oslo::pub_key (key.as_private_key ()) != account)
							{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									oslo::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << oslo::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = oslo::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<oslo::inactive_node> oslo::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = oslo::inactive_node_flag_defaults ();
	oslo::update_flags (node_flags, vm_a);
	return std::make_unique<oslo::inactive_node> (path_a, node_flags);
}
...
{
	oslo::daemon_config config (data_path);
	auto error = oslo::read_node_config_toml (data_path, config);
	if (!error)
	{
		...
		if (use_rocksdb)
		{
			...
			ec = oslo::error_cli::database_write_error;
#endif
			return (OSLO_ROCKSDB == 1);
		}
	}
	else
	{
		ec = oslo::error_cli::reading_config;
	}
