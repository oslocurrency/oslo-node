...
oslo::telemetry::telemetry (oslo::network & network_a, oslo::alarm & alarm_a, oslo::worker & worker_a, oslo::observer_set<oslo::telemetry_data const &, oslo::endpoint const &> & observers_a, oslo::stat & stats_a, oslo::network_params & network_params_a, bool disable_ongoing_requests_a) :
network (network_a),
alarm (alarm_a),
worker (worker_a),
...
void oslo::telemetry::stop ()
{
	stopped = true;
}

void oslo::telemetry::set (oslo::telemetry_ack const & message_a, oslo::transport::channel const & channel_a)
{
	if (!stopped)
	{
		oslo::unique_lock<std::mutex> lk (mutex);
		oslo::endpoint endpoint = channel_a.get_endpoint ();
		...
		if (it == recent_or_initial_request_telemetry_data.cend () || !it->undergoing_request)
		{
			...
			stats.inc (oslo::stat::type::telemetry, oslo::stat::detail::unsolicited_telemetry_ack);
			return;
		}

		recent_or_initial_request_telemetry_data.modify (it, [&message_a](oslo::telemetry_info & telemetry_info_a) {
...
bool oslo::telemetry::verify_message (oslo::telemetry_ack const & message_a, oslo::transport::channel const & channel_a)
{
	...
	if (!node_id_mismatch)
	{
		...
		if (!remove_channel)
		{
			...
			if (remove_channel)
			{
				stats.inc (oslo::stat::type::telemetry, oslo::stat::detail::different_genesis_hash);
			}
		}
		else
		{
			stats.inc (oslo::stat::type::telemetry, oslo::stat::detail::invalid_signature);
		}
	}
	else
	{
		stats.inc (oslo::stat::type::telemetry, oslo::stat::detail::node_id_mismatch);
	}
...
std::chrono::milliseconds oslo::telemetry::cache_plus_buffer_cutoff_time () const
{
...
bool oslo::telemetry::within_cache_plus_buffer_cutoff (telemetry_info const & telemetry_info) const
{
...
bool oslo::telemetry::within_cache_cutoff (telemetry_info const & telemetry_info) const
{
	auto is_within = (telemetry_info.last_response + cache_cutoff) >= std::chrono::steady_clock::now ();
	return !telemetry_info.awaiting_first_response () && is_within;
}

void oslo::telemetry::ongoing_req_all_peers (std::chrono::milliseconds next_request_interval)
{
		...
		if (auto this_l = this_w.lock ())
		{
			...
			if (!this_l->stopped)
			{
				...
				struct channel_wrapper
				{
					...
					channel_wrapper (std::shared_ptr<oslo::transport::channel> const & channel_a) :
					channel (channel_a)
					{
					}
				...
				mi::indexed_by<
					mi::hashed_unique<mi::tag<tag_endpoint>,
						mi::const_mem_fun<channel_wrapper, oslo::endpoint, &channel_wrapper::endpoint>>,
					mi::hashed_unique<mi::tag<tag_channel>,
						mi::member<channel_wrapper, std::shared_ptr<oslo::transport::channel>, &channel_wrapper::channel>>>> peers;
				// clang-format on
...

				{
					...
					oslo::lock_guard<std::mutex> guard (this_l->mutex);
...
std::unordered_map<oslo::endpoint, oslo::telemetry_data> oslo::telemetry::get_metrics ()
{
	std::unordered_map<oslo::endpoint, oslo::telemetry_data> telemetry_data;

	oslo::lock_guard<std::mutex> guard (mutex);
	auto range = boost::make_iterator_range (recent_or_initial_request_telemetry_data);
	// clang-format off
	oslo::transform_if (range.begin (), range.end (), std::inserter (telemetry_data, telemetry_data.end ()),
		[this](auto const & telemetry_info) { return this->within_cache_plus_buffer_cutoff (telemetry_info); },
		[](auto const & telemetry_info) { return std::pair<const oslo::endpoint, oslo::telemetry_data>{ telemetry_info.endpoint, telemetry_info.data }; });
	// clang-format on

	return telemetry_data;
}

void oslo::telemetry::get_metrics_single_peer_async (std::shared_ptr<oslo::transport::channel> const & channel_a, std::function<void(telemetry_data_response const &)> const & callback_a)
{
	auto invoke_callback_with_error = [&callback_a, &worker = this->worker, channel_a]() {
		...
		worker.push_task ([callback_a, endpoint]() {
			...
			callback_a ({ oslo::telemetry_data{}, endpoint, error });
		});
	};
	...
	if (!stopped)
	{
		...
		{
			auto add_callback_async = [& worker = this->worker, &callback_a](telemetry_data const & telemetry_data_a, oslo::endpoint const & endpoint_a) {
			...
			else
			{
				...
				else
				{
					if (it == recent_or_initial_request_telemetry_data.cend ())
					{
						...
						recent_or_initial_request_telemetry_data.emplace (channel_a->get_endpoint (), oslo::telemetry_data (), std::chrono::steady_clock::now (), true);
						it = recent_or_initial_request_telemetry_data.find (channel_a->get_endpoint ());
					}
					else
					{
						recent_or_initial_request_telemetry_data.modify (it, [](oslo::telemetry_info & telemetry_info_a) {
...
oslo::telemetry_data_response oslo::telemetry::get_metrics_single_peer (std::shared_ptr<oslo::transport::channel> const & channel_a)
{
...
void oslo::telemetry::fire_request_message (std::shared_ptr<oslo::transport::channel> const & channel_a)
{
	...
	std::weak_ptr<oslo::telemetry> this_w (shared_from_this ());
	...
	channel_a->send (message, [this_w, endpoint = channel_a->get_endpoint (), round_l](boost::system::error_code const & ec, size_t size_a) {
		if (auto this_l = this_w.lock ())
		{
			if (ec)
			{
				...
				this_l->stats.inc (oslo::stat::type::telemetry, oslo::stat::detail::failed_send_telemetry_req);
				oslo::lock_guard<std::mutex> guard (this_l->mutex);
			...
			else
			{
				...
				this_l->alarm.add (std::chrono::steady_clock::now () + this_l->response_time_cutoff, [round_l, this_w, endpoint]() {
					if (auto this_l = this_w.lock ())
					{
						...
						{
							this_l->stats.inc (oslo::stat::type::telemetry, oslo::stat::detail::no_response_received);
							this_l->channel_processed (endpoint, true);
						}
					}
				});			
	...
	oslo::buffer_drop_policy::no_socket_drop);
	// clang-format on
}

void oslo::telemetry::channel_processed (oslo::endpoint const & endpoint_a, bool error_a)
{
...
void oslo::telemetry::flush_callbacks_async (oslo::endpoint const & endpoint_a, bool error_a)
{
	// Post to worker so that it's truly async and not on the calling thread (same problem as std::async otherwise)
	worker.push_task ([endpoint_a, error_a, this_w = std::weak_ptr<oslo::telemetry> (shared_from_this ())]() {
		if (auto this_l = this_w.lock ())
		{
			oslo::unique_lock<std::mutex> lk (this_l->mutex);
...
void oslo::telemetry::invoke_callbacks (oslo::endpoint const & endpoint_a, bool error_a)
{
	std::vector<std::function<void(telemetry_data_response const &)>> callbacks_l;
	telemetry_data_response response_data{ oslo::telemetry_data (), endpoint_a, error_a };
	{
...
size_t oslo::telemetry::telemetry_data_size ()
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return recent_or_initial_request_telemetry_data.size ();
}

oslo::telemetry_info::telemetry_info (oslo::endpoint const & endpoint_a, oslo::telemetry_data const & data_a, std::chrono::steady_clock::time_point last_response_a, bool undergoing_request_a) :
endpoint (endpoint_a),
data (data_a),
...
bool oslo::telemetry_info::awaiting_first_response () const
{
	return data == oslo::telemetry_data ();
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (telemetry & telemetry, const std::string & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
	size_t callbacks_count;
	{
		oslo::lock_guard<std::mutex> guard (telemetry.mutex);
...
oslo::telemetry_data oslo::consolidate_telemetry_data (std::vector<oslo::telemetry_data> const & telemetry_datas)
{
	...
	std::unordered_map<oslo::block_hash, int> genesis_blocks;
	...
	auto strip_outliers_and_sum = [num_either_side_to_remove](auto & counts) {
		...
		return std::accumulate (counts.begin (), counts.end (), oslo::uint128_t (0), [](oslo::uint128_t total, auto count) {
			return total += count;
		});
	};
	...
	oslo::telemetry_data consolidated_data;
...
oslo::telemetry_data oslo::local_telemetry_data (oslo::ledger_cache const & ledger_cache_a, oslo::network & network_a, uint64_t bandwidth_limit_a, oslo::network_params const & network_params_a, std::chrono::steady_clock::time_point statup_time_a, uint64_t active_difficulty_a, oslo::keypair const & node_id_a)
{
	...
	telemetry_data.peer_count = oslo::narrow_cast<decltype (telemetry_data.peer_count)> (network_a.size ());
	telemetry_data.account_count = ledger_cache_a.account_count;
	telemetry_data.major_version = oslo::get_major_node_version ();
	telemetry_data.minor_version = oslo::get_minor_node_version ();
	telemetry_data.patch_version = oslo::get_patch_node_version ();
	telemetry_data.pre_release_version = oslo::get_pre_release_node_version ();
