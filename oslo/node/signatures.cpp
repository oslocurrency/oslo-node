...
oslo::signature_checker::signature_checker (unsigned num_threads) :
...
void oslo::signature_checker::verify (oslo::signature_check_set & check_a)
{
...
void oslo::signature_checker::stop ()
{
...
void oslo::signature_checker::flush ()
{
	while (!stopped && tasks_remaining != 0)
		;
}

bool oslo::signature_checker::verify_batch (const oslo::signature_check_set & check_a, size_t start_index, size_t size)
{
	oslo::validate_message_batch (check_a.messages + start_index, check_a.message_lengths + start_index, check_a.pub_keys + start_index, check_a.signatures + start_index, size, check_a.verifications + start_index);
 ...
 */
void oslo::signature_checker::verify_async (oslo::signature_check_set & check_a, size_t num_batches, std::promise<void> & promise)
{
...
void oslo::signature_checker::set_thread_names (unsigned num_threads)
{
	...
	for (auto i = 0u; i < num_threads; ++i)
	{
		boost::asio::post (thread_pool, [& promise = promises[i]]() {
			oslo::thread_role::set (oslo::thread_role::name::signature_checking);
			promise.set_value ();
		});
	}
