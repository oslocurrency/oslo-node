...
class socket : public std::enable_shared_from_this<oslo::socket>
{
...
public:
	 ...
	 */
	explicit socket (std::shared_ptr<oslo::node> node, boost::optional<std::chrono::seconds> io_timeout = boost::none, concurrency = concurrency::single_writer);
	virtual ~socket ();
	...
	void async_write (oslo::shared_const_buffer const &, std::function<void(boost::system::error_code const &, size_t)> = nullptr, oslo::buffer_drop_policy = oslo::buffer_drop_policy::limiter);
...
public:
	 ...
	 */
	explicit server_socket (std::shared_ptr<oslo::node> node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a, concurrency concurrency_a = concurrency::single_writer);
	...
	void on_connection (std::function<bool(std::shared_ptr<oslo::socket> new_connection, boost::system::error_code const &)>);
...
private:
	std::vector<std::weak_ptr<oslo::socket>> connections;
