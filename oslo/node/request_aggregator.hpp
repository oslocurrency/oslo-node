...
class request_aggregator final
{
	...
	{
		channel_pool () = delete;
		explicit channel_pool (std::shared_ptr<oslo::transport::channel> & channel_a) :
		channel (channel_a),
		endpoint (oslo::transport::map_endpoint_to_v6 (channel_a->get_endpoint ()))
		{
		}
		std::vector<std::pair<oslo::block_hash, oslo::root>> hashes_roots;
...
public:
	...
	request_aggregator (oslo::network_constants const &, oslo::node_config const & config, oslo::stat & stats_a, oslo::votes_cache &, oslo::ledger &, oslo::wallets &, oslo::active_transactions &);

	/** Add a new request by \p channel_a for hashes \p hashes_roots_a */
	void add (std::shared_ptr<oslo::transport::channel> & channel_a, std::vector<std::pair<oslo::block_hash, oslo::root>> const & hashes_roots_a);
	void stop ();
...
private:
	...
	void erase_duplicates (std::vector<std::pair<oslo::block_hash, oslo::root>> &) const;
	/** Aggregate \p requests_a and send cached votes to \p channel_a . Return the remaining hashes that need vote generation **/
	std::vector<oslo::block_hash> aggregate (oslo::transaction const &, std::vector<std::pair<oslo::block_hash, oslo::root>> const & requests_a, std::shared_ptr<oslo::transport::channel> & channel_a) const;
	/** Generate votes from \p hashes_a and send to \p channel_a **/
	void generate (oslo::transaction const &, std::vector<oslo::block_hash> const & hashes_a, std::shared_ptr<oslo::transport::channel> & channel_a) const;
	...
	mi::indexed_by<
		mi::hashed_unique<mi::tag<tag_endpoint>,
			mi::member<channel_pool, oslo::endpoint, &channel_pool::endpoint>>,
