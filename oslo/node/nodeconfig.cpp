...
node_config (0, oslo::logging ())
{
}

oslo::node_config::node_config (uint16_t peering_port_a, oslo::logging const & logging_a) :
...
external_address (boost::asio::ip::address_v6{}.to_string ())
{
	...
	switch (network_params.network.network ())
	{
		...
		case oslo::oslo_networks::oslo_live_network:
			...
			preconfigured_representatives.emplace_back ("658C588F0EF8884530A3973E70AED17C9C6C76DEEC9C4D18571A875715D36460");
			break;
		default:
			debug_assert (false);
			break;
...
oslo::error oslo::node_config::serialize_toml (oslo::tomlconfig & toml) const
{
	...
	oslo::tomlconfig experimental_l;
...
oslo::error oslo::node_config::deserialize_toml (oslo::tomlconfig & toml)
{
	try
	{
		...
		oslo::network_constants network;
		...
		if (frontiers_confirmation == oslo::frontiers_confirmation_mode::invalid)
		{
...
oslo::error oslo::node_config::serialize_json (oslo::jsonconfig & json) const
{
...
bool oslo::node_config::upgrade_json (unsigned version_a, oslo::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
		...
		case 2:
		{
			json.put ("inactive_supply", oslo::uint128_union (0).to_string_dec ());
		...
		case 3:
			...
			json.put ("receive_minimum", oslo::xrb_ratio.convert_to<std::string> ());
		case 4:
			json.erase ("receive_minimum");
			json.put ("receive_minimum", oslo::xrb_ratio.convert_to<std::string> ());
		case 5:
		...
		case 9:
			json.put ("state_block_parse_canary", oslo::block_hash (0).to_string ());
			json.put ("state_block_generate_canary", oslo::block_hash (0).to_string ());
		case 10:
			json.put ("online_weight_minimum", online_weight_minimum.to_string_dec ());
		...
		case 15:
		{
			...
			// Update to the new preconfigured_peers url for rebrand if it is found (rai -> oslo)
...
oslo::error oslo::node_config::deserialize_json (bool & upgraded_a, oslo::jsonconfig & json)
{
	try
	{
...
std::string oslo::node_config::serialize_frontiers_confirmation (oslo::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case oslo::frontiers_confirmation_mode::always:
			return "always";
		case oslo::frontiers_confirmation_mode::automatic:
			return "auto";
		case oslo::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
oslo::frontiers_confirmation_mode oslo::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return oslo::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return oslo::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return oslo::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return oslo::frontiers_confirmation_mode::invalid;
	}
}

void oslo::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
oslo::account oslo::node_config::random_representative () const
{
	debug_assert (!preconfigured_representatives.empty ());
	size_t index (oslo::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
