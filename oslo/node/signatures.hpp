...
#include <oslo/boost/asio/thread_pool.hpp>
...
private:
	struct Task final
	{
		Task (oslo::signature_check_set & check, size_t pending) :
		...
		oslo::signature_check_set & check;
		std::atomic<size_t> pending;
	};

	bool verify_batch (const oslo::signature_check_set & check_a, size_t index, size_t size);
	void verify_async (oslo::signature_check_set & check_a, size_t num_batches, std::promise<void> & promise);
