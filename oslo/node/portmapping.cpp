...
std::string oslo::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

void oslo::port_mapping::refresh_devices ()
{
...
oslo::endpoint oslo::port_mapping::external_address ()
{
	oslo::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	oslo::lock_guard<std::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = oslo::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void oslo::port_mapping::refresh_mapping ()
{
	if (!network_params.network.is_test_network ())
	{
		oslo::lock_guard<std::mutex> guard_l (mutex);
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
		{
			auto upnp_description = std::string ("Oslo Node (") + network_params.network.get_current_network_as_string () + ")";
...
int oslo::port_mapping::check_mapping ()
{
	int result_l (3600);
	if (!network_params.network.is_test_network ())
	{
...
void oslo::port_mapping::check_mapping_loop ()
{
...
oslo::upnp_state & oslo::upnp_state::operator= (oslo::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
