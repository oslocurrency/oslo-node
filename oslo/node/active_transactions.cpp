...
oslo::active_transactions::active_transactions (oslo::node & node_a, oslo::confirmation_height_processor & confirmation_height_processor_a) :
...
})
{
	...
	confirmation_height_processor.add_cemented_observer ([this](std::shared_ptr<oslo::block> callback_block_a) {
		this->block_cemented_callback (callback_block_a);
	});

	// Register a callback which will get called if a block is already cemented
	confirmation_height_processor.add_block_already_cemented_observer ([this](oslo::block_hash const & hash_a) {
...
oslo::active_transactions::~active_transactions ()
{
	stop ();
}

void oslo::active_transactions::confirm_prioritized_frontiers (oslo::transaction const & transaction_a)
{
	...
	bool agressive_mode (half_princpal_representative || node.config.frontiers_confirmation == oslo::frontiers_confirmation_mode::always);
...
void oslo::active_transactions::block_cemented_callback (std::shared_ptr<oslo::block> const & block_a)
{
	auto transaction = node.store.tx_begin_read ();

	boost::optional<oslo::election_status_type> election_status_type;
	...
	else
	{
		...
		election_status_type = oslo::election_status_type::active_confirmed_quorum;
	}

	if (election_status_type.is_initialized ())
	{
		if (election_status_type == oslo::election_status_type::inactive_confirmation_height)
		{
			oslo::account account (0);
			oslo::uint128_t amount (0);
			bool is_state_send (false);
			oslo::account pending_account (0);
			node.process_confirmed_data (transaction, block_a, block_a->hash (), account, amount, is_state_send, pending_account);
			node.observers.blocks.notify (oslo::election_status{ block_a, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, oslo::election_status_type::inactive_confirmation_height }, account, amount, is_state_send);
		}
		else
		{
			auto hash (block_a->hash ());
			oslo::unique_lock<std::mutex> election_winners_lk (election_winner_details_mutex);
...
void oslo::active_transactions::add_election_winner_details (oslo::block_hash const & hash_a, std::shared_ptr<oslo::election> const & election_a)
{
	oslo::lock_guard<std::mutex> guard (election_winner_details_mutex);
	election_winner_details.emplace (hash_a, election_a);
}

void oslo::active_transactions::remove_election_winner_details (oslo::block_hash const & hash_a)
{
	oslo::lock_guard<std::mutex> guard (election_winner_details_mutex);
	election_winner_details.erase (hash_a);
}

void oslo::active_transactions::block_already_cemented_callback (oslo::block_hash const & hash_a)
{
...
void oslo::active_transactions::request_confirm (oslo::unique_lock<std::mutex> & lock_a)
{
	debug_assert (!mutex.try_lock ());

	// Only representatives ready to receive batched confirm_req
	oslo::confirmation_solicitor solicitor (node.network, node.network_params.network);
	solicitor.prepare (node.rep_crawler.principal_representatives (std::numeric_limits<size_t>::max ()));

	oslo::vote_generator_session generator_session (generator);
	...
	oslo::timer<std::chrono::milliseconds> elapsed (oslo::timer_state::started);
...
void oslo::active_transactions::frontiers_confirmation (oslo::unique_lock<std::mutex> & lock_a)
{
	/*
	...
	auto disabled_confirmation_mode = (node.config.frontiers_confirmation == oslo::frontiers_confirmation_mode::disabled);
...
void oslo::active_transactions::activate_dependencies (oslo::unique_lock<std::mutex> & lock_a)
{
	...
	auto first_unconfirmed = [this](oslo::transaction const & transaction_a, oslo::account const & account_a, oslo::block_hash const & confirmed_frontier_a) {
		...
		{
			oslo::account_info account_info_l;
	...
	std::vector<std::pair<std::shared_ptr<oslo::block>, oslo::block_hash>> activate_l;
	{
		auto transaction = node.store.tx_begin_read ();
		for (auto const & entry_l : pending_l)
		{
			...
			if (block_l)
			{
				...
				if (!previous_hash_l.is_zero ())
				{
					...
					oslo::confirmation_height_info conf_info_l;
...
void oslo::active_transactions::request_loop ()
{
	oslo::unique_lock<std::mutex> lock (mutex);
	started = true;
	lock.unlock ();
...
void oslo::active_transactions::prioritize_account_for_confirmation (oslo::active_transactions::prioritize_num_uncemented & cementable_frontiers_a, size_t & cementable_frontiers_size_a, oslo::account const & account_a, oslo::account_info const & info_a, uint64_t confirmation_height)
{
	if (info_a.block_count > confirmation_height && !confirmation_height_processor.is_processing_block (info_a.head))
	{
		...
		if (it != cementable_frontiers_a.get<tag_account> ().end ())
		{
			if (it->blocks_uncemented != num_uncemented)
			{
				...
				cementable_frontiers_a.get<tag_account> ().modify (it, [num_uncemented](oslo::cementable_account & info) {
...
void oslo::active_transactions::prioritize_frontiers_for_confirmation (oslo::transaction const & transaction_a, std::chrono::milliseconds ledger_account_traversal_max_time_a, std::chrono::milliseconds wallet_account_traversal_max_time_a)
{
	...
	if (confirmation_height_processor.awaiting_processing_size () < confirmed_frontiers_max_pending_size)
	{
		...
		oslo::timer<std::chrono::milliseconds> wallet_account_timer;
		...
		if (!skip_wallets)
		{
			...
			{
				oslo::lock_guard<std::mutex> lock (node.wallets.mutex);
...
void oslo::active_transactions::stop ()
{
	oslo::unique_lock<std::mutex> lock (mutex);
	if (!started)
	{
...
oslo::election_insertion_result oslo::active_transactions::insert_impl (std::shared_ptr<oslo::block> const & block_a, boost::optional<oslo::uint128_t> const & previous_balance_a, std::function<void(std::shared_ptr<oslo::block>)> const & confirmation_action_a)
{
	debug_assert (block_a->has_sideband ());
	oslo::election_insertion_result result;
	if (!stopped)
	{
		...
		if (existing == roots.get<tag_root> ().end ())
		{
			if (recently_confirmed.get<tag_root> ().find (root) == recently_confirmed.get<tag_root> ().end ())
			{
				...
				oslo::uint128_t previous_balance (previous_balance_a.value_or (0));
				...
				result.election = oslo::make_shared<oslo::election> (node, block_a, confirmation_action_a, prioritized);
				roots.get<tag_root> ().emplace (oslo::active_transactions::conflict_info{ root, multiplier, multiplier, result.election, epoch, previous_balance });
				blocks.emplace (hash, result.election);
				add_adjust_difficulty (hash);
				result.election->insert_inactive_votes_cache (hash);
				node.stats.inc (oslo::stat::type::election, prioritized ? oslo::stat::detail::election_priority : oslo::stat::detail::election_non_priority);
			}
		}
		else
		{
...
oslo::election_insertion_result oslo::active_transactions::insert (std::shared_ptr<oslo::block> const & block_a, boost::optional<oslo::uint128_t> const & previous_balance_a, std::function<void(std::shared_ptr<oslo::block>)> const & confirmation_action_a)
{
...
oslo::vote_code oslo::active_transactions::vote (std::shared_ptr<oslo::vote> vote_a)
{
	...
	bool processed (false);
	{
		...
		{
			oslo::election_vote_result result;
			auto & recently_confirmed_by_hash (recently_confirmed.get<tag_hash> ());
			if (vote_block.which ())
			{
				auto block_hash (boost::get<oslo::block_hash> (vote_block));
	...
	if (at_least_one)
	{
		...
		return replay ? oslo::vote_code::replay : oslo::vote_code::vote;
	}
	else if (recently_confirmed_counter == vote_a->blocks.size ())
	{
		return oslo::vote_code::replay;
	}
	else
	{
		return oslo::vote_code::indeterminate;
	}
}

bool oslo::active_transactions::active (oslo::qualified_root const & root_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return roots.get<tag_root> ().find (root_a) != roots.get<tag_root> ().end ();
}

bool oslo::active_transactions::active (oslo::block const & block_a)
{
	return active (block_a.qualified_root ());
}

std::shared_ptr<oslo::election> oslo::active_transactions::election (oslo::qualified_root const & root_a) const
{
	std::shared_ptr<oslo::election> result;
	oslo::lock_guard<std::mutex> lock (mutex);
...
std::shared_ptr<oslo::block> oslo::active_transactions::winner (oslo::block_hash const & hash_a) const
{
...
oslo::election_insertion_result oslo::active_transactions::activate (oslo::account const & account_a)
{
	oslo::election_insertion_result result;
	auto transaction (node.store.tx_begin_read ());
...
bool oslo::active_transactions::update_difficulty (oslo::block const & block_a)
{
	oslo::lock_guard<std::mutex> guard (mutex);
...
bool oslo::active_transactions::update_difficulty_impl (oslo::active_transactions::roots_iterator const & root_it_a, oslo::block const & block_a)
{
	debug_assert (!mutex.try_lock ());
	...
	if (!error)
	{
		...
		roots.get<tag_root> ().modify (root_it_a, [multiplier](oslo::active_transactions::conflict_info & info_a) {
			info_a.multiplier = multiplier;
		});
		add_adjust_difficulty (block_a.hash ());
		node.stats.inc (oslo::stat::type::election, oslo::stat::detail::election_difficulty_update);
	}
	return error;
}

bool oslo::active_transactions::restart (std::shared_ptr<oslo::block> const & block_a, oslo::write_transaction const & transaction_a)
{
		...
		if (ledger_block != nullptr && ledger_block->block_work () != block_a->block_work () && !node.block_confirmed_or_being_confirmed (transaction_a, hash))
		{
			if (block_a->difficulty () > ledger_block->difficulty ())
			{
				...
				if (insert_result.inserted)
				{
					...
					node.stats.inc (oslo::stat::type::election, oslo::stat::detail::election_restart);
				}
			}
		}
	}
...
double oslo::active_transactions::normalized_multiplier (oslo::block const & block_a, boost::optional<oslo::active_transactions::roots_iterator> const & root_it_a) const
{
	debug_assert (!mutex.try_lock ());
	...
	{
		threshold = oslo::work_threshold (block_a.work_version (), block_a.sideband ().details);
	}
	else if (root_it_a.is_initialized ())
	{
		...
		{
			threshold = oslo::work_threshold (block_a.work_version (), find_block->second->sideband ().details);
		}
		else
		{
			...
			oslo::block_details details (election->epoch, is_send, is_receive, false);

			threshold = oslo::work_threshold (block_a.work_version (), details);
			sideband_not_found = true;
		}
	}
	double multiplier (oslo::difficulty::to_multiplier (difficulty, threshold));
	debug_assert (multiplier >= 1 || sideband_not_found);
	if (multiplier >= 1)
	{
		multiplier = oslo::normalized_multiplier (multiplier, threshold);
	}
	else
	{
...
void oslo::active_transactions::add_adjust_difficulty (oslo::block_hash const & hash_a)
{
	debug_assert (!mutex.try_lock ());
	adjust_difficulty_list.push_back (hash_a);
}

void oslo::active_transactions::update_adjusted_multiplier ()
{
	debug_assert (!mutex.try_lock ());
	std::unordered_set<oslo::block_hash> processed_blocks;
	while (!adjust_difficulty_list.empty ())
	{
		auto const & adjust_difficulty_item (adjust_difficulty_list.front ());
		std::deque<std::pair<oslo::block_hash, int64_t>> remaining_blocks;
		remaining_blocks.emplace_back (adjust_difficulty_item, 0);
		adjust_difficulty_list.pop_front ();
		std::vector<std::pair<oslo::qualified_root, int64_t>> elections_list;
		double sum (0.);
		...
		while (!remaining_blocks.empty ())
		{
			...
			if (processed_blocks.find (hash) == processed_blocks.end ())
			{
				...
				if (existing != blocks.end () && !existing->second->confirmed () && existing->second->status.winner->hash () == hash)
				{
					...
					oslo::qualified_root root (previous, existing->second->status.winner->root ());
		...
		if (!elections_list.empty ())
		{
			...
			for (auto & item : elections_list)
			{
				...
				{
					roots.get<tag_root> ().modify (existing_root, [multiplier_a](oslo::active_transactions::conflict_info & info_a) {
...
void oslo::active_transactions::update_active_multiplier (oslo::unique_lock<std::mutex> & lock_a)
{
	debug_assert (!mutex.try_lock ());
	...
	debug_assert (multiplier >= oslo::difficulty::to_multiplier (node.network_params.network.publish_thresholds.entry, node.network_params.network.publish_thresholds.base));
	...
	auto difficulty = oslo::difficulty::from_multiplier (avg_multiplier, node.default_difficulty (oslo::work_version::work_1));
...
uint64_t oslo::active_transactions::active_difficulty ()
{
	return oslo::difficulty::from_multiplier (active_multiplier (), node.default_difficulty (oslo::work_version::work_1));
}

uint64_t oslo::active_transactions::limited_active_difficulty (oslo::block const & block_a)
{
	uint64_t threshold (0);
	if (block_a.has_sideband ())
	{
...
uint64_t oslo::active_transactions::limited_active_difficulty (oslo::work_version const version_a, uint64_t const threshold_a)
{
	auto difficulty (oslo::difficulty::from_multiplier (oslo::denormalized_multiplier (active_multiplier (), threshold_a), threshold_a));
	return std::min (difficulty, node.max_work_generate_difficulty (version_a));
}

double oslo::active_transactions::active_multiplier ()
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
std::deque<std::shared_ptr<oslo::block>> oslo::active_transactions::list_blocks ()
{
	std::deque<std::shared_ptr<oslo::block>> result;
	oslo::lock_guard<std::mutex> lock (mutex);
	for (auto & root : roots)
	{
...
std::deque<oslo::election_status> oslo::active_transactions::list_recently_cemented ()
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return recently_cemented;
}

void oslo::active_transactions::add_recently_cemented (oslo::election_status const & status_a)
{
...
void oslo::active_transactions::add_recently_confirmed (oslo::qualified_root const & root_a, oslo::block_hash const & hash_a)
{
...
void oslo::active_transactions::erase (oslo::block const & block_a)
{
	oslo::unique_lock<std::mutex> lock (mutex);
...
bool oslo::active_transactions::empty ()
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return roots.empty ();
}

size_t oslo::active_transactions::size ()
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return roots.size ();
}

bool oslo::active_transactions::publish (std::shared_ptr<oslo::block> block_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	...
	if (existing != roots.get<tag_root> ().end ())
	{
		...
		if (!result)
		{
			...
			node.stats.inc (oslo::stat::type::election, oslo::stat::detail::election_block_conflict);
		}
	}
	return result;
}
...
boost::optional<oslo::election_status_type> oslo::active_transactions::confirm_block (oslo::transaction const & transaction_a, std::shared_ptr<oslo::block> block_a)
{
	auto hash (block_a->hash ());
	oslo::unique_lock<std::mutex> lock (mutex);
	auto existing (blocks.find (hash));
	boost::optional<oslo::election_status_type> status_type;
	if (existing != blocks.end ())
	{
		if (existing->second->status.winner && existing->second->status.winner->hash () == hash)
		{
			...
			{
				existing->second->confirm_once (oslo::election_status_type::active_confirmation_height);
				status_type = oslo::election_status_type::active_confirmation_height;
			}
			else
			{
#ifndef NDEBUG
...
#endif
				status_type = oslo::election_status_type::active_confirmed_quorum;
			}
		}
		else
		{
			status_type = boost::optional<oslo::election_status_type>{};
		}
	}
	else
	{
		status_type = oslo::election_status_type::inactive_confirmation_height;
	}

	return status_type;
}

size_t oslo::active_transactions::priority_cementable_frontiers_size ()
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return priority_cementable_frontiers.size ();
}

size_t oslo::active_transactions::priority_wallet_cementable_frontiers_size ()
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return priority_wallet_cementable_frontiers.size ();
}

boost::circular_buffer<double> oslo::active_transactions::difficulty_trend ()
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return multipliers_cb;
}

size_t oslo::active_transactions::inactive_votes_cache_size ()
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return inactive_votes_cache.size ();
}

void oslo::active_transactions::add_inactive_votes_cache (oslo::block_hash const & hash_a, oslo::account const & representative_a)
{
	...
	if (node.ledger.weight (representative_a) > node.minimum_principal_weight ())
	{
		...
		if (existing != inactive_by_hash.end ())
		{
			...
			{
				auto is_new (false);
				inactive_by_hash.modify (existing, [representative_a, &is_new](oslo::inactive_cache_information & info) {
				...
				if (is_new)
				{
					...
					{
						inactive_by_hash.modify (existing, [](oslo::inactive_cache_information & info) {
		...
		{
			std::vector<oslo::account> representative_vector (1, representative_a);
			bool confirmed (false);
			...
			inactive_by_arrival.emplace (oslo::inactive_cache_information{ std::chrono::steady_clock::now (), hash_a, representative_vector, start_bootstrap, confirmed });
...
oslo::inactive_cache_information oslo::active_transactions::find_inactive_votes_cache (oslo::block_hash const & hash_a)
{
	auto & inactive_by_hash (inactive_votes_cache.get<tag_hash> ());
	auto existing (inactive_by_hash.find (hash_a));
	if (existing != inactive_by_hash.end ())
	...
	{
		return oslo::inactive_cache_information{};
	}
}

void oslo::active_transactions::erase_inactive_votes_cache (oslo::block_hash const & hash_a)
{
	inactive_votes_cache.get<tag_hash> ().erase (hash_a);
}

bool oslo::active_transactions::inactive_votes_bootstrap_check (std::vector<oslo::account> const & voters_a, oslo::block_hash const & hash_a, bool & confirmed_a)
{
	uint128_t tally;
...
size_t oslo::active_transactions::election_winner_details_size ()
{
	oslo::lock_guard<std::mutex> guard (election_winner_details_mutex);
	return election_winner_details.size ();
}

oslo::cementable_account::cementable_account (oslo::account const & account_a, size_t blocks_uncemented_a) :
account (account_a), blocks_uncemented (blocks_uncemented_a)
{
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (active_transactions & active_transactions, const std::string & name)
{
	size_t roots_count;
	size_t blocks_count;
	...
	{
		oslo::lock_guard<std::mutex> guard (active_transactions.mutex);
	...
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "priority_wallet_cementable_frontiers_count", active_transactions.priority_wallet_cementable_frontiers_size (), sizeof (oslo::cementable_account) }));
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "priority_cementable_frontiers_count", active_transactions.priority_cementable_frontiers_size (), sizeof (oslo::cementable_account) }));
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "inactive_votes_cache_count", active_transactions.inactive_votes_cache_size (), sizeof (oslo::gap_information) }));
	composite->add_component (collect_container_info (active_transactions.generator, "generator"));
	return composite;
}

oslo::dropped_elections::dropped_elections (oslo::stat & stats_a) :
stats (stats_a)
{
}

void oslo::dropped_elections::add (oslo::qualified_root const & root_a)
{
	stats.inc (oslo::stat::type::election, oslo::stat::detail::election_drop);
	oslo::lock_guard<std::mutex> guard (mutex);
	auto & items_by_sequence = items.get<tag_sequence> ();
	items_by_sequence.emplace_back (oslo::election_timepoint{ std::chrono::steady_clock::now (), root_a });
...
void oslo::dropped_elections::erase (oslo::qualified_root const & root_a)
{
	oslo::lock_guard<std::mutex> guard (mutex);
	items.get<tag_root> ().erase (root_a);
}

std::chrono::steady_clock::time_point oslo::dropped_elections::find (oslo::qualified_root const & root_a) const
{
	oslo::lock_guard<std::mutex> guard (mutex);
...
size_t oslo::dropped_elections::size () const
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return items.size ();
}
