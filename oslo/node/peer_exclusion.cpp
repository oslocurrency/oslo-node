...
constexpr std::chrono::hours oslo::peer_exclusion::exclude_time_hours;
constexpr std::chrono::hours oslo::peer_exclusion::exclude_remove_hours;
constexpr size_t oslo::peer_exclusion::size_max;
constexpr double oslo::peer_exclusion::peers_percentage_limit;

uint64_t oslo::peer_exclusion::add (oslo::tcp_endpoint const & endpoint_a, size_t const network_peers_count_a)
{
	uint64_t result (0);
...
bool oslo::peer_exclusion::check (oslo::tcp_endpoint const & endpoint_a)
{
	bool excluded (false);
	oslo::lock_guard<std::mutex> guard (mutex);
	auto & peers_by_endpoint (peers.get<tag_endpoint> ());
...
void oslo::peer_exclusion::remove (oslo::tcp_endpoint const & endpoint_a)
{
	oslo::lock_guard<std::mutex> guard (mutex);
	peers.get<tag_endpoint> ().erase (endpoint_a.address ());
}

size_t oslo::peer_exclusion::limited_size (size_t const network_peers_count_a) const
{
	return std::min<size_t> (size_max, network_peers_count_a * peers_percentage_limit);
}

size_t oslo::peer_exclusion::size () const
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return peers.size ();
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (oslo::peer_exclusion const & excluded_peers, const std::string & name)
{
	auto composite = std::make_unique<container_info_composite> (name);

	size_t excluded_peers_count = excluded_peers.size ();
	auto sizeof_excluded_peers_element = sizeof (oslo::peer_exclusion::ordered_endpoints::value_type);
