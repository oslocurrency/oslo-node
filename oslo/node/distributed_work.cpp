...
#include <oslo/boost/asio/post.hpp>
...
std::shared_ptr<request_type> oslo::distributed_work::peer_request::get_prepared_json_request (std::string const & request_string_a) const
{
...
oslo::distributed_work::distributed_work (oslo::node & node_a, oslo::work_request const & request_a, std::chrono::seconds const & backoff_a) :
node (node_a),
...
elapsed (oslo::timer_state::started, "distributed work generation timer")
{
	debug_assert (!finished);
	debug_assert (status == work_generation_status::ongoing);
}

oslo::distributed_work::~distributed_work ()
{
	debug_assert (status != work_generation_status::ongoing);
	if (auto node_l = node_w.lock ())
	{
		if (!node_l->stopped && node_l->websocket_server && node_l->websocket_server->any_subscriber (oslo::websocket::topic::work))
		{
...
void oslo::distributed_work::start ()
{
	...
	for (auto const & peer : need_resolve)
	{
		...
		if (!ec)
		{
			do_request (oslo::tcp_endpoint (parsed_address, peer.second));
		}
		else
		{
			...
			node.network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (peer.first, std::to_string (peer.second)), [peer, this_l, &extra = resolved_extra](boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
				if (!ec)
				{
					this_l->do_request (oslo::tcp_endpoint (i_a->endpoint ().address (), i_a->endpoint ().port ()));
					++i_a;
					for (auto & i : boost::make_iterator_range (i_a, {}))
					{
						++extra;
						this_l->do_request (oslo::tcp_endpoint (i.endpoint ().address (), i.endpoint ().port ()));
					}
				}
				else
				{
...
void oslo::distributed_work::start_local ()
{
	auto this_l (shared_from_this ());
...
void oslo::distributed_work::do_request (oslo::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
	...
	[this_l, connection](boost::system::error_code const & ec) {
		if (!ec && !this_l->stopped)
		{
			std::string request_string;
			{
				...
				request.put ("difficulty", oslo::to_string_hex (this_l->request.difficulty));
...
void oslo::distributed_work::do_cancel (oslo::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
...
void oslo::distributed_work::success (std::string const & body_a, oslo::tcp_endpoint const & endpoint_a)
{
	bool error = true;
	try
	{
		...
		uint64_t work;
		if (!oslo::from_string_hex (work_text, work))
		{
			if (oslo::work_difficulty (request.version, request.root, work) >= request.difficulty)
			{
				error = false;
			...
			{
				node.logger.try_log (boost::str (boost::format ("Incorrect work response from %1%:%2% for root %3% with diffuculty %4%: %5%") % endpoint_a.address () % endpoint_a.port () % request.root.to_string () % oslo::to_string_hex (request.difficulty) % work_text));
			}
		}
		else
		{
...
void oslo::distributed_work::stop_once (bool const local_stop_a)
{
...
void oslo::distributed_work::set_once (uint64_t const work_a, std::string const & source_a)
{
	if (!finished.exchange (true))
	{
		...
		if (node.config.logging.work_generation_time ())
		{
			...
			auto multiplier_text_l (oslo::to_string (oslo::difficulty::to_multiplier (request.difficulty, node.default_difficulty (request.version)), 2));
			node.logger.try_log (boost::str (unformatted_l % request.root.to_string () % oslo::to_string_hex (request.difficulty) % multiplier_text_l % elapsed.value ().count ()));
		}
	}
}

void oslo::distributed_work::cancel ()
{
	if (!finished.exchange (true))
	{
		elapsed.stop ();
...
void oslo::distributed_work::failure ()
{
...
void oslo::distributed_work::handle_failure ()
{
	if (!finished)
	{
...
void oslo::distributed_work::add_bad_peer (oslo::tcp_endpoint const & endpoint_a)
{
	oslo::lock_guard<std::mutex> guard (mutex);
