...
oslo::mdb_env::mdb_env (bool & error_a, boost::filesystem::path const & path_a, oslo::mdb_env::options options_a)
{
	init (error_a, path_a, options_a);
}

void oslo::mdb_env::init (bool & error_a, boost::filesystem::path const & path_a, oslo::mdb_env::options options_a)
{
	boost::system::error_code error_mkdir, error_chmod;
	if (path_a.has_parent_path ())
	{
		boost::filesystem::create_directories (path_a.parent_path (), error_mkdir);
		oslo::set_secure_perm_directory (path_a.parent_path (), error_chmod);
		if (!error_mkdir)
		{
			...
			if (options_a.config.sync == oslo::lmdb_config::sync_strategy::nosync_safe)
			{
				environment_flags |= MDB_NOMETASYNC;
			}
			else if (options_a.config.sync == oslo::lmdb_config::sync_strategy::nosync_unsafe)
			{
				environment_flags |= MDB_NOSYNC;
			}
			else if (options_a.config.sync == oslo::lmdb_config::sync_strategy::nosync_unsafe_large_memory)
			{
...
oslo::mdb_env::operator MDB_env * () const
{
	return environment;
}

oslo::read_transaction oslo::mdb_env::tx_begin_read (mdb_txn_callbacks mdb_txn_callbacks) const
{
	return oslo::read_transaction{ std::make_unique<oslo::read_mdb_txn> (*this, mdb_txn_callbacks) };
}

oslo::write_transaction oslo::mdb_env::tx_begin_write (mdb_txn_callbacks mdb_txn_callbacks) const
{
	return oslo::write_transaction{ std::make_unique<oslo::write_mdb_txn> (*this, mdb_txn_callbacks) };
}

MDB_txn * oslo::mdb_env::tx (oslo::transaction const & transaction_a) const
{
