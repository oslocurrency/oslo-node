...
{
public:
	mdb_iterator (oslo::transaction const & transaction_a, MDB_dbi db_a)
	{
	...
	mdb_iterator (oslo::transaction const & transaction_a, MDB_dbi db_a, MDB_val const & val_a)
	{
		auto status (mdb_cursor_open (tx (transaction_a), db_a, &cursor));
		release_assert (status == 0);
		current.first = val_a;
	...
	mdb_iterator (oslo::mdb_iterator<T, U> && other_a)
	{
	...
	mdb_iterator (oslo::mdb_iterator<T, U> const &) = delete;
	...
	oslo::store_iterator_impl<T, U> & operator++ () override
	{
	...
	std::pair<oslo::db_val<MDB_val>, oslo::db_val<MDB_val>> * operator-> ()
	{
		return &current;
	}

	bool operator== (oslo::store_iterator_impl<T, U> const & base_a) const override
	{
		auto const other_a (boost::polymorphic_downcast<oslo::mdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = oslo::db_val<MDB_val> ();
		current.second = oslo::db_val<MDB_val> ();
		debug_assert (is_end_sentinal ());
	}

	oslo::mdb_iterator<T, U> & operator= (oslo::mdb_iterator<T, U> && other_a)
	{
		if (cursor != nullptr)
		{
			mdb_cursor_close (cursor);
	...
	std::pair<oslo::db_val<MDB_val>, oslo::db_val<MDB_val>> current;
...
{
public:
	mdb_merge_iterator (oslo::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a) :
	impl1 (std::make_unique<oslo::mdb_iterator<T, U>> (transaction_a, db1_a)),
	impl2 (std::make_unique<oslo::mdb_iterator<T, U>> (transaction_a, db2_a))
	{
	}

	mdb_merge_iterator () :
	impl1 (std::make_unique<oslo::mdb_iterator<T, U>> ()),
	impl2 (std::make_unique<oslo::mdb_iterator<T, U>> ())
	{
	}

	mdb_merge_iterator (oslo::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a, MDB_val const & val_a) :
	impl1 (std::make_unique<oslo::mdb_iterator<T, U>> (transaction_a, db1_a, val_a)),
	impl2 (std::make_unique<oslo::mdb_iterator<T, U>> (transaction_a, db2_a, val_a))
	{
	}

	mdb_merge_iterator (oslo::mdb_merge_iterator<T, U> && other_a)
	{
		impl1 = std::move (other_a.impl1);
		impl2 = std::move (other_a.impl2);
	}

	mdb_merge_iterator (oslo::mdb_merge_iterator<T, U> const &) = delete;
	...
	bool operator== (oslo::store_iterator_impl<T, U> const & base_a) const override
	{
		debug_assert ((dynamic_cast<oslo::mdb_merge_iterator<T, U> const *> (&base_a) != nullptr) && "Incompatible iterator comparison");
		auto & other (static_cast<oslo::mdb_merge_iterator<T, U> const &> (base_a));
	...
	oslo::mdb_merge_iterator<T, U> & operator= (oslo::mdb_merge_iterator<T, U> &&) = default;
	oslo::mdb_merge_iterator<T, U> & operator= (oslo::mdb_merge_iterator<T, U> const &) = delete;

	mutable bool from_first_database{ false };

private:
	oslo::mdb_iterator<T, U> & least_iterator () const
	{
		oslo::mdb_iterator<T, U> * result;
	...
	std::unique_ptr<oslo::mdb_iterator<T, U>> impl1;
	std::unique_ptr<oslo::mdb_iterator<T, U>> impl2;
};
}
