...
oslo::mdb_store::mdb_store (oslo::logger_mt & logger_a, boost::filesystem::path const & path_a, oslo::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, oslo::lmdb_config const & lmdb_config_a, size_t const batch_size_a, bool backup_before_upgrade_a) :
logger (logger_a),
env (error, path_a, oslo::mdb_env::options::make ().set_config (lmdb_config_a).set_use_no_mem_init (true)),
...
bool oslo::mdb_store::vacuum_after_upgrade (boost::filesystem::path const & path_a, oslo::lmdb_config const & lmdb_config_a)
{
	...
	if (vacuum_success)
	{
		...
		auto options = oslo::mdb_env::options::make ()
...
void oslo::mdb_store::serialize_mdb_tracker (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	mdb_txn_tracker.serialize_json (json, min_read_time, min_write_time);
}

oslo::write_transaction oslo::mdb_store::tx_begin_write (std::vector<oslo::tables> const &, std::vector<oslo::tables> const &)
{
	return env.tx_begin_write (create_txn_callbacks ());
}

oslo::read_transaction oslo::mdb_store::tx_begin_read ()
{
	return env.tx_begin_read (create_txn_callbacks ());
}

std::string oslo::mdb_store::vendor_get () const
{
	return boost::str (boost::format ("LMDB %1%.%2%.%3%") % MDB_VERSION_MAJOR % MDB_VERSION_MINOR % MDB_VERSION_PATCH);
}

oslo::mdb_txn_callbacks oslo::mdb_store::create_txn_callbacks ()
{
	oslo::mdb_txn_callbacks mdb_txn_callbacks;
	if (txn_tracking_enabled)
	{
		mdb_txn_callbacks.txn_start = ([& mdb_txn_tracker = mdb_txn_tracker](const oslo::transaction_impl * transaction_impl) {
			mdb_txn_tracker.add (transaction_impl);
		});
		mdb_txn_callbacks.txn_end = ([& mdb_txn_tracker = mdb_txn_tracker](const oslo::transaction_impl * transaction_impl) {
...
void oslo::mdb_store::open_databases (bool & error_a, oslo::transaction const & transaction_a, unsigned flags)
{
...
bool oslo::mdb_store::do_upgrades (oslo::write_transaction & transaction_a, bool & needs_vacuuming, size_t batch_size_a)
{
	auto error (false);
...
void oslo::mdb_store::upgrade_v1_to_v2 (oslo::write_transaction const & transaction_a)
{
	version_put (transaction_a, 2);
	oslo::account account (1);
	while (!account.is_zero ())
	{
		oslo::mdb_iterator<oslo::account, oslo::account_info_v1> i (transaction_a, accounts_v0, oslo::mdb_val (account));
		std::cerr << std::hex;
		if (i != oslo::mdb_iterator<oslo::account, oslo::account_info_v1>{})
		{
			account = oslo::account (i->first);
			oslo::account_info_v1 v1 (i->second);
			oslo::account_info_v5 v2;
			...
			auto status (mdb_put (env.tx (transaction_a), accounts_v0, oslo::mdb_val (account), oslo::mdb_val (sizeof (v2), &v2), 0));
...
void oslo::mdb_store::upgrade_v2_to_v3 (oslo::write_transaction const & transaction_a)
{
	version_put (transaction_a, 3);
	mdb_drop (env.tx (transaction_a), representation, 0);
	for (auto i (std::make_unique<oslo::mdb_iterator<oslo::account, oslo::account_info_v5>> (transaction_a, accounts_v0)), n (std::make_unique<oslo::mdb_iterator<oslo::account, oslo::account_info_v5>> ()); *i != *n; ++(*i))
	{
		oslo::account account_l ((*i)->first);
		oslo::account_info_v5 info ((*i)->second);
		...
		auto impl (boost::polymorphic_downcast<oslo::mdb_iterator<oslo::account, oslo::account_info_v5> *> (i.get ()));
		mdb_cursor_put (impl->cursor, oslo::mdb_val (account_l), oslo::mdb_val (sizeof (info), &info), MDB_CURRENT);
	}
}

void oslo::mdb_store::upgrade_v3_to_v4 (oslo::write_transaction const & transaction_a)
{
	version_put (transaction_a, 4);
	std::queue<std::pair<oslo::pending_key, oslo::pending_info_v14>> items;
	for (auto i (oslo::store_iterator<oslo::block_hash, oslo::pending_info_v3> (std::make_unique<oslo::mdb_iterator<oslo::block_hash, oslo::pending_info_v3>> (transaction_a, pending_v0))), n (oslo::store_iterator<oslo::block_hash, oslo::pending_info_v3> (nullptr)); i != n; ++i)
	{
		oslo::block_hash const & hash (i->first);
		oslo::pending_info_v3 const & info (i->second);
		items.emplace (oslo::pending_key (info.destination, hash), oslo::pending_info_v14 (info.source, info.amount, oslo::epoch::epoch_0));
	}
	mdb_drop (env.tx (transaction_a), pending_v0, 0);
	while (!items.empty ())
	{
		auto status (mdb_put (env.tx (transaction_a), pending, oslo::mdb_val (items.front ().first), oslo::mdb_val (items.front ().second), 0));
		debug_assert (success (status));
		items.pop ();
	}
}

void oslo::mdb_store::upgrade_v4_to_v5 (oslo::write_transaction const & transaction_a)
{
	version_put (transaction_a, 5);
	for (auto i (oslo::store_iterator<oslo::account, oslo::account_info_v5> (std::make_unique<oslo::mdb_iterator<oslo::account, oslo::account_info_v5>> (transaction_a, accounts_v0))), n (oslo::store_iterator<oslo::account, oslo::account_info_v5> (nullptr)); i != n; ++i)
	{
		oslo::account_info_v5 const & info (i->second);
		...
		while (block != nullptr)
		{
			...
			if (block_successor (transaction_a, hash).is_zero () && !successor.is_zero ())
			{
				std::vector<uint8_t> vector;
				{
					...
					oslo::write (stream, successor.bytes);
				}
				...
				if (!block->previous ().is_zero ())
				{
					...
					std::copy (hash.bytes.begin (), hash.bytes.end (), data.end () - oslo::block_sideband::size (type));
...
void oslo::mdb_store::upgrade_v5_to_v6 (oslo::write_transaction const & transaction_a)
{
	version_put (transaction_a, 6);
	std::deque<std::pair<oslo::account, oslo::account_info_v13>> headers;
	for (auto i (oslo::store_iterator<oslo::account, oslo::account_info_v5> (std::make_unique<oslo::mdb_iterator<oslo::account, oslo::account_info_v5>> (transaction_a, accounts_v0))), n (oslo::store_iterator<oslo::account, oslo::account_info_v5> (nullptr)); i != n; ++i)
	{
		oslo::account const & account (i->first);
		oslo::account_info_v5 info_old (i->second);
		...
		headers.emplace_back (account, oslo::account_info_v13{ info_old.head, info_old.rep_block, info_old.open_block, info_old.balance, info_old.modified, block_count, oslo::epoch::epoch_0 });
	}
	for (auto i (headers.begin ()), n (headers.end ()); i != n; ++i)
	{
		auto status (mdb_put (env.tx (transaction_a), accounts_v0, oslo::mdb_val (i->first), oslo::mdb_val (i->second), 0));
		release_assert (status == 0);
	}
}

void oslo::mdb_store::upgrade_v6_to_v7 (oslo::write_transaction const & transaction_a)
{
	version_put (transaction_a, 7);
	mdb_drop (env.tx (transaction_a), unchecked, 0);
}

void oslo::mdb_store::upgrade_v7_to_v8 (oslo::write_transaction const & transaction_a)
{
...
void oslo::mdb_store::upgrade_v8_to_v9 (oslo::write_transaction const & transaction_a)
{
	...
	std::shared_ptr<oslo::block> block (std::move (genesis.open));
	oslo::keypair junk;
	for (oslo::mdb_iterator<oslo::account, uint64_t> i (transaction_a, sequence), n (oslo::mdb_iterator<oslo::account, uint64_t>{}); i != n; ++i)
	{
		oslo::bufferstream stream (reinterpret_cast<uint8_t const *> (i->second.data ()), i->second.size ());
		uint64_t sequence;
		auto error (oslo::try_read (stream, sequence));
		(void)error;
		// Create a dummy vote with the same sequence number for easy upgrading.  This won't have a valid signature.
		oslo::vote dummy (oslo::account (i->first), junk.prv, sequence, block);
		std::vector<uint8_t> vector;
		{
			oslo::vectorstream stream (vector);
			dummy.serialize (stream);
		}
		auto status1 (mdb_put (env.tx (transaction_a), vote, oslo::mdb_val (i->first), oslo::mdb_val (vector.size (), vector.data ()), 0));
...
void oslo::mdb_store::upgrade_v10_to_v11 (oslo::write_transaction const & transaction_a)
{
...
void oslo::mdb_store::upgrade_v11_to_v12 (oslo::write_transaction const & transaction_a)
{
...
void oslo::mdb_store::upgrade_v12_to_v13 (oslo::write_transaction & transaction_a, size_t const batch_size)
{
	size_t cost (0);
	...
	{
		oslo::account first (0);
		oslo::account_info_v13 second;
		{
			oslo::mdb_merge_iterator<oslo::account, oslo::account_info_v13> current (transaction_a, accounts_v0, accounts_v1, oslo::mdb_val (account));
			oslo::mdb_merge_iterator<oslo::account, oslo::account_info_v13> end{};
			if (current != end)
			{
				first = oslo::account (current->first);
				second = oslo::account_info_v13 (current->second);
			}
		}
		if (!first.is_zero ())
		{
			...
			while (!hash.is_zero ())
			{
				...
				if (sideband.height == 0)
				{
					...
					oslo::mdb_val value{ vector.size (), (void *)vector.data () };
					MDB_dbi database = is_state_block_v1 ? state_blocks_v1 : table_to_dbi (block_database (block->type ()));

					auto status = mdb_put (env.tx (transaction_a), database, oslo::mdb_val (hash), value, 0);
					release_assert (success (status));

					oslo::block_predecessor_set<MDB_val, oslo::mdb_store> predecessor (transaction_a, *this);
...
void oslo::mdb_store::upgrade_v13_to_v14 (oslo::write_transaction const & transaction_a)
{
	// Upgrade all accounts to have a confirmation of 0 (except genesis which should have 1)
	version_put (transaction_a, 14);
	oslo::mdb_merge_iterator<oslo::account, oslo::account_info_v13> i (transaction_a, accounts_v0, accounts_v1);
	oslo::mdb_merge_iterator<oslo::account, oslo::account_info_v13> n{};
	std::vector<std::pair<oslo::account, oslo::account_info_v14>> account_infos;
	account_infos.reserve (count (transaction_a, accounts_v0) + count (transaction_a, accounts_v1));
	for (; i != n; ++i)
	{
		oslo::account account (i->first);
		oslo::account_info_v13 account_info_v13 (i->second);
		...
		account_infos.emplace_back (account, oslo::account_info_v14{ account_info_v13.head, account_info_v13.rep_block, account_info_v13.open_block, account_info_v13.balance, account_info_v13.modified, account_info_v13.block_count, confirmation_height, i.from_first_database ? oslo::epoch::epoch_0 : oslo::epoch::epoch_1 });
	}

	for (auto const & account_info : account_infos)
	{
		auto status1 (mdb_put (env.tx (transaction_a), account_info.second.epoch == oslo::epoch::epoch_0 ? accounts_v0 : accounts_v1, oslo::mdb_val (account_info.first), oslo::mdb_val (account_info.second), 0));
		release_assert (status1 == 0);
	}
	...
	auto error (mdb_del (env.tx (transaction_a), meta, oslo::mdb_val (node_id_mdb_key), nullptr));
	release_assert (!error || error == MDB_NOTFOUND);
}

void oslo::mdb_store::upgrade_v14_to_v15 (oslo::write_transaction & transaction_a)
{
	logger.always_log ("Preparing v14 to v15 database upgrade...");

	std::vector<std::pair<oslo::account, oslo::account_info>> account_infos;
	...
	oslo::mdb_merge_iterator<oslo::account, oslo::account_info_v14> i_account (transaction_a, accounts_v0, accounts_v1);
	oslo::mdb_merge_iterator<oslo::account, oslo::account_info_v14> n_account{};
	for (; i_account != n_account; ++i_account)
	{
		oslo::account account (i_account->first);
		oslo::account_info_v14 account_info_v14 (i_account->second);
		...
		account_infos.emplace_back (account, oslo::account_info{ account_info_v14.head, rep_block->representative (), account_info_v14.open_block, account_info_v14.balance, account_info_v14.modified, account_info_v14.block_count, i_account.from_first_database ? oslo::epoch::epoch_0 : oslo::epoch::epoch_1 });
		// Move confirmation height from account_info database to its own table
		mdb_put (env.tx (transaction_a), confirmation_height, oslo::mdb_val (account), oslo::mdb_val (account_info_v14.confirmation_height), MDB_APPEND);
	...
	for (auto const & account_account_info_pair : account_infos)
	{
		...
		mdb_put (env.tx (transaction_a), accounts, oslo::mdb_val (account_account_info_pair.first), oslo::mdb_val (account_info), MDB_APPEND);
	}
	...
	oslo::mdb_merge_iterator<oslo::block_hash, oslo::state_block_w_sideband_v14> i_state (transaction_a, state_blocks_v0, state_blocks_v1);
	oslo::mdb_merge_iterator<oslo::block_hash, oslo::state_block_w_sideband_v14> n_state{};
	auto num = 0u;
	for (; i_state != n_state; ++i_state, ++num)
	{
		oslo::block_hash hash (i_state->first);
		oslo::state_block_w_sideband_v14 state_block_w_sideband_v14 (i_state->second);
		auto & sideband_v14 = state_block_w_sideband_v14.sideband;

		oslo::block_sideband sideband (sideband_v14.account, sideband_v14.successor, sideband_v14.balance, sideband_v14.height, sideband_v14.timestamp, i_state.from_first_database ? oslo::epoch::epoch_0 : oslo::epoch::epoch_1, false, false, false);
		...
		oslo::mdb_val value{ data.size (), (void *)data.data () };
		auto s = mdb_put (env.tx (transaction_a), state_blocks_new, oslo::mdb_val (hash), value, MDB_APPEND);
	...
	std::vector<std::pair<oslo::pending_key, oslo::pending_info>> pending_infos;
	pending_infos.reserve (pending_counters.before_v0 + pending_counters.before_v1);

	oslo::mdb_merge_iterator<oslo::pending_key, oslo::pending_info_v14> i_pending (transaction_a, pending_v0, pending_v1);
	oslo::mdb_merge_iterator<oslo::pending_key, oslo::pending_info_v14> n_pending{};
	for (; i_pending != n_pending; ++i_pending)
	{
		oslo::pending_info_v14 info (i_pending->second);
		pending_infos.emplace_back (oslo::pending_key (i_pending->first), oslo::pending_info{ info.source, info.amount, i_pending.from_first_database ? oslo::epoch::epoch_0 : oslo::epoch::epoch_1 });
	...
	{
		mdb_put (env.tx (transaction_a), pending, oslo::mdb_val (pending_key_pending_info_pair.first), oslo::mdb_val (pending_key_pending_info_pair.second), MDB_APPEND);
	}
...
void oslo::mdb_store::upgrade_v15_to_v16 (oslo::write_transaction const & transaction_a)
{
...
void oslo::mdb_store::upgrade_v16_to_v17 (oslo::write_transaction const & transaction_a)
{
	...
	std::vector<std::pair<oslo::account, oslo::confirmation_height_info>> confirmation_height_infos;
	auto num = 0u;
	for (oslo::mdb_iterator<oslo::account, uint64_t> i (transaction_a, confirmation_height), n (oslo::mdb_iterator<oslo::account, uint64_t>{}); i != n; ++i, ++account_info_i, ++num)
	{
		oslo::account account (i->first);
		...
		{
			confirmation_height_infos.emplace_back (account, confirmation_height_info{ 0, oslo::block_hash (0) });
		}
		else
		{
	...
	{
		mdb_put (env.tx (transaction_a), confirmation_height, oslo::mdb_val (confirmation_height_info_pair.first), oslo::mdb_val (confirmation_height_info_pair.second), MDB_APPEND);
	}
...
void oslo::mdb_store::upgrade_v17_to_v18 (oslo::write_transaction const & transaction_a)
{
	...
	auto num = 0u;
	for (oslo::mdb_iterator<oslo::block_hash, oslo::state_block_w_sideband> state_i (transaction_a, state_blocks), state_n{}; state_i != state_n; ++state_i, ++num)
	{
		oslo::state_block_w_sideband block_sideband (state_i->second);
		...
		oslo::block_sideband new_sideband (sideband.account, sideband.successor, sideband.balance, sideband.height, sideband.timestamp, sideband.details.epoch, is_send, is_receive, is_epoch);
		// Write these out
		std::vector<uint8_t> data;
		{
			oslo::vectorstream stream (data);
...
void oslo::mdb_store::create_backup_file (oslo::mdb_env & env_a, boost::filesystem::path const & filepath_a, oslo::logger_mt & logger_a)
{
...
void oslo::mdb_store::version_put (oslo::write_transaction const & transaction_a, int version_a)
{
	oslo::uint256_union version_key (1);
	oslo::uint256_union version_value (version_a);
	auto status (mdb_put (env.tx (transaction_a), meta, oslo::mdb_val (version_key), oslo::mdb_val (version_value), 0));
	release_assert (status == 0);
...
bool oslo::mdb_store::block_info_get (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a, oslo::block_info & block_info_a) const
{
	debug_assert (!full_sideband (transaction_a));
	oslo::mdb_val value;
	auto status (mdb_get (env.tx (transaction_a), blocks_info, oslo::mdb_val (hash_a), value));
	...
	if (status != MDB_NOTFOUND)
	{
		...
		auto error1 (oslo::try_read (stream, block_info_a.account));
		(void)error1;
		debug_assert (!error1);
		auto error2 (oslo::try_read (stream, block_info_a.balance));
		(void)error2;
		debug_assert (!error2);
	}
	return result;
...
bool oslo::mdb_store::exists (oslo::transaction const & transaction_a, tables table_a, oslo::mdb_val const & key_a) const
{
	oslo::mdb_val junk;
...
int oslo::mdb_store::get (oslo::transaction const & transaction_a, tables table_a, oslo::mdb_val const & key_a, oslo::mdb_val & value_a) const
{
	return mdb_get (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a);
}

int oslo::mdb_store::put (oslo::write_transaction const & transaction_a, tables table_a, oslo::mdb_val const & key_a, const oslo::mdb_val & value_a) const
{
	return (mdb_put (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a, 0));
}

int oslo::mdb_store::del (oslo::write_transaction const & transaction_a, tables table_a, oslo::mdb_val const & key_a) const
{
	return (mdb_del (env.tx (transaction_a), table_to_dbi (table_a), key_a, nullptr));
}

int oslo::mdb_store::drop (oslo::write_transaction const & transaction_a, tables table_a)
{
	return clear (transaction_a, table_to_dbi (table_a));
}

int oslo::mdb_store::clear (oslo::write_transaction const & transaction_a, MDB_dbi handle_a)
{
	return mdb_drop (env.tx (transaction_a), handle_a, 0);
}

size_t oslo::mdb_store::count (oslo::transaction const & transaction_a, tables table_a) const
{
	return count (transaction_a, table_to_dbi (table_a));
}

size_t oslo::mdb_store::count (oslo::transaction const & transaction_a, MDB_dbi db_a) const
{
	MDB_stat stats;
...
MDB_dbi oslo::mdb_store::table_to_dbi (tables table_a) const
{
	switch (table_a)
	{
...
bool oslo::mdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool oslo::mdb_store::success (int status) const
{
	return (MDB_SUCCESS == status);
}

int oslo::mdb_store::status_code_not_found () const
{
	return MDB_NOTFOUND;
}

bool oslo::mdb_store::copy_db (boost::filesystem::path const & destination_file)
{
	return !mdb_env_copy2 (env.environment, destination_file.string ().c_str (), MDB_CP_COMPACT);
}

void oslo::mdb_store::rebuild_db (oslo::write_transaction const & transaction_a)
{
	...
	for (auto const & table : tables)
	{
		...
		for (auto i (oslo::store_iterator<oslo::uint256_union, oslo::mdb_val> (std::make_unique<oslo::mdb_iterator<oslo::uint256_union, oslo::mdb_val>> (transaction_a, table))), n (oslo::store_iterator<oslo::uint256_union, oslo::mdb_val> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, oslo::mdb_val (i->first), i->second, MDB_APPEND);
			release_assert (success (s));
		}
		...
		for (auto i (oslo::store_iterator<oslo::uint256_union, oslo::mdb_val> (std::make_unique<oslo::mdb_iterator<oslo::uint256_union, oslo::mdb_val>> (transaction_a, temp))), n (oslo::store_iterator<oslo::uint256_union, oslo::mdb_val> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), table, oslo::mdb_val (i->first), i->second, MDB_APPEND);
			release_assert (success (s));
		}
		release_assert (count (transaction_a, table) == count (transaction_a, temp));
	...
	// Pending table
	{
		...
		mdb_dbi_open (env.tx (transaction_a), "temp_table", MDB_CREATE, &temp);
		// Copy all values to temporary table
		for (auto i (oslo::store_iterator<oslo::pending_key, oslo::pending_info> (std::make_unique<oslo::mdb_iterator<oslo::pending_key, oslo::pending_info>> (transaction_a, pending))), n (oslo::store_iterator<oslo::pending_key, oslo::pending_info> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, oslo::mdb_val (i->first), oslo::mdb_val (i->second), MDB_APPEND);
			release_assert (success (s));
		}
		release_assert (count (transaction_a, pending) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), pending, 0);
		// Put values from copy
		for (auto i (oslo::store_iterator<oslo::pending_key, oslo::pending_info> (std::make_unique<oslo::mdb_iterator<oslo::pending_key, oslo::pending_info>> (transaction_a, temp))), n (oslo::store_iterator<oslo::pending_key, oslo::pending_info> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), pending, oslo::mdb_val (i->first), oslo::mdb_val (i->second), MDB_APPEND);
			release_assert (success (s));
		}
		release_assert (count (transaction_a, pending) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), temp, 1);
...
bool oslo::mdb_store::init_error () const
{
	return error;
}

// All the v14 functions below are only needed during upgrades
bool oslo::mdb_store::entry_has_sideband_v14 (size_t entry_size_a, oslo::block_type type_a) const
{
	return (entry_size_a == oslo::block::size (type_a) + oslo::block_sideband_v14::size (type_a));
}

size_t oslo::mdb_store::block_successor_offset_v14 (oslo::transaction const & transaction_a, size_t entry_size_a, oslo::block_type type_a) const
{
	size_t result;
	if (full_sideband (transaction_a) || entry_has_sideband_v14 (entry_size_a, type_a))
	{
		result = entry_size_a - oslo::block_sideband_v14::size (type_a);
	}
	else
	{
		// Read old successor-only sideband
		debug_assert (entry_size_a == oslo::block::size (type_a) + sizeof (oslo::uint256_union));
		result = entry_size_a - sizeof (oslo::uint256_union);
	}
	return result;
}

oslo::block_hash oslo::mdb_store::block_successor_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a) const
{
	oslo::block_type type;
	...
	if (value.size () != 0)
	{
		...
		oslo::bufferstream stream (reinterpret_cast<uint8_t const *> (value.data ()) + block_successor_offset_v14 (transaction_a, value.size (), type), result.bytes.size ());
...
oslo::mdb_val oslo::mdb_store::block_raw_get_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a, oslo::block_type & type_a, bool * is_state_v1) const
{
	oslo::mdb_val result;
...
boost::optional<oslo::mdb_val> oslo::mdb_store::block_raw_get_by_type_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a, oslo::block_type & type_a, bool * is_state_v1) const
{
	oslo::mdb_val value;
	oslo::mdb_val hash (hash_a);
	...
	boost::optional<oslo::mdb_val> result;
	if (success (status))
	{
		result = value;
	}
...
oslo::account oslo::mdb_store::block_account_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a) const
{
	oslo::block_sideband_v14 sideband;
	auto block (block_get_v14 (transaction_a, hash_a, &sideband));
	oslo::account result (block->account ());
	if (result.is_zero ())
	{
...
oslo::account oslo::mdb_store::block_account_computed_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a) const
{
	debug_assert (!full_sideband (transaction_a));
...
oslo::uint128_t oslo::mdb_store::block_balance_computed_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a) const
{
	debug_assert (!full_sideband (transaction_a));
...
std::shared_ptr<oslo::block> oslo::mdb_store::block_get_v14 (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a, oslo::block_sideband_v14 * sideband_a, bool * is_state_v1) const
{
	oslo::block_type type;
...
oslo::mdb_store::upgrade_counters::upgrade_counters (uint64_t count_before_v0, uint64_t count_before_v1) :
before_v0 (count_before_v0),
before_v1 (count_before_v1)
{
}

bool oslo::mdb_store::upgrade_counters::are_equal () const
{
	return (before_v0 == after_v0) && (before_v1 == after_v1);
}

// Explicitly instantiate
template class oslo::block_store_partial<MDB_val, oslo::mdb_store>;
