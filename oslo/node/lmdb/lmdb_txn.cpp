...
{
public:
	explicit matches_txn (const oslo::transaction_impl * transaction_impl_a) :
	transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (oslo::mdb_txn_stats const & mdb_txn_stats)
	{
		return (mdb_txn_stats.transaction_impl == transaction_impl);
	}

private:
	const oslo::transaction_impl * transaction_impl;
};
}

oslo::read_mdb_txn::read_mdb_txn (oslo::mdb_env const & environment_a, oslo::mdb_txn_callbacks txn_callbacks_a) :
...
oslo::read_mdb_txn::~read_mdb_txn ()
{
...
void oslo::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void oslo::read_mdb_txn::renew ()
{
...
void * oslo::read_mdb_txn::get_handle () const
{
	return handle;
}

oslo::write_mdb_txn::write_mdb_txn (oslo::mdb_env const & environment_a, oslo::mdb_txn_callbacks txn_callbacks_a) :
env (environment_a),
txn_callbacks (txn_callbacks_a)
{
	renew ();
...
oslo::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void oslo::write_mdb_txn::commit () const
{
	auto status (mdb_txn_commit (handle));
	release_assert (status == MDB_SUCCESS);
	txn_callbacks.txn_end (this);
...
void * oslo::write_mdb_txn::get_handle () const
{
	return handle;
}

bool oslo::write_mdb_txn::contains (oslo::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

oslo::mdb_txn_tracker::mdb_txn_tracker (oslo::logger_mt & logger_a, oslo::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
logger (logger_a),
...
void oslo::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			oslo::jsonconfig mdb_lock_config;
			...
			oslo::jsonconfig stack (stacktrace_config);
...
void oslo::mdb_txn_tracker::output_finished (oslo::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == oslo::thread_role::get_string (oslo::thread_role::name::block_processing);
...
void oslo::mdb_txn_tracker::add (const oslo::transaction_impl * transaction_impl)
{
	oslo::lock_guard<std::mutex> guard (mutex);
...
void oslo::mdb_txn_tracker::erase (const oslo::transaction_impl * transaction_impl)
{
	oslo::lock_guard<std::mutex> guard (mutex);
...
oslo::mdb_txn_stats::mdb_txn_stats (const oslo::transaction_impl * transaction_impl) :
transaction_impl (transaction_impl),
thread_name (oslo::thread_role::get_string ()),
stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool oslo::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<const oslo::write_transaction_impl *> (transaction_impl) != nullptr);
}
