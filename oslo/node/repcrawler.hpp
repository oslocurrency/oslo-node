...
public:
	...
	representative (oslo::account account_a, oslo::amount weight_a, std::shared_ptr<oslo::transport::channel> channel_a) :
	account (account_a), weight (weight_a), channel (channel_a)
	{
		debug_assert (channel != nullptr);
	}
	std::reference_wrapper<oslo::transport::channel const> channel_ref () const
	{
		return *channel;
	};
	bool operator== (oslo::representative const & other_a) const
	{
		return account == other_a.account;
	}
	oslo::account account{ 0 };
	oslo::amount weight{ 0 };
...
class rep_crawler final
{
	...
	mi::indexed_by<
		mi::hashed_unique<mi::member<representative, oslo::account, &representative::account>>,
		mi::random_access<>,
		...
		mi::ordered_non_unique<mi::tag<tag_weight>,
			mi::member<representative, oslo::amount, &representative::weight>, std::greater<oslo::amount>>,
		mi::hashed_non_unique<mi::tag<tag_channel_ref>,
			mi::const_mem_fun<representative, std::reference_wrapper<oslo::transport::channel const>, &representative::channel_ref>>>>;
	// clang-format on

public:
	rep_crawler (oslo::node & node_a);
	...
	void query (std::vector<std::shared_ptr<oslo::transport::channel>> const & channels_a);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::shared_ptr<oslo::transport::channel> const & channel_a);

	/** Query if a peer manages a principle representative */
	bool is_pr (oslo::transport::channel const &) const;
	 ...
	 */
	bool response (std::shared_ptr<oslo::transport::channel> &, std::shared_ptr<oslo::vote> &);

	/** Get total available weight from representatives */
	oslo::uint128_t total_weight () const;
	...
	std::vector<representative> representatives (size_t count_a = std::numeric_limits<size_t>::max (), oslo::uint128_t const weight_a = 0, boost::optional<decltype (oslo::protocol_constants::protocol_version)> const & opt_version_min_a = boost::none);
	...
	std::vector<representative> principal_representatives (size_t count_a = std::numeric_limits<size_t>::max (), boost::optional<decltype (oslo::protocol_constants::protocol_version)> const & opt_version_min_a = boost::none);

	/** Request a list of the top \p count_a known representative endpoints. */
	std::vector<std::shared_ptr<oslo::transport::channel>> representative_endpoints (size_t count_a);
...
private:
	...
	std::unordered_set<oslo::block_hash> active;
	...
	std::vector<std::shared_ptr<oslo::transport::channel>> get_crawl_targets (oslo::uint128_t total_weight_a);

	/** When a rep request is made, this is called to update the last-request timestamp. */
	void on_rep_request (std::shared_ptr<oslo::transport::channel> channel_a);
	...
	std::deque<std::pair<std::shared_ptr<oslo::transport::channel>, std::shared_ptr<oslo::vote>>> responses;
};
}
