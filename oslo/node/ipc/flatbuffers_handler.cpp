...
namespace
{
auto handler_map = oslo::ipc::action_handler::handler_map ();
...
oslo::ipc::flatbuffers_handler::flatbuffers_handler (oslo::node & node_a, oslo::ipc::ipc_server & ipc_server_a, std::shared_ptr<oslo::ipc::subscriber> const & subscriber_a, oslo::ipc::ipc_config const & ipc_config_a) :
node (node_a),
...
std::shared_ptr<flatbuffers::Parser> oslo::ipc::flatbuffers_handler::make_flatbuffers_parser (oslo::ipc::ipc_config const & ipc_config_a)
{
	...
	if (!api_path)
	{
		throw oslo::error ("Internal IPC error: unable to find api path");
	}

	const char * include_directories[] = { api_path->string ().c_str (), nullptr };
	std::string schemafile;
	if (!flatbuffers::LoadFile ((*api_path / "osloapi.fbs").string ().c_str (), false, &schemafile))
	{
		throw oslo::error ("Internal IPC error: unable to load schema file");
	}
	...
	if (!parse_success)
	{
		...
		throw oslo::error (parser_error);
	}
	return parser;
}

void oslo::ipc::flatbuffers_handler::process_json (const uint8_t * message_buffer_a, size_t buffer_size_a,
...
void oslo::ipc::flatbuffers_handler::process (const uint8_t * message_buffer_a, size_t buffer_size_a,
std::function<void(std::shared_ptr<flatbuffers::FlatBufferBuilder>)> response_handler)
{
	...
	try
	{
		...
		if (ipc_config.flatbuffers.verify_buffers)
		{
			...
			{
				throw oslo::error ("Envelope buffer did not pass verifier");
			}
		}

		auto incoming = osloapi::GetEnvelope (message_buffer_a);
		if (incoming == nullptr)
		{
			oslo::error err ("Invalid message");
		...
		{
			oslo::error err ("Unknown message type");
			actionhandler->make_error (err.error_code_as_int (), err.get_message ());
		}
	}
	catch (oslo::error const & err)
