...
#include <oslo/boost/asio/local/stream_protocol.hpp>
...
#include <oslo/node/ipc/flatbuffers_handler.hpp>
...
class session final : public oslo::ipc::socket_base, public std::enable_shared_from_this<session<SOCKET_TYPE>>
{
public:
	session (oslo::ipc::ipc_server & server_a, boost::asio::io_context & io_ctx_a, oslo::ipc::ipc_config_transport & config_transport_a) :
	...
	std::shared_ptr<oslo::ipc::subscriber> get_subscriber ()
	{
		class subscriber_impl final : public oslo::ipc::subscriber, public std::enable_shared_from_this<subscriber_impl>
		{
		public:
			...
			virtual void async_send_message (uint8_t const * data_a, size_t length_a, std::function<void(oslo::error const &)> broadcast_completion_handler_a) override
			{
			...
			oslo::ipc::payload_encoding get_active_encoding () const override
			{
				oslo::ipc::payload_encoding encoding{ oslo::ipc::payload_encoding::flatbuffers };
				if (auto session_l = session_m.lock ())
				{
		...
		oslo::unique_lock<std::mutex> lock (subscriber_mutex);
	 ...
	 * @note This function explicitely doesn't use oslo::shared_const_buffer, as buffers usually originate from Flatbuffers
	...
	void write_queued_messages ()
	{
		...
		oslo::unsafe_async_write (socket, msg.buffer,
	...
	void handle_json_query (bool allow_unsafe)
	{
		...
		// This is called when oslo::rpc_handler#process_request is done. We convert to
		...
		node.stats.inc (oslo::stat::type::ipc, oslo::stat::detail::invocations);
		auto body (std::string (reinterpret_cast<char *> (buffer.data ()), buffer.size ()));

		// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
		auto handler (std::make_shared<oslo::json_handler> (node, server.node_rpc_config, body, response_handler_l, [& server = server]() {
			server.stop ();
	...
	void read_next_request ()
	{
		...
		async_read_exactly (buffer.data (), buffer.size (), std::chrono::seconds::max (), [this_l]() {
			auto encoding (this_l->buffer[oslo::ipc::preamble_offset::encoding]);
			this_l->active_encoding = static_cast<oslo::ipc::payload_encoding> (encoding);
			if (this_l->buffer[oslo::ipc::preamble_offset::lead] != 'N' || this_l->buffer[oslo::ipc::preamble_offset::reserved_1] != 0 || this_l->buffer[oslo::ipc::preamble_offset::reserved_2] != 0)
			{
				if (this_l->node.config.logging.log_ipc ())
				{
			...
			else if (encoding == static_cast<uint8_t> (oslo::ipc::payload_encoding::json_v1) || encoding == static_cast<uint8_t> (oslo::ipc::payload_encoding::json_v1_unsafe))
			{
				auto allow_unsafe (encoding == static_cast<uint8_t> (oslo::ipc::payload_encoding::json_v1_unsafe));
				// Length of payload
			...
			else if (encoding == static_cast<uint8_t> (oslo::ipc::payload_encoding::flatbuffers) || encoding == static_cast<uint8_t> (oslo::ipc::payload_encoding::flatbuffers_json))
			{
				// Length of payload
				this_l->async_read_exactly (&this_l->buffer_size, sizeof (this_l->buffer_size), [this_l, encoding]() {
					...
					this_l->async_read_exactly (this_l->buffer.data (), this_l->buffer_size, [this_l, encoding]() {
						...
						{
							this_l->flatbuffers_handler = std::make_shared<oslo::ipc::flatbuffers_handler> (this_l->node, this_l->server, this_l->get_subscriber (), this_l->node.config.ipc_config);
						}

						if (encoding == static_cast<uint8_t> (oslo::ipc::payload_encoding::flatbuffers_json))
						{
...
private:
	...
	oslo::locked<std::string> service_name;
	 ...
	 */
	std::atomic<oslo::ipc::payload_encoding> active_encoding;

	/** Timer for measuring the duration of ipc calls */
	oslo::timer<std::chrono::microseconds> session_timer;
	...
	oslo::ipc::ipc_config_transport & config_transport;

	/** Handler for Flatbuffers requests. This is created lazily on the first request. */
	std::shared_ptr<oslo::ipc::flatbuffers_handler> flatbuffers_handler;

	/** Session subscriber */
	std::shared_ptr<oslo::ipc::subscriber> subscriber;
};

/** Domain and TCP socket transport */
template <typename ACCEPTOR_TYPE, typename SOCKET_TYPE, typename ENDPOINT_TYPE>
class socket_transport : public oslo::ipc::transport
{
public:
	socket_transport (oslo::ipc::ipc_server & server_a, ENDPOINT_TYPE endpoint_a, oslo::ipc::ipc_config_transport & config_transport_a, int concurrency_a) :
	server (server_a), config_transport (config_transport_a)
	{
		...
		if (concurrency_a > 0)
		{
			runner = std::make_unique<oslo::thread_runner> (*io_ctx, static_cast<unsigned> (concurrency_a));
		}
	}
 ...
 */
void await_hup_signal (std::shared_ptr<boost::asio::signal_set> const & signals, oslo::ipc::ipc_server & server_a)
{
...
oslo::ipc::ipc_server::ipc_server (oslo::node & node_a, oslo::node_rpc_config const & node_rpc_config_a) :
node (node_a),
...
broker (node_a)
{
	try
	{
		oslo::error access_config_error (reload_access_config ());
...
oslo::ipc::broker & oslo::ipc::ipc_server::get_broker ()
{
	return broker;
}

oslo::ipc::access & oslo::ipc::ipc_server::get_access ()
{
	return access;
}

oslo::error oslo::ipc::ipc_server::reload_access_config ()
{
	oslo::error access_config_error (oslo::ipc::read_access_config_toml (node.application_path, access));
	if (access_config_error)
	{
