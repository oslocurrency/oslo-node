...
oslo::ipc::access_permission from_string (std::string permission)
{
	if (permission == "unrestricted")
		return oslo::ipc::access_permission::unrestricted;
	if (permission == "api_account_weight")
		return oslo::ipc::access_permission::api_account_weight;
	if (permission == "api_service_register")
		return oslo::ipc::access_permission::api_service_register;
	if (permission == "api_service_stop")
		return oslo::ipc::access_permission::api_service_stop;
	if (permission == "api_topic_service_stop")
		return oslo::ipc::access_permission::api_topic_service_stop;
	if (permission == "api_topic_confirmation")
		return oslo::ipc::access_permission::api_topic_confirmation;
	if (permission == "account_query")
		return oslo::ipc::access_permission::account_query;
	if (permission == "epoch_upgrade")
		return oslo::ipc::access_permission::epoch_upgrade;
	if (permission == "service")
		return oslo::ipc::access_permission::service;
	if (permission == "wallet")
		return oslo::ipc::access_permission::wallet;
	if (permission == "wallet_read")
		return oslo::ipc::access_permission::wallet_read;
	if (permission == "wallet_write")
		return oslo::ipc::access_permission::wallet_write;
	if (permission == "wallet_seed_change")
		return oslo::ipc::access_permission::wallet_seed_change;

	return oslo::ipc::access_permission::invalid;
}
}

void oslo::ipc::access::set_effective_permissions (oslo::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a)
{
	...
	for (auto const & permission : allow_strings_l)
	{
		if (!permission.empty ())
		{
			...
			if (permission_enum != oslo::ipc::access_permission::invalid)
			{
...
void oslo::ipc::access::clear ()
{
	...
	default_user.permissions.insert (oslo::ipc::access_permission::api_account_weight);
}

oslo::error oslo::ipc::access::deserialize_toml (oslo::tomlconfig & toml)
{
...
bool oslo::ipc::access::has_access (std::string const & credentials_a, oslo::ipc::access_permission permssion_a) const
{
	oslo::unique_lock<std::mutex> lock (mutex);
	...
	if (user != users.end ())
	{
		...
		if (!permitted)
		{
			permitted = user->second.permissions.find (oslo::ipc::access_permission::unrestricted) != user->second.permissions.end ();
		}
	}
	return permitted;
}

bool oslo::ipc::access::has_access_to_all (std::string const & credentials_a, std::initializer_list<oslo::ipc::access_permission> permissions_a) const
{
	oslo::unique_lock<std::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
bool oslo::ipc::access::has_access_to_oneof (std::string const & credentials_a, std::initializer_list<oslo::ipc::access_permission> permissions_a) const
{
	oslo::unique_lock<std::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
void oslo::ipc::access_subject::clear ()
{
	permissions.clear ();
}

void oslo::ipc::access_user::clear ()
{
...
namespace ipc
{
	oslo::error read_access_config_toml (boost::filesystem::path const & data_path_a, oslo::ipc::access & config_a)
	{
		oslo::error error;
		auto toml_config_path = oslo::get_access_toml_config_path (data_path_a);
