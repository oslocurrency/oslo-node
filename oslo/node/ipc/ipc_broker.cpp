...
oslo::ipc::broker::broker (oslo::node & node_a) :
node (node_a)
{
}

std::shared_ptr<flatbuffers::Parser> oslo::ipc::subscriber::get_parser (oslo::ipc::ipc_config const & ipc_config_a)
{
	if (!parser)
	{
		parser = oslo::ipc::flatbuffers_handler::make_flatbuffers_parser (ipc_config_a);
	}
	return parser;
}

void oslo::ipc::broker::start ()
{
	node.observers.blocks.add ([this](oslo::election_status const & status_a, oslo::account const & account_a, oslo::amount const & amount_a, bool is_state_send_a) {
		...
		try
		{
			...
			{
				auto confirmation (std::make_shared<osloapi::EventConfirmationT> ());
				...
				switch (status_a.type)
				{
					case oslo::election_status_type::active_confirmed_quorum:
						confirmation->confirmation_type = osloapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
						break;
					case oslo::election_status_type::active_confirmation_height:
						confirmation->confirmation_type = osloapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height;
						break;
					case oslo::election_status_type::inactive_confirmation_height:
						confirmation->confirmation_type = osloapi::TopicConfirmationType::TopicConfirmationType_inactive;
						break;
					default:
						debug_assert (false);
						break;
				...
				confirmation->confirmation_type = osloapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
				confirmation->block = oslo::ipc::flatbuffers_builder::block_to_union (*status_a.winner, amount_a, is_state_send_a);
				confirmation->election_info = std::make_unique<osloapi::ElectionInfoT> ();
...
void subscribe_or_unsubscribe (oslo::logger_mt & logger, COLL & subscriber_collection, std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a, TOPIC_TYPE topic_a)
{
...
void oslo::ipc::broker::subscribe (std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a, std::shared_ptr<osloapi::TopicConfirmationT> const & confirmation_a)
{
...
void oslo::ipc::broker::broadcast (std::shared_ptr<osloapi::EventConfirmationT> const & confirmation_a)
{
	using Filter = osloapi::TopicConfirmationTypeFilter;
	decltype (confirmation_a->election_info) election_info;
	osloapi::BlockUnion block;
	...
	while (itr != confirmation_subscribers->end ())
	{
		if (auto subscriber_l = itr->subscriber.lock ())
		{
			auto should_filter = [this, &itr, confirmation_a]() {
				...
				if ((confirmation_a->confirmation_type == osloapi::TopicConfirmationType::TopicConfirmationType_active_quorum || confirmation_a->confirmation_type == osloapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height) && (all_filter || active_filter))
				{
					should_filter_conf_type_l = false;
				}
				else if (confirmation_a->confirmation_type == osloapi::TopicConfirmationType::TopicConfirmationType_inactive && (all_filter || inactive_filter))
				{
					should_filter_conf_type_l = false;
				}
			...
			{
				auto fb (oslo::ipc::flatbuffer_producer::make_buffer (*confirmation_a));

				if (subscriber_l->get_active_encoding () == oslo::ipc::payload_encoding::flatbuffers_json)
				{
					...
					{
						throw oslo::error ("Couldn't serialize response to JSON");
					}

					subscriber_l->async_send_message (reinterpret_cast<uint8_t const *> (json->data ()), json->size (), [json](const oslo::error & err) {});
				}
				else
				{
					subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb](const oslo::error & err) {});
				}
			}
			...
			if (block.type != osloapi::Block::Block_NONE)
			{
...
size_t oslo::ipc::broker::confirmation_subscriber_count () const
{
	return confirmation_subscribers->size ();
}

void oslo::ipc::broker::service_register (std::string const & service_name_a, std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a)
{
...
void oslo::ipc::broker::service_stop (std::string const & service_name_a)
{
	...
	for (auto & subcription : subscribers.get ())
	{
		if (auto subscriber_l = subcription.subscriber.lock ())
		{
			...
			{
				osloapi::EventServiceStopT event_stop;
				auto fb (oslo::ipc::flatbuffer_producer::make_buffer (event_stop));
				subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb](const oslo::error & err) {});
...
void oslo::ipc::broker::subscribe (std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a, std::shared_ptr<osloapi::TopicServiceStopT> const & service_stop_a)
{
	auto subscribers = service_stop_subscribers.lock ();
