...
namespace ipc
{
	class ipc_config;
	/**
	 * A subscriber represents a live session, and is weakly referenced oslo::ipc::subscription whenever a subscription is made.
	...
	public:
		 ...
		 */
		virtual void async_send_message (uint8_t const * data_a, size_t length_a, std::function<void(oslo::error const &)> broadcast_completion_handler_a) = 0;
		...
		virtual oslo::ipc::payload_encoding get_active_encoding () const = 0;

		/** Get flatbuffer parser instance for this subscriber; create it if necessary */
		std::shared_ptr<flatbuffers::Parser> get_parser (oslo::ipc::ipc_config const & ipc_config_a);
	...
	{
	public:
		subscription (std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a, std::shared_ptr<TopicType> const & topic_a) :
	...
	{
	public:
		broker (oslo::node & node_a);
		/** Starts the broker by setting up observers */
		void start ();
		/** Subscribe to block confirmations */
		void subscribe (std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a, std::shared_ptr<osloapi::TopicConfirmationT> const & confirmation_a);
		/** Subscribe to EventServiceStop notifications for \p subscriber_a. The subscriber must first have called ServiceRegister. */
		void subscribe (std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a, std::shared_ptr<osloapi::TopicServiceStopT> const & service_stop_a);
		...
		void service_register (std::string const & service_name_a, std::weak_ptr<oslo::ipc::subscriber> const & subscriber_a);
	...
	private:
		...
		void broadcast (std::shared_ptr<osloapi::EventConfirmationT> const & confirmation_a);

		oslo::node & node;
		mutable oslo::locked<std::vector<subscription<osloapi::TopicConfirmationT>>> confirmation_subscribers;
		mutable oslo::locked<std::vector<subscription<osloapi::TopicServiceStopT>>> service_stop_subscribers;
	};
}
}
