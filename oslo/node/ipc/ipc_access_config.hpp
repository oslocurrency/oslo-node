...
namespace ipc
{
	...
	{
	public:
		std::unordered_set<oslo::ipc::access_permission, enum_hash> permissions;
	...
	public:
		...
		std::string id;
		std::vector<oslo::ipc::access_role> roles;
		void clear () override;
	};
	...
	{
	public:
		bool has_access (std::string const & credentials_a, oslo::ipc::access_permission permission_a) const;
		bool has_access_to_all (std::string const & credentials_a, std::initializer_list<oslo::ipc::access_permission> permissions_a) const;
		bool has_access_to_oneof (std::string const & credentials_a, std::initializer_list<oslo::ipc::access_permission> permissions_a) const;
		oslo::error deserialize_toml (oslo::tomlconfig &);

	private:
		/** Process allow and deny entries for the given subject */
		void set_effective_permissions (oslo::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a);

		/** Clear current users, roles and default permissions */
		void clear ();

		std::unordered_map<std::string, oslo::ipc::access_user> users;
		std::unordered_map<std::string, oslo::ipc::access_role> roles;
