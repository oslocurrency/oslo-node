...
oslo::error oslo::ipc::ipc_config::serialize_toml (oslo::tomlconfig & toml) const
{
	oslo::tomlconfig tcp_l;
...
oslo::error oslo::ipc::ipc_config::deserialize_toml (oslo::tomlconfig & toml)
{
...
oslo::error oslo::ipc::ipc_config::serialize_json (oslo::jsonconfig & json) const
{
	oslo::jsonconfig tcp_l;
	// Only write out experimental config values if they're previously set explicitly in the config file
	if (transport_tcp.io_threads >= 0)
	{
...
oslo::error oslo::ipc::ipc_config::deserialize_json (bool & upgraded_a, oslo::jsonconfig & json)
{
