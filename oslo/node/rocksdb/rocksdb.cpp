...
oslo::rocksdb_store::rocksdb_store (oslo::logger_mt & logger_a, boost::filesystem::path const & path_a, oslo::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
logger (logger_a),
...
oslo::rocksdb_store::~rocksdb_store ()
{
...
void oslo::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
oslo::write_transaction oslo::rocksdb_store::tx_begin_write (std::vector<oslo::tables> const & tables_requiring_locks_a, std::vector<oslo::tables> const & tables_no_locks_a)
{
	std::unique_ptr<oslo::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<oslo::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<oslo::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return oslo::write_transaction{ std::move (txn) };
}

oslo::read_transaction oslo::rocksdb_store::tx_begin_read ()
{
	return oslo::read_transaction{ std::make_unique<oslo::read_rocksdb_txn> (db) };
}

std::string oslo::rocksdb_store::vendor_get () const
{
	return boost::str (boost::format ("RocksDB %1%.%2%.%3%") % ROCKSDB_MAJOR % ROCKSDB_MINOR % ROCKSDB_PATCH);
}

rocksdb::ColumnFamilyHandle * oslo::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool oslo::rocksdb_store::exists (oslo::transaction const & transaction_a, tables table_a, oslo::rocksdb_val const & key_a) const
{
...
int oslo::rocksdb_store::del (oslo::write_transaction const & transaction_a, tables table_a, oslo::rocksdb_val const & key_a)
{
...
bool oslo::rocksdb_store::block_info_get (oslo::transaction const &, oslo::block_hash const &, oslo::block_info &) const
{
...
void oslo::rocksdb_store::version_put (oslo::write_transaction const & transaction_a, int version_a)
{
	...
	auto status (put (transaction_a, tables::meta, version_key, oslo::rocksdb_val (version_value)));
	release_assert (success (status));
}

rocksdb::Transaction * oslo::rocksdb_store::tx (oslo::transaction const & transaction_a) const
{
	debug_assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int oslo::rocksdb_store::get (oslo::transaction const & transaction_a, tables table_a, oslo::rocksdb_val const & key_a, oslo::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
bool oslo::rocksdb_store::is_caching_counts (oslo::tables table_a) const
{
	switch (table_a)
	{
		case tables::send_blocks:
...
int oslo::rocksdb_store::increment (oslo::write_transaction const & transaction_a, tables table_a, oslo::rocksdb_val const & key_a, uint64_t amount_a)
{
	release_assert (transaction_a.contains (table_a));
	uint64_t base;
	oslo::rocksdb_val value;
	...
	return put (transaction_a, table_a, key_a, oslo::rocksdb_val (base + amount_a));
}

int oslo::rocksdb_store::decrement (oslo::write_transaction const & transaction_a, tables table_a, oslo::rocksdb_val const & key_a, uint64_t amount_a)
{
	release_assert (transaction_a.contains (table_a));
	oslo::rocksdb_val value;
...
int oslo::rocksdb_store::put (oslo::write_transaction const & transaction_a, tables table_a, oslo::rocksdb_val const & key_a, oslo::rocksdb_val const & value_a)
{
	debug_assert (transaction_a.contains (table_a));
...
bool oslo::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool oslo::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int oslo::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t oslo::rocksdb_store::count (oslo::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle) const
{
	uint64_t count = 0;
	oslo::rocksdb_val val;
	auto const & key = handle->GetName ();
	auto status = get (transaction_a, tables::cached_counts, oslo::rocksdb_val (key.size (), (void *)key.data ()), val);
	if (success (status))
	{
...
size_t oslo::rocksdb_store::count (oslo::transaction const & transaction_a, tables table_a) const
{
	size_t sum = 0;
...
int oslo::rocksdb_store::drop (oslo::write_transaction const & transaction_a, tables table_a)
{
	debug_assert (transaction_a.contains (table_a));
	...
	{
		// Reset counter to 0
		status = put (transaction_a, tables::cached_counts, oslo::rocksdb_val (rocksdb::Slice (col->GetName ())), oslo::rocksdb_val (uint64_t{ 0 }));
	}
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peers_begin (transaction_a), n = peers_end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, oslo::rocksdb_val (i->first));
				release_assert (success (status));
			}
			return status;
		}
...
int oslo::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
void oslo::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options oslo::rocksdb_store::get_db_options () const
{
...
rocksdb::BlockBasedTableOptions oslo::rocksdb_store::get_table_options () const
{
...
rocksdb::ColumnFamilyOptions oslo::rocksdb_store::get_cf_options () const
{
...
std::vector<oslo::tables> oslo::rocksdb_store::all_tables () const
{
	return std::vector<oslo::tables>{ tables::accounts, tables::cached_counts, tables::change_blocks, tables::confirmation_height, tables::frontiers, tables::meta, tables::online_weight, tables::open_blocks, tables::peers, tables::pending, tables::receive_blocks, tables::representation, tables::send_blocks, tables::state_blocks, tables::unchecked, tables::vote };
}

bool oslo::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		oslo::rocksdb_store rocksdb_store (logger, destination_path.string (), rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

void oslo::rocksdb_store::rebuild_db (oslo::write_transaction const & transaction_a)
{
	release_assert (false && "Not available for RocksDB");
}

bool oslo::rocksdb_store::init_error () const
{
	return error;
}
// Explicitly instantiate
template class oslo::block_store_partial<rocksdb::Slice, oslo::rocksdb_store>;
