...
namespace
{
inline bool is_read (oslo::transaction const & transaction_a)
{
	return (dynamic_cast<const oslo::read_transaction *> (&transaction_a) != nullptr);
}

inline rocksdb::ReadOptions const & snapshot_options (oslo::transaction const & transaction_a)
{
...
{
public:
	rocksdb_iterator (rocksdb::DB * db, oslo::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle_a)
	{
	...
	rocksdb_iterator (rocksdb::DB * db, oslo::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle_a, rocksdb_val const & val_a)
	{
		rocksdb::Iterator * iter;
		if (is_read (transaction_a))
		{
	...
	rocksdb_iterator (oslo::rocksdb_iterator<T, U> && other_a)
	{
	...
	rocksdb_iterator (oslo::rocksdb_iterator<T, U> const &) = delete;
	...
	std::pair<oslo::rocksdb_val, oslo::rocksdb_val> * operator-> ()
	{
		return &current;
	}
	...
	{
		auto const other_a (boost::polymorphic_downcast<oslo::rocksdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = oslo::rocksdb_val{};
		current.second = oslo::rocksdb_val{};
		debug_assert (is_end_sentinal ());
	}
	oslo::rocksdb_iterator<T, U> & operator= (oslo::rocksdb_iterator<T, U> && other_a)
	{
	...
	std::pair<oslo::rocksdb_val, oslo::rocksdb_val> current;
