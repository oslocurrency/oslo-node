...
int constexpr oslo::election::passive_duration_factor;
int constexpr oslo::election::active_request_count_min;
int constexpr oslo::election::active_broadcasting_duration_factor;
int constexpr oslo::election::confirmed_duration_factor;

std::chrono::milliseconds oslo::election::base_latency () const
{
	return node.network_params.network.is_test_network () ? 25ms : 1000ms;
}

oslo::election_vote_result::election_vote_result (bool replay_a, bool processed_a)
{
	replay = replay_a;
	processed = processed_a;
}

oslo::election::election (oslo::node & node_a, std::shared_ptr<oslo::block> block_a, std::function<void(std::shared_ptr<oslo::block>)> const & confirmation_action_a, bool prioritized_a) :
confirmation_action (confirmation_action_a),
prioritized_m (prioritized_a),
node (node_a),
status ({ block_a, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, oslo::election_status_type::ongoing }),
height (block_a->sideband ().height)
{
	last_votes.emplace (node.network_params.random.not_an_account, oslo::vote_info{ std::chrono::steady_clock::now (), 0, block_a->hash () });
...
void oslo::election::confirm_once (oslo::election_status_type type_a)
{
	debug_assert (!node.active.mutex.try_lock ());
	// This must be kept above the setting of election state, as dependent confirmed elections require up to date changes to election_winner_details
	oslo::unique_lock<std::mutex> election_winners_lk (node.active.election_winner_details_mutex);
	if (state_m.exchange (oslo::election::state_t::confirmed) != oslo::election::state_t::confirmed && (node.active.election_winner_details.count (status.winner->hash ()) == 0))
	{
		status.election_end = std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ());
		status.election_duration = std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::steady_clock::now () - election_start);
		status.confirmation_request_count = confirmation_request_count;
		status.block_count = oslo::narrow_cast<decltype (status.block_count)> (blocks.size ());
		status.voter_count = oslo::narrow_cast<decltype (status.voter_count)> (last_votes.size ());
		status.type = type_a;
...
bool oslo::election::valid_change (oslo::election::state_t expected_a, oslo::election::state_t desired_a) const
{
	bool result = false;
	switch (expected_a)
	{
		case oslo::election::state_t::idle:
			switch (desired_a)
			{
				case oslo::election::state_t::passive:
				case oslo::election::state_t::active:
					result = true;
					break;
				default:
					break;
		...
		case oslo::election::state_t::passive:
			switch (desired_a)
			{
				case oslo::election::state_t::idle:
				case oslo::election::state_t::active:
				case oslo::election::state_t::confirmed:
				case oslo::election::state_t::expired_unconfirmed:
					result = true;
					break;
				default:
					break;
		...
		case oslo::election::state_t::active:
			...
			{
				case oslo::election::state_t::idle:
				case oslo::election::state_t::broadcasting:
				case oslo::election::state_t::confirmed:
				case oslo::election::state_t::expired_unconfirmed:
					result = true;
					break;
		...
		case oslo::election::state_t::broadcasting:
			...
			{
				case oslo::election::state_t::idle:
				case oslo::election::state_t::backtracking:
				case oslo::election::state_t::confirmed:
				case oslo::election::state_t::expired_unconfirmed:
					result = true;
					break;
		...
		case oslo::election::state_t::confirmed:
			switch (desired_a)
			{
				case oslo::election::state_t::expired_confirmed:
					result = true;
					break;
				default:
					break;
...
bool oslo::election::state_change (oslo::election::state_t expected_a, oslo::election::state_t desired_a)
{
...
void oslo::election::send_confirm_req (oslo::confirmation_solicitor & solicitor_a)
{
...
void oslo::election::transition_passive ()
{
	oslo::lock_guard<std::mutex> guard (timepoints_mutex);
	transition_passive_impl ();
}

void oslo::election::transition_passive_impl ()
{
	state_change (oslo::election::state_t::idle, oslo::election::state_t::passive);
}

void oslo::election::transition_active ()
{
	oslo::lock_guard<std::mutex> guard (timepoints_mutex);
	transition_active_impl ();
}

void oslo::election::transition_active_impl ()
{
	state_change (oslo::election::state_t::idle, oslo::election::state_t::active);
}

bool oslo::election::idle () const
{
	return state_m == oslo::election::state_t::idle;
}

bool oslo::election::confirmed () const
{
	return state_m == oslo::election::state_t::confirmed || state_m == oslo::election::state_t::expired_confirmed;
}

void oslo::election::activate_dependencies ()
{
	debug_assert (!node.active.mutex.try_lock ());
	node.active.pending_dependencies.emplace_back (status.winner->hash (), height);
}

void oslo::election::broadcast_block (oslo::confirmation_solicitor & solicitor_a)
{
...
bool oslo::election::transition_time (oslo::confirmation_solicitor & solicitor_a)
{
	debug_assert (!node.active.mutex.try_lock ());
	oslo::lock_guard<std::mutex> guard (timepoints_mutex);
	bool result = false;
	switch (state_m)
	{
		...
		case oslo::election::state_t::passive:
		{
			...
			{
				state_change (oslo::election::state_t::passive, oslo::election::state_t::active);
			}
			break;
		}
		case oslo::election::state_t::active:
			...
			{
				state_change (oslo::election::state_t::active, oslo::election::state_t::broadcasting);
			}
			break;
		case oslo::election::state_t::broadcasting:
			...
			{
				state_change (oslo::election::state_t::broadcasting, oslo::election::state_t::backtracking);
		...
		case oslo::election::state_t::confirmed:
			...
			{
				result = true;
				state_change (oslo::election::state_t::confirmed, oslo::election::state_t::expired_confirmed);
			}
			break;
		case oslo::election::state_t::expired_unconfirmed:
		case oslo::election::state_t::expired_confirmed:
	...
	{
		result = true;
		state_change (state_m.load (), oslo::election::state_t::expired_unconfirmed);
		status.type = oslo::election_status_type::stopped;
		log_votes (tally ());
	}
	return result;
}

bool oslo::election::have_quorum (oslo::tally_t const & tally_a, oslo::uint128_t tally_sum) const
{
	bool result = false;
...
oslo::tally_t oslo::election::tally ()
{
	std::unordered_map<oslo::block_hash, oslo::uint128_t> block_weights;
...
void oslo::election::confirm_if_quorum ()
{
	auto tally_l (tally ());
	...
	if (have_quorum (tally_l, sum))
	{
		...
		confirm_once (oslo::election_status_type::active_confirmed_quorum);
	}
}

void oslo::election::log_votes (oslo::tally_t const & tally_a) const
{
...
oslo::election_vote_result oslo::election::vote (oslo::account rep, uint64_t sequence, oslo::block_hash block_hash)
{
	...
	if (node.network_params.network.is_test_network () || weight > node.minimum_principal_weight (online_stake))
	{
		...
		if (should_process)
		{
			node.stats.inc (oslo::stat::type::election, oslo::stat::detail::vote_new);
	...
	return oslo::election_vote_result (replay, should_process);
}

bool oslo::election::publish (std::shared_ptr<oslo::block> block_a)
{
...
size_t oslo::election::last_votes_size ()
{
	oslo::lock_guard<std::mutex> lock (node.active.mutex);
	return last_votes.size ();
}

void oslo::election::update_dependent ()
{
	debug_assert (!node.active.mutex.try_lock ());
	std::vector<oslo::block_hash> blocks_search;
...
void oslo::election::adjust_dependent_difficulty ()
{
...
size_t oslo::election::insert_inactive_votes_cache (oslo::block_hash const & hash_a)
{
	auto cache (node.active.find_inactive_votes_cache (hash_a));
	for (auto const & rep : cache.voters)
	{
		auto inserted (last_votes.emplace (rep, oslo::vote_info{ std::chrono::steady_clock::time_point::min (), 0, hash_a }));
		if (inserted.second)
		{
			node.stats.inc (oslo::stat::type::election, oslo::stat::detail::vote_cached);
		}
	}
	if (!confirmed () && !cache.voters.empty ())
	{
		...
		{
			node.stats.inc (oslo::stat::type::election, oslo::stat::detail::late_block);
			node.stats.add (oslo::stat::type::election, oslo::stat::detail::late_block_seconds, oslo::stat::dir::in, delay.count (), true);
		}
		confirm_if_quorum ();
	}
...
bool oslo::election::prioritized () const
{
	return prioritized_m;
}

void oslo::election::prioritize_election (oslo::vote_generator_session & generator_session_a)
{
	debug_assert (!node.active.mutex.try_lock ());
...
void oslo::election::try_generate_votes (oslo::block_hash const & hash_a)
{
...
void oslo::election::generate_votes (oslo::block_hash const & hash_a)
{
...
void oslo::election::remove_votes (oslo::block_hash const & hash_a)
{
	if (node.config.enable_voting && node.wallets.reps ().voting > 0)
	{
