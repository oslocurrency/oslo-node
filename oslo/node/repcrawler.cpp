...
oslo::rep_crawler::rep_crawler (oslo::node & node_a) :
node (node_a)
{
	if (!node.flags.disable_rep_crawler)
	{
		node.observers.endpoint.add ([this](std::shared_ptr<oslo::transport::channel> channel_a) {
			this->query (channel_a);
		});
	}
}

void oslo::rep_crawler::remove (oslo::block_hash const & hash_a)
{
	oslo::lock_guard<std::mutex> lock (active_mutex);
	active.erase (hash_a);
}

void oslo::rep_crawler::start ()
{
	ongoing_crawl ();
}

void oslo::rep_crawler::validate ()
{
	...
	for (auto const & i : responses_l)
	{
		...
		oslo::uint128_t rep_weight = node.ledger.weight (vote->account);
		if (rep_weight > minimum)
		{
			auto updated_or_inserted = false;
			oslo::unique_lock<std::mutex> lock (probable_reps_mutex);
			auto existing (probable_reps.find (vote->account));
			if (existing != probable_reps.end ())
			{
				probable_reps.modify (existing, [rep_weight, &updated_or_inserted, &vote, &channel](oslo::representative & info) {
			...
			{
				probable_reps.emplace (oslo::representative (vote->account, rep_weight, channel));
				updated_or_inserted = true;
			}
			lock.unlock ();
...
std::vector<std::shared_ptr<oslo::transport::channel>> oslo::rep_crawler::get_crawl_targets (oslo::uint128_t total_weight_a)
{
	...
	std::vector<std::shared_ptr<oslo::transport::channel>> result;
	result.insert (result.end (), random_peers.begin (), random_peers.end ());
	return result;
}

void oslo::rep_crawler::query (std::vector<std::shared_ptr<oslo::transport::channel>> const & channels_a)
{
	auto transaction (node.store.tx_begin_read ());
	std::shared_ptr<oslo::block> block (node.store.block_random (transaction));
...
void oslo::rep_crawler::query (std::shared_ptr<oslo::transport::channel> const & channel_a)
{
	std::vector<std::shared_ptr<oslo::transport::channel>> peers;
	peers.emplace_back (channel_a);
	query (peers);
}

bool oslo::rep_crawler::is_pr (oslo::transport::channel const & channel_a) const
{
	oslo::lock_guard<std::mutex> lock (probable_reps_mutex);
...
bool oslo::rep_crawler::response (std::shared_ptr<oslo::transport::channel> & channel_a, std::shared_ptr<oslo::vote> & vote_a)
{
	bool error = true;
	oslo::lock_guard<std::mutex> lock (active_mutex);
...
oslo::uint128_t oslo::rep_crawler::total_weight () const
{
	oslo::lock_guard<std::mutex> lock (probable_reps_mutex);
...
void oslo::rep_crawler::on_rep_request (std::shared_ptr<oslo::transport::channel> channel_a)
{
	oslo::lock_guard<std::mutex> lock (probable_reps_mutex);
	if (channel_a->get_tcp_endpoint ().address () != boost::asio::ip::address_v6::any ())
	{
		...
		{
			channel_ref_index.modify (itr_pair.first, [](oslo::representative & value_a) {
...
void oslo::rep_crawler::cleanup_reps ()
{
	std::vector<std::shared_ptr<oslo::transport::channel>> channels;
	{
	...
	{
		bool equal (false);
		if (i->get_type () == oslo::transport::transport_type::tcp)
		{
			auto find_channel (node.network.tcp_channels.find_channel (i->get_tcp_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<oslo::transport::channel_tcp *> (i.get ()))
			{
				equal = true;
			}
		}
		else if (i->get_type () == oslo::transport::transport_type::udp)
		{
			auto find_channel (node.network.udp_channels.channel (i->get_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<oslo::transport::channel_udp *> (i.get ()))
			{
				equal = true;
			}
		}
...
void oslo::rep_crawler::update_weights ()
{
	oslo::lock_guard<std::mutex> lock (probable_reps_mutex);
	for (auto i (probable_reps.get<tag_last_request> ().begin ()), n (probable_reps.get<tag_last_request> ().end ()); i != n;)
	{
		...
		if (weight > 0)
		{
			...
			{
				probable_reps.get<tag_last_request> ().modify (i, [weight](oslo::representative & info) {
					info.weight = weight;
				});
			}
			++i;
...
std::vector<oslo::representative> oslo::rep_crawler::representatives (size_t count_a, oslo::uint128_t const weight_a, boost::optional<decltype (oslo::protocol_constants::protocol_version)> const & opt_version_min_a)
{
...
std::vector<oslo::representative> oslo::rep_crawler::principal_representatives (size_t count_a, boost::optional<decltype (oslo::protocol_constants::protocol_version)> const & opt_version_min_a)
{
	return representatives (count_a, node.minimum_principal_weight (), opt_version_min_a);
}

std::vector<std::shared_ptr<oslo::transport::channel>> oslo::rep_crawler::representative_endpoints (size_t count_a)
{
	std::vector<std::shared_ptr<oslo::transport::channel>> result;
...
size_t oslo::rep_crawler::representative_count ()
{
	oslo::lock_guard<std::mutex> lock (probable_reps_mutex);
