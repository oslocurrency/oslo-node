...
oslo::transport::channel_tcp::channel_tcp (oslo::node & node_a, std::weak_ptr<oslo::socket> socket_a) :
channel (node_a),
socket (socket_a)
{
}

oslo::transport::channel_tcp::~channel_tcp ()
{
...
size_t oslo::transport::channel_tcp::hash_code () const
{
...
bool oslo::transport::channel_tcp::operator== (oslo::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<oslo::transport::channel_tcp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void oslo::transport::channel_tcp::send_buffer (oslo::shared_const_buffer const & buffer_a, oslo::stat::detail detail_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a, oslo::buffer_drop_policy drop_policy_a)
{
	if (auto socket_l = socket.lock ())
	{
...
std::function<void(boost::system::error_code const &, size_t)> oslo::transport::channel_tcp::callback (oslo::stat::detail detail_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a) const
{
	return callback_a;
}

std::function<void(boost::system::error_code const &, size_t)> oslo::transport::channel_tcp::tcp_callback (oslo::stat::detail detail_a, oslo::tcp_endpoint const & endpoint_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a) const
{
	return [endpoint_a, node = std::weak_ptr<oslo::node> (node.shared ()), callback_a](boost::system::error_code const & ec, size_t size_a) {
		if (auto node_l = node.lock ())
		{
			...
			{
				node_l->stats.inc (oslo::stat::type::error, oslo::stat::detail::unreachable_host, oslo::stat::dir::out);
			}
			if (callback_a)
			{
...
std::string oslo::transport::channel_tcp::to_string () const
{
	if (auto socket_l = socket.lock ())
	{
...
oslo::transport::tcp_channels::tcp_channels (oslo::node & node_a) :
node (node_a)
{
}

bool oslo::transport::tcp_channels::insert (std::shared_ptr<oslo::transport::channel_tcp> channel_a, std::shared_ptr<oslo::socket> socket_a, std::shared_ptr<oslo::bootstrap_server> bootstrap_server_a)
{
	auto endpoint (channel_a->get_tcp_endpoint ());
	debug_assert (endpoint.address ().is_v6 ());
	auto udp_endpoint (oslo::transport::map_tcp_to_endpoint (endpoint));
	bool error (true);
...
void oslo::transport::tcp_channels::erase (oslo::tcp_endpoint const & endpoint_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

size_t oslo::transport::tcp_channels::size () const
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<oslo::transport::channel_tcp> oslo::transport::tcp_channels::find_channel (oslo::tcp_endpoint const & endpoint_a) const
{
	oslo::lock_guard<std::mutex> lock (mutex);
	std::shared_ptr<oslo::transport::channel_tcp> result;
...
std::unordered_set<std::shared_ptr<oslo::transport::channel>> oslo::transport::tcp_channels::random_set (size_t count_a, uint8_t min_version, bool include_temporary_channels_a) const
{
	std::unordered_set<std::shared_ptr<oslo::transport::channel>> result;
	...
	if (!channels.empty ())
	{
		...
		{
			auto index (oslo::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (peers_size - 1)));
...
void oslo::transport::tcp_channels::random_fill (std::array<oslo::endpoint, 8> & target_a) const
{
...
bool oslo::transport::tcp_channels::store_all (bool clear_peers)
{
	// We can't hold the mutex while starting a write transaction, so
	// we collect endpoints to be saved and then relase the lock.
	std::vector<oslo::endpoint> endpoints;
	{
		oslo::lock_guard<std::mutex> lock (mutex);
		endpoints.reserve (channels.size ());
		std::transform (channels.begin (), channels.end (),
		std::back_inserter (endpoints), [](const auto & channel) { return oslo::transport::map_tcp_to_endpoint (channel.endpoint ()); });
	}
	bool result (false);
	if (!endpoints.empty ())
	{
...
std::shared_ptr<oslo::transport::channel_tcp> oslo::transport::tcp_channels::find_node_id (oslo::account const & node_id_a)
{
	std::shared_ptr<oslo::transport::channel_tcp> result;
	oslo::lock_guard<std::mutex> lock (mutex);
...
oslo::tcp_endpoint oslo::transport::tcp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
...
void oslo::transport::tcp_channels::process_messages ()
{
	while (!stopped)
	{
...
void oslo::transport::tcp_channels::process_message (oslo::message const & message_a, oslo::tcp_endpoint const & endpoint_a, oslo::account const & node_id_a, std::shared_ptr<oslo::socket> socket_a, oslo::bootstrap_server_type type_a)
{
	if (!stopped && message_a.header.version_using >= protocol_constants ().protocol_version_min (node.ledger.cache.epoch_2_started))
	{
		auto channel (node.network.find_channel (oslo::transport::map_tcp_to_endpoint (endpoint_a)));
		if (channel)
		{
		...
		else
		{
			...
			else if (!node.network.excluded_peers.check (endpoint_a))
			{
				if (!node_id_a.is_zero ())
				{
					...
					socket_a->set_writer_concurrency (oslo::socket::concurrency::multi_writer);
					auto temporary_channel (std::make_shared<oslo::transport::channel_tcp> (node, socket_a));
					...
					debug_assert (type_a == oslo::bootstrap_server_type::realtime || type_a == oslo::bootstrap_server_type::realtime_response_server);
					// Don't insert temporary channels for response_server
					if (type_a == oslo::bootstrap_server_type::realtime)
					{
				...
				else
				{
					...
					debug_assert (message_a.header.type == oslo::message_type::node_id_handshake);
					debug_assert (type_a == oslo::bootstrap_server_type::undefined);
...
void oslo::transport::tcp_channels::start ()
{
	ongoing_keepalive ();
}

void oslo::transport::tcp_channels::stop ()
{
	stopped = true;
	oslo::unique_lock<std::mutex> lock (mutex);
...
bool oslo::transport::tcp_channels::max_ip_connections (oslo::tcp_endpoint const & endpoint_a)
{
	bool result (false);
...
bool oslo::transport::tcp_channels::reachout (oslo::endpoint const & endpoint_a)
{
	auto tcp_endpoint (oslo::transport::map_endpoint_to_tcp (endpoint_a));
...
std::unique_ptr<oslo::container_info_component> oslo::transport::tcp_channels::collect_container_info (std::string const & name)
{
...
void oslo::transport::tcp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
void oslo::transport::tcp_channels::ongoing_keepalive ()
{
	oslo::keepalive message;
	node.network.random_fill (message.peers);
	oslo::unique_lock<std::mutex> lock (mutex);
	// Wake up channels
	std::vector<std::shared_ptr<oslo::transport::channel_tcp>> send_list;
	...
	oslo::tcp_endpoint invalid_endpoint (boost::asio::ip::address_v6::any (), 0);
	if (!node.network_params.network.is_test_network () && !node.flags.disable_udp)
	{
		...
		for (auto i (0); i <= random_count; ++i)
		{
			...
			{
				start_tcp (oslo::transport::map_tcp_to_endpoint (tcp_endpoint));
			}
		}
	}
...
void oslo::transport::tcp_channels::list_below_version (std::vector<std::shared_ptr<oslo::transport::channel>> & channels_a, uint8_t cutoff_version_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	// clang-format off
	oslo::transform_if (channels.get<random_access_tag> ().begin (), channels.get<random_access_tag> ().end (), std::back_inserter (channels_a),
...
void oslo::transport::tcp_channels::list (std::deque<std::shared_ptr<oslo::transport::channel>> & deque_a, uint8_t minimum_version_a, bool include_temporary_channels_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	// clang-format off
	oslo::transform_if (channels.get<random_access_tag> ().begin (), channels.get<random_access_tag> ().end (), std::back_inserter (deque_a),
...
void oslo::transport::tcp_channels::modify (std::shared_ptr<oslo::transport::channel_tcp> channel_a, std::function<void(std::shared_ptr<oslo::transport::channel_tcp>)> modify_callback_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
void oslo::transport::tcp_channels::update (oslo::tcp_endpoint const & endpoint_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
	if (existing != channels.get<endpoint_tag> ().end ())
...
bool oslo::transport::tcp_channels::node_id_handhake_sockets_empty () const
{
	oslo::lock_guard<std::mutex> guard (mutex);
	return node_id_handshake_sockets.empty ();
}

void oslo::transport::tcp_channels::push_node_id_handshake_socket (std::shared_ptr<oslo::socket> const & socket_a)
{
	oslo::lock_guard<std::mutex> guard (mutex);
	node_id_handshake_sockets.push_back (socket_a);
}

void oslo::transport::tcp_channels::remove_node_id_handshake_socket (std::shared_ptr<oslo::socket> const & socket_a)
{
	std::weak_ptr<oslo::node> node_w (node.shared ());
	if (auto node_l = node_w.lock ())
	{
...
void oslo::transport::tcp_channels::start_tcp (oslo::endpoint const & endpoint_a, std::function<void(std::shared_ptr<oslo::transport::channel>)> const & callback_a)
{
	...
	auto socket (std::make_shared<oslo::socket> (node.shared_from_this (), boost::none, oslo::socket::concurrency::multi_writer));
	std::weak_ptr<oslo::socket> socket_w (socket);
	auto channel (std::make_shared<oslo::transport::channel_tcp> (node, socket_w));
	std::weak_ptr<oslo::node> node_w (node.shared ());
	socket->async_connect (oslo::transport::map_endpoint_to_tcp (endpoint_a),
	[node_w, channel, socket, endpoint_a, callback_a](boost::system::error_code const & ec) {
		if (auto node_l = node_w.lock ())
		{
			if (!ec && channel)
			{
				...
				oslo::node_id_handshake message (cookie, boost::none);
				...
				channel->send_buffer (bytes, oslo::stat::detail::node_id_handshake, [node_w, channel, endpoint_a, receive_buffer, callback_a](boost::system::error_code const & ec, size_t size_a) {
					if (auto node_l = node_w.lock ())
					{
						if (!ec && channel)
						{
...
void oslo::transport::tcp_channels::start_tcp_receive_node_id (std::shared_ptr<oslo::transport::channel_tcp> channel_a, oslo::endpoint const & endpoint_a, std::shared_ptr<std::vector<uint8_t>> receive_buffer_a, std::function<void(std::shared_ptr<oslo::transport::channel>)> const & callback_a)
{
	std::weak_ptr<oslo::node> node_w (node.shared ());
	if (auto socket_l = channel_a->socket.lock ())
	{
		auto cleanup_node_id_handshake_socket = [socket_w = channel_a->socket, node_w](oslo::endpoint const & endpoint_a, std::function<void(std::shared_ptr<oslo::transport::channel>)> const & callback_a) {
			if (auto node_l = node_w.lock ())
			{
		...
		auto cleanup_and_udp_fallback = [socket_w = channel_a->socket, node_w, cleanup_node_id_handshake_socket](oslo::endpoint const & endpoint_a, std::function<void(std::shared_ptr<oslo::transport::channel>)> const & callback_a) {
			if (auto node_l = node_w.lock ())
			{
				node_l->network.tcp_channels.udp_fallback (endpoint_a, callback_a);
		...
		socket_l->async_read (receive_buffer_a, 8 + sizeof (oslo::account) + sizeof (oslo::account) + sizeof (oslo::signature), [node_w, channel_a, endpoint_a, receive_buffer_a, callback_a, cleanup_and_udp_fallback, cleanup_node_id_handshake_socket](boost::system::error_code const & ec, size_t size_a) {
			if (auto node_l = node_w.lock ())
			{
				if (!ec && channel_a)
				{
					node_l->stats.inc (oslo::stat::type::message, oslo::stat::detail::node_id_handshake, oslo::stat::dir::in);
					auto error (false);
					oslo::bufferstream stream (receive_buffer_a->data (), size_a);
					oslo::message_header header (error, stream);
					if (!error && header.type == oslo::message_type::node_id_handshake)
					{
						if (header.version_using >= node_l->network_params.protocol.protocol_version_min (node_l->ledger.cache.epoch_2_started))
						{
							oslo::node_id_handshake message (error, stream, header);
							if (!error && message.response && message.query)
							{
								...
								if (process)
								{
									...
									boost::optional<std::pair<oslo::account, oslo::signature>> response (std::make_pair (node_l->node_id.pub, oslo::sign_message (node_l->node_id.prv, node_l->node_id.pub, *message.query)));
									oslo::node_id_handshake response_message (boost::none, response);
									...
									channel_a->send_buffer (bytes, oslo::stat::detail::node_id_handshake, [node_w, channel_a, endpoint_a, callback_a, cleanup_and_udp_fallback](boost::system::error_code const & ec, size_t size_a) {
										if (auto node_l = node_w.lock ())
										{
											if (!ec && channel_a)
											{
												...
												if (auto socket_l = channel_a->socket.lock ())
												{
													...
													auto response_server = std::make_shared<oslo::bootstrap_server> (socket_l, node_l);
													...
													response_server->type = oslo::bootstrap_server_type::realtime_response_server;
													...
													{
														node_l->telemetry->get_metrics_single_peer_async (channel_a, [](oslo::telemetry_data_response /* unused */) {
						...
						else
						{
							...
							cleanup_node_id_handshake_socket (endpoint_a, callback_a);
							{
								oslo::lock_guard<std::mutex> lock (node_l->network.tcp_channels.mutex);
								node_l->network.tcp_channels.attempts.get<endpoint_tag> ().erase (oslo::transport::map_endpoint_to_tcp (endpoint_a));
							}
						}
					}
					else
...
void oslo::transport::tcp_channels::udp_fallback (oslo::endpoint const & endpoint_a, std::function<void(std::shared_ptr<oslo::transport::channel>)> const & callback_a)
{
	{
		oslo::lock_guard<std::mutex> lock (mutex);
		attempts.get<endpoint_tag> ().erase (oslo::transport::map_endpoint_to_tcp (endpoint_a));
	}
