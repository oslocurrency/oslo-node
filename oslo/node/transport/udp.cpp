...
oslo::transport::channel_udp::channel_udp (oslo::transport::udp_channels & channels_a, oslo::endpoint const & endpoint_a, uint8_t protocol_version_a) :
...
size_t oslo::transport::channel_udp::hash_code () const
{
	std::hash<::oslo::endpoint> hash;
	return hash (endpoint);
}

bool oslo::transport::channel_udp::operator== (oslo::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<oslo::transport::channel_udp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void oslo::transport::channel_udp::send_buffer (oslo::shared_const_buffer const & buffer_a, oslo::stat::detail detail_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a, oslo::buffer_drop_policy drop_policy_a)
{
	set_last_packet_sent (std::chrono::steady_clock::now ());
	channels.send (buffer_a, endpoint, callback (detail_a, callback_a));
}

std::function<void(boost::system::error_code const &, size_t)> oslo::transport::channel_udp::callback (oslo::stat::detail detail_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a) const
{
	return [node = std::weak_ptr<oslo::node> (channels.node.shared ()), callback_a](boost::system::error_code const & ec, size_t size_a) {
		if (auto node_l = node.lock ())
		{
			...
			if (size_a > 0)
			{
				node_l->stats.add (oslo::stat::type::traffic_udp, oslo::stat::dir::out, size_a);
			}
...
std::string oslo::transport::channel_udp::to_string () const
{
	return boost::str (boost::format ("%1%") % endpoint);
}

oslo::transport::udp_channels::udp_channels (oslo::node & node_a, uint16_t port_a) :
node (node_a),
strand (node_a.io_ctx.get_executor ())
{
	if (!node.flags.disable_udp)
	{
		socket = std::make_unique<boost::asio::ip::udp::socket> (node_a.io_ctx, oslo::endpoint (boost::asio::ip::address_v6::any (), port_a));
		...
		local_endpoint = oslo::endpoint (boost::asio::ip::address_v6::loopback (), port);
	}
	else
	{
		local_endpoint = oslo::endpoint (boost::asio::ip::address_v6::loopback (), 0);
		stopped = true;
	}
}

void oslo::transport::udp_channels::send (oslo::shared_const_buffer const & buffer_a, oslo::endpoint endpoint_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a)
{
...
std::shared_ptr<oslo::transport::channel_udp> oslo::transport::udp_channels::insert (oslo::endpoint const & endpoint_a, unsigned network_version_a)
{
	debug_assert (endpoint_a.address ().is_v6 ());
	std::shared_ptr<oslo::transport::channel_udp> result;
	if (!node.network.not_a_peer (endpoint_a, node.config.allow_local_peers) && (node.network_params.network.is_test_network () || !max_ip_connections (endpoint_a)))
	{
		...
		{
			result = std::make_shared<oslo::transport::channel_udp> (*this, endpoint_a, network_version_a);
...
void oslo::transport::udp_channels::erase (oslo::endpoint const & endpoint_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

size_t oslo::transport::udp_channels::size () const
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<oslo::transport::channel_udp> oslo::transport::udp_channels::channel (oslo::endpoint const & endpoint_a) const
{
	oslo::lock_guard<std::mutex> lock (mutex);
	std::shared_ptr<oslo::transport::channel_udp> result;
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
...
std::unordered_set<std::shared_ptr<oslo::transport::channel>> oslo::transport::udp_channels::random_set (size_t count_a, uint8_t min_version) const
{
...
void oslo::transport::udp_channels::random_fill (std::array<oslo::endpoint, 8> & target_a) const
{
...
bool oslo::transport::udp_channels::store_all (bool clear_peers)
{
...
std::shared_ptr<oslo::transport::channel_udp> oslo::transport::udp_channels::find_node_id (oslo::account const & node_id_a)
{
	std::shared_ptr<oslo::transport::channel_udp> result;
	oslo::lock_guard<std::mutex> lock (mutex);
...
void oslo::transport::udp_channels::clean_node_id (oslo::account const & node_id_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	channels.get<node_id_tag> ().erase (node_id_a);
}

void oslo::transport::udp_channels::clean_node_id (oslo::endpoint const & endpoint_a, oslo::account const & node_id_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
oslo::tcp_endpoint oslo::transport::udp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
	...
	for (auto i (channels.get<last_bootstrap_attempt_tag> ().begin ()), n (channels.get<last_bootstrap_attempt_tag> ().end ()); i != n;)
	{
		...
		{
			result = oslo::transport::map_endpoint_to_tcp (i->endpoint ());
...
void oslo::transport::udp_channels::receive ()
{
	if (!stopped)
	{
		...
		socket->async_receive_from (boost::asio::buffer (data->buffer, oslo::network::buffer_size), data->endpoint,
...
void oslo::transport::udp_channels::start ()
{
...
void oslo::transport::udp_channels::stop ()
{
...
void oslo::transport::udp_channels::close_socket ()
{
...
oslo::endpoint oslo::transport::udp_channels::get_local_endpoint () const
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
namespace
{
class udp_message_visitor : public oslo::message_visitor
{
public:
	udp_message_visitor (oslo::node & node_a, oslo::endpoint const & endpoint_a) :
	node (node_a),
	endpoint (endpoint_a)
	{
	}
	void keepalive (oslo::keepalive const & message_a) override
	{
		if (!node.network.udp_channels.max_ip_connections (endpoint))
		{
			...
			if (cookie)
			{
				...
				else if (!node.network.tcp_channels.find_channel (oslo::transport::map_endpoint_to_tcp (endpoint)))
				{
					// Don't start connection if TCP channel to same IP:port exists
					find_channel = std::make_shared<oslo::transport::channel_udp> (node.network.udp_channels, endpoint, node.network_params.protocol.protocol_version);
					node.network.send_node_id_handshake (find_channel, *cookie, boost::none);
				}
			}
			...
			{
				oslo::endpoint new_endpoint (endpoint.address (), peer0.port ());
	...
	void telemetry_req (oslo::telemetry_req const & message_a) override
	{
		...
		if (find_channel)
		{
			...
			auto cache_exceeded = std::chrono::steady_clock::now () >= find_channel->get_last_telemetry_req () + oslo::telemetry_cache_cutoffs::network_to_time (node.network_params.network);
			if (is_very_first_message || cache_exceeded)
			{
				node.network.udp_channels.modify (find_channel, [](std::shared_ptr<oslo::transport::channel_udp> channel_a) {
	...
	void node_id_handshake (oslo::node_id_handshake const & message_a) override
	{
		...
		boost::optional<oslo::uint256_union> out_query;
		boost::optional<oslo::uint256_union> out_respond_to;
		if (message_a.query)
		{
		...
		if (message_a.response)
		{
			if (!node.network.syn_cookies.validate (endpoint, message_a.response->first, message_a.response->second))
			{
				...
				if (message_a.response->first != node.node_id.pub && !node.network.tcp_channels.find_node_id (message_a.response->first))
				{
					...
					if (new_channel)
					{
						node.network.udp_channels.modify (new_channel, [&message_a](std::shared_ptr<oslo::transport::channel_udp> channel_a) {
	...
	void message (oslo::message const & message_a)
	{
		auto find_channel (node.network.udp_channels.channel (endpoint));
		if (find_channel)
		{
...
void oslo::transport::udp_channels::receive_action (oslo::message_buffer * data_a)
{
	...
	else if (oslo::transport::reserved_address (data_a->endpoint, node.config.allow_local_peers))
	{
		allowed_sender = false;
	}
	if (allowed_sender)
	{
		...
		oslo::message_parser parser (node.network.publish_filter, node.block_uniquer, node.vote_uniquer, visitor, node.work, node.ledger.cache.epoch_2_started);
		parser.deserialize_buffer (data_a->buffer, data_a->size);
		if (parser.status == oslo::message_parser::parse_status::success)
		{
			node.stats.add (oslo::stat::type::traffic_udp, oslo::stat::dir::in, data_a->size);
		}
		else if (parser.status == oslo::message_parser::parse_status::duplicate_publish_message)
		{
			node.stats.inc (oslo::stat::type::filter, oslo::stat::detail::duplicate_publish);
		}
		else
		{
			node.stats.inc (oslo::stat::type::error);

			switch (parser.status)
			{
				case oslo::message_parser::parse_status::insufficient_work:
					// We've already increment error count, update detail only
					node.stats.inc_detail_only (oslo::stat::type::error, oslo::stat::detail::insufficient_work);
					break;
				case oslo::message_parser::parse_status::invalid_magic:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_magic);
					break;
				case oslo::message_parser::parse_status::invalid_network:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_network);
					break;
				case oslo::message_parser::parse_status::invalid_header:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_header);
					break;
				case oslo::message_parser::parse_status::invalid_message_type:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_message_type);
					break;
				case oslo::message_parser::parse_status::invalid_keepalive_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_keepalive_message);
					break;
				case oslo::message_parser::parse_status::invalid_publish_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_publish_message);
					break;
				case oslo::message_parser::parse_status::invalid_confirm_req_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_confirm_req_message);
					break;
				case oslo::message_parser::parse_status::invalid_confirm_ack_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_confirm_ack_message);
					break;
				case oslo::message_parser::parse_status::invalid_node_id_handshake_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_node_id_handshake_message);
					break;
				case oslo::message_parser::parse_status::invalid_telemetry_req_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_telemetry_req_message);
					break;
				case oslo::message_parser::parse_status::invalid_telemetry_ack_message:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::invalid_telemetry_ack_message);
					break;
				case oslo::message_parser::parse_status::outdated_version:
					node.stats.inc (oslo::stat::type::udp, oslo::stat::detail::outdated_version);
					break;
	...
	else
	{
		...
		node.stats.inc_detail_only (oslo::stat::type::error, oslo::stat::detail::bad_sender);
	}
}

void oslo::transport::udp_channels::process_packets ()
{
	while (!stopped)
	{
...
std::shared_ptr<oslo::transport::channel> oslo::transport::udp_channels::create (oslo::endpoint const & endpoint_a)
{
	return std::make_shared<oslo::transport::channel_udp> (*this, endpoint_a, node.network_params.protocol.protocol_version);
}

bool oslo::transport::udp_channels::max_ip_connections (oslo::endpoint const & endpoint_a)
{
	bool result (false);
...
bool oslo::transport::udp_channels::reachout (oslo::endpoint const & endpoint_a)
{
	// Don't overload single IP
	bool error = max_ip_connections (endpoint_a);
	if (!error && !node.flags.disable_udp)
	{
		auto endpoint_l (oslo::transport::map_endpoint_to_v6 (endpoint_a));
...
std::unique_ptr<oslo::container_info_component> oslo::transport::udp_channels::collect_container_info (std::string const & name)
{
...
void oslo::transport::udp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
void oslo::transport::udp_channels::ongoing_keepalive ()
{
	oslo::keepalive message;
	node.network.random_fill (message.peers);
	std::vector<std::shared_ptr<oslo::transport::channel_udp>> send_list;
	oslo::unique_lock<std::mutex> lock (mutex);
...
void oslo::transport::udp_channels::list_below_version (std::vector<std::shared_ptr<oslo::transport::channel>> & channels_a, uint8_t cutoff_version_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	// clang-format off
...
void oslo::transport::udp_channels::list (std::deque<std::shared_ptr<oslo::transport::channel>> & deque_a, uint8_t minimum_version_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	// clang-format off
...
void oslo::transport::udp_channels::modify (std::shared_ptr<oslo::transport::channel_udp> channel_a, std::function<void(std::shared_ptr<oslo::transport::channel_udp>)> modify_callback_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
