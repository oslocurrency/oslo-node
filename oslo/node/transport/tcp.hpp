...
{
public:
	std::shared_ptr<oslo::message> message;
	oslo::tcp_endpoint endpoint;
	oslo::account node_id;
	std::shared_ptr<oslo::socket> socket;
	oslo::bootstrap_server_type type;
};
namespace transport
{
	class tcp_channels;
	class channel_tcp : public oslo::transport::channel
	{
		friend class oslo::transport::tcp_channels;

	public:
		channel_tcp (oslo::node &, std::weak_ptr<oslo::socket>);
		~channel_tcp ();
		...
		std::function<void(boost::system::error_code const &, size_t)> tcp_callback (oslo::stat::detail, oslo::tcp_endpoint const &, std::function<void(boost::system::error_code const &, size_t)> const & = nullptr) const;
		std::string to_string () const override;
		bool operator== (oslo::transport::channel_tcp const & other_a) const
		{
			return &node == &other_a.node && socket.lock () == other_a.socket.lock ();
		}
		std::weak_ptr<oslo::socket> socket;
		std::weak_ptr<oslo::bootstrap_server> response_server;
		...
		oslo::endpoint get_endpoint () const override
		{
			...
			{
				return oslo::transport::map_tcp_to_endpoint (socket_l->remote_endpoint ());
			}
			else
			{
				return oslo::endpoint (boost::asio::ip::address_v6::any (), 0);
			}
		}
		...
		oslo::tcp_endpoint get_tcp_endpoint () const override
		{
			...
			{
				return oslo::tcp_endpoint (boost::asio::ip::address_v6::any (), 0);
			}
		}

		oslo::transport::transport_type get_type () const override
		{
			return oslo::transport::transport_type::tcp;
		}
	};
	class tcp_channels final
	{
		friend class oslo::transport::channel_tcp;
		friend class telemetry_simultaneous_requests_Test;

	public:
		tcp_channels (oslo::node &);
		bool insert (std::shared_ptr<oslo::transport::channel_tcp>, std::shared_ptr<oslo::socket>, std::shared_ptr<oslo::bootstrap_server>);
		void erase (oslo::tcp_endpoint const &);
		size_t size () const;
		std::shared_ptr<oslo::transport::channel_tcp> find_channel (oslo::tcp_endpoint const &) const;
		...
		std::shared_ptr<oslo::transport::channel_tcp> find_node_id (oslo::account const &);
		...
		void process_message (oslo::message const &, oslo::tcp_endpoint const &, oslo::account const &, std::shared_ptr<oslo::socket>, oslo::bootstrap_server_type);
		bool max_ip_connections (oslo::tcp_endpoint const &);
		...
		void list (std::deque<std::shared_ptr<oslo::transport::channel>> &, uint8_t = 0, bool = true);
		void modify (std::shared_ptr<oslo::transport::channel_tcp>, std::function<void(std::shared_ptr<oslo::transport::channel_tcp>)>);
		void update (oslo::tcp_endpoint const &);
		// Connection start
		void start_tcp (oslo::endpoint const &, std::function<void(std::shared_ptr<oslo::transport::channel>)> const & = nullptr);
		void start_tcp_receive_node_id (std::shared_ptr<oslo::transport::channel_tcp>, oslo::endpoint const &, std::shared_ptr<std::vector<uint8_t>>, std::function<void(std::shared_ptr<oslo::transport::channel>)> const &);
		void udp_fallback (oslo::endpoint const &, std::function<void(std::shared_ptr<oslo::transport::channel>)> const &);
		void push_node_id_handshake_socket (std::shared_ptr<oslo::socket> const & socket_a);
		void remove_node_id_handshake_socket (std::shared_ptr<oslo::socket> const & socket_a);
	...
	private:
		...
		public:
			...
			std::shared_ptr<oslo::socket> socket;
			std::shared_ptr<oslo::bootstrap_server> response_server;
			channel_tcp_wrapper (std::shared_ptr<oslo::transport::channel_tcp> const & channel_a, std::shared_ptr<oslo::socket> const & socket_a, std::shared_ptr<oslo::bootstrap_server> const & server_a) :
			channel (channel_a), socket (socket_a), response_server (server_a)
			{
			}
			oslo::tcp_endpoint endpoint () const
			{
		...
		public:
			...
			explicit tcp_endpoint_attempt (oslo::tcp_endpoint const & endpoint_a) :
			endpoint (endpoint_a),
		...
		mi::indexed_by<
			...
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::const_mem_fun<channel_tcp_wrapper, oslo::tcp_endpoint, &channel_tcp_wrapper::endpoint>>,
			mi::hashed_non_unique<mi::tag<node_id_tag>,
				mi::const_mem_fun<channel_tcp_wrapper, oslo::account, &channel_tcp_wrapper::node_id>>,
		...
		mi::indexed_by<
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::member<tcp_endpoint_attempt, oslo::tcp_endpoint, &tcp_endpoint_attempt::endpoint>>,
			mi::hashed_non_unique<mi::tag<ip_address_tag>,
		...
		std::vector<std::shared_ptr<oslo::socket>> node_id_handshake_sockets;
