...
#include <oslo/boost/beast/core.hpp>
#include <oslo/boost/beast/websocket.hpp>
...
namespace websocket
{
	...
	{
	public:
		message (oslo::websocket::topic topic_a) :
		topic (topic_a)
		{
		}
		message (oslo::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
		topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		oslo::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<oslo::block> block_a, oslo::account const & account_a, oslo::amount const & amount_a, std::string subtype, bool include_block, oslo::election_status const & election_status_a, oslo::websocket::confirmation_options const & options_a);
		message stopped_election (oslo::block_hash const & hash_a);
		message vote_received (std::shared_ptr<oslo::vote> vote_a, oslo::vote_code code_a);
		message difficulty_changed (uint64_t publish_threshold_a, uint64_t difficulty_active_a);
		message work_generation (oslo::work_version const version_a, oslo::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (oslo::work_version const version_a, oslo::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (oslo::work_version const version_a, oslo::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		...
		message telemetry_received (oslo::telemetry_data const &, oslo::endpoint const &);
		message new_block_arrived (oslo::block const & block_a);
	...
	{
	public:
		confirmation_options (oslo::wallets & wallets_a);
		confirmation_options (boost::property_tree::ptree const & options_a, oslo::wallets & wallets_a, oslo::logger_mt & logger_a);
	...
	private:
		...
		boost::optional<oslo::logger_mt &> logger;
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, oslo::logger_mt & logger_a);
	...
	public:
		...
		explicit session (oslo::websocket::listener & listener_a, socket_type socket_a);
		~session ();
		...
		void write (oslo::websocket::message message_a);

	private:
		/** The owning listener */
		oslo::websocket::listener & ws_listener;
		/** Websocket */
	...
	{
	public:
		listener (oslo::logger_mt & logger_a, oslo::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<oslo::block> block_a, oslo::account const & account_a, oslo::amount const & amount_a, std::string subtype, oslo::election_status const & election_status_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (oslo::websocket::message message_a);

		oslo::logger_mt & get_logger () const
		{
			return logger;
		}

		oslo::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (oslo::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		size_t subscriber_count (oslo::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend oslo::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (oslo::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (oslo::websocket::topic const & topic_a);
