...
__kernel void oslo_work (__global ulong const * attempt, __global ulong * result_a, __global uchar const * item_a, __global ulong const * difficulty_a)
{
...
oslo::opencl_environment::opencl_environment (bool & error_a)
{
...
void oslo::opencl_environment::dump (std::ostream & stream)
{
	auto index (0);
...
oslo::opencl_work::opencl_work (bool & error_a, oslo::opencl_config const & config_a, oslo::opencl_environment & environment_a, oslo::logger_mt & logger_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logger (logger_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			oslo::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (oslo::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "oslo_work", &kernel_error);
...
boost::optional<uint64_t> oslo::opencl_work::generate_work (oslo::work_version const version_a, oslo::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (version_a, root_a, difficulty_a, ticket_l);
}

boost::optional<uint64_t> oslo::opencl_work::generate_work (oslo::work_version const version_a, oslo::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while (oslo::work_difficulty (version_a, root_a, result) < difficulty_a && !error && ticket_a == ticket_l)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (oslo::root), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<oslo::opencl_work> oslo::opencl_work::create (bool create_a, oslo::opencl_config const & config_a, oslo::logger_mt & logger_a)
{
	std::unique_ptr<oslo::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new oslo::opencl_work (error, config_a, environment, logger_a));
			if (error)
			{
				result.reset ();
			}
