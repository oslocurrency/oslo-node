...
#include <oslo/node/confirmation_height_unbounded.hpp>
...
void oslo::confirmation_height_unbounded::process ()
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	std::vector<oslo::block_hash> orig_block_callback_data;
	...
	do
	{
		...
		std::vector<oslo::block_hash> block_callback_datas_required;
...
void oslo::confirmation_height_unbounded::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, oslo::block_hash const & hash_a, oslo::account const & account_a, oslo::read_transaction const & transaction_a, std::vector<receive_source_pair> & receive_source_pairs_a, std::vector<oslo::block_hash> & block_callback_data_a, std::vector<oslo::block_hash> & orig_block_callback_data_a)
{
	auto hash (hash_a);
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.source_exists (transaction_a, source))
			{
				...
				receive_source_pairs_a.emplace_back (std::make_shared<conf_height_details> (account_a, hash, block_height, 1, std::vector<oslo::block_hash>{ hash }), source);
			}
...
void oslo::confirmation_height_unbounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
 ...
 */
void oslo::confirmation_height_unbounded::cement_blocks (oslo::write_guard & scoped_write_guard_a)
{
	oslo::timer<std::chrono::milliseconds> cemented_batch_timer;
	...
	auto error = false;
	{
		...
		while (!pending_writes.empty ())
		{
			...
			if (!error && pending.height > confirmation_height)
			{
				...
				ledger.stats.add (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in, pending.height - confirmation_height);
				ledger.stats.add (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_unbounded, oslo::stat::dir::in, pending.height - confirmation_height);
...
std::shared_ptr<oslo::block> oslo::confirmation_height_unbounded::get_block_and_sideband (oslo::block_hash const & hash_a, oslo::transaction const & transaction_a)
{
...
bool oslo::confirmation_height_unbounded::pending_empty () const
{
	return pending_writes.empty ();
}

void oslo::confirmation_height_unbounded::clear_process_vars ()
{
...
oslo::confirmation_height_unbounded::conf_height_details::conf_height_details (oslo::account const & account_a, oslo::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<oslo::block_hash> const & block_callback_data_a) :
account (account_a),
hash (hash_a),
height (height_a),
...
oslo::confirmation_height_unbounded::receive_source_pair::receive_source_pair (std::shared_ptr<conf_height_details> const & receive_details_a, const block_hash & source_a) :
receive_details (receive_details_a),
source_hash (source_a)
{
}

oslo::confirmation_height_unbounded::confirmed_iterated_pair::confirmed_iterated_pair (uint64_t confirmed_height_a, uint64_t iterated_height_a) :
confirmed_height (confirmed_height_a),
iterated_height (iterated_height_a)
{
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (confirmation_height_unbounded & confirmation_height_unbounded, const std::string & name_a)
{
