...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<oslo::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (oslo::tcp_endpoint const &, size_t const);
	bool check (oslo::tcp_endpoint const &);
	void remove (oslo::tcp_endpoint const &);
