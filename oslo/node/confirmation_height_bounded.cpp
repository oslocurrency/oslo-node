...
#include <oslo/node/confirmation_height_bounded.hpp>
...
oslo::confirmation_height_bounded::confirmation_height_bounded (oslo::ledger & ledger_a, oslo::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, oslo::logger_mt & logger_a, std::atomic<bool> & stopped_a, oslo::block_hash const & original_hash_a, uint64_t & batch_write_size_a, std::function<void(std::vector<std::shared_ptr<oslo::block>> const &)> const & notify_observers_callback_a, std::function<void(oslo::block_hash const &)> const & notify_block_already_cemented_observers_callback_a, std::function<uint64_t ()> const & awaiting_processing_size_callback_a) :
ledger (ledger_a),
...
oslo::confirmation_height_bounded::top_and_next_hash oslo::confirmation_height_bounded::get_next_block (boost::optional<top_and_next_hash> const & next_in_receive_chain_a, boost::circular_buffer_space_optimized<oslo::block_hash> const & checkpoints_a, boost::circular_buffer_space_optimized<receive_source_pair> const & receive_source_pairs, boost::optional<receive_chain_details> & receive_details_a)
{
...
void oslo::confirmation_height_bounded::process ()
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	boost::circular_buffer_space_optimized<oslo::block_hash> checkpoints{ max_items };
	...
	do
	{
		...
		oslo::account account (block->account ());
		if (account.is_zero ())
		{
		...
		if (!hit_receive || (receive_source_pairs.size () == 1 && top_most_non_receive_block_hash != current))
		{
			...
			if ((max_batch_write_size_reached || should_output || force_write) && !pending_writes.empty ())
			{
				...
				if (write_database_queue.process (oslo::writer::confirmation_height))
				{
...
oslo::block_hash oslo::confirmation_height_bounded::get_least_unconfirmed_hash_from_top_level (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a, oslo::account const & account_a, oslo::confirmation_height_info const & confirmation_height_info_a, uint64_t & block_height_a)
{
	oslo::block_hash least_unconfirmed_hash = hash_a;
...
bool oslo::confirmation_height_bounded::iterate (oslo::read_transaction const & transaction_a, uint64_t bottom_height_a, oslo::block_hash const & bottom_hash_a, boost::circular_buffer_space_optimized<oslo::block_hash> & checkpoints_a, oslo::block_hash & top_most_non_receive_block_hash_a, oslo::block_hash const & top_level_hash_a, boost::circular_buffer_space_optimized<receive_source_pair> & receive_source_pairs_a, oslo::account const & account_a)
{
	...
	while (!hash.is_zero () && !reached_target && !stopped)
	{
		...
		if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.source_exists (transaction_a, source))
		{
			...
			auto next = !sideband.successor.is_zero () && sideband.successor != top_level_hash_a ? boost::optional<oslo::block_hash> (sideband.successor) : boost::none;
...
void oslo::confirmation_height_bounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
...
void oslo::confirmation_height_bounded::cement_blocks (oslo::write_guard & scoped_write_guard_a)
{
	...
	std::vector<std::shared_ptr<oslo::block>> cemented_blocks;
	...
	oslo::timer<> cemented_batch_timer;
	auto error = false;
	{
		// This only writes to the confirmation_height table and is the only place to do so in a single process
		auto transaction (ledger.store.tx_begin_write ({}, { oslo::tables::confirmation_height }));
		...
		while (!error && !pending_writes.empty ())
		{
			...
			auto write_confirmation_height = [&account, &ledger = ledger, &transaction](uint64_t num_blocks_cemented, uint64_t confirmation_height, oslo::block_hash const & confirmed_frontier) {
#ifndef NDEBUG
				// Extra debug checks
				oslo::confirmation_height_info confirmation_height_info;
...
#endif
				ledger.store.confirmation_height_put (transaction, account, oslo::confirmation_height_info{ confirmation_height, confirmed_frontier });
				ledger.cache.cemented_count += num_blocks_cemented;
				ledger.stats.add (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in, num_blocks_cemented);
				ledger.stats.add (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_bounded, oslo::stat::dir::in, num_blocks_cemented);
			};
			...
			if (!error && pending.top_height > confirmation_height_info.height)
			{
				...
				oslo::block_hash new_cemented_frontier;
				...
				for (auto num_blocks_iterated = 0; num_blocks_confirmed - num_blocks_iterated != 0; ++num_blocks_iterated)
				{
					...
					if (cemented_blocks.size () > batch_write_size + (batch_write_size / 10))
					{
						...
						{
							scoped_write_guard_a = write_database_queue.wait (oslo::writer::confirmation_height);
							transaction.renew ();
						}
	...
	if (!network_params.network.is_test_network ())
	{
		...
		auto blocks_confirmed_stats = ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed);
		auto observer_stats = ledger.stats.count (oslo::stat::type::confirmation_observer, oslo::stat::detail::all, oslo::stat::dir::out);
...
bool oslo::confirmation_height_bounded::pending_empty () const
{
	return pending_writes.empty ();
}

void oslo::confirmation_height_bounded::clear_process_vars ()
{
	accounts_confirmed_info.clear ();
	accounts_confirmed_info_size = 0;
}

oslo::confirmation_height_bounded::receive_chain_details::receive_chain_details (oslo::account const & account_a, uint64_t height_a, oslo::block_hash const & hash_a, oslo::block_hash const & top_level_a, boost::optional<oslo::block_hash> next_a, uint64_t bottom_height_a, oslo::block_hash const & bottom_most_a) :
account (account_a),
height (height_a),
hash (hash_a),
...
oslo::confirmation_height_bounded::write_details::write_details (oslo::account const & account_a, uint64_t bottom_height_a, oslo::block_hash const & bottom_hash_a, uint64_t top_height_a, oslo::block_hash const & top_hash_a) :
account (account_a),
bottom_height (bottom_height_a),
...
oslo::confirmation_height_bounded::receive_source_pair::receive_source_pair (confirmation_height_bounded::receive_chain_details const & receive_details_a, const block_hash & source_a) :
receive_details (receive_details_a),
source_hash (source_a)
{
}

oslo::confirmation_height_bounded::confirmed_info::confirmed_info (uint64_t confirmed_height_a, oslo::block_hash const & iterated_frontier_a) :
confirmed_height (confirmed_height_a),
iterated_frontier (iterated_frontier_a)
{
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (confirmation_height_bounded & confirmation_height_bounded, const std::string & name_a)
{
