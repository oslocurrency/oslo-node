...
inline std::string get_default_pow_server_filepath ()
{
	...
	// Construct the oslo_pow_server executable file path based on where the currently running executable is found.
	auto pow_server_filepath = running_executable_filepath.parent_path () / "oslo_pow_server";
...
public:
	...
	bool enable{ false };
	std::string pow_server_path{ oslo::get_default_pow_server_filepath () };
};
}
