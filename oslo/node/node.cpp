...
double constexpr oslo::node::price_max;
double constexpr oslo::node::free_cutoff;
size_t constexpr oslo::block_arrival::arrival_size_min;
std::chrono::seconds constexpr oslo::block_arrival::arrival_time_min;

namespace oslo
{
extern unsigned char oslo_bootstrap_weights_live[];
extern size_t oslo_bootstrap_weights_live_size;
extern unsigned char oslo_bootstrap_weights_beta[];
extern size_t oslo_bootstrap_weights_beta_size;
}

void oslo::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a](boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				auto endpoint (oslo::transport::map_endpoint_to_v6 (i->endpoint ()));
				std::weak_ptr<oslo::node> node_w (node_l);
				auto channel (node_l->network.find_channel (endpoint));
				if (!channel)
				{
					node_l->network.tcp_channels.start_tcp (endpoint, [node_w](std::shared_ptr<oslo::transport::channel> channel_a) {
...
std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (rep_crawler & rep_crawler, const std::string & name)
{
	size_t count;
	{
		oslo::lock_guard<std::mutex> guard (rep_crawler.active_mutex);
...
oslo::node::node (boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, oslo::alarm & alarm_a, oslo::logging const & logging_a, oslo::work_pool & work_a, oslo::node_flags flags_a, unsigned seq) :
node (io_ctx_a, application_path_a, alarm_a, oslo::node_config (peering_port_a, logging_a), work_a, flags_a, seq)
{
}

oslo::node::node (boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, oslo::alarm & alarm_a, oslo::node_config const & config_a, oslo::work_pool & work_a, oslo::node_flags flags_a, unsigned seq) :
io_ctx (io_ctx_a),
...
store_impl (oslo::make_store (logger, application_path_a, flags.read_only, true, config_a.rocksdb_config, config_a.diagnostics_config.txn_tracking, config_a.block_processor_batch_max_time, config_a.lmdb_config, flags.sideband_batch_size, config_a.backup_before_upgrade, config_a.rocksdb_config.enable)),
store (*store_impl),
wallets_store_impl (std::make_unique<oslo::mdb_wallets_store> (application_path_a / "wallets.ldb", config_a.lmdb_config)),
...
telemetry (std::make_shared<oslo::telemetry> (network, alarm, worker, observers.telemetry, stats, network_params, flags.disable_ongoing_telemetry_requests)),
...
block_processor_thread ([this]() {
	oslo::thread_role::set (oslo::thread_role::name::block_processing);
...
node_seq (seq)
{
	if (!init_error ())
	{
		...
		{
			auto endpoint_l (oslo::tcp_endpoint (boost::asio::ip::make_address_v6 (config.websocket_config.address), config.websocket_config.port));
			websocket_server = std::make_shared<oslo::websocket::listener> (logger, wallets, io_ctx, endpoint_l);
		...
		network.channel_observer = [this](std::shared_ptr<oslo::transport::channel> channel_a) {
		...
		{
			observers.blocks.add ([this](oslo::election_status const & status_a, oslo::account const & account_a, oslo::amount const & amount_a, bool is_state_send_a) {
				auto block_a (status_a.winner);
				if ((status_a.type == oslo::election_status_type::active_confirmed_quorum || status_a.type == oslo::election_status_type::active_confirmation_height) && this->block_arrival.recent (block_a->hash ()))
				{
					auto node_l (shared_from_this ());
					background ([node_l, block_a, account_a, amount_a, is_state_send_a]() {
						...
						// Subtype field
						else if (block_a->type () == oslo::block_type::state)
						{
						...
						resolver->async_resolve (boost::asio::ip::tcp::resolver::query (address, std::to_string (port)), [node_l, address, port, target, body, resolver](boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
							...
							else
							{
								...
								node_l->stats.inc (oslo::stat::type::error, oslo::stat::detail::http_callback, oslo::stat::dir::out);
							}
						});
					});
				}
		...
		{
			observers.blocks.add ([this](oslo::election_status const & status_a, oslo::account const & account_a, oslo::amount const & amount_a, bool is_state_send_a) {
				debug_assert (status_a.type != oslo::election_status_type::ongoing);

				if (this->websocket_server->any_subscriber (oslo::websocket::topic::confirmation))
				{
					auto block_a (status_a.winner);
					std::string subtype;
					if (is_state_send_a)
			...
			observers.active_stopped.add ([this](oslo::block_hash const & hash_a) {
				if (this->websocket_server->any_subscriber (oslo::websocket::topic::stopped_election))
				{
			...
			observers.difficulty.add ([this](uint64_t active_difficulty) {
				if (this->websocket_server->any_subscriber (oslo::websocket::topic::active_difficulty))
				{
					oslo::websocket::message_builder builder;
					auto msg (builder.difficulty_changed (this->default_difficulty (oslo::work_version::work_1), active_difficulty));
					this->websocket_server->broadcast (msg);
				}
			});

			observers.telemetry.add ([this](oslo::telemetry_data const & telemetry_data, oslo::endpoint const & endpoint) {
				if (this->websocket_server->any_subscriber (oslo::websocket::topic::telemetry))
				{
					oslo::websocket::message_builder builder;
		...
		observers.blocks.add ([this](oslo::election_status const & status_a, oslo::account const & account_a, oslo::amount const & amount_a, bool is_state_send_a) {
			...
			{
				case oslo::election_status_type::active_confirmed_quorum:
					this->stats.inc (oslo::stat::type::confirmation_observer, oslo::stat::detail::active_quorum, oslo::stat::dir::out);
					break;
				case oslo::election_status_type::active_confirmation_height:
					this->stats.inc (oslo::stat::type::confirmation_observer, oslo::stat::detail::active_conf_height, oslo::stat::dir::out);
					break;
				case oslo::election_status_type::inactive_confirmation_height:
					this->stats.inc (oslo::stat::type::confirmation_observer, oslo::stat::detail::inactive_conf_height, oslo::stat::dir::out);
					break;
				default:
					break;
			}
		});
		observers.endpoint.add ([this](std::shared_ptr<oslo::transport::channel> channel_a) {
			if (channel_a->get_type () == oslo::transport::transport_type::udp)
			{
		...
		observers.vote.add ([this](std::shared_ptr<oslo::vote> vote_a, std::shared_ptr<oslo::transport::channel> channel_a, oslo::vote_code code_a) {
			debug_assert (code_a != oslo::vote_code::invalid);
			if (code_a != oslo::vote_code::replay)
			{
				auto active_in_rep_crawler (!this->rep_crawler.response (channel_a, vote_a));
				if (active_in_rep_crawler || code_a == oslo::vote_code::vote)
				{
					// Representative is defined as online if replying to live votes or rep_crawler queries
					this->online_reps.observe (vote_a->account);
				}
			}
			if (code_a == oslo::vote_code::indeterminate)
			{
		...
		{
			observers.vote.add ([this](std::shared_ptr<oslo::vote> vote_a, std::shared_ptr<oslo::transport::channel> channel_a, oslo::vote_code code_a) {
				if (this->websocket_server->any_subscriber (oslo::websocket::topic::vote))
				{
					oslo::websocket::message_builder builder;
		...
		observers.work_cancel.add ([this](oslo::root const & root_a) {
...
void oslo::node::do_rpc_callback (boost::asio::ip::tcp::resolver::iterator i_a, std::string const & address, uint16_t port, std::shared_ptr<std::string> target, std::shared_ptr<std::string> body, std::shared_ptr<boost::asio::ip::tcp::resolver> resolver)
{
					...
					if (!ec)
					{
						...
						boost::beast::http::async_read (*sock, *sb, *resp, [node_l, sb, resp, sock, address, port, i_a, target, body, resolver](boost::system::error_code const & ec, size_t bytes_transferred) mutable {
							if (!ec)
							{
								...
								{
									node_l->stats.inc (oslo::stat::type::http_callback, oslo::stat::detail::initiate, oslo::stat::dir::out);
								}
								else
								{
									if (node_l->config.logging.callback_logging ())
...
bool oslo::node::copy_with_compaction (boost::filesystem::path const & destination)
{
	return store.copy_db (destination);
}

void oslo::node::process_fork (oslo::transaction const & transaction_a, std::shared_ptr<oslo::block> block_a)
{
	auto root (block_a->root ());
	if (!store.block_exists (transaction_a, block_a->type (), block_a->hash ()) && store.root_exists (transaction_a, block_a->root ()))
	{
		std::shared_ptr<oslo::block> ledger_block (ledger.forked_block (transaction_a, *block_a));
		...
		{
			std::weak_ptr<oslo::node> this_w (shared_from_this ());
			auto election = active.insert (ledger_block, boost::none, [this_w, root](std::shared_ptr<oslo::block>) {
				if (auto this_l = this_w.lock ())
				{
					auto attempt (this_l->bootstrap_initiator.current_attempt ());
					if (attempt && attempt->mode == oslo::bootstrap_mode::legacy)
					{
						auto transaction (this_l->store.tx_begin_read ());
						auto account (this_l->ledger.store.frontier_get (transaction, root));
						if (!account.is_zero ())
						{
							this_l->bootstrap_initiator.connections->requeue_pull (oslo::pull_info (account, root, root, attempt->incremental_id));
						}
						else if (this_l->ledger.store.account_exists (transaction, root))
						{
							this_l->bootstrap_initiator.connections->requeue_pull (oslo::pull_info (root, oslo::block_hash (0), oslo::block_hash (0), attempt->incremental_id));
						}
					}
				}
			});
...
std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (node & node, const std::string & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
...
void oslo::node::process_active (std::shared_ptr<oslo::block> incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming, oslo::seconds_since_epoch ());
}

oslo::process_return oslo::node::process (oslo::block & block_a)
{
...
oslo::process_return oslo::node::process_local (std::shared_ptr<oslo::block> block_a, bool const work_watcher_a)
{
	...
	auto transaction (store.tx_begin_write ({ tables::accounts, tables::cached_counts, tables::change_blocks, tables::frontiers, tables::open_blocks, tables::pending, tables::receive_blocks, tables::representation, tables::send_blocks, tables::state_blocks }, { tables::confirmation_height }));
	return block_processor.process_one (transaction, events, info, work_watcher_a, oslo::block_origin::local);
}

void oslo::node::start ()
{
...
void oslo::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
...
oslo::block_hash oslo::node::latest (oslo::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.latest (transaction, account_a);
}

oslo::uint128_t oslo::node::balance (oslo::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.account_balance (transaction, account_a);
}

std::shared_ptr<oslo::block> oslo::node::block (oslo::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return store.block_get (transaction, hash_a);
}

std::pair<oslo::uint128_t, oslo::uint128_t> oslo::node::balance_pending (oslo::account const & account_a)
{
	std::pair<oslo::uint128_t, oslo::uint128_t> result;
	auto transaction (store.tx_begin_read ());
...
oslo::uint128_t oslo::node::weight (oslo::account const & account_a)
{
	return ledger.weight (account_a);
}

oslo::block_hash oslo::node::rep_block (oslo::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
...
oslo::uint128_t oslo::node::minimum_principal_weight ()
{
	return minimum_principal_weight (online_reps.online_stake ());
}

oslo::uint128_t oslo::node::minimum_principal_weight (oslo::uint128_t const & online_stake)
{
	return online_stake / network_params.network.principal_weight_factor;
}

void oslo::node::long_inactivity_cleanup ()
{
...
void oslo::node::ongoing_rep_calculation ()
{
	auto now (std::chrono::steady_clock::now ());
	vote_processor.calculate_weights ();
	std::weak_ptr<oslo::node> node_w (shared_from_this ());
...
void oslo::node::ongoing_bootstrap ()
{
...
void oslo::node::ongoing_store_flush ()
{
	{
...
void oslo::node::ongoing_peer_store ()
{
...
void oslo::node::backup_wallet ()
{
	...
	for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n; ++i)
	{
		...
		oslo::set_secure_perm_directory (backup_path, error_chmod);
...
void oslo::node::bootstrap_wallet ()
{
	std::deque<oslo::account> accounts;
	{
		oslo::lock_guard<std::mutex> lock (wallets.mutex);
		auto transaction (wallets.tx_begin_read ());
		for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n && accounts.size () < 128; ++i)
		{
			auto & wallet (*i->second);
			oslo::lock_guard<std::recursive_mutex> wallet_lock (wallet.store.mutex);
			for (auto j (wallet.store.begin (transaction)), m (wallet.store.end ()); j != m && accounts.size () < 128; ++j)
			{
				oslo::account account (j->first);
...
void oslo::node::unchecked_cleanup ()
{
	std::vector<oslo::uint128_t> digests;
	std::deque<oslo::unchecked_key> cleaning_list;
	...
	{
		auto now (oslo::seconds_since_epoch ());
		auto transaction (store.tx_begin_read ());
		// Max 1M records to clean, max 2 minutes reading to prevent slow i/o systems issues
		for (auto i (store.unchecked_begin (transaction)), n (store.unchecked_end ()); i != n && cleaning_list.size () < 1024 * 1024 && oslo::seconds_since_epoch () - now < 120; ++i)
		{
...
void oslo::node::ongoing_unchecked_cleanup ()
{
	unchecked_cleanup ();
	auto this_l (shared ());
...
int oslo::node::price (oslo::uint128_t const & balance_a, int amount_a)
{
	debug_assert (balance_a >= amount_a * oslo::Gxrb_ratio);
...
uint64_t oslo::node::default_difficulty (oslo::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case oslo::work_version::work_1:
			result = ledger.cache.epoch_2_started ? oslo::work_threshold_base (version_a) : network_params.network.publish_thresholds.epoch_1;
			break;
		default:
...
uint64_t oslo::node::max_work_generate_difficulty (oslo::work_version const version_a) const
{
	return oslo::difficulty::from_multiplier (config.max_work_generate_multiplier, default_difficulty (version_a));
}

bool oslo::node::local_work_generation_enabled () const
{
	return config.work_threads > 0 || work.opencl;
}

bool oslo::node::work_generation_enabled () const
{
	return work_generation_enabled (config.work_peers);
}

bool oslo::node::work_generation_enabled (std::vector<std::pair<std::string, uint16_t>> const & peers_a) const
{
	return !peers_a.empty () || local_work_generation_enabled ();
}

boost::optional<uint64_t> oslo::node::work_generate_blocking (oslo::block & block_a, uint64_t difficulty_a)
{
...
void oslo::node::work_generate (oslo::work_version const version_a, oslo::root const & root_a, uint64_t difficulty_a, std::function<void(boost::optional<uint64_t>)> callback_a, boost::optional<oslo::account> const & account_a, bool secondary_work_peers_a)
{
...
boost::optional<uint64_t> oslo::node::work_generate_blocking (oslo::work_version const version_a, oslo::root const & root_a, uint64_t difficulty_a, boost::optional<oslo::account> const & account_a)
{
...
boost::optional<uint64_t> oslo::node::work_generate_blocking (oslo::block & block_a)
{
	debug_assert (network_params.network.is_test_network ());
	return work_generate_blocking (block_a, default_difficulty (oslo::work_version::work_1));
}

boost::optional<uint64_t> oslo::node::work_generate_blocking (oslo::root const & root_a)
{
	debug_assert (network_params.network.is_test_network ());
	return work_generate_blocking (root_a, default_difficulty (oslo::work_version::work_1));
}

boost::optional<uint64_t> oslo::node::work_generate_blocking (oslo::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_params.network.is_test_network ());
	return work_generate_blocking (oslo::work_version::work_1, root_a, difficulty_a);
}

void oslo::node::add_initial_peers ()
{
	auto transaction (store.tx_begin_read ());
	for (auto i (store.peers_begin (transaction)), n (store.peers_end ()); i != n; ++i)
	{
		oslo::endpoint endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ());
		if (!network.reachout (endpoint, config.allow_local_peers))
		{
			std::weak_ptr<oslo::node> node_w (shared_from_this ());
			network.tcp_channels.start_tcp (endpoint, [node_w](std::shared_ptr<oslo::transport::channel> channel_a) {
				if (auto node_l = node_w.lock ())
				{
					node_l->network.send_keepalive (channel_a);
...
void oslo::node::block_confirm (std::shared_ptr<oslo::block> block_a)
{
...
bool oslo::node::block_confirmed (oslo::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return store.block_exists (transaction, hash_a) && ledger.block_confirmed (transaction, hash_a);
}

bool oslo::node::block_confirmed_or_being_confirmed (oslo::transaction const & transaction_a, oslo::block_hash const & hash_a)
{
	return confirmation_height_processor.is_processing_block (hash_a) || ledger.block_confirmed (transaction_a, hash_a);
}

oslo::uint128_t oslo::node::delta () const
{
	auto result ((online_reps.online_stake () / 100) * config.online_weight_quorum);
	return result;
}

void oslo::node::ongoing_online_weight_calculation_queue ()
{
	std::weak_ptr<oslo::node> node_w (shared_from_this ());
...
namespace
{
class confirmed_visitor : public oslo::block_visitor
{
public:
	confirmed_visitor (oslo::transaction const & transaction_a, oslo::node & node_a, std::shared_ptr<oslo::block> const & block_a, oslo::block_hash const & hash_a) :
	...
	void scan_receivable (oslo::account const & account_a)
	{
		for (auto i (node.wallets.items.begin ()), n (node.wallets.items.end ()); i != n; ++i)
		{
			...
			if (wallet->store.exists (transaction_l, account_a))
			{
				...
				auto error (node.store.pending_get (transaction, oslo::pending_key (account_a, hash), pending));
				if (!error)
				{
					auto node_l (node.shared ());
					auto amount (pending.amount.number ());
					wallet->receive_async (block, representative, amount, [](std::shared_ptr<oslo::block>) {});
				}
				else
				{
...
void oslo::node::receive_confirmed (oslo::transaction const & transaction_a, std::shared_ptr<oslo::block> block_a, oslo::block_hash const & hash_a)
{
	confirmed_visitor visitor (transaction_a, *this, block_a, hash_a);
	block_a->visit (visitor);
}

void oslo::node::process_confirmed_data (oslo::transaction const & transaction_a, std::shared_ptr<oslo::block> block_a, oslo::block_hash const & hash_a, oslo::account & account_a, oslo::uint128_t & amount_a, bool & is_state_send_a, oslo::account & pending_account_a)
{
	...
	if (auto state = dynamic_cast<oslo::state_block *> (block_a.get ()))
	{
	...
	if (auto send = dynamic_cast<oslo::send_block *> (block_a.get ()))
	{
		pending_account_a = send->hashables.destination;
	}
}

void oslo::node::process_confirmed (oslo::election_status const & status_a, uint64_t iteration_a)
{
	auto block_a (status_a.winner);
	...
	{
		iteration_a++;
		std::weak_ptr<oslo::node> node_w (shared ());
...
bool oslo::block_arrival::add (oslo::block_hash const & hash_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	auto inserted (arrival.get<tag_sequence> ().emplace_back (oslo::block_arrival_info{ now, hash_a }));
	auto result (!inserted.second);
	return result;
}

bool oslo::block_arrival::recent (oslo::block_hash const & hash_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (block_arrival & block_arrival, const std::string & name)
{
	size_t count = 0;
	{
		oslo::lock_guard<std::mutex> guard (block_arrival.mutex);
...
std::shared_ptr<oslo::node> oslo::node::shared ()
{
	return shared_from_this ();
}

int oslo::node::store_version ()
{
	auto transaction (store.tx_begin_read ());
	return store.version_get (transaction);
}

bool oslo::node::init_error () const
{
	return store.init_error () || wallets_store.init_error ();
}

bool oslo::node::epoch_upgrader (oslo::private_key const & prv_a, oslo::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	...
	if (!error)
	{
		...
		if (!error)
		{
			*epoch_upgrade = std::async (std::launch::async, &oslo::node::epoch_upgrader_impl, this, prv_a, epoch_a, count_limit, threads);
		}
	}
	return error;
}

void oslo::node::epoch_upgrader_impl (oslo::private_key const & prv_a, oslo::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	oslo::thread_role::set (oslo::thread_role::name::epoch_upgrader);
	auto upgrader_process = [](oslo::node & node_a, std::atomic<uint64_t> & counter, std::shared_ptr<oslo::block> epoch, uint64_t difficulty, oslo::public_key const & signer_a, oslo::root const & root_a, oslo::account const & account_a) {
		epoch->block_work_set (node_a.work_generate_blocking (oslo::work_version::work_1, root_a, difficulty).value_or (0));
		bool valid_signature (!oslo::validate_message (signer_a, epoch->hash (), epoch->block_signature ()));
		bool valid_work (epoch->difficulty () >= difficulty);
		oslo::process_result result (oslo::process_result::old);
		if (valid_signature && valid_work)
		{
			result = node_a.process_local (epoch).code;
		}
		if (result == oslo::process_result::progress)
		{
			++counter;
		}
		else
		{
			bool fork (result == oslo::process_result::fork);
	...
	oslo::raw_key raw_key;
	raw_key.data = prv_a;
	auto signer (oslo::pub_key (prv_a));
	debug_assert (signer == ledger.epoch_signer (link));

	std::mutex upgrader_mutex;
	oslo::condition_variable upgrader_condition;
	...
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>,
			boost::multi_index::member<account_upgrade_item, oslo::account, &account_upgrade_item::account>>>>
	accounts_list;
	// clang-format on
	...
	while (!finished_upgrade && !stopped)
	{
		...
		{
			{
				...
				for (auto i (store.latest_begin (transaction)), n (store.latest_end ()); i != n && accounts_list.size () < count_limit; ++i)
				{
					...
					{
						release_assert (oslo::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped; ++i)
			{
				auto transaction (store.tx_begin_read ());
				oslo::account_info info;
				oslo::account const & account (i->account);
				if (!store.account_get (transaction, account, info) && info.epoch () < epoch_a)
				{
					++attempts;
					auto difficulty (oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (epoch_a, false, false, true)));
					oslo::root const & root (info.head);
					std::shared_ptr<oslo::block> epoch = builder.state ()
					                                     .account (account)
					                                     .previous (info.head)
					...
					{
						{
							oslo::unique_lock<std::mutex> lock (upgrader_mutex);
							++workers;
						...
						worker.push_task ([node_l = shared_from_this (), &upgrader_process, &upgrader_mutex, &upgrader_condition, &upgraded_accounts, &workers, epoch, difficulty, signer, root, account]() {
							...
							{
								oslo::lock_guard<std::mutex> lock (upgrader_mutex);
								--workers;
							}
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			uint64_t attempts (0);
			auto transaction (store.tx_begin_read ());
			for (auto i (store.pending_begin (transaction, oslo::pending_key (1, 0))), n (store.pending_end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account_exists (transaction, key.account))
				{
					...
					if (info.epoch < epoch_a)
					{
						...
						release_assert (oslo::epochs::is_sequential (info.epoch, epoch_a));
						auto difficulty (oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (epoch_a, false, false, true)));
						oslo::root const & root (key.account);
						oslo::account const & account (key.account);
						std::shared_ptr<oslo::block> epoch = builder.state ()
						                                     .account (key.account)
						                                     .previous (0)
						                                     .representative (0)
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<oslo::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = store.pending_begin (transaction, oslo::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
...
std::pair<uint64_t, decltype (oslo::ledger::bootstrap_weights)> oslo::node::get_bootstrap_weights () const
{
	std::unordered_map<oslo::account, oslo::uint128_t> weights;
	const uint8_t * weight_buffer = network_params.network.is_live_network () ? oslo_bootstrap_weights_live : oslo_bootstrap_weights_beta;
	size_t weight_size = network_params.network.is_live_network () ? oslo_bootstrap_weights_live_size : oslo_bootstrap_weights_beta_size;
	oslo::bufferstream weight_stream ((const uint8_t *)weight_buffer, weight_size);
	oslo::uint128_union block_height;
	uint64_t max_blocks = 0;
	if (!oslo::try_read (weight_stream, block_height))
	{
		max_blocks = oslo::narrow_cast<uint64_t> (block_height.number ());
		while (true)
		{
			oslo::account account;
			if (oslo::try_read (weight_stream, account.bytes))
			{
				break;
			}
			oslo::amount weight;
			if (oslo::try_read (weight_stream, weight.bytes))
			{
				break;
			}
...
oslo::inactive_node::inactive_node (boost::filesystem::path const & path_a, oslo::node_flags const & node_flags_a) :
...
work (1)
{
	...
	oslo::set_secure_perm_directory (path_a, error_chmod);
	oslo::daemon_config daemon_config (path_a);
	auto error = oslo::read_node_config_toml (path_a, daemon_config, node_flags_a.config_overrides);
	if (error)
	{
	...
	node = std::make_shared<oslo::node> (*io_context, path_a, alarm, node_config, work, node_flags_a);
	node->active.stop ();
}

oslo::inactive_node::~inactive_node ()
{
	node->stop ();
}

oslo::node_flags const & oslo::inactive_node_flag_defaults ()
{
	static oslo::node_flags node_flags;
...
std::unique_ptr<oslo::block_store> oslo::make_store (oslo::logger_mt & logger, boost::filesystem::path const & path, bool read_only, bool add_db_postfix, oslo::rocksdb_config const & rocksdb_config, oslo::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, oslo::lmdb_config const & lmdb_config_a, size_t batch_size, bool backup_before_upgrade, bool use_rocksdb_backend)
{
#if OSLO_ROCKSDB
	auto make_rocksdb = [&logger, add_db_postfix, &path, &rocksdb_config, read_only]() {
		return std::make_unique<oslo::rocksdb_store> (logger, add_db_postfix ? path / "rocksdb" : path, rocksdb_config, read_only);
	};
#endif
	...
	if (use_rocksdb_backend)
	{
		...
		return make_rocksdb ();
#else
		logger.always_log (std::error_code (oslo::error_config::rocksdb_enabled_but_not_supported).message ());
	...
	return std::make_unique<oslo::mdb_store> (logger, add_db_postfix ? path / "data.ldb" : path, txn_tracking_config_a, block_processor_batch_max_time_a, lmdb_config_a, batch_size, backup_before_upgrade);
}
