...
oslo::websocket::confirmation_options::confirmation_options (oslo::wallets & wallets_a) :
wallets (wallets_a)
{
}

oslo::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, oslo::wallets & wallets_a, oslo::logger_mt & logger_a) :
wallets (wallets_a),
logger (logger_a)
{
...
bool oslo::websocket::confirmation_options::should_filter (oslo::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			oslo::account source_l (0), destination_l (0);
...
bool oslo::websocket::confirmation_options::update (boost::property_tree::ptree const & options_a)
{
...
void oslo::websocket::confirmation_options::check_filter_empty () const
{
...
oslo::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, oslo::logger_mt & logger_a)
{
...
bool oslo::websocket::vote_options::should_filter (oslo::websocket::message const & message_a) const
{
...
oslo::websocket::session::session (oslo::websocket::listener & listener_a, socket_type socket_a) :
...
oslo::websocket::session::~session ()
{
	{
		oslo::unique_lock<std::mutex> lk (subscriptions_mutex);
...
void oslo::websocket::session::handshake ()
{
...
void oslo::websocket::session::close ()
{
...
void oslo::websocket::session::write (oslo::websocket::message message_a)
{
	oslo::unique_lock<std::mutex> lk (subscriptions_mutex);
	auto subscription (subscriptions.find (message_a.topic));
	if (message_a.topic == oslo::websocket::topic::ack || (subscription != subscriptions.end () && !subscription->second->should_filter (message_a)))
	{
		lk.unlock ();
		auto this_l (shared_from_this ());
...
void oslo::websocket::session::write_queued_messages ()
{
	auto msg (send_queue.front ().to_string ());
	auto this_l (shared_from_this ());

	ws.async_write (oslo::shared_const_buffer (msg),
...
void oslo::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
oslo::websocket::topic to_topic (std::string const & topic_a)
{
	oslo::websocket::topic topic = oslo::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = oslo::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = oslo::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = oslo::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = oslo::websocket::topic::ack;
	}
	else if (topic_a == "active_difficulty")
	{
		topic = oslo::websocket::topic::active_difficulty;
	}
	else if (topic_a == "work")
	{
		topic = oslo::websocket::topic::work;
	}
	else if (topic_a == "bootstrap")
	{
		topic = oslo::websocket::topic::bootstrap;
	}
	else if (topic_a == "telemetry")
	{
		topic = oslo::websocket::topic::telemetry;
	}
	else if (topic_a == "new_unconfirmed_block")
	{
		topic = oslo::websocket::topic::new_unconfirmed_block;
	}

	return topic;
}

std::string from_topic (oslo::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == oslo::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == oslo::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == oslo::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == oslo::websocket::topic::ack)
	{
		topic = "ack";
	}
	else if (topic_a == oslo::websocket::topic::active_difficulty)
	{
		topic = "active_difficulty";
	}
	else if (topic_a == oslo::websocket::topic::work)
	{
		topic = "work";
	}
	else if (topic_a == oslo::websocket::topic::bootstrap)
	{
		topic = "bootstrap";
	}
	else if (topic_a == oslo::websocket::topic::telemetry)
	{
		topic = "telemetry";
	}
	else if (topic_a == oslo::websocket::topic::new_unconfirmed_block)
	{
...
void oslo::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	...
	oslo::websocket::message msg (oslo::websocket::topic::ack);
...
void oslo::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != oslo::websocket::topic::invalid)
	{
		auto options_text_l (message_a.get_child_optional ("options"));
		oslo::lock_guard<std::mutex> lk (subscriptions_mutex);
		std::unique_ptr<oslo::websocket::options> options_l{ nullptr };
		if (options_text_l && topic_l == oslo::websocket::topic::confirmation)
		{
			options_l = std::make_unique<oslo::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_wallets (), ws_listener.get_logger ());
		}
		else if (options_text_l && topic_l == oslo::websocket::topic::vote)
		{
			options_l = std::make_unique<oslo::websocket::vote_options> (options_text_l.get (), ws_listener.get_logger ());
		}
		else
		{
			options_l = std::make_unique<oslo::websocket::options> ();
		}
	...
	else if (action == "unsubscribe" && topic_l != oslo::websocket::topic::invalid)
	{
		oslo::lock_guard<std::mutex> lk (subscriptions_mutex);
...
void oslo::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();

	oslo::lock_guard<std::mutex> lk (sessions_mutex);
...
oslo::websocket::listener::listener (oslo::logger_mt & logger_a, oslo::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a) :
logger (logger_a),
wallets (wallets_a),
acceptor (io_ctx_a),
socket (io_ctx_a)
...
void oslo::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void oslo::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void oslo::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		auto session (std::make_shared<oslo::websocket::session> (*this, std::move (socket)));
		sessions_mutex.lock ();
...
void oslo::websocket::listener::broadcast_confirmation (std::shared_ptr<oslo::block> block_a, oslo::account const & account_a, oslo::amount const & amount_a, std::string subtype, oslo::election_status const & election_status_a)
{
	oslo::websocket::message_builder builder;

	oslo::lock_guard<std::mutex> lk (sessions_mutex);
	boost::optional<oslo::websocket::message> msg_with_block;
	boost::optional<oslo::websocket::message> msg_without_block;
	for (auto & weak_session : sessions)
	{
		auto session_ptr (weak_session.lock ());
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (oslo::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				oslo::websocket::confirmation_options default_options (wallets);
				auto conf_options (dynamic_cast<oslo::websocket::confirmation_options *> (subscription->second.get ()));
...
void oslo::websocket::listener::broadcast (oslo::websocket::message message_a)
{
	oslo::lock_guard<std::mutex> lk (sessions_mutex);
	for (auto & weak_session : sessions)
	{
...
void oslo::websocket::listener::increase_subscriber_count (oslo::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void oslo::websocket::listener::decrease_subscriber_count (oslo::websocket::topic const & topic_a)
{
...
oslo::websocket::message oslo::websocket::message_builder::stopped_election (oslo::block_hash const & hash_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::stopped_election);
...
oslo::websocket::message oslo::websocket::message_builder::block_confirmed (std::shared_ptr<oslo::block> block_a, oslo::account const & account_a, oslo::amount const & amount_a, std::string subtype, bool include_block_a, oslo::election_status const & election_status_a, oslo::websocket::confirmation_options const & options_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::confirmation);
	set_common_fields (message_l);
...
oslo::websocket::message oslo::websocket::message_builder::vote_received (std::shared_ptr<oslo::vote> vote_a, oslo::vote_code code_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::vote);
	set_common_fields (message_l);
...
oslo::websocket::message oslo::websocket::message_builder::difficulty_changed (uint64_t publish_threshold_a, uint64_t difficulty_active_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::active_difficulty);
	set_common_fields (message_l);

	// Active difficulty information
	boost::property_tree::ptree difficulty_l;
	difficulty_l.put ("network_minimum", oslo::to_string_hex (publish_threshold_a));
	difficulty_l.put ("network_current", oslo::to_string_hex (difficulty_active_a));
	auto multiplier = oslo::difficulty::to_multiplier (difficulty_active_a, publish_threshold_a);
	difficulty_l.put ("multiplier", oslo::to_string (multiplier));

	message_l.contents.add_child ("message", difficulty_l);
	return message_l;
}

oslo::websocket::message oslo::websocket::message_builder::work_generation (oslo::work_version const version_a, oslo::block_hash const & root_a, uint64_t work_a, uint64_t difficulty_a, uint64_t publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool completed_a, bool cancelled_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::work);
	set_common_fields (message_l);
	...
	request_l.put ("version", oslo::to_string (version_a));
	request_l.put ("hash", root_a.to_string ());
	request_l.put ("difficulty", oslo::to_string_hex (difficulty_a));
	auto request_multiplier_l (oslo::difficulty::to_multiplier (difficulty_a, publish_threshold_a));
	request_l.put ("multiplier", oslo::to_string (request_multiplier_l));
	...
	if (completed_a)
	{
		...
		result_l.put ("work", oslo::to_string_hex (work_a));
		auto result_difficulty_l (oslo::work_difficulty (version_a, root_a, work_a));
		result_l.put ("difficulty", oslo::to_string_hex (result_difficulty_l));
		auto result_multiplier_l (oslo::difficulty::to_multiplier (result_difficulty_l, publish_threshold_a));
		result_l.put ("multiplier", oslo::to_string (result_multiplier_l));
...
oslo::websocket::message oslo::websocket::message_builder::work_cancelled (oslo::work_version const version_a, oslo::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, true);
}

oslo::websocket::message oslo::websocket::message_builder::work_failed (oslo::work_version const version_a, oslo::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, false);
}

oslo::websocket::message oslo::websocket::message_builder::bootstrap_started (std::string const & id_a, std::string const & mode_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
oslo::websocket::message oslo::websocket::message_builder::bootstrap_exited (std::string const & id_a, std::string const & mode_a, std::chrono::steady_clock::time_point const start_time_a, uint64_t const total_blocks_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
oslo::websocket::message oslo::websocket::message_builder::telemetry_received (oslo::telemetry_data const & telemetry_data_a, oslo::endpoint const & endpoint_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::telemetry);
	set_common_fields (message_l);

	// Telemetry information
	oslo::jsonconfig telemetry_l;
...
oslo::websocket::message oslo::websocket::message_builder::new_block_arrived (oslo::block const & block_a)
{
	oslo::websocket::message message_l (oslo::websocket::topic::new_unconfirmed_block);
	set_common_fields (message_l);

	boost::property_tree::ptree block_l;
	block_a.serialize_json (block_l);
	auto subtype (oslo::state_subtype (block_a.sideband ().details));
...
void oslo::websocket::message_builder::set_common_fields (oslo::websocket::message & message_a)
{
...
std::string oslo::websocket::message::to_string () const
{
