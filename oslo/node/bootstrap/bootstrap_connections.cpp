...
constexpr double oslo::bootstrap_limits::bootstrap_connection_scale_target_blocks;
constexpr double oslo::bootstrap_limits::bootstrap_minimum_blocks_per_sec;
constexpr double oslo::bootstrap_limits::bootstrap_minimum_termination_time_sec;
constexpr unsigned oslo::bootstrap_limits::bootstrap_max_new_connections;
constexpr unsigned oslo::bootstrap_limits::requeued_pulls_processed_blocks_factor;

oslo::bootstrap_client::bootstrap_client (std::shared_ptr<oslo::node> node_a, std::shared_ptr<oslo::bootstrap_connections> connections_a, std::shared_ptr<oslo::transport::channel_tcp> channel_a, std::shared_ptr<oslo::socket> socket_a) :
node (node_a),
...
oslo::bootstrap_client::~bootstrap_client ()
{
	--connections->connections_count;
}

double oslo::bootstrap_client::block_rate () const
{
	auto elapsed = std::max (elapsed_seconds (), oslo::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate);
	return static_cast<double> (block_count.load () / elapsed);
}

void oslo::bootstrap_client::set_start_time (std::chrono::steady_clock::time_point start_time_a)
{
	oslo::lock_guard<std::mutex> guard (start_time_mutex);
	start_time_m = start_time_a;
}

double oslo::bootstrap_client::elapsed_seconds () const
{
	oslo::lock_guard<std::mutex> guard (start_time_mutex);
	return std::chrono::duration_cast<std::chrono::duration<double>> (std::chrono::steady_clock::now () - start_time_m).count ();
}

void oslo::bootstrap_client::stop (bool force)
{
	pending_stop = true;
	if (force)
	{
...
oslo::bootstrap_connections::bootstrap_connections (oslo::node & node_a) :
node (node_a)
{
}

std::shared_ptr<oslo::bootstrap_client> oslo::bootstrap_connections::connection (std::shared_ptr<oslo::bootstrap_attempt> attempt_a, bool use_front_connection)
{
	...
	std::shared_ptr<oslo::bootstrap_client> result;
...
void oslo::bootstrap_connections::pool_connection (std::shared_ptr<oslo::bootstrap_client> client_a, bool new_client, bool push_front)
{
	oslo::unique_lock<std::mutex> lock (mutex);
...
void oslo::bootstrap_connections::add_connection (oslo::endpoint const & endpoint_a)
{
	connect_client (oslo::tcp_endpoint (endpoint_a.address (), endpoint_a.port ()), true);
}

std::shared_ptr<oslo::bootstrap_client> oslo::bootstrap_connections::find_connection (oslo::tcp_endpoint const & endpoint_a)
{
...
void oslo::bootstrap_connections::connect_client (oslo::tcp_endpoint const & endpoint_a, bool push_front)
{
	++connections_count;
	auto socket (std::make_shared<oslo::socket> (node.shared ()));
	...
	[this_l, socket, endpoint_a, push_front](boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			auto client (std::make_shared<oslo::bootstrap_client> (this_l->node.shared (), this_l, std::make_shared<oslo::transport::channel_tcp> (*this_l->node.shared (), socket), socket));
...
unsigned oslo::bootstrap_connections::target_connections (size_t pulls_remaining, size_t attempts_count)
{
	...
	double step_scale = std::min (1.0, std::max (0.0, (double)pulls_remaining / oslo::bootstrap_limits::bootstrap_connection_scale_target_blocks));
...
{
	bool operator() (const std::shared_ptr<oslo::bootstrap_client> & lhs, const std::shared_ptr<oslo::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

void oslo::bootstrap_connections::populate_connections (bool repeat)
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	size_t attempts_count = node.bootstrap_initiator.attempts.size ();
	std::priority_queue<std::shared_ptr<oslo::bootstrap_client>, std::vector<std::shared_ptr<oslo::bootstrap_client>>, block_rate_cmp> sorted_connections;
	std::unordered_set<oslo::tcp_endpoint> endpoints;
	{
		oslo::unique_lock<std::mutex> lock (mutex);
		num_pulls = pulls.size ();
		std::deque<std::weak_ptr<oslo::bootstrap_client>> new_clients;
		for (auto & c : clients)
		{
			if (auto client = c.lock ())
			{
				if (auto socket_l = client->channel->socket.lock ())
				{
					...
					if (client->elapsed_seconds () > oslo::bootstrap_limits::bootstrap_connection_warmup_time_sec && client->block_count > 0)
					{
					...
					if (elapsed_sec > oslo::bootstrap_limits::bootstrap_minimum_termination_time_sec && blocks_per_sec < oslo::bootstrap_limits::bootstrap_minimum_blocks_per_sec)
					{
						if (node.config.logging.bulk_pull_logging ())
						{
							node.logger.try_log (boost::str (boost::format ("Stopping slow peer %1% (elapsed sec %2%s > %3%s and %4% blocks per second < %5%)") % client->channel->to_string () % elapsed_sec % oslo::bootstrap_limits::bootstrap_minimum_termination_time_sec % blocks_per_sec % oslo::bootstrap_limits::bootstrap_minimum_blocks_per_sec));
						}
	...
	{
		auto delta = std::min ((target - connections_count) * 2, oslo::bootstrap_limits::bootstrap_max_new_connections);
		// TODO - tune this better
		// Not many peers respond, need to try to make more connections than we need.
		for (auto i = 0u; i < delta; i++)
		{
			auto endpoint (node.network.bootstrap_peer (true));
			if (endpoint != oslo::tcp_endpoint (boost::asio::ip::address_v6::any (), 0) && (node.flags.allow_bootstrap_peers_duplicates || endpoints.find (endpoint) == endpoints.end ()) && !node.network.excluded_peers.check (endpoint))
			{
	...
	if (!stopped && repeat)
	{
		std::weak_ptr<oslo::bootstrap_connections> this_w (shared_from_this ());
...
void oslo::bootstrap_connections::start_populate_connections ()
{
...
void oslo::bootstrap_connections::add_pull (oslo::pull_info const & pull_a)
{
...
void oslo::bootstrap_connections::request_pull (oslo::unique_lock<std::mutex> & lock_a)
{
	lock_a.unlock ();
	...
	{
		std::shared_ptr<oslo::bootstrap_attempt> attempt_l;
		oslo::pull_info pull;
		...
		while (attempt_l == nullptr && !pulls.empty ())
		{
			...
			if (attempt_l != nullptr && attempt_l->mode == oslo::bootstrap_mode::lazy && !pull.head.is_zero () && attempt_l->lazy_processed_or_exists (pull.head))
			{
		...
		{
			if (attempt_l->mode == oslo::bootstrap_mode::legacy)
			{
			...
			node.background ([connection_l, attempt_l, pull]() {
				auto client (std::make_shared<oslo::bulk_pull_client> (connection_l, attempt_l, pull));
				client->request ();
			});
		}
	}
...
void oslo::bootstrap_connections::requeue_pull (oslo::pull_info const & pull_a, bool network_error)
{
	auto pull (pull_a);
	if (!network_error)
	{
	...
	if (attempt_l != nullptr)
	{
		...
		else if (attempt_l->mode == oslo::bootstrap_mode::lazy)
		{
			pull.count = attempt_l->lazy_batch_size ();
		}
		if (pull.attempts < pull.retry_limit + (pull.processed / oslo::bootstrap_limits::requeued_pulls_processed_blocks_factor))
		{
			{
				oslo::lock_guard<std::mutex> lock (mutex);
		...
		else if (attempt_l->mode == oslo::bootstrap_mode::lazy && (pull.retry_limit == std::numeric_limits<unsigned>::max () || pull.attempts <= pull.retry_limit + (pull.processed / node.network_params.bootstrap.lazy_max_pull_blocks)))
		{
		...
		else
		{
			...
			node.stats.inc (oslo::stat::type::bootstrap, oslo::stat::detail::bulk_pull_failed_account, oslo::stat::dir::in);

			if (attempt_l->mode == oslo::bootstrap_mode::lazy && pull.processed > 0)
			{
				attempt_l->lazy_add (pull);
			}
			else if (attempt_l->mode == oslo::bootstrap_mode::legacy)
			{
...
void oslo::bootstrap_connections::clear_pulls (uint64_t bootstrap_id_a)
{
	{
		oslo::lock_guard<std::mutex> lock (mutex);
		auto i (pulls.begin ());
...
void oslo::bootstrap_connections::run ()
{
...
void oslo::bootstrap_connections::stop ()
{
	oslo::unique_lock<std::mutex> lock (mutex);
	stopped = true;
	lock.unlock ();
