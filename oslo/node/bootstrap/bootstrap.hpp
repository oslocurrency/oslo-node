...
public:
	...
	oslo::uint512_union account_head;
	oslo::block_hash new_head;
};
class pulls_cache final
{
public:
	void add (oslo::pull_info const &);
	void update_pull (oslo::pull_info &);
	void remove (oslo::pull_info const &);
	std::mutex pulls_cache_mutex;
	class account_head_tag
	{
	};
	// clang-format off
	boost::multi_index_container<oslo::cached_pulls,
	mi::indexed_by<
		mi::ordered_non_unique<
			mi::member<oslo::cached_pulls, std::chrono::steady_clock::time_point, &oslo::cached_pulls::time>>,
		mi::hashed_unique<mi::tag<account_head_tag>,
			mi::member<oslo::cached_pulls, oslo::uint512_union, &oslo::cached_pulls::account_head>>>>
	cache;
	// clang-format on
...
{
public:
	void add (std::shared_ptr<oslo::bootstrap_attempt>);
	void remove (uint64_t);
	void clear ();
	std::shared_ptr<oslo::bootstrap_attempt> find (uint64_t);
	size_t size ();
	std::atomic<uint64_t> incremental{ 0 };
	std::mutex bootstrap_attempts_mutex;
	std::map<uint64_t, std::shared_ptr<oslo::bootstrap_attempt>> attempts;
};

class bootstrap_initiator final
{
public:
	explicit bootstrap_initiator (oslo::node &);
	~bootstrap_initiator ();
	void bootstrap (oslo::endpoint const &, bool add_to_peers = true, bool frontiers_confirmed = false, std::string id_a = "");
	void bootstrap (bool force = false, std::string id_a = "");
	void bootstrap_lazy (oslo::hash_or_account const &, bool force = false, bool confirmed = true, std::string id_a = "");
	void bootstrap_wallet (std::deque<oslo::account> &);
	void run_bootstrap ();
	void lazy_requeue (oslo::block_hash const &, oslo::block_hash const &, bool);
	void notify_listeners (bool);
	void add_observer (std::function<void(bool)> const &);
	bool in_progress ();
	std::shared_ptr<oslo::bootstrap_connections> connections;
	std::shared_ptr<oslo::bootstrap_attempt> new_attempt ();
	bool has_new_attempts ();
	std::shared_ptr<oslo::bootstrap_attempt> current_attempt ();
	std::shared_ptr<oslo::bootstrap_attempt> current_lazy_attempt ();
	std::shared_ptr<oslo::bootstrap_attempt> current_wallet_attempt ();
	oslo::pulls_cache cache;
	oslo::bootstrap_attempts attempts;
	void stop ();

private:
	oslo::node & node;
	std::shared_ptr<oslo::bootstrap_attempt> find_attempt (oslo::bootstrap_mode);
	void remove_attempt (std::shared_ptr<oslo::bootstrap_attempt>);
	void stop_attempts ();
	std::vector<std::shared_ptr<oslo::bootstrap_attempt>> attempts_list;
