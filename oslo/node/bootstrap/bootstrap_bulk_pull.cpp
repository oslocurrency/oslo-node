...
#include <oslo/node/bootstrap/bootstrap_connections.hpp>
...
oslo::pull_info::pull_info (oslo::hash_or_account const & account_or_head_a, oslo::block_hash const & head_a, oslo::block_hash const & end_a, uint64_t bootstrap_id_a, count_t count_a, unsigned retry_limit_a) :
...
oslo::bulk_pull_client::bulk_pull_client (std::shared_ptr<oslo::bootstrap_client> connection_a, std::shared_ptr<oslo::bootstrap_attempt> attempt_a, oslo::pull_info const & pull_a) :
...
oslo::bulk_pull_client::~bulk_pull_client ()
{
	// If received end block is not expected end block
	if (expected != pull.end)
	{
		pull.head = expected;
		if (attempt->mode != oslo::bootstrap_mode::legacy)
		{
...
void oslo::bulk_pull_client::request ()
{
	...
	req, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (oslo::stat::type::bootstrap, oslo::stat::detail::bulk_pull_request_failure, oslo::stat::dir::in);
		}
	},
	oslo::buffer_drop_policy::no_limiter_drop);
}

void oslo::bulk_pull_client::throttled_receive_block ()
{
...
void oslo::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	if (auto socket_l = connection->channel->socket.lock ())
	{
		socket_l->async_read (connection->receive_buffer, 1, [this_l](boost::system::error_code const & ec, size_t size_a) {
			...
			else
			{
				...
				this_l->connection->node->stats.inc (oslo::stat::type::bootstrap, oslo::stat::detail::bulk_pull_receive_block_failure, oslo::stat::dir::in);
...
void oslo::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	oslo::block_type type (static_cast<oslo::block_type> (connection->receive_buffer->data ()[0]));
	...
	if (auto socket_l = connection->channel->socket.lock ())
	{
		switch (type)
		{
			...
			{
				socket_l->async_read (connection->receive_buffer, oslo::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
			...
			{
				socket_l->async_read (connection->receive_buffer, oslo::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			...
			{
				socket_l->async_read (connection->receive_buffer, oslo::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			...
			{
				socket_l->async_read (connection->receive_buffer, oslo::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			...
			{
				socket_l->async_read (connection->receive_buffer, oslo::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
...
void oslo::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, oslo::block_type type_a)
{
	if (!ec)
	{
		oslo::bufferstream stream (connection->receive_buffer->data (), size_a);
		std::shared_ptr<oslo::block> block (oslo::deserialize_block (stream, type_a));
		if (block != nullptr && !oslo::work_validate_entry (*block))
		{
			...
			// Unconfirmed head is used only for lazy destinations if legacy bootstrap is not available, see oslo::bootstrap_attempt::lazy_destinations_increment (...)
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (attempt->mode != oslo::bootstrap_mode::legacy || unexpected_count < 16384)
				{
		...
		else
		{
			...
			connection->node->stats.inc (oslo::stat::type::bootstrap, oslo::stat::detail::bulk_pull_deserialize_receive_block, oslo::stat::dir::in);
		}
	}
	else
	{
		...
		connection->node->stats.inc (oslo::stat::type::bootstrap, oslo::stat::detail::bulk_pull_receive_block_failure, oslo::stat::dir::in);
		network_error = true;
	}
}

oslo::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<oslo::bootstrap_client> connection_a, std::shared_ptr<oslo::bootstrap_attempt> attempt_a, oslo::account const & account_a) :
connection (connection_a),
attempt (attempt_a),
account (account_a),
pull_blocks (0)
...
oslo::bulk_pull_account_client::~bulk_pull_account_client ()
{
	attempt->pull_finished ();
}

void oslo::bulk_pull_account_client::request ()
{
	oslo::bulk_pull_account req;
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = oslo::bulk_pull_account_flags::pending_hash_and_amount;
	if (connection->node->config.logging.bulk_pull_logging ())
	{
	...
	req, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (oslo::stat::type::bootstrap, oslo::stat::detail::bulk_pull_error_starting_request, oslo::stat::dir::in);
		}
	},
	oslo::buffer_drop_policy::no_limiter_drop);
}

void oslo::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (oslo::uint256_union) + sizeof (oslo::uint128_union));
	if (auto socket_l = connection->channel->socket.lock ())
	{
		socket_l->async_read (connection->receive_buffer, size_l, [this_l, size_l](boost::system::error_code const & ec, size_t size_a) {
			...
			if (size_a == size_l)
			{
				if (!ec)
				{
					oslo::block_hash pending;
					oslo::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (oslo::uint256_union));
					auto error1 (oslo::try_read (frontier_stream, pending));
					(void)error1;
					debug_assert (!error1);
					oslo::amount balance;
					oslo::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (oslo::uint256_union), sizeof (oslo::uint128_union));
					auto error2 (oslo::try_read (balance_stream, balance));
					(void)error2;
					debug_assert (!error2);
 ...
 */
void oslo::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void oslo::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		...
		{
			oslo::vectorstream stream (send_buffer);
			oslo::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
		...
		connection->socket->async_write (oslo::shared_const_buffer (std::move (send_buffer)), [this_l](boost::system::error_code const & ec, size_t size_a) {
...
std::shared_ptr<oslo::block> oslo::bulk_pull_server::get_next ()
{
...
void oslo::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void oslo::bulk_pull_server::send_finished ()
{
	oslo::shared_const_buffer send_buffer (static_cast<uint8_t> (oslo::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void oslo::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
...
oslo::bulk_pull_server::bulk_pull_server (std::shared_ptr<oslo::bootstrap_server> const & connection_a, std::unique_ptr<oslo::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void oslo::bulk_pull_account_server::set_params ()
{
	debug_assert (request != nullptr);
	...
	if (request->flags == oslo::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == oslo::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == oslo::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void oslo::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	if (!invalid_request)
	{
		...
		oslo::uint128_union account_frontier_balance (account_frontier_balance_int);

		// Write the frontier block hash and balance into a buffer
		std::vector<uint8_t> send_buffer;
		{
			oslo::vectorstream output_stream (send_buffer);
...
void oslo::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<oslo::pending_key>, std::unique_ptr<oslo::pending_info>> oslo::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<oslo::pending_key>, std::unique_ptr<oslo::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == oslo::store_iterator<oslo::pending_key, oslo::pending_info> (nullptr))
		{
			break;
		}

		oslo::pending_key key (stream->first);
		oslo::pending_info info (stream->second);
		...
		result.first = std::make_unique<oslo::pending_key> (key);
		result.second = std::make_unique<oslo::pending_info> (info);
...
void oslo::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void oslo::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		oslo::vectorstream output_stream (send_buffer);
		oslo::uint256_union account_zero (0);
		oslo::uint128_union balance_zero (0);
...
void oslo::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
oslo::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<oslo::bootstrap_server> const & connection_a, std::unique_ptr<oslo::bulk_pull_account> request_a) :
connection (connection_a),
request (std::move (request_a)),
current_key (0, 0)
{
