...
constexpr std::chrono::seconds oslo::bootstrap_limits::lazy_flush_delay_sec;
constexpr unsigned oslo::bootstrap_limits::lazy_destinations_request_limit;
constexpr uint64_t oslo::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit;
constexpr double oslo::bootstrap_limits::lazy_batch_pull_count_resize_ratio;
constexpr size_t oslo::bootstrap_limits::lazy_blocks_restart_limit;

oslo::bootstrap_attempt_lazy::bootstrap_attempt_lazy (std::shared_ptr<oslo::node> node_a, uint64_t incremental_id_a, std::string id_a) :
oslo::bootstrap_attempt (node_a, oslo::bootstrap_mode::lazy, incremental_id_a, id_a)
{
	node->bootstrap_initiator.notify_listeners (true);
}

oslo::bootstrap_attempt_lazy::~bootstrap_attempt_lazy ()
{
	debug_assert (lazy_blocks.size () == lazy_blocks_count);
	node->bootstrap_initiator.notify_listeners (false);
}

void oslo::bootstrap_attempt_lazy::lazy_start (oslo::hash_or_account const & hash_or_account_a, bool confirmed)
{
...
void oslo::bootstrap_attempt_lazy::lazy_add (oslo::hash_or_account const & hash_or_account_a, unsigned retry_limit)
{
...
void oslo::bootstrap_attempt_lazy::lazy_add (oslo::pull_info const & pull_a)
{
...
void oslo::bootstrap_attempt_lazy::lazy_requeue (oslo::block_hash const & hash_a, oslo::block_hash const & previous_a, bool confirmed_a)
{
	oslo::unique_lock<std::mutex> lock (mutex);
	...
	if (lazy_blocks_processed (hash_a))
	{
		...
		lock.unlock ();
		node->bootstrap_initiator.connections->requeue_pull (oslo::pull_info (hash_a, hash_a, previous_a, incremental_id, static_cast<oslo::pull_info::count_t> (1), confirmed_a ? std::numeric_limits<unsigned>::max () : node->network_params.bootstrap.lazy_destinations_retry_limit));
	}
}

uint32_t oslo::bootstrap_attempt_lazy::lazy_batch_size ()
{
	auto result (node->network_params.bootstrap.lazy_max_pull_blocks);
	if (total_blocks > oslo::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit && lazy_blocks_count != 0)
	{
		double lazy_blocks_ratio (total_blocks / lazy_blocks_count);
		if (lazy_blocks_ratio > oslo::bootstrap_limits::lazy_batch_pull_count_resize_ratio)
		{
			// Increasing blocks ratio weight as more important (^3). Small batch count should lower blocks ratio below target
			double lazy_blocks_factor (std::pow (lazy_blocks_ratio / oslo::bootstrap_limits::lazy_batch_pull_count_resize_ratio, 3.0));
			// Decreasing total block count weight as less important (sqrt)
			double total_blocks_factor (std::sqrt (total_blocks / oslo::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit));
...
void oslo::bootstrap_attempt_lazy::lazy_pull_flush (oslo::unique_lock<std::mutex> & lock_a)
{
	static size_t const max_pulls (oslo::bootstrap_limits::bootstrap_connection_scale_target_blocks * 3);
	if (pulling < max_pulls)
	{
		debug_assert (node->network_params.bootstrap.lazy_max_pull_blocks <= std::numeric_limits<oslo::pull_info::count_t>::max ());
		oslo::pull_info::count_t batch_count (lazy_batch_size ());
		...
		while (!lazy_pulls.empty () && count < max_pulls)
		{
			...
			{
				lock_a.unlock ();
				node->bootstrap_initiator.connections->add_pull (oslo::pull_info (pull_start.first, pull_start.first, oslo::block_hash (0), incremental_id, batch_count, pull_start.second));
				++pulling;
				++count;
				lock_a.lock ();
			}
...
bool oslo::bootstrap_attempt_lazy::lazy_finished ()
{
	debug_assert (!mutex.try_lock ());
	if (stopped)
	{
...
bool oslo::bootstrap_attempt_lazy::lazy_has_expired () const
{
	bool result (false);
	...
	else if (!node->flags.disable_legacy_bootstrap && lazy_blocks_count > oslo::bootstrap_limits::lazy_blocks_restart_limit)
	{
		result = true;
	}
	return result;
...
void oslo::bootstrap_attempt_lazy::run ()
{
	...
	while ((still_pulling () || !lazy_finished ()) && !lazy_has_expired ())
	{
		...
		{
			condition.wait (lock, [& stopped = stopped, &pulling = pulling, &lazy_pulls = lazy_pulls, this_l] { return stopped || pulling == 0 || (pulling < oslo::bootstrap_limits::bootstrap_connection_scale_target_blocks && !lazy_pulls.empty ()) || this_l->lazy_has_expired (); });
			++iterations;
			// Flushing lazy pulls
...
bool oslo::bootstrap_attempt_lazy::process_block (std::shared_ptr<oslo::block> block_a, oslo::account const & known_account_a, uint64_t pull_blocks, oslo::bulk_pull::count_t max_blocks, bool block_expected, unsigned retry_limit)
{
...
bool oslo::bootstrap_attempt_lazy::process_block_lazy (std::shared_ptr<oslo::block> block_a, oslo::account const & known_account_a, uint64_t pull_blocks, oslo::bulk_pull::count_t max_blocks, unsigned retry_limit)
{
	bool stop_pull (false);
	...
	if (!lazy_blocks_processed (hash))
	{
		...
		else if (block_a->type () == oslo::block_type::send)
		{
			std::shared_ptr<oslo::send_block> block_l (std::static_pointer_cast<oslo::send_block> (block_a));
		...
		if (pull_blocks == 0 && (block_a->type () == oslo::block_type::state || block_a->type () == oslo::block_type::send))
		{
		...
		lock.unlock ();
		oslo::unchecked_info info (block_a, known_account_a, 0, oslo::signature_verification::unknown, retry_limit == std::numeric_limits<unsigned>::max ());
...
void oslo::bootstrap_attempt_lazy::lazy_block_state (std::shared_ptr<oslo::block> block_a, unsigned retry_limit)
{
	std::shared_ptr<oslo::state_block> block_l (std::static_pointer_cast<oslo::state_block> (block_a));
	if (block_l != nullptr)
	{
		auto transaction (node->store.tx_begin_read ());
		oslo::uint128_t balance (block_l->hashables.balance.number ());
		...
		if (!link.is_zero () && !node->ledger.is_epoch_link (link) && !lazy_blocks_processed (link) && !node->store.block_exists (transaction, link))
		{
			...
			{
				lazy_state_backlog.emplace (previous, oslo::lazy_state_backlog_item{ link, balance, retry_limit });
			}
		}
	}
}

void oslo::bootstrap_attempt_lazy::lazy_block_state_backlog_check (std::shared_ptr<oslo::block> block_a, oslo::block_hash const & hash_a)
{
	...
	if (find_state != lazy_state_backlog.end ())
	{
		...
		if (block_a->type () == oslo::block_type::state || block_a->type () == oslo::block_type::send)
		{
...
void oslo::bootstrap_attempt_lazy::lazy_backlog_cleanup ()
{
	uint64_t read_count (0);
	auto transaction (node->store.tx_begin_read ());
...
void oslo::bootstrap_attempt_lazy::lazy_destinations_increment (oslo::account const & destination_a)
{
	...
	if (node->flags.disable_legacy_bootstrap)
	{
		...
		{
			lazy_destinations.get<account_tag> ().modify (existing, [](oslo::lazy_destinations_item & item_a) {
				++item_a.count;
			});
		}
		else
		{
			lazy_destinations.emplace (oslo::lazy_destinations_item{ destination_a, 1 });
		}
	}
}

void oslo::bootstrap_attempt_lazy::lazy_destinations_flush ()
{
	debug_assert (!mutex.try_lock ());
	lazy_destinations_flushed = true;
	size_t count (0);
	for (auto it (lazy_destinations.get<count_tag> ().begin ()), end (lazy_destinations.get<count_tag> ().end ()); it != end && count < oslo::bootstrap_limits::lazy_destinations_request_limit && !stopped;)
	{
...
void oslo::bootstrap_attempt_lazy::lazy_blocks_insert (oslo::block_hash const & hash_a)
{
	debug_assert (!mutex.try_lock ());
	auto inserted (lazy_blocks.insert (std::hash<::oslo::block_hash> () (hash_a)));
	if (inserted.second)
	{
		++lazy_blocks_count;
...
void oslo::bootstrap_attempt_lazy::lazy_blocks_erase (oslo::block_hash const & hash_a)
{
	debug_assert (!mutex.try_lock ());
	auto erased (lazy_blocks.erase (std::hash<::oslo::block_hash> () (hash_a)));
	if (erased)
	{
		--lazy_blocks_count;
...
bool oslo::bootstrap_attempt_lazy::lazy_blocks_processed (oslo::block_hash const & hash_a)
{
	return lazy_blocks.find (std::hash<::oslo::block_hash> () (hash_a)) != lazy_blocks.end ();
}

bool oslo::bootstrap_attempt_lazy::lazy_processed_or_exists (oslo::block_hash const & hash_a)
{
	bool result (false);
	oslo::unique_lock<std::mutex> lock (mutex);
	if (lazy_blocks_processed (hash_a))
...
void oslo::bootstrap_attempt_lazy::get_information (boost::property_tree::ptree & tree_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
...
oslo::bootstrap_attempt_wallet::bootstrap_attempt_wallet (std::shared_ptr<oslo::node> node_a, uint64_t incremental_id_a, std::string id_a) :
oslo::bootstrap_attempt (node_a, oslo::bootstrap_mode::wallet_lazy, incremental_id_a, id_a)
{
	node->bootstrap_initiator.notify_listeners (true);
}

oslo::bootstrap_attempt_wallet::~bootstrap_attempt_wallet ()
{
	node->bootstrap_initiator.notify_listeners (false);
}

void oslo::bootstrap_attempt_wallet::request_pending (oslo::unique_lock<std::mutex> & lock_a)
{
	lock_a.unlock ();
	...
	if (connection_l && !stopped)
	{
		...
		node->background ([connection_l, this_l, account]() {
			auto client (std::make_shared<oslo::bulk_pull_account_client> (connection_l, this_l, account));
			client->request ();
		});
	}
}

void oslo::bootstrap_attempt_wallet::requeue_pending (oslo::account const & account_a)
{
...
void oslo::bootstrap_attempt_wallet::wallet_start (std::deque<oslo::account> & accounts_a)
{
	{
		oslo::lock_guard<std::mutex> lock (mutex);
...
bool oslo::bootstrap_attempt_wallet::wallet_finished ()
{
	debug_assert (!mutex.try_lock ());
...
void oslo::bootstrap_attempt_wallet::run ()
{
	debug_assert (started);
...
size_t oslo::bootstrap_attempt_wallet::wallet_size ()
{
	oslo::lock_guard<std::mutex> lock (mutex);
	return wallet_accounts.size ();
}

void oslo::bootstrap_attempt_wallet::get_information (boost::property_tree::ptree & tree_a)
{
	oslo::lock_guard<std::mutex> lock (mutex);
