...
oslo::request_aggregator::request_aggregator (oslo::network_constants const & network_constants_a, oslo::node_config const & config_a, oslo::stat & stats_a, oslo::votes_cache & cache_a, oslo::ledger & ledger_a, oslo::wallets & wallets_a, oslo::active_transactions & active_a) :
...
void oslo::request_aggregator::add (std::shared_ptr<oslo::transport::channel> & channel_a, std::vector<std::pair<oslo::block_hash, oslo::root>> const & hashes_roots_a)
{
	debug_assert (wallets.reps ().voting > 0);
	bool error = true;
	auto const endpoint (oslo::transport::map_endpoint_to_v6 (channel_a->get_endpoint ()));
	oslo::unique_lock<std::mutex> lock (mutex);
	...
	stats.inc (oslo::stat::type::aggregator, !error ? oslo::stat::detail::aggregator_accepted : oslo::stat::detail::aggregator_dropped);
}

void oslo::request_aggregator::run ()
{
	oslo::thread_role::set (oslo::thread_role::name::request_aggregator);
	oslo::unique_lock<std::mutex> lock (mutex);
	started = true;
	lock.unlock ();
	condition.notify_all ();
...
void oslo::request_aggregator::stop ()
{
	{
...
std::size_t oslo::request_aggregator::size ()
{
	oslo::unique_lock<std::mutex> lock (mutex);
	return requests.size ();
}

bool oslo::request_aggregator::empty ()
{
	return size () == 0;
}

void oslo::request_aggregator::erase_duplicates (std::vector<std::pair<oslo::block_hash, oslo::root>> & requests_a) const
{
...
std::vector<oslo::block_hash> oslo::request_aggregator::aggregate (oslo::transaction const & transaction_a, std::vector<std::pair<oslo::block_hash, oslo::root>> const & requests_a, std::shared_ptr<oslo::transport::channel> & channel_a) const
{
	size_t cached_hashes = 0;
	std::vector<oslo::block_hash> to_generate;
	std::vector<std::shared_ptr<oslo::vote>> cached_votes;
	for (auto const & hash_root : requests_a)
	{
		...
		else
		{
			...
			if (block)
			{
				...
				{
					stats.inc (oslo::stat::type::requests, oslo::stat::detail::requests_cannot_vote, stat::dir::in);
				}
			...
			{
				stats.inc (oslo::stat::type::requests, oslo::stat::detail::requests_unknown, stat::dir::in);
			}
		}
	}
	// Unique votes
	...
	stats.add (oslo::stat::type::requests, oslo::stat::detail::requests_cached_hashes, stat::dir::in, cached_hashes);
	stats.add (oslo::stat::type::requests, oslo::stat::detail::requests_cached_votes, stat::dir::in, cached_votes.size ());
	return to_generate;
}

void oslo::request_aggregator::generate (oslo::transaction const & transaction_a, std::vector<oslo::block_hash> const & hashes_a, std::shared_ptr<oslo::transport::channel> & channel_a) const
{
	...
	while (i != n)
	{
		...
		for (; i != n && hashes_l.size () < oslo::network::confirm_ack_hashes_max; ++i)
		{
			hashes_l.push_back (*i);
		}
		wallets.foreach_representative ([this, &generated_l, &hashes_l, &channel_a, &transaction_a](oslo::public_key const & pub_a, oslo::raw_key const & prv_a) {
	...
	stats.add (oslo::stat::type::requests, oslo::stat::detail::requests_generated_hashes, stat::dir::in, hashes_a.size ());
	stats.add (oslo::stat::type::requests, oslo::stat::detail::requests_generated_votes, stat::dir::in, generated_l);
}

std::unique_ptr<oslo::container_info_component> oslo::collect_container_info (oslo::request_aggregator & aggregator, const std::string & name)
{
