...
{
public:
	confirmation_height_processor (oslo::ledger &, oslo::write_database_queue &, std::chrono::milliseconds, oslo::logger_mt &, boost::latch & initialized_latch, confirmation_height_mode = confirmation_height_mode::automatic);
	~confirmation_height_processor ();
	void pause ();
	void unpause ();
	void stop ();
	void add (oslo::block_hash const & hash_a);
	void run (confirmation_height_mode);
	size_t awaiting_processing_size ();
	bool is_processing_block (oslo::block_hash const &);
	oslo::block_hash current ();

	void add_cemented_observer (std::function<void(std::shared_ptr<oslo::block>)> const &);
	void add_block_already_cemented_observer (std::function<void(oslo::block_hash const &)> const &);
...
private:
	...
	std::vector<std::function<void(std::shared_ptr<oslo::block>)>> cemented_observers;
	std::vector<std::function<void(oslo::block_hash const &)>> block_already_cemented_observers;
	...
	void set_next_hash ();
	void notify_observers (std::vector<std::shared_ptr<oslo::block>> const &);
	void notify_observers (oslo::block_hash const &);
