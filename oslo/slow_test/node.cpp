...
TEST (system, receive_while_synchronizing)
{
	std::vector<boost::thread> threads;
	{
		...
		auto channel (std::make_shared<oslo::transport::channel_udp> (node1->network.udp_channels, system.nodes[0]->network.endpoint (), node1->network_params.protocol.protocol_version));
		node1->network.send_keepalive (channel);
		auto wallet (node1->wallets.create (1));
		wallet->insert_adhoc (oslo::test_genesis_key.prv); // For voting
		...
		system.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (200), ([&system, &key]() {
			auto hash (system.wallet (0)->send_sync (oslo::test_genesis_key.pub, key.pub, system.nodes[0]->config.receive_minimum.number ()));
			auto transaction (system.nodes[0]->store.tx_begin_read ());
...
TEST (ledger, deep_account_compute)
{
	...
	oslo::keypair key;
	auto balance (oslo::genesis_amount - 1);
	oslo::send_block send (genesis.hash (), key.pub, balance, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (genesis.hash ()));
	ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, send).code);
	oslo::open_block open (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *pool.generate (key.pub));
	...
	{
		balance -= 1;
		oslo::send_block send (sprevious, key.pub, balance, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (sprevious));
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, send).code);
		sprevious = send.hash ();
		oslo::receive_block receive (rprevious, send.hash (), key.prv, key.pub, *pool.generate (rprevious));
...
TEST (wallet, multithreaded_send_async)
{
	std::vector<boost::thread> threads;
	{
		...
		wallet_l->insert_adhoc (oslo::test_genesis_key.prv);
		...
		for (auto i (0); i < 20; ++i)
		{
			threads.push_back (boost::thread ([wallet_l, &key]() {
				...
				{
					wallet_l->send_async (oslo::test_genesis_key.pub, key.pub, 1000, [](std::shared_ptr<oslo::block> block_a) {
		...
		while (system.nodes[0]->balance (oslo::test_genesis_key.pub) != (oslo::genesis_amount - 20 * 1000 * 1000))
		{
		...
		threads.push_back (boost::thread ([&system]() {
			for (auto i (0); i != 1000; ++i)
			{
				...
				for (auto j (0); j != 10; ++j)
				{
					...
					system.nodes[0]->store.confirmation_height_put (transaction, account, { 0, oslo::block_hash (0) });
					system.nodes[0]->store.account_put (transaction, account, oslo::account_info ());
				}
			}
		}));
	}
...
TEST (node, fork_storm)
{
	...
	oslo::system system (64, oslo::transport::transport_type::tcp, flags);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto previous (system.nodes[0]->latest (oslo::test_genesis_key.pub));
	auto balance (system.nodes[0]->balance (oslo::test_genesis_key.pub));
	ASSERT_FALSE (previous.is_zero ());
	for (auto j (0); j != system.nodes.size (); ++j)
	{
		balance -= 1;
		oslo::keypair key;
		oslo::send_block send (previous, key.pub, balance, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0);
		system.nodes[j]->work_generate_blocking (send);
		previous = send.hash ();
		for (auto i (0); i != system.nodes.size (); ++i)
		{
			auto send_result (system.nodes[i]->process (send));
			ASSERT_EQ (oslo::process_result::progress, send_result.code);
			oslo::keypair rep;
			auto open (std::make_shared<oslo::open_block> (previous, rep.pub, key.pub, key.prv, key.pub, 0));
			system.nodes[i]->work_generate_blocking (*open);
			auto open_result (system.nodes[i]->process (*open));
			ASSERT_EQ (oslo::process_result::progress, open_result.code);
	...
	while (again)
	{
		auto empty = 0;
		auto single = 0;
		std::for_each (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<oslo::node> const & node_a) {
			...
			{
				oslo::lock_guard<std::mutex> lock (node_a->active.mutex);
		...
		for (auto i (nodes.begin ()), n (nodes.end ()); i != n; ++i)
		{
			switch (*i)
			{
				...
				case 1:
					...
					{
						++message_count;
						auto entry (oslo::random_pool::generate_word32 (0, node_count - 1));
						switch (nodes[entry])
						{
							case 0:
								nodes[entry] = 1;
...
TEST (store, unchecked_load)
{
	...
	auto block (std::make_shared<oslo::send_block> (0, 0, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, 0));
...
TEST (store, vote_load)
{
	...
	for (auto i (0); i < 1000000; ++i)
	{
		auto vote (std::make_shared<oslo::vote> (oslo::test_genesis_key.pub, oslo::test_genesis_key.prv, i, block));
		node.vote_processor.vote (vote, std::make_shared<oslo::transport::channel_udp> (system.nodes[0]->network.udp_channels, system.nodes[0]->network.endpoint (), system.nodes[0]->network_params.protocol.protocol_version));
	}
}
...
TEST (wallets, rep_scan)
{
	...
	node.wallets.foreach_representative ([](oslo::public_key const & pub_a, oslo::raw_key const & prv_a) {
	});
...
TEST (node, mass_vote_by_hash)
{
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	oslo::block_hash previous (oslo::genesis_hash);
	oslo::keypair key;
	std::vector<std::shared_ptr<oslo::state_block>> blocks;
	for (auto i (0); i < 10000; ++i)
	{
		auto block (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, previous, oslo::test_genesis_key.pub, oslo::genesis_amount - (i + 1) * oslo::Gxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (previous)));
		previous = block->hash ();
		blocks.push_back (block);
	}
	for (auto i (blocks.begin ()), n (blocks.end ()); i != n; ++i)
	{
		system.nodes[0]->block_processor.add (*i, oslo::seconds_since_epoch ());
	}
}
...
TEST (confirmation_height, many_accounts_single_confirmation)
{
	...
	// The number of frontiers should be more than the oslo::confirmation_height::unbounded_cutoff to test the amount of blocks confirmed is correct.
	node->confirmation_height_processor.batch_write_size = 500;
	auto const num_accounts = oslo::confirmation_height::unbounded_cutoff * 2 + 50;
	oslo::keypair last_keypair = oslo::test_genesis_key;
	auto last_open_hash = node->latest (oslo::test_genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			oslo::send_block send (last_open_hash, key.pub, node->config.online_weight_minimum.number (), last_keypair.prv, last_keypair.pub, *system.work.generate (last_open_hash));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send).code);
			oslo::open_block open (send.hash (), last_keypair.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in), num_accounts * 2 - 2);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_bounded, oslo::stat::dir::in), num_accounts * 2 - 2);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_unbounded, oslo::stat::dir::in), 0);

	system.deadline_set (40s);
	while ((node->ledger.cache.cemented_count - 1) != node->stats.count (oslo::stat::type::confirmation_observer, oslo::stat::detail::all, oslo::stat::dir::out))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (confirmation_height, many_accounts_many_confirmations)
{
	...
	node->confirmation_height_processor.batch_write_size = 500;
	auto const num_accounts = oslo::confirmation_height::unbounded_cutoff * 2 + 50;
	auto latest_genesis = node->latest (oslo::test_genesis_key.pub);
	std::vector<std::shared_ptr<oslo::open_block>> open_blocks;
	{
		auto transaction = node->store.tx_begin_write ();
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			oslo::send_block send (latest_genesis, key.pub, node->config.online_weight_minimum.number (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest_genesis));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send).code);
			auto open = std::make_shared<oslo::open_block> (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *open).code);
	...
	while (node->stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in) != num_blocks_to_confirm)
	{
		ASSERT_NO_ERROR (system.poll ());
	}

	auto num_confirmed_bounded = node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_bounded, oslo::stat::dir::in);
	ASSERT_GE (num_confirmed_bounded, oslo::confirmation_height::unbounded_cutoff);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_unbounded, oslo::stat::dir::in), num_blocks_to_confirm - num_confirmed_bounded);
...
TEST (confirmation_height, long_chains)
{
	...
	node->confirmation_height_processor.batch_write_size = 500;
	auto const num_blocks = oslo::confirmation_height::unbounded_cutoff * 2 + 50;

	// First open the other account
	oslo::send_block send (latest, key1.pub, oslo::genesis_amount - oslo::Gxrb_ratio + num_blocks + 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest));
	...
	auto previous_destination_chain_hash = open.hash ();
	{
		...
		{
			oslo::send_block send (previous_genesis_chain_hash, key1.pub, oslo::genesis_amount - oslo::Gxrb_ratio + i + 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send).code);
			oslo::receive_block receive (previous_destination_chain_hash, send.hash (), key1.prv, key1.pub, *system.work.generate (previous_destination_chain_hash));
	...
	oslo::send_block send1 (previous_destination_chain_hash, oslo::test_genesis_key.pub, oslo::Gxrb_ratio - 2, key1.prv, key1.pub, *system.work.generate (previous_destination_chain_hash));
	auto receive1 (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, previous_genesis_chain_hash, oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio + 1, send1.hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash)));
	...
	oslo::keypair key2;
	oslo::state_block send2 (oslo::genesis_account, receive1->hash (), oslo::genesis_account, oslo::genesis_amount - oslo::Gxrb_ratio, key2.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (receive1->hash ()));
	...
	ASSERT_EQ (cemented_count, node->ledger.cache.cemented_count);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in), num_blocks * 2 + 2);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_bounded, oslo::stat::dir::in), num_blocks * 2 + 2);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_unbounded, oslo::stat::dir::in), 0);
...
TEST (confirmation_height, dynamic_algorithm)
{
	...
	oslo::keypair key;
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	auto const num_blocks = oslo::confirmation_height::unbounded_cutoff;
	auto latest_genesis = node->latest (oslo::test_genesis_key.pub);
	std::vector<std::shared_ptr<oslo::state_block>> state_blocks;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto send (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, latest_genesis, oslo::test_genesis_key.pub, oslo::genesis_amount - i - 1, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest_genesis)));
		latest_genesis = send->hash ();
	...
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in), num_blocks);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_bounded, oslo::stat::dir::in), 1);
	ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_unbounded, oslo::stat::dir::in), num_blocks - 1);
	system.deadline_set (10s);
	while (node->active.election_winner_details_size () > 0)
	{
		ASSERT_NO_ERROR (system.poll ());
...
TEST (confirmation_height, dynamic_algorithm_no_transition_while_pending)
{
	...
	for (auto _ = 0; _ < 3; ++_)
	{
		...
		auto latest_genesis = node->latest (oslo::test_genesis_key.pub);
		std::vector<std::shared_ptr<oslo::state_block>> state_blocks;
		auto const num_blocks = oslo::confirmation_height::unbounded_cutoff - 2;

		auto add_block_to_genesis_chain = [&](oslo::write_transaction & transaction) {
			static int num = 0;
			auto send (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, latest_genesis, oslo::test_genesis_key.pub, oslo::genesis_amount - num - 1, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest_genesis)));
			latest_genesis = send->hash ();
			state_blocks.push_back (send);
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *send).code);
			++num;
		...
		ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in), num_blocks);
		ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_bounded, oslo::stat::dir::in), 0);
		ASSERT_EQ (node->ledger.stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed_unbounded, oslo::stat::dir::in), num_blocks);
		system.deadline_set (10s);
		while (node->active.election_winner_details_size () > 0)
		{
			ASSERT_NO_ERROR (system.poll ());
...
TEST (confirmation_height, many_accounts_send_receive_self)
{
	...
	node_flags.confirmation_height_processor_mode = oslo::confirmation_height_mode::unbounded;
	auto node = system.add_node (node_config);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	...
	std::vector<std::shared_ptr<oslo::open_block>> open_blocks;
	{
		...
		for (auto i = 0; i < num_accounts; ++i)
		{
			...
			oslo::send_block send (latest_genesis, key.pub, oslo::genesis_amount - 1 - i, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest_genesis));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send).code);
			auto open = std::make_shared<oslo::open_block> (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, *open).code);
			open_blocks.push_back (std::move (open));
	...
	std::vector<std::shared_ptr<oslo::send_block>> send_blocks;
	std::vector<std::shared_ptr<oslo::receive_block>> receive_blocks;

	for (int i = 0; i < open_blocks.size (); ++i)
	{
		auto open_block = open_blocks[i];
		auto & keypair = keys[i];
		send_blocks.emplace_back (std::make_shared<oslo::send_block> (open_block->hash (), keypair.pub, 1, keypair.prv, keypair.pub, *system.work.generate (open_block->hash ())));
		receive_blocks.emplace_back (std::make_shared<oslo::receive_block> (send_blocks.back ()->hash (), send_blocks.back ()->hash (), keypair.prv, keypair.pub, *system.work.generate (send_blocks.back ()->hash ())));
	}
...
TEST (confirmation_height, many_accounts_send_receive_self_no_elections)
{
	...
	oslo::block_hash block_hash_being_processed{ 0 };
	oslo::confirmation_height_processor confirmation_height_processor{ ledger, write_database_queue, 10ms, logger, initialized_latch, confirmation_height_mode::automatic };

	auto const num_accounts = 100000;

	auto latest_genesis = oslo::genesis_hash;
	std::vector<oslo::keypair> keys;
	std::vector<std::shared_ptr<oslo::open_block>> open_blocks;
...

	{
		...
		for (auto i = 0; i < num_accounts; ++i)
		{
			oslo::keypair key;
			keys.emplace_back (key);
			oslo::send_block send (latest_genesis, key.pub, oslo::genesis_amount - 1 - i, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *pool.generate (latest_genesis));
			ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, send).code);
			auto open = std::make_shared<oslo::open_block> (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *pool.generate (key.pub));
			ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, *open).code);
			open_blocks.push_back (std::move (open));
			latest_genesis = send.hash ();
		}
	...
	system.deadline_set (1000s);
	auto num_blocks_to_confirm = num_accounts * 2;
	while (stats.count (oslo::stat::type::confirmation_height, oslo::stat::detail::blocks_confirmed, oslo::stat::dir::in) != num_blocks_to_confirm)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	// Now add all send/receive blocks
	{
		...
		for (int i = 0; i < open_blocks.size (); ++i)
		{
			...
			ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, *send_blocks.back ()).code);
			ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, *receive_blocks.back ()).code);
		}
	}
...
TEST (confirmation_height, prioritize_frontiers_overwrite)
{
...

	{
		...
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			oslo::send_block send (last_open_hash, key.pub, oslo::Gxrb_ratio - 1, last_keypair.prv, last_keypair.pub, *system.work.generate (last_open_hash));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send).code);
			oslo::open_block open (send.hash (), last_keypair.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
			ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, open).code);
			last_open_hash = open.hash ();
	...
	oslo::keypair key;
	auto latest_genesis = node->latest (oslo::test_genesis_key.pub);
	oslo::send_block send (latest_genesis, key.pub, oslo::Gxrb_ratio - 1, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest_genesis));
	oslo::open_block open (send.hash (), oslo::test_genesis_key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, open).code);
	...
	oslo::send_block send1 (send.hash (), key.pub, oslo::Gxrb_ratio - 2, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send.hash ()));
	oslo::receive_block receive (open.hash (), send1.hash (), key.prv, key.pub, *system.work.generate (open.hash ()));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, send1).code);
		ASSERT_EQ (oslo::process_result::progress, node->ledger.process (transaction, receive).code);
...
{
public:
	oslo::util::counted_completion write_completion{ 0 };
...
TEST (telemetry, ongoing_requests)
{
	...
	ASSERT_EQ (0, node_client->stats.count (oslo::stat::type::bootstrap, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in));
	ASSERT_EQ (0, node_client->stats.count (oslo::stat::type::bootstrap, oslo::stat::detail::telemetry_req, oslo::stat::dir::out));

	system.deadline_set (20s);
	while (node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in) != 1 || node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in) != 1)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	ASSERT_EQ (2, node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in));
	ASSERT_EQ (2, node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::in));
	ASSERT_EQ (2, node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::out));
	ASSERT_EQ (2, node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in));
	ASSERT_EQ (2, node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::in));
	ASSERT_EQ (2, node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::out));
}
				...
				while (std::any_of (node_data.cbegin (), node_data.cend (), [](auto const & data) { return data.keep_requesting_metrics.load (); }))
				{
					for (auto & data : node_data)
					{
						...
						if (data.keep_requesting_metrics)
						{
							...
							data.node->telemetry->get_metrics_single_peer_async (peer, [&shared_data, &data, &node_data](oslo::telemetry_data_response const & telemetry_data_response_a) {
...
TEST (telemetry, under_load)
{
	...
	auto send (std::make_shared<oslo::state_block> (oslo::test_genesis_key.pub, latest_genesis, oslo::test_genesis_key.pub, oslo::genesis_amount - num_blocks, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest_genesis)));
	node->process_active (send);
	latest_genesis = send->hash ();
	auto open (std::make_shared<oslo::state_block> (key.pub, 0, key.pub, num_blocks, send->hash (), key.prv, key.pub, *system.work.generate (key.pub)));
	node->process_active (open);
	auto latest_key = open->hash ();

	auto thread_func = [key1, &system, node, num_blocks](oslo::keypair const & keypair, oslo::block_hash const & latest, oslo::uint128_t const initial_amount) {
		auto latest_l = latest;
		for (int i = 0; i < num_blocks; ++i)
		{
			auto send (std::make_shared<oslo::state_block> (keypair.pub, latest_l, keypair.pub, initial_amount - i - 1, key1.pub, keypair.prv, keypair.pub, *system.work.generate (latest_l)));
			latest_l = send->hash ();
			node->process_active (send);
		}
	};

	std::thread thread1 (thread_func, oslo::test_genesis_key, latest_genesis, oslo::genesis_amount - num_blocks);
	...
	{
		ASSERT_EQ (0, node->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::failed_send_telemetry_req));
		ASSERT_EQ (0, node->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::request_within_protection_cache_zone));
		ASSERT_EQ (0, node->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::unsolicited_telemetry_ack));
		ASSERT_EQ (0, node->stats.count (oslo::stat::type::telemetry, oslo::stat::detail::no_response_received));
	}
}
...
TEST (telemetry, all_peers_use_single_request_cache)
{
	...
	oslo::telemetry_data telemetry_data;
	{
		...
		node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &telemetry_data](oslo::telemetry_data_response const & response_a) {
	...
	ASSERT_EQ (1, node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in));
	ASSERT_EQ (0, node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::in));
	ASSERT_EQ (1, node_client->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::out));
	ASSERT_EQ (0, node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_ack, oslo::stat::dir::in));
	ASSERT_EQ (1, node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::in));
	ASSERT_EQ (0, node_server->stats.count (oslo::stat::type::message, oslo::stat::detail::telemetry_req, oslo::stat::dir::out));
...
TEST (telemetry, many_nodes)
{
	...
	const auto num_nodes = (is_sanitizer_build || oslo::running_within_valgrind ()) ? 4 : 10;
	for (auto i = 0; i < num_nodes; ++i)
	{
		oslo::node_config node_config (oslo::get_available_port (), system.logging);
		...
		auto node = std::make_shared<oslo::node> (system.io_ctx, oslo::unique_path (), system.alarm, node_config, system.work, node_flags);
		node->start ();
	...
	oslo::keypair key;
	oslo::genesis genesis;
	oslo::state_block send (oslo::test_genesis_key.pub, genesis.hash (), oslo::test_genesis_key.pub, oslo::genesis_amount - oslo::Mxrb_ratio, key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (genesis.hash ()));
	...
	{
		node_client->telemetry->get_metrics_single_peer_async (peer, [&telemetry_datas, &mutex](oslo::telemetry_data_response const & response_a) {
	...
	for (auto & data : telemetry_datas)
	{
		...
		ASSERT_EQ (data.major_version, oslo::get_major_node_version ());
		ASSERT_EQ (data.minor_version, oslo::get_minor_node_version ());
		ASSERT_EQ (data.patch_version, oslo::get_patch_node_version ());
		ASSERT_EQ (data.pre_release_version, oslo::get_pre_release_node_version ());
...
TEST (signature_checker, mass_boundary_checks)
{
	...
	for (auto i = 1; i <= 10; ++i)
	{
		add_boundary (oslo::signature_checker::batch_size * i);
	}

	for (auto num_threads = 0; num_threads < 5; ++num_threads)
	{
		oslo::signature_checker checker (num_threads);
		auto max_size = *(sizes.end () - 1);
		std::vector<oslo::uint256_union> hashes;
		...
		oslo::keypair key;
		oslo::state_block block (key.pub, 0, key.pub, 0, 0, key.prv, key.pub, 0);
...
TEST (node, mass_epoch_upgrader)
{
	auto perform_test = [](size_t const batch_size) {
		...
		auto balance = node.balance (oslo::test_genesis_key.pub);
		auto latest = node.latest (oslo::test_genesis_key.pub);
		oslo::uint128_t amount = 1;
		...
		for (auto & accounts : all)
		{
			for (auto & info : *accounts)
			{
				...
				auto block = builder
				             ...
				             .work (*node.work_generate_blocking (latest, oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (oslo::epoch::epoch_0, false, false, false))))
				             .build (ec);
				ASSERT_FALSE (ec);
				ASSERT_NE (nullptr, block);
		...
		for (auto const & info : opened)
		{
			...
			auto block = builder
			             ...
			             .work (*node.work_generate_blocking (info.key.pub, oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (oslo::epoch::epoch_0, false, false, false))))
			             .build (ec);
			ASSERT_FALSE (ec);
			ASSERT_NE (nullptr, block);
			ASSERT_EQ (oslo::process_result::progress, node.process (*block).code);
		...
		while (node.ledger.cache.block_count != block_count_before + total_to_upgrade)
		{
			...
			ASSERT_FALSE (node.epoch_upgrader (epoch_signer.prv.as_private_key (), oslo::epoch::epoch_1, upgrade_count, threads));
			// Already ongoing - should fail
			ASSERT_TRUE (node.epoch_upgrader (epoch_signer.prv.as_private_key (), oslo::epoch::epoch_1, upgrade_count, threads));
			system.deadline_set (60s);
...
TEST (node, mass_block_new)
{
	...
	// Upgrade to epoch_2
	system.upgrade_genesis_epoch (node, oslo::epoch::epoch_1);
	system.upgrade_genesis_epoch (node, oslo::epoch::epoch_2);

	auto next_block_count = num_blocks + 3;
	auto process_all = [&](std::vector<std::shared_ptr<oslo::state_block>> const & blocks_a) {
	...
	oslo::genesis genesis;
	oslo::keypair key;
	std::vector<oslo::keypair> keys (num_blocks);
	oslo::state_block_builder builder;
	std::vector<std::shared_ptr<oslo::state_block>> send_blocks;
	auto send_threshold (oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (oslo::epoch::epoch_2, true, false, false)));
	auto latest_genesis = node.latest (oslo::test_genesis_key.pub);
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto send = builder.make_block ()
		            ...
		            .link (keys[i].pub)
		            .sign (oslo::test_genesis_key.prv, oslo::test_genesis_key.pub)
		            .work (*system.work.generate (oslo::work_version::work_1, latest_genesis, send_threshold))
		            .build ();
		latest_genesis = send->hash ();
	...
	std::vector<std::shared_ptr<oslo::state_block>> open_blocks;
	auto receive_threshold (oslo::work_threshold (oslo::work_version::work_1, oslo::block_details (oslo::epoch::epoch_2, false, true, false)));
	for (auto i = 0; i < num_blocks; ++i)
	{
		...
		auto open = builder.make_block ()
		            ...
		            .work (*system.work.generate (oslo::work_version::work_1, key.pub, receive_threshold))
		            .build ();
		open_blocks.push_back (std::move (open));
	}
	...
	std::vector<std::shared_ptr<oslo::state_block>> send_blocks2;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto const & key = keys[i];
		...
		auto send2 = builder.make_block ()
		             ...
		             .link (key.pub)
		             .sign (key.prv, key.pub)
		             .work (*system.work.generate (oslo::work_version::work_1, latest->hash (), send_threshold))
		             .build ();
	...
	std::vector<std::shared_ptr<oslo::state_block>> receive_blocks;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto const & key = keys[i];
		...
		auto send2 = builder.make_block ()
		             ...
		             .link (latest->hash ())
		             .sign (key.prv, key.pub)
		             .work (*system.work.generate (oslo::work_version::work_1, latest->hash (), receive_threshold))
		             .build ();
