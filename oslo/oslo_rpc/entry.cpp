...
void run (boost::filesystem::path const & data_path, std::vector<std::string> const & config_overrides)
{
	...
	auto error = oslo::read_rpc_config_toml (data_path, rpc_config, config_overrides);
	if (!error)
	{
		...
		try
		{
			oslo::ipc_rpc_processor ipc_rpc_processor (io_ctx, rpc_config);
			auto rpc = oslo::get_rpc (io_ctx, rpc_config, ipc_rpc_processor);
			rpc->start ();
			...
			runner = std::make_unique<oslo::thread_runner> (io_ctx, rpc_config.rpc_process.io_threads);
			runner->join ();
