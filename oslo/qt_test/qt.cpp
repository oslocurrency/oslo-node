...
TEST (wallet, construction)
{
	...
	auto wallet_l (system.nodes[0]->wallets.create (oslo::random_wallet_id ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet](oslo_qt::status_types status_ty) {
	...
	system.nodes[0]->network.udp_channels.insert (oslo::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (oslo_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (oslo_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (oslo_qt::status_types::disconnected));
}
...
TEST (wallet, startup_balance)
{
	...
	ASSERT_EQ ("Balance: 0 OSLO", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	oslo::system system (1);
	auto wallet_l (system.nodes[0]->wallets.create (oslo::random_wallet_id ()));
	oslo::public_key key1 (wallet_l->deterministic_insert ());
	oslo::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (oslo::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
...
TEST (wallet, send)
{
	...
	oslo::public_key key1 (system.wallet (1)->insert_adhoc (oslo::keypair ().prv));
	auto account (oslo::test_genesis_key.pub);
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, oslo::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	oslo::uint128_t amount (wallet->node.balance (key1));
...
TEST (wallet, process_block)
{
	...
	oslo::system system (1);
	oslo::account account;
	oslo::block_hash latest (system.nodes[0]->latest (oslo::genesis_account));
	system.wallet (0)->insert_adhoc (oslo::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	...
	oslo::send_block send (latest, key1.pub, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest));
	std::string previous;
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, oslo::test_genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	oslo::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (oslo::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (oslo::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	oslo::system system (1);
	system.wallet (0)->insert_adhoc (oslo::test_genesis_key.prv);
	system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, 100);
	oslo::block_hash latest1 (system.nodes[0]->latest (oslo::test_genesis_key.pub));
	system.wallet (0)->send_action (oslo::test_genesis_key.pub, key.pub, 100);
	oslo::block_hash latest2 (system.nodes[0]->latest (oslo::test_genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, oslo::test_genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	oslo::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (oslo::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (oslo::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	oslo::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (oslo::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (oslo::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	oslo::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (oslo::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (oslo::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (history, short_text)
{
	...
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	oslo::mdb_store store (system.nodes[0]->logger, oslo::unique_path ());
	ASSERT_TRUE (!store.init_error ());
	oslo::genesis genesis;
	oslo::ledger ledger (store, system.nodes[0]->stats);
	{
		auto transaction (store.tx_begin_write ());
		store.initialize (transaction, genesis, ledger.cache);
		oslo::keypair key;
		auto latest (ledger.latest (transaction, oslo::test_genesis_key.pub));
		oslo::send_block send (latest, oslo::test_genesis_key.pub, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, send).code);
		oslo::receive_block receive (send.hash (), send.hash (), oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (send.hash ()));
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, receive).code);
		oslo::change_block change (receive.hash (), key.pub, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (receive.hash ()));
		ASSERT_EQ (oslo::process_result::progress, ledger.process (transaction, change).code);
	}
	oslo_qt::history history (ledger, oslo::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, oslo::test_genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		again = wallet->wallet_m->store.work_get (transaction, oslo::test_genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	oslo::block_hash hash;
	{
		...
		auto latest (system.nodes[0]->ledger.latest (transaction, oslo::test_genesis_key.pub));
		oslo::send_block block (latest, key.pub, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system.work.generate (latest));
		hash = block.hash ();
		ASSERT_EQ (oslo::process_result::progress, system.nodes[0]->ledger.process (transaction, block).code);
	}
	auto account (oslo::test_genesis_key.pub);
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	...
	ASSERT_FALSE (system.nodes[1]->balance (oslo::test_genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (oslo::test_genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	oslo::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, oslo::test_genesis_key.prv.data.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	oslo::system system (1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	ASSERT_GE (oslo::work_difficulty (oslo::work_version::work_1, system.nodes[0]->ledger.latest_root (transaction, pub), work), system.nodes[0]->default_difficulty (oslo::work_version::work_1));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<oslo_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		auto latest (system1.nodes[0]->ledger.latest (transaction, oslo::genesis_account));
		oslo::send_block send (latest, key1, 0, oslo::test_genesis_key.prv, oslo::test_genesis_key.pub, *system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (oslo_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (oslo_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (oslo_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
